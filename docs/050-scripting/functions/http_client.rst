...
+==========================+===========+================================+
| method                   | string    | HTTP method, e.g. ``GET`` or   |
|                          |           | ``POST``                       |
+--------------------------+-----------+--------------------------------+
| uri                      | string    | Full uri to call, e. g.        |
|                          |           | ``https://api.liste            |
|                          |           | nbrainz.org/1/submit-listens`` |
+--------------------------+-----------+--------------------------------+
| extra_headers            | string    | Additional headers, must be    |
|                          |           | terminated by ``\r\n``         |
+--------------------------+-----------+--------------------------------+
...
If you set the ``out`` argument to an empty string, the file will be
kept in the http client cache. You can serve the file directly from the
cache with the ``mympd.http_serve_file_from_cache`` function.
...
1. The ``filename`` is set to the ``out`` argument if it was not empty,
   else the ``filename`` is populated with the http client cache
   filepath.

Get a http header from the response
-----------------------------------

Gets the value of a http header. Name of the header is matched case
insensitive as per rfc.
