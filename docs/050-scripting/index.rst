...
myMPD integrates `Lua <http://www.lua.org>`__ for scripting purposes. Scripts are executed asynchronously and can not block the main thread of myMPD. There are two types of scripts.
...
- :doc:`Triggers </060-references/trigger>`
...
+-----------------------------------+-------------------------------------------------------------------+
| ``<argument>\|select;opt1;opt2;`` | Selectbox, options are separated by semicolon.                    |
+-----------------------------------+-------------------------------------------------------------------+
| ``<argument>\|checkbox``          | Simple checkbox, returns the string ``true`` if checked.          |
+-----------------------------------+-------------------------------------------------------------------+
| ``<argument>\|radio;r1;r2;r3``    | Radioboxes, returns the selected option. Options are separated by |
|                                   | semicolon.                                                        |
+-----------------------------------+-------------------------------------------------------------------+
| ``<argument>\|list;r1;r2;r3``     | List of values, returns the selected values separated by ``;;``.  |
...
- :doc:`mympd.init <functions/mympd_init>`
...
- :doc:`List of all functions <functions/index>`

Lua manual
----------

- `Lua manual <https://www.lua.org/manual/5.4/>`__
...
Complete scripts that can be imported are in the `mympd-scripts repository <https://github.com/jcorporation/mympd-scripts>`__.
...
.. code-block:: lua
   ...
   if rc == 0
       ...
       return "Error message:" .. result.message end

Script file format
------------------

Scripts are saved in the directory ``/var/lib/mympd/scripts`` with the
extension ``.lua``. The metadata is saved in the first line in a lua
...
+--------------+-----------------------------------------------------------------------------+
| arguments    | Name of the keys for the script arguments, the gui asks for this arguments. |
|              | Arguments are populated in a lua table called ``mympd_arguments``.          |
+--------------+-----------------------------------------------------------------------------+
