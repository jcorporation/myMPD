Multiroom audio system using MPD/myMPD and pulseaudio
=====================================================
...
- A few low-powered embedded devices connected to audio systems in
  different rooms throughout your house.

.. figure:: ../assets/multiroomaudio/madiag.png
   :alt: image

   image

You can use mpd/mympd’s partition feature to enable a multiroom audio
system leveraging Pulseaudio RTP streaming between your NAS and your
low-powered devices.

You then get on the central myMPD interface the option to select to
which output your player will be outputting: |image|

In this case, you could can either stream to all rooms using the
“Multiroom” output, which then uses pulseaudio rtp multicast output to
stream a single synchronized audio flow to all receivers, or output to a
single room, which then uses pulseaudio rtp unicast output to stream to
the selected receiver.

Please note that perfect audio synchronisation with multiple audio
receivers can only be achieved using a multicast output.

If you want to play different audio/playlists to differents zones, just
create a new partition |image1|

Move the outputs you want to link to that partition: |image2|

.. figure:: ../assets/multiroomaudio/partitionoutputmove2.png
   :alt: image

   image

.. figure:: ../assets/multiroomaudio/partitionoutputmove3.png
   :alt: image

   image

Now, the partition “Jane” has its own player, its own playlist and its
own output(s).

Configuration
-------------

Let’s use the following table as configuration reference in this
example:
...
- the emitter (NAS)
- and the receivers (Raspberry pis or orther low powered receiver
  device).
...
Notice that a distinct sink is configured for each destination device,
plus another one for synchronised multiroom audio.
...
On the receivers, the incoming RTP flows (unicast and multicast) should
appear under the audio interface that’s outputting them.
...
|image3| Example here with a Seagate Dockstar (a low powered ARM plug
computer) being used as audio receiver.

All operations regarding the scanning of the music collection, the
playlists, etc only happens on the central node.

.. |image| image:: ../assets/multiroomaudio/outputselection.png
.. |image1| image:: ../assets/multiroomaudio/partition.png
.. |image2| image:: ../assets/multiroomaudio/partitionoutputmove1.png
.. |image3| image:: ../assets/multiroomaudio/satellite-dev-cpu-usage.png
