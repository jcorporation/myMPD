...
var CACHE="myMPD-cache-v6.10.0",subdir=self.location.pathname.replace("/sw.js","").replace(/\/$/,""),urlsToCache=[subdir+"/",subdir+"/css/combined.css",subdir+"/js/combined.js",subdir+"/assets/appicon-192.png",subdir+"/assets/appicon-512.png",subdir+"/assets/coverimage-stream.svg",subdir+"/assets/coverimage-notavailable.svg",subdir+"/assets/coverimage-loading.svg",subdir+"/assets/coverimage-mympd.svg",subdir+"/assets/favicon.ico",subdir+"/assets/MaterialIcons-Regular.woff2"],ignoreRequests=new RegExp("("+
[subdir+"/api",subdir+"/ca.crt",subdir+"/ws",subdir+"/tagpics/(.*)",subdir+"/albumart/(.*)",subdir+"/browse/(.*)"].join("(/?)|\\")+")$");self.addEventListener("install",function(a){a.waitUntil(caches.open(CACHE).then(function(c){urlsToCache.map(function(e){return c.add(e).catch(function(g){return console.log("ServiceWorker: "+String(g)+" "+e)})})}))});
