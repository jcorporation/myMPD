...
var CACHE="myMPD-cache-v5.5.0",subdir=self.location.pathname.replace("/sw.min.js","").replace(/\/$/,""),urlsToCache=[subdir+"/",subdir+"/css/bootstrap.min.css",subdir+"/css/mympd.min.css",subdir+"/js/bootstrap-native-v4.min.js",subdir+"/js/mympd.min.js",subdir+"/js/keymap.min.js",subdir+"/js/i18n.min.js",subdir+"/assets/appicon-167.png",subdir+"/assets/appicon-192.png",subdir+"/assets/appicon-512.png",subdir+"/assets/coverimage-stream.svg",subdir+"/assets/coverimage-notavailable.svg",subdir+"/assets/coverimage-loading.svg",
subdir+"/assets/favicon.ico",subdir+"/assets/MaterialIcons-Regular.woff2"],ignoreRequests=new RegExp("("+[subdir+"/api",subdir+"/ws",subdir+"/library/(.*)",subdir+"/pics/(.*)"].join("(/?)|\\")+")$");self.addEventListener("install",function(a){a.waitUntil(caches.open(CACHE).then(function(a){urlsToCache.map(function(b){return a.add(b).catch(function(a){return console.log("ServiceWorker: "+String(a)+" "+b)})})}))});
