...
function gotoBrowse(){if(!1!==settings.featAdvsearch){var a=event.target,b=a.getAttribute("data-tag"),c=decodeURI(a.getAttribute("data-name"));null===b&&(b=a.parentNode.getAttribute("data-tag"),c=decodeURI(a.parentNode.getAttribute("data-name")));if(""!==b&&""!==c&&"-"!==c&&settings.browsetags.includes(b))if("Album"===b){var f=a.getAttribute("data-albumartist");null===f&&(f=a.parentNode.getAttribute("data-albumartist"));null!==f?appGoto("Browse","Database","Detail","0",b,tagAlbumArtist,c,decodeURI(f)):
appGoto("Browse","Database","List","0",b,tagAlbumArtist,"Album","("+b+" == '"+c+"')")}else appGoto("Browse","Database","List","0",b,tagAlbumArtist,"Album","("+b+" == '"+c+"')")}}
...
ligatureMore+"</a></td></tr>";d=a.result.data[g].Disc}b.innerHTML=f;document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto("Browse","Database","List")}function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);_addAlbum(a,c,b)}
function _addAlbum(a,b,c){b="((Album == '"+c+"') AND ("+tagAlbumArtist+" == '"+b+"'))";"appendQueue"===a?addAllFromSearchPlist("queue",b,!1):"replaceQueue"===a?addAllFromSearchPlist("queue",b,!0):"addPlaylist"===a&&showAddToPlaylist("ALBUM",b)}
...
var socket=null,websocketConnected=!1,websocketTimer=null,socketRetry=0,lastSong="",lastSongObj={},lastState,currentSong={},playstate="",settingsLock=!1,settingsParsed=!1,settingsNew={},settings={loglevel:2},alertTimeout=null,progressTimer=null,deferredA2HSprompt,dragSrc,dragEl,appInited=!1,subdir="",uiEnabled=!0,locale=navigator.language||navigator.userLanguage,scale="1.0",isMobile=/iPhone|iPad|iPod|Android/i.test(navigator.userAgent),ligatureMore="menu",progressBarTransition="width 1s linear",tagAlbumArtist=
"AlbumArtist",app={apps:{Home:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Playback:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Queue:{active:"Current",tabs:{Current:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0},LastPlayed:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0},Jukebox:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0}}},Browse:{active:"Database",tabs:{Filesystem:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},
Playlists:{active:"All",views:{All:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Detail:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0}}},Database:{active:"List",views:{List:{page:0,filter:"AlbumArtist",sort:"AlbumArtist",tag:"Album",search:"",scrollPos:0},Detail:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0}}}}},Search:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0}},current:{app:"Home",tab:void 0,view:void 0,page:0,filter:"",search:"",sort:"",tag:"",
scrollPos:0},last:{app:void 0,tab:void 0,view:void 0,filter:"",search:"",sort:"",tag:"",scrollPos:0}},domCache={};domCache.counter=document.getElementById("counter");domCache.volumePrct=document.getElementById("volumePrct");domCache.volumeControl=document.getElementById("volumeControl");domCache.volumeMenu=document.getElementById("volumeMenu");domCache.btnsPlay=document.getElementsByClassName("btnPlay");domCache.btnsPlayLen=domCache.btnsPlay.length;domCache.btnPrev=document.getElementById("btnPrev");
...
"-";appGoto("Browse","Filesystem",void 0,"0",app.current.filter,app.current.sort,"-",decodeURI(d.target.parentNode.getAttribute("data-uri")));break;case "song":appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"))}else"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",
...
decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("SearchList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseDatabaseCards").addEventListener("click",function(d){if("Album"===
app.current.tag)if(d.target.classList.contains("card-body"))appGoto("Browse","Database","Detail","0","Album","AlbumArtist",decodeURI(d.target.parentNode.getAttribute("data-album")),decodeURI(d.target.parentNode.getAttribute("data-albumartist")));else{if(d.target.classList.contains("card-footer")){showMenu(d.target,d);for(var g=document.getElementById("BrowseDatabaseCards").getElementsByClassName("selected"),k=0;k<g.length;k++)g[k].classList.remove("selected");d.target.parentNode.classList.add("selected")}}else app.current.search=
"",document.getElementById("searchDatabaseStr").value="",appGoto(app.current.app,app.current.card,void 0,"0","Album","AlbumArtist","Album","("+app.current.tag+" == '"+decodeURI(d.target.parentNode.getAttribute("data-tag"))+"')")},!1);document.getElementById("BrowseDatabaseCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);!1===isMobile&&(document.getElementById("BrowseDatabaseCards").addEventListener("mouseover",function(d){if(d.target.classList.contains("card-body")&&
0===d.target.childNodes.length){var g=document.getElementById("BrowseDatabaseCards").getElementsByClassName("album-grid-mouseover");if(1<g.length)for(var k=0;k<g.length;k++)g[k].remove();addPlayButton(d.target)}},!1),document.getElementById("BrowseDatabaseCards").addEventListener("mouseout",function(d){!d.target.classList.contains("card-body")||null!==d.relatedTarget&&d.relatedTarget.classList.contains("album-grid-mouseover")||(d.target.innerHTML="")},!1));document.getElementById("BrowseDatabaseDetailList").addEventListener("click",
function(d){"TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemAddAllSongsDropdown").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromBrowseFilesystem():"Add all to playlist"===d.target.getAttribute("data-phrase")&&showAddToPlaylist(app.current.search,
""))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue",null,!1):"Add all to playlist"===d.target.getAttribute("data-phrase")?showAddToPlaylist("SEARCH",""):"Save as smart playlist"===d.target.getAttribute("data-phrase")&&saveSearchAsSmartPlaylist())},!1);document.getElementById("searchtags").addEventListener("click",function(d){"BUTTON"===
d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),doSearch(domCache.searchstr.value))},!1);document.getElementById("searchDatabaseTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("databaseSortDesc").addEventListener("click",function(d){toggleBtnChk(this);
d.stopPropagation();d.preventDefault();"-"===app.current.sort.charAt(0)?app.current.sort=app.current.sort.substr(1):app.current.sort="-"+app.current.sort;appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search)},!1);document.getElementById("databaseSortTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),d.stopPropagation(),app.current.sort=d.target.getAttribute("data-tag"),appGoto(app.current.app,
app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavPlaylistsDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavFilesystemDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("dropdownSortPlaylistTags").addEventListener("click",
function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),playlistSort(d.target.getAttribute("data-tag")))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,"-",this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,
app.current.page,d.target.getAttribute("data-tag"),app.current.sort,"-",app.current.search)},!1);document.getElementById("searchFilesystemStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",""!==this.value?this.value:"-",app.current.sort,"-",app.current.search)},!1);document.getElementById("searchPlaylistsStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,
"0",app.current.filter,app.current.sort,"-",this.value)},!1);a=["PlaybackColsDropdown"];for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&d.target.classList.contains("material-icons")&&(d.stopPropagation(),d.preventDefault(),toggleBtnChk(d.target))},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);document.getElementById("searchdatabase").addEventListener("submit",
function(){return!1},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&"Album"===app.current.tag)if(""!==this.value){d=document.getElementById("searchDatabaseMatch");var g=document.createElement("button");g.classList.add("btn","btn-light","mr-2");g.setAttribute("data-filter",encodeURI(app.current.filter+" "+d.options[d.selectedIndex].value+" '"+this.value+"'"));g.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+
" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";document.getElementById("searchDatabaseCrumb").appendChild(g)}else searchAlbumgrid(this.value);else"Album"===app.current.tag?searchAlbumgrid(this.value):appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,this.value)},!1);document.getElementById("searchDatabaseCrumb").addEventListener("click",function(d){d.preventDefault();d.stopPropagation();if("SPAN"===
d.target.nodeName)d.target.parentNode.remove(),searchAlbumgrid("");else if("BUTTON"===d.target.nodeName){var g=decodeURI(d.target.getAttribute("data-filter"));document.getElementById("searchDatabaseStr").value=g.substring(g.indexOf("'")+1,g.length-1);var k=g.substring(g.indexOf(" ")+1);k=k.substring(0,k.indexOf(" "));document.getElementById("searchDatabaseMatch").value=k;g=g.substring(0,g.indexOf(" "));selectTag("searchDatabaseTags","searchDatabaseTagsDesc",g);d.target.remove();searchAlbumgrid(document.getElementById("searchDatabaseStr").value)}},
!1);domCache.searchstr.addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&settings.featAdvsearch)if(""!==this.value){d=document.getElementById("searchMatch");var g=document.createElement("button");g.classList.add("btn","btn-light","mr-2");g.setAttribute("data-filter",encodeURI(app.current.filter+" "+d.options[d.selectedIndex].value+" '"+this.value+"'"));g.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';
this.value="";domCache.searchCrumb.appendChild(g)}else doSearch(this.value);else doSearch(this.value)},!1);domCache.searchCrumb.addEventListener("click",function(d){if("SPAN"===d.target.nodeName)d.preventDefault(),d.stopPropagation(),d.target.parentNode.remove(),doSearch("");else if("BUTTON"===d.target.nodeName){d.preventDefault();d.stopPropagation();var g=decodeURI(d.target.getAttribute("data-filter"));domCache.searchstr.value=g.substring(g.indexOf("'")+1,g.length-1);var k=g.substring(0,g.indexOf(" "));
selectTag("searchtags","searchtagsdesc",k);g=g.substring(g.indexOf(" ")+1);g=g.substring(0,g.indexOf(" "));document.getElementById("searchMatch").value=g;d.target.remove();doSearch(domCache.searchstr.value)}},!1);document.getElementById("searchMatch").addEventListener("change",function(){doSearch(domCache.searchstr.value)},!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(d){if(settings.featAdvsearch&&"TH"===d.target.nodeName&&""!==d.target.innerHTML){var g=
d.target.getAttribute("data-col");if("Duration"!==g){var k=app.current.sort,l=!0;if(k===g||k==="-"+g)0===k.indexOf("-")?(l=!0,g=k.substring(1)):l=!1;!1===l?(k="-"+g,l=!0):(l=!1,k=g);for(var m=document.getElementById("SearchList").getElementsByClassName("sort-dir"),n=0;n<m.length;n++)m[n].remove();app.current.sort=k;d.target.innerHTML=t(g)+'<span class="sort-dir material-icons pull-right">'+(!0===l?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,
app.current.page,app.current.filter,app.current.sort,"-",app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(d){"Enter"===d.key&&(d.preventDefault(),d.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(d){"OPTION"===d.target.nodeName&&removeScriptArgument(d)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");
dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("QueueJukebox");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},!1);document.addEventListener("keydown",function(d){if("INPUT"!==d.target.tagName&&
"SELECT"!==d.target.tagName&&"TEXTAREA"!==d.target.tagName&&!d.ctrlKey&&!d.altKey){var g=keymap[d.key];g&&"function"===typeof window[g.cmd]&&(void 0===keymap[d.key].req||!0===settings[keymap[d.key].req])&&parseCmd(d,g)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(d){navigateTable(this,d.key)},!1);var h="";Object.keys(themes).forEach(function(d){h+='<option value="'+e(d)+'">'+t(themes[d])+"</option>"});
document.getElementById("selectTheme").innerHTML=h;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("localPlayer").addEventListener("click",
...
f=decodeURI(a.parentNode.getAttribute("data-album")),d+=addMenuItem({cmd:"_addAlbum",options:["appendQueue",b,f]},t("Append to queue"))+addMenuItem({cmd:"_addAlbum",options:["replaceQueue",b,f]},t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"_addAlbum",options:["addPlaylist",b,f]},t("Add to playlist")):"");else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"All"===app.current.view)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",
...
n.innerText="keyboard_arrow_right"===n.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new BSN.Collapse(m));document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));d.show()}
...
localeDate(lastSongObj[settings.colsPlayback[b]]):""):"Fileformat"===settings.colsPlayback[b]?a+(lastState?fileformat(lastState.audioFormat):""):a+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):""),a+="</p></div>";document.getElementById("cardPlaybackTags").innerHTML=a;(a=document.getElementById("currentLyrics"))&&lastSongObj.uri&&(b=a.getElementsByTagName("small")[0],b.classList.add("clickable"),b.addEventListener("click",function(k){k.target.parentNode.children[1].classList.toggle("expanded")},
...
function getLyrics(a,b){void 0===a?b.innerHTML=t("No lyrics found"):(b.classList.add("opacity05"),sendAPI("MPD_API_LYRICS_UNSYNCED_GET",{uri:a},function(c){if(c.error)b.innerText=t(c.error.message);else if(c.result.message)b.innerText=t(c.result.message);else{for(var f='<span class="lyricsHeader" class="btn-group-toggle" data-toggle="buttons">',h='<div class="lyricsTextContainer">',d=0;d<c.result.returnedEntities;d++){var g=c.result.data[d].desc;g=""!==g&&""!==c.result.data[d].lang?g+(" ("+c.result.data[d].lang+
")"):""!==c.result.data[d].lang?c.result.data[d].lang:d;f+='<label data-num="'+d+'" class="btn btn-sm btn-outline-secondary mr-2'+(0===d?" active":"")+'">'+g+"</label>";h+='<div class="lyricsText'+(0<d?" hide":"")+'">'+e(c.result.data[d].text).replace(/\n/g,"<br/>")+"</div>"}b.innerHTML=(1<c.result.returnedEntities?f+"</span>":"")+(h+"</div>");b.getElementsByClassName("lyricsHeader")[0].addEventListener("click",function(k){if("LABEL"===k.target.nodeName){k.target.parentNode.getElementsByClassName("active")[0].classList.remove("active");
k.target.classList.add("active");k=parseInt(k.target.getAttribute("data-num"));for(var l=b.getElementsByClassName("lyricsText"),m=0;m<l.length;m++)m===k?l[m].classList.remove("hide"):l[m].classList.add("hide")}},!1)}b.classList.remove("opacity05")},!0))}function loveSong(){sendAPI("MPD_API_LOVE",{})}
