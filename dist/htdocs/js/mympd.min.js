...
genId("database"+a.result.data[g].Album+a.result.data[g].AlbumArtist);var l=subdir+"/albumart/"+encodeURI(a.result.data[g].FirstSongUri);var m='<div class="card card-grid clickable" data-picture="'+l+'" data-uri="'+encodeURI(a.result.data[g].FirstSongUri.replace(/\/[^/]+$/,""))+'" data-type="dir" data-name="'+encodeURI(a.result.data[g].Album)+'" data-album="'+encodeURI(a.result.data[g].Album)+'" data-albumartist="'+encodeURI(a.result.data[g].AlbumArtist)+'" tabindex="0"><div class="card-body album-cover-loading album-cover-grid bg-white d-flex" id="'+
k+'"></div><div class="card-footer card-footer-grid p-2" title="'+e(a.result.data[g].AlbumArtist)+": "+e(a.result.data[g].Album)+'">'+e(a.result.data[g].Album)+"<br/><small>"+e(a.result.data[g].AlbumArtist)+"</small></div></div>"}else k=genId("database"+a.result.data[g].value),l=subdir+"/tagpics/"+a.result.tag+"/"+encodeURI(a.result.data[g].value),m='<div class="card card-grid clickable" data-picture="'+l+'" data-tag="'+encodeURI(a.result.data[g].value)+'" tabindex="0">'+(!0===a.result.pics?'<div class="card-body album-cover-loading album-cover-grid bg-white" id="'+
k+'"></div>':"")+'<div class="card-footer card-footer-grid p-2" title="'+e(a.result.data[g].value)+'">'+e(a.result.data[g].value)+"<br/></div></div>";h.innerHTML=m;m=!1;g<d.length?d[g].firstChild.getAttribute("data-picture")!==h.firstChild.getAttribute("data-picture")&&(d[g].replaceWith(h),m=!0):(c.append(h),m=!0);!0===m&&(!0===f?(new IntersectionObserver(setGridImage,{root:null,rootMargin:"0px"})).observe(h):h.firstChild.firstChild.style.backgroundImage=l);"Album"===a.result.tag&&!0===isMobile&&
addPlayButton(document.getElementById(k))}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}
...
function addPlayButton(a){var b=document.createElement("div");b.classList.add("align-self-end","album-grid-mouseover","material-icons","rounded-circle","clickable");b.innerText="play_arrow";a.appendChild(b);b.addEventListener("click",function(a){a.preventDefault();a.stopPropagation();replaceQueue("dir",decodeURI(a.target.parentNode.parentNode.getAttribute("data-uri")),decodeURI(a.target.parentNode.parentNode.getAttribute("data-name")))},!1)}
...
'<div class="card home-icons clickable" draggable="true" tabindex="0" data-pos="'+f+"\" data-href='"+e(h)+"'  title=\""+e(a.result.data[f].name)+'"><div class="card-body material-icons">'+e(a.result.data[f].ligature)+'</div><div class="card-footer card-footer-grid p-2">'+e(a.result.data[f].name)+"</div></div>";g.innerHTML=h;f<d.length?d[f].replaceWith(g):c.append(g);""!==a.result.data[f].image&&(g.getElementsByClassName("card-body")[0].style.backgroundImage='url("'+subdir+"/browse/pics/"+a.result.data[f].image+
...
a.preventDefault();a=this.getElementsByTagName("span")[0];a.innerText="keyboard_arrow_right"===a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1);document.getElementById("navbar-main").addEventListener("click",function(a){a.preventDefault();var b=a.target.getAttribute("data-href");null===b&&(b=a.target.parentNode.getAttribute("data-href"));null!==b&&parseCmd(a,b)},!1);document.getElementById("volumeMenu").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",
{},parseOutputs)});document.getElementById("btnDropdownNeighbors").parentNode.addEventListener("show.bs.dropdown",function(){!0===settings.featNeighbors?sendAPI("MPD_API_MOUNT_NEIGHBOR_LIST",{},parseNeighbors,!0):document.getElementById("dropdownNeighbors").children[0].innerHTML='<div class="list-group-item"><span class="material-icons">warning</span> '+t("Neighbors are disabled")+"</div>"});document.getElementById("dropdownNeighbors").children[0].addEventListener("click",function(a){a.preventDefault();
"A"===a.target.nodeName&&(a=a.target.getAttribute("data-value").match(/^(\w+:\/\/)(.+)$/),document.getElementById("selectMountUrlhandler").value=a[1],document.getElementById("inputMountUrl").value=a[2])});document.getElementById("BrowseFilesystemBookmark").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)});document.getElementById("playDropdown").parentNode.addEventListener("show.bs.dropdown",function(){showPlayDropdown()});document.getElementById("playDropdown").addEventListener("click",
...
function(){document.getElementById("bgColorPreview").style.backgroundColor=this.value},!1);document.getElementById("selectTheme").addEventListener("change",function(){var a=getSelectValue(this);"theme-default"===a?document.getElementById("inputBgColor").value="#ccc":"theme-light"===a?document.getElementById("inputBgColor").value="#fff":"theme-dark"===a&&(document.getElementById("inputBgColor").value="#000");document.getElementById("bgColorPreview").style.backgroundColor=document.getElementById("inputBgColor").value},
!1);document.getElementById("modalAddToQueue").addEventListener("shown.bs.modal",function(){document.getElementById("inputAddToQueueQuantity").classList.remove("is-invalid");document.getElementById("warnJukeboxPlaylist2").classList.add("hide");!0===settings.featPlaylists&&sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(a){getAllPlaylists(a,"selectAddToQueuePlaylist")})});document.getElementById("modalUpdateDB").addEventListener("hidden.bs.modal",function(){document.getElementById("updateDBprogress").classList.remove("updateDBprogressAnimate")});
...
"disabled"),document.getElementById("inputAddToQueueQuantity").value="1",document.getElementById("selectAddToQueuePlaylist").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").value="Database"):"1"===a&&(document.getElementById("inputAddToQueueQuantity").removeAttribute("disabled"),document.getElementById("selectAddToQueuePlaylist").removeAttribute("disabled"))});document.getElementById("addToPlaylistPlaylist").addEventListener("change",function(){"new"===this.options[this.selectedIndex].value?
...
"readonly")):"none"===this.options[this.selectedIndex].value?(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").removeAttribute("readonly"))},!1);document.getElementById("syscmds").addEventListener("click",function(a){"A"===a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",
function(a){"A"===a.target.nodeName&&execScript(a.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++){!1===a[c].classList.contains("notclickable")&&a[c].classList.add("clickable");var d=a[c].parentNode.classList.contains("noInitChilds")?!0:!1;!1===d&&(d=a[c].parentNode.parentNode.classList.contains("noInitChilds")?!0:!1);!0!==d&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1)}document.getElementById("HomeCards").addEventListener("click",
function(a){if(a.target.classList.contains("card-body")){var b=a.target.parentNode.getAttribute("data-href");null!==b&&parseCmd(a,b)}else if(a.target.classList.contains("card-footer")){b=document.getElementById("HomeCards").getElementsByClassName("selected");for(var c=0;c<b.length;c++)b[c].classList.remove("selected");a.target.parentNode.classList.add("selected");showMenu(a.target,a);a.stopPropagation()}},!1);document.getElementById("HomeCards").addEventListener("keydown",function(a){navigateGrid(a.target,
a.key)},!1);dragAndDropHome();document.getElementById("selectHomeIconCmd").addEventListener("change",function(){showHomeIconCmdOptions()},!1);document.getElementById("inputHomeIconLigature").addEventListener("change",function(a){document.getElementById("homeIconPreview").innerText=a.target.value;""!==a.target.value&&(document.getElementById("selectHomeIconImage").value="",document.getElementById("homeIconPreview").style.backgroundImage="")},!1);document.getElementById("inputHomeIconBgcolor").addEventListener("change",
...
document.getElementById("homeIconPreview").innerText=document.getElementById("inputHomeIconLigature").value)},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"P"===a.target.nodeName&&gotoBrowse()},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",function(a){"A"===
...
c.classList.add("spinner-border","spinner-border-sm");a.target.classList.add("hide");b.appendChild(c)}else null!==a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),gotoBrowse());else"BUTTON"===a.target.nodeName&&a.target.getAttribute("data-href")&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(a){"BUTTON"===a.target.nodeName?(a.stopPropagation(),a.preventDefault(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",
...
"')"))},!1);document.getElementById("BrowseDatabaseCards").addEventListener("keydown",function(a){navigateGrid(a.target,a.key)},!1);!1===isMobile&&(document.getElementById("BrowseDatabaseCards").addEventListener("mouseover",function(a){if(a.target.classList.contains("card-body")&&0===a.target.childNodes.length){var b=document.getElementById("BrowseDatabaseCards").getElementsByClassName("album-grid-mouseover");if(1<b.length)for(var c=0;c<b.length;c++)b[c].remove();addPlayButton(a.target)}},!1),document.getElementById("BrowseDatabaseCards").addEventListener("mouseout",
function(a){!a.target.classList.contains("card-body")||null!==a.relatedTarget&&a.relatedTarget.classList.contains("album-grid-mouseover")||(a.target.innerHTML="")},!1));document.getElementById("BrowseDatabaseDetailList").addEventListener("click",function(a){"TD"===a.target.nodeName?appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name")):"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseFilesystemAddAllSongsDropdown").addEventListener("click",
...
document.getElementById("searchMatch");var b=document.createElement("button");b.classList.add("btn","btn-light","mr-2");b.setAttribute("data-filter",encodeURI(app.current.filter+" "+a.options[a.selectedIndex].value+" '"+this.value+"'"));b.innerHTML=e(app.current.filter)+" "+e(a.options[a.selectedIndex].value)+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";domCache.searchCrumb.appendChild(b)}else doSearch(this.value);else doSearch(this.value)},!1);domCache.searchCrumb.addEventListener("click",
function(a){a.preventDefault();a.stopPropagation();if("SPAN"===a.target.nodeName)a.target.parentNode.remove(),doSearch("");else if("BUTTON"===a.target.nodeName){var b=decodeURI(a.target.getAttribute("data-filter"));domCache.searchstr.value=b.substring(b.indexOf("'")+1,b.length-1);var c=b.substring(0,b.indexOf(" "));selectTag("searchtags","searchtagsdesc",c);b=b.substring(b.indexOf(" ")+1);b=b.substring(0,b.indexOf(" "));document.getElementById("searchMatch").value=b;a.target.remove();doSearch(domCache.searchstr.value)}},
!1);document.getElementById("searchMatch").addEventListener("change",function(){doSearch(domCache.searchstr.value)},!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(a){if(settings.featAdvsearch&&"TH"===a.target.nodeName&&""!==a.target.innerHTML){var b=a.target.getAttribute("data-col");if("Duration"!==b){var c=app.current.sort,d=!0;if(c===b||c==="-"+b)0===c.indexOf("-")?(d=!0,b=c.substring(1)):d=!1;!1===d?(c="-"+b,d=!0):(d=!1,c=b);for(var f=
document.getElementById("SearchList").getElementsByClassName("sort-dir"),g=0;g<f.length;g++)f[g].remove();app.current.sort=c;a.target.innerHTML=t(b)+'<span class="sort-dir material-icons pull-right">'+(!0===d?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page,app.current.filter,app.current.sort,"-",app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(a){"Enter"===a.key&&(a.preventDefault(),
a.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(a){"OPTION"===a.target.nodeName&&removeScriptArgument(a)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("QueueJukebox");dragAndDropTableHeader("Search");
dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},!1);document.addEventListener("keydown",function(a){if("INPUT"!==a.target.tagName&&"SELECT"!==a.target.tagName&&"TEXTAREA"!==a.target.tagName&&!a.ctrlKey&&!a.altKey){var b=keymap[a.key];b&&"function"===typeof window[b.cmd]&&(void 0===keymap[a.key].req||!0===settings[keymap[a.key].req])&&
parseCmd(a,b)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);var f="";Object.keys(themes).forEach(function(a){f+='<option value="'+e(a)+'">'+t(themes[a])+"</option>"});document.getElementById("selectTheme").innerHTML=f;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",
function(a){a.stopPropagation();a.preventDefault();clickCheckLocalPlayerState(a)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();clickCheckLocalPlayerState(a)},!1);document.getElementById("localPlayer").addEventListener("click",function(a){a.stopPropagation()});document.getElementById("localPlayer").addEventListener("canplay",function(){logDebug("localPlayer event: canplay");document.getElementById("alertLocalPlayback").classList.add("hide");
...
a+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):""),a+="</p></div>";document.getElementById("cardPlaybackTags").innerHTML=a;(a=document.getElementById("currentLyrics"))&&lastSongObj.uri&&(b=a.getElementsByTagName("small")[0],b.classList.add("clickable"),b.addEventListener("click",function(a){a.target.parentNode.children[1].classList.toggle("expanded")},!1),getLyrics(lastSongObj.uri,a.getElementsByTagName("p")[0]))}settings.tags.includes("Title")&&(app.apps.Search.sort=
"Title");settings.tags.includes("AlbumArtist")||"AlbumArtist"!==app.apps.Browse.tabs.Database.views.List.filter||(app.apps.Browse.tabs.Database.views.List.filter="Artist",app.apps.Browse.tabs.Database.views.List.sort="Artist");!0===settings.featPlaylists?sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(a){getAllPlaylists(a,"selectJukeboxPlaylist",settings.jukeboxPlaylist)}):document.getElementById("selectJukeboxPlaylist").innerHTML='<option value="Database">'+t("Database")+"</option>";
...
app.current.app).classList.add("active");for(b=0;b<domCache.navbarBtnsLen;b++)domCache.navbarBtns[b].firstChild.setAttribute("data-href",JSON.stringify({cmd:"appGoto",options:settings.navbarIcons[b].options}))}function resetValue(a){var b=document.getElementById(a);switch(a){case "inputBgCssFilter":b.value="grayscale(100%) opacity(5%)"}}function songDetails(a){sendAPI("MPD_API_DATABASE_SONGDETAILS",{uri:a},parseSongDetails);modalSongDetails.show()}
...
function setCols(a,b){var c=document.getElementById(a+"ColsDropdown");c&&(c.firstChild.innerHTML=setColsChecklist(a));var d=app.current.sort;"Search"===a&&"Title"===app.apps.Search.sort&&(d=settings.tags.includes("Title")?"Title":!1===settings.featTags?"Filename":"-");if("Playback"!==a){c="";for(var f=0;f<settings["cols"+a].length;f++){var g=settings["cols"+a][f];c+='<th draggable="true" data-col="'+g+'">';if("Track"===g||"Pos"===g)g="#";c+=t(g);"Search"!==a||g!==d&&"-"+g!==d||(g=!1,0===app.current.sort.indexOf("-")&&
