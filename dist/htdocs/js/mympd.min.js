...
function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");window.addEventListener("hashchange",appRoute,!1);window.addEventListener("focus",function(){sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState)},!1);document.addEventListener("keydown",function(a){if("INPUT"!=
a.target.tagName&&"SELECT"!=a.target.tagName&&!a.ctrlKey&&!a.altKey){var b=keymap[a.key];b&&"function"===typeof window[b.cmd]&&(void 0==keymap[a.key].req||1==settings[keymap[a.key].req])&&parseCmd(a,b)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a});window.addEventListener("beforeinstallprompt",
...
toggleBtnChk("btnSmartpls",settings.smartpls);a=["featLocalplayer","featSyscmds","featMixramp","featCacert"];for(var b=0;b<a.length;b++)for(var c=document.getElementsByClassName(a[b]),d=c.length,f=1==settings[a[b]]?"":"none",g=0;g<d;g++)c[g].style.display=f;if(settings.featSyscmds){a="";b=settings.syscmdList.length;if(0<b)for(a=4<b?"":'<div class="dropdown-divider"></div>',c=0;c<b;c++)a="HR"==settings.syscmdList[c]?a+'<div class="dropdown-divider"></div>':a+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+
...
'" data-name="'+encodeURI(lastSongObj.data?lastSongObj.data[settings.colsPlayback[b]]:"")+'"><small>'+t(settings.colsPlayback[b])+"</small><h4",settings.browsetags.includes(settings.colsPlayback[b])&&(a+=' class="clickable"'),a+=">"+(lastSongObj.data?e(lastSongObj.data[settings.colsPlayback[b]]):"")+"</h4></div>";document.getElementById("cardPlaybackTags").innerHTML=a}!settings.tags.includes("AlbumArtist")&&settings.featTags&&(settings.tags.includes("Artist")?app.apps.Browse.tabs.Database.active=
...
(g=!0),c+='<span class="sort-dir material-icons pull-right">'+(1==g?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c+="<th></th>";if(void 0==b)document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c;else for(a=document.querySelectorAll(b),b=0;b<a.length;b++)a[b].getElementsByTagName("tr")[0].innerHTML=c}}function getSettings(a){0==settingsLock&&(settingsLock=!0,sendAPI({cmd:"MYMPD_API_SETTINGS_GET"},getMpdSettings,a))}
...
{cmd:"MYMPD_API_COLS_SAVE",data:{table:"cols"+a,cols:[]}};c=b.getElementsByTagName("th");for(b=0;b<c.length;b++)(d=c[b].getAttribute("data-col"))&&a.data.cols.push(d);sendAPI(a,getSettings)}
...
a,cols:[]}};c=c.getElementsByTagName("div");for(b=0;b<c.length;b++)(d=c[b].getAttribute("data-tag"))&&a.data.cols.push(d);sendAPI(a,getSettings)}
...
p.innerHTML=n}k<h.length?l=1==replaceTblRow(h[k],p)?k:l:g.append(p)}for(b=h.length-1;b>=d;b--)h[b].remove();1==f&&focusTable(0);setPagination(a.totalEntities,a.returnedEntities);0==d&&(g.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+c+'">'+t("Empty list")+"</td></tr>");document.getElementById(app.current.app+(void 0==app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05");document.getElementById("cardFooterBrowse").innerText=t("Num entries",
a.totalEntities)}
...
gtPage("Num songs",a.returnedEntities,a.totalEntities)}for(g=f.length-1;g>=b;g--)f[g].remove();1==c&&focusTable(0);setPagination(a.totalEntities,a.returnedEntities);0==b&&(d.innerHTML="All"==app.current.view?'<tr><td><span class="material-icons">error_outline</span></td><td colspan="3>'+t("No playlists found")+"</td></tr>":'<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+(settings.colsBrowsePlaylistsDetail.length-1)+'">'+t("Empty playlist")+"</td></tr>");document.getElementById(app.current.app+
...
function updateDBstarted(a){1==a?(document.getElementById("updateDBfinished").innerText="",document.getElementById("updateDBfooter").classList.add("hide"),a=document.getElementById("updateDBprogress"),a.style.width="20px",a.style.marginLeft="-20px",modalUpdateDB.show(),a.classList.add("updateDBprogressAnimate")):showNotification(t("Database update started"),"","","success")}
...
!0:!1,notificationPage:document.getElementById("btnNotifyPage").classList.contains("active")?!0:!1,jukeboxMode:f.options[f.selectedIndex].value,jukeboxPlaylist:c.options[c.selectedIndex].value,jukeboxQueueLength:document.getElementById("inputJukeboxQueueLength").value,autoPlay:document.getElementById("btnAutoPlay").classList.contains("active")?!0:!1,bgCover:document.getElementById("btnBgCover").classList.contains("active")?!0:!1,bgColor:document.getElementById("inputBgColor").value,bgCssFilter:document.getElementById("inputBgCssFilter").value,
featLocalplayer:document.getElementById("btnFeatLocalplayer").classList.contains("active")?!0:!1,localplayerAutoplay:document.getElementById("btnLocalplayerAutoplay").classList.contains("active")?!0:!1,streamUrl:b,streamPort:d,coverimage:document.getElementById("btnCoverimage").classList.contains("active")?!0:!1,coverimageName:document.getElementById("inputCoverimageName").value,coverimageSize:document.getElementById("inputCoverimageSize").value,locale:g.options[g.selectedIndex].value,love:document.getElementById("btnLoveEnable").classList.contains("active")?
!0:!1,loveChannel:document.getElementById("inputLoveChannel").value,loveMessage:document.getElementById("inputLoveMessage").value,maxElementsPerPage:document.getElementById("inputMaxElementsPerPage").value,stickers:document.getElementById("btnStickers").classList.contains("active")?!0:!1,lastPlayedCount:document.getElementById("inputLastPlayedCount").value,smartpls:document.getElementById("btnSmartpls").classList.contains("active")?!0:!1,taglist:document.getElementById("inputEnabledTags").value.replace(/\s/g,
""),searchtaglist:document.getElementById("inputSearchTags").value.replace(/\s/g,""),browsetaglist:document.getElementById("inputBrowseTags").value.replace(/\s/g,"")}},getSettings);modalSettings.hide()}}function resetSettings(){sendAPI({cmd:"MYMPD_API_SETTINGS_RESET"},getSettings)}function addAllFromBrowseFilesystem(){sendAPI({cmd:"MPD_API_QUEUE_ADD_TRACK",data:{uri:app.current.search}});showNotification(t("Added all songs"),"","","success")}
...
function logWarn(a){logLog(1,"WARN: "+a)}function logInfo(a){logLog(2,"INFO: "+a)}function logVerbose(a){logLog(3,"VERBOSE: "+a)}function logDebug(a){logLog(4,"DEBUG: "+a)}function logLog(a,b){settings.loglevel>=a&&(0==a?console.error(b):1==a?console.warn(b):console.log(b))}function e(a){return isNaN(a)?a.replace(/([<>])/g,function(a,c){if("<"==c)return"&lt;";if(">"==c)return"&gt;"}):a}appInitStart();
