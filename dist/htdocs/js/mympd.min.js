...
k.style.backgroundImage='url("'+subdir+"/browse/music/"+a.result.images[g]+'"),url("assets/coverimage-loading.svg")';d.appendChild(k)}}d=a.result.returnedEntities;g=f.getElementsByTagName("tr");c=document.activeElement.parentNode.parentNode===c?!0:!1;for(var l=k=0;l<d;l++){var m=encodeURI(a.result.data[l].uri),n=document.createElement("tr"),p="";n.setAttribute("data-type",a.result.data[l].Type);n.setAttribute("data-uri",m);n.setAttribute("tabindex",0);"song"===a.result.data[l].Type?n.setAttribute("data-name",
a.result.data[l].Title):n.setAttribute("data-name",a.result.data[l].name);switch(a.result.data[l].Type){case "dir":case "smartpls":case "plist":for(m=0;m<settings["cols"+b].length;m++)p+='<td data-col="'+settings["cols"+b][m]+'">',"Type"===settings["cols"+b][m]?p="dir"===a.result.data[l].Type?p+'<span class="material-icons">folder_open</span>':p+('<span class="material-icons">'+("smartpls"===a.result.data[l].Type?"queue_music":"list")+"</span>"):"Title"===settings["cols"+b][m]&&(p+=e(a.result.data[l].name)),
p+="</td>";p+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";n.innerHTML=p;break;case "song":a.result.data[l].Duration=beautifySongDuration(a.result.data[l].Duration);for(m=0;m<settings["cols"+b].length;m++)p+='<td data-col="'+settings["cols"+b][m]+'">',p="Type"===settings["cols"+b][m]?p+'<span class="material-icons">music_note</span>':p+e(a.result.data[l][settings["cols"+b][m]]),p+="</td>";p+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+
ligatureMore+"</a></td>";n.innerHTML=p}l<g.length?k=!0===replaceTblRow(g[l],n)?l:k:f.append(n)}for(b=g.length-1;b>=d;b--)g[b].remove();!0===c&&focusTable(0);setPagination(a.result.totalEntities,a.result.returnedEntities);0===d&&(f.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+h+'">'+t("Empty list")+"</td></tr>")}document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05")}
...
f.length-1;h>=b;h--)f[h].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}
...
ligatureMore+"</a></td></tr>";d=a.result.data[g].Disc}b.innerHTML=f;document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto("Browse","Database","List")}
...
domCache.body=document.getElementsByTagName("body")[0];domCache.footer=document.getElementsByTagName("footer")[0];domCache.header=document.getElementById("header");domCache.mainMenu=document.getElementById("mainMenu");
...
function appInit(){document.getElementById("btnChVolumeDown").addEventListener("click",function(d){d.stopPropagation()},!1);document.getElementById("btnChVolumeUp").addEventListener("click",function(d){d.stopPropagation()},!1);domCache.volumeBar.addEventListener("click",function(d){d.stopPropagation()},!1);domCache.volumeBar.addEventListener("change",function(){sendAPI("MPD_API_PLAYER_VOLUME_SET",{volume:domCache.volumeBar.value})},!1);domCache.progress.addEventListener("click",function(d){currentSong&&
0<=currentSong.currentSongId&&0<currentSong.totalTime&&(domCache.progressBar.style.transition="none",domCache.progressBar.style.width=d.clientX+"px",setTimeout(function(){domCache.progressBar.style.transition=progressBarTransition},10),sendAPI("MPD_API_PLAYER_SEEK",{songid:currentSong.currentSongId,seek:Math.ceil(currentSong.totalTime*d.clientX/d.target.offsetWidth)}))},!1);domCache.progress.addEventListener("mousemove",function(d){if(("pause"===playstate||"play"===playstate)&&0<currentSong.totalTime){domCache.progressPos.innerText=
beautifySongDuration(Math.ceil(currentSong.totalTime/d.target.offsetWidth*d.clientX));domCache.progressPos.style.display="block";var g=domCache.progressPos.offsetWidth/2;domCache.progressPos.style.left=(d.clientX<g?d.clientX:d.clientX<window.innerWidth-g?d.clientX-g:d.clientX-2*g)+"px"}},!1);domCache.progress.addEventListener("mouseout",function(){domCache.progressPos.style.display="none"},!1);for(var a=document.querySelectorAll(".subMenu"),b=a.length,c=0;c<b;c++)a[c].addEventListener("click",function(d){d.stopPropagation();
d.preventDefault();d=this.getElementsByTagName("span")[0];d.innerText="keyboard_arrow_right"===d.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1);document.getElementById("navbar-main").addEventListener("click",function(d){d.preventDefault();var g=d.target.getAttribute("data-href");null===g&&(g=d.target.parentNode.getAttribute("data-href"));null!==g&&parseCmd(d,g)},!1);document.getElementById("volumeMenu").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",
{},parseOutputs)});document.getElementById("btnDropdownNeighbors").parentNode.addEventListener("show.bs.dropdown",function(){!0===settings.featNeighbors?sendAPI("MPD_API_MOUNT_NEIGHBOR_LIST",{},parseNeighbors,!0):document.getElementById("dropdownNeighbors").children[0].innerHTML='<div class="list-group-item"><span class="material-icons">warning</span> '+t("Neighbors are disabled")+"</div>"});document.getElementById("dropdownNeighbors").children[0].addEventListener("click",function(d){d.preventDefault();
"A"===d.target.nodeName&&(d=d.target.getAttribute("data-value").match(/^(\w+:\/\/)(.+)$/),document.getElementById("selectMountUrlhandler").value=d[1],document.getElementById("inputMountUrl").value=d[2])});document.getElementById("BrowseFilesystemBookmark").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)});document.getElementById("playDropdown").parentNode.addEventListener("show.bs.dropdown",function(){showPlayDropdown()});document.getElementById("playDropdown").addEventListener("click",
function(d){d.preventDefault();d.stopPropagation()});a=document.querySelectorAll(".dropdown-toggle");for(b=0;b<a.length;b++)a[b].parentNode.addEventListener("show.bs.dropdown",function(){alignDropdown(this)});document.getElementById("modalTimer").addEventListener("shown.bs.modal",function(){showListTimer()});document.getElementById("modalMounts").addEventListener("shown.bs.modal",function(){showListMounts()});document.getElementById("modalScripts").addEventListener("shown.bs.modal",function(){showListScripts()});
document.getElementById("modalTrigger").addEventListener("shown.bs.modal",function(){showListTrigger()});document.getElementById("modalPartitions").addEventListener("shown.bs.modal",function(){showListPartitions()});document.getElementById("modalPartitionOutputs").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{partition:"default"},parsePartitionOutputsList,!1)});document.getElementById("modalAbout").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_DATABASE_STATS",
{},parseStats);getServerinfo();var d="",g;for(g in keymap)if(void 0===keymap[g].req||!0===settings[keymap[g].req])d+='<tr><td><div class="key'+(keymap[g].key&&1<keymap[g].key.length?" material-icons material-icons-small":"")+'">'+(void 0!==keymap[g].key?keymap[g].key:g)+"</div></td><td>"+t(keymap[g].desc)+"</td></tr>";document.getElementById("tbodyShortcuts").innerHTML=d});document.getElementById("modalAddToPlaylist").addEventListener("shown.bs.modal",function(){document.getElementById("addStreamFrm").classList.contains("hide")?
...
document.getElementById("homeIconPreview").innerText=document.getElementById("inputHomeIconLigature").value)},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&gotoPage(d.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(d){"P"===d.target.nodeName&&gotoBrowse()},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",function(d){"A"===
d.target.nodeName&&(d.preventDefault(),appGoto("Browse","Filesystem",void 0,"0",app.current.filter,app.current.sort,"-",decodeURI(d.target.getAttribute("data-uri"))))},!1);document.getElementById("tbodySongDetails").addEventListener("click",function(d){if("A"===d.target.nodeName)if("calcFingerprint"===d.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(d.target.getAttribute("data-uri"))},parseFingerprint);d.preventDefault();var g=d.target.parentNode,k=document.createElement("div");
k.classList.add("spinner-border","spinner-border-sm");d.target.classList.add("hide");g.appendChild(k)}else null!==d.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),d.preventDefault(),gotoBrowse());else"BUTTON"===d.target.nodeName&&d.target.getAttribute("data-href")&&parseCmd(d,d.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(d){"BUTTON"===d.target.nodeName?(d.stopPropagation(),d.preventDefault(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",
{output:d.target.getAttribute("data-output-id"),state:d.target.classList.contains("active")?0:1}),toggleBtn(d.target.id)):"A"===d.target.nodeName&&(d.preventDefault(),showListOutputAttributes(decodeURI(d.target.parentNode.getAttribute("data-output-name"))))},!1);document.getElementById("listTimerList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName?d.target.parentNode.classList.contains("not-clickable")||showEditTimer(d.target.parentNode.getAttribute("data-id")):
"A"===d.target.nodeName?deleteTimer(d.target.parentNode.parentNode.getAttribute("data-id")):"BUTTON"===d.target.nodeName&&toggleTimer(d.target,d.target.parentNode.parentNode.getAttribute("data-id"))},!1);document.getElementById("listMountsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(""===d.target.parentNode.getAttribute("data-point"))return!1;showEditMount(decodeURI(d.target.parentNode.getAttribute("data-url")),decodeURI(d.target.parentNode.getAttribute("data-point")))}else if("A"===
d.target.nodeName){var g=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===g?unmountMount(k):"update"===g&&updateMount(d.target,k)}},!1);document.getElementById("listScriptsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(!1===settings.featScripteditor||""===d.target.parentNode.getAttribute("data-script"))return!1;showEditScript(decodeURI(d.target.parentNode.getAttribute("data-script")))}else if("A"===
d.target.nodeName){var g=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-script"));"delete"===g?deleteScript(k):"execute"===g?execScript(d.target.getAttribute("data-href")):"add2home"===g&&addScriptToHome(k,d.target.getAttribute("data-href"))}},!1);document.getElementById("listTriggerList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){var g=decodeURI(d.target.parentNode.getAttribute("data-trigger-id"));
showEditTrigger(g)}else"A"===d.target.nodeName&&(g=d.target.getAttribute("data-action"),d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-trigger-id")),"delete"===g&&deleteTrigger(d))},!1);document.getElementById("listPartitionsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("A"===d.target.nodeName){var g=d.target.getAttribute("data-action");d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-partition"));"delete"===g?deletePartition(d):
"switch"===g&&switchPartition(d)}},!1);document.getElementById("partitionOutputsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName&&(d=decodeURI(d.target.parentNode.getAttribute("data-output")),moveOutput(d),modalPartitionOutputs.hide())},!1);document.getElementById("QueueCurrentList").addEventListener("click",function(d){"TD"===d.target.nodeName?sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:d.target.parentNode.getAttribute("data-trackid")}):"A"===
d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",function(d){"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueJukeboxList").addEventListener("click",function(d){"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(d){if("TD"===d.target.nodeName)switch(d.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":appGoto("Browse",
"Filesystem",void 0,"0",app.current.filter,app.current.sort,"-",decodeURI(d.target.parentNode.getAttribute("data-uri")));break;case "song":appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"))}else"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",
function(d){if("A"===d.target.nodeName){var g=d.target.parentNode.parentNode.getAttribute("data-id"),k=d.target.parentNode.parentNode.getAttribute("data-type"),l=decodeURI(d.target.parentNode.parentNode.getAttribute("data-uri")),m=d.target.parentNode.parentNode.firstChild.innerText,n=d.target.getAttribute("data-href");"delete"===n?(sendAPI("MYMPD_API_BOOKMARK_RM",{id:g},function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)}),d.preventDefault(),d.stopPropagation()):"edit"===n?showBookmarkSave(g,
m,l,k):"goto"===n&&appGoto("Browse","Filesystem",void 0,"0","-","-","-",l)}},!1);document.getElementById("BrowsePlaylistsAllList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("plist",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowsePlaylistsDetailList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("plist",
decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("SearchList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseDatabaseCards").addEventListener("click",function(d){"Album"===
app.current.tag?d.target.classList.contains("card-body")?appGoto("Browse","Database","Detail","0","Album","AlbumArtist",decodeURI(d.target.parentNode.getAttribute("data-album")),decodeURI(d.target.parentNode.getAttribute("data-albumartist"))):d.target.classList.contains("card-footer")&&showMenu(d.target,d):(app.current.search="",document.getElementById("searchDatabaseStr").value="",appGoto(app.current.app,app.current.card,void 0,"0","Album","AlbumArtist","Album","("+app.current.tag+" == '"+decodeURI(d.target.parentNode.getAttribute("data-tag"))+
"')"))},!1);document.getElementById("BrowseDatabaseCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);!1===isMobile&&(document.getElementById("BrowseDatabaseCards").addEventListener("mouseover",function(d){if(d.target.classList.contains("card-body")&&0===d.target.childNodes.length){var g=document.getElementById("BrowseDatabaseCards").getElementsByClassName("album-grid-mouseover");if(1<g.length)for(var k=0;k<g.length;k++)g[k].remove();addPlayButton(d.target)}},!1),document.getElementById("BrowseDatabaseCards").addEventListener("mouseout",
...
function(d){"BUTTON"===d.target.nodeName&&("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromBrowseFilesystem():"Add all to playlist"===d.target.getAttribute("data-phrase")&&showAddToPlaylist(app.current.search,""))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue",null,!1):"Add all to playlist"===d.target.getAttribute("data-phrase")?
showAddToPlaylist("SEARCH",""):"Save as smart playlist"===d.target.getAttribute("data-phrase")&&saveSearchAsSmartPlaylist())},!1);document.getElementById("searchtags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),doSearch(domCache.searchstr.value))},!1);document.getElementById("searchDatabaseTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),appGoto(app.current.app,
...
!1);document.getElementById("databaseSortTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),d.stopPropagation(),app.current.sort=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavPlaylistsDropdown").addEventListener("click",
function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavFilesystemDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("dropdownSortPlaylistTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),playlistSort(d.target.getAttribute("data-tag")))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,
"0",app.current.filter,app.current.sort,"-",this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page,d.target.getAttribute("data-tag"),app.current.sort,"-",app.current.search)},!1);document.getElementById("searchFilesystemStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",""!==
this.value?this.value:"-",app.current.sort,"-",app.current.search)},!1);document.getElementById("searchPlaylistsStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,"-",this.value)},!1);a=["PlaybackColsDropdown"];for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&d.target.classList.contains("material-icons")&&(d.stopPropagation(),
d.preventDefault(),toggleBtnChk(d.target))},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);document.getElementById("searchdatabase").addEventListener("submit",function(){return!1},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&"Album"===app.current.tag)if(""!==this.value){d=
...
dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},!1);document.addEventListener("keydown",function(d){if("INPUT"!==d.target.tagName&&"SELECT"!==d.target.tagName&&"TEXTAREA"!==d.target.tagName&&!d.ctrlKey&&!d.altKey){var g=keymap[d.key];g&&"function"===typeof window[g.cmd]&&(void 0===keymap[d.key].req||!0===settings[keymap[d.key].req])&&
parseCmd(d,g)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(d){navigateTable(this,d.key)},!1);var h="";Object.keys(themes).forEach(function(d){h+='<option value="'+e(d)+'">'+t(themes[d])+"</option>"});document.getElementById("selectTheme").innerHTML=h;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",
function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("localPlayer").addEventListener("click",function(d){d.stopPropagation()});document.getElementById("localPlayer").addEventListener("canplay",function(){logDebug("localPlayer event: canplay");document.getElementById("alertLocalPlayback").classList.add("hide");
document.getElementById("errorLocalPlayback").classList.add("hide")});document.getElementById("localPlayer").addEventListener("error",function(){logError("localPlayer event: error");document.getElementById("errorLocalPlayback").classList.remove("hide")})}
...
function showNotification(a,b,c,f){if(!0===settings.notificationWeb){var h=new Notification(a,{icon:"assets/favicon.ico",body:b});setTimeout(h.close.bind(h),3E3)}if(!0===settings.notificationPage||"danger"===f||"warning"===f){alertTimeout&&clearTimeout(alertTimeout);document.getElementById("alertBox")?h=document.getElementById("alertBox"):(h=document.createElement("div"),h.setAttribute("id","alertBox"),h.classList.add("toast"));var d='<div class="toast-header">';d=("success"===f?d+'<span class="material-icons text-success mr-2">info</span>':
"warning"===f?d+'<span class="material-icons text-warning mr-2">warning</span>':d+'<span class="material-icons text-danger mr-2">error</span>')+('<strong class="mr-auto">'+e(a)+'</strong><button type="button" class="ml-2 mb-1 close">&times;</button></div>');if(""!==c||""!==b)d+='<div class="toast-body">'+(""===c?e(b):c)+"</div>";h.innerHTML=d+"</div>";document.getElementById("alertBox")||(document.getElementsByTagName("main")[0].append(h),requestAnimationFrame(function(){var g=document.getElementById("alertBox");
g&&g.classList.add("alertBoxActive")}));h.getElementsByTagName("button")[0].addEventListener("click",function(){hideNotification()},!1);alertTimeout=setTimeout(function(){hideNotification()},3E3)}setStateIcon();logMessage(a,b,c,f)}
...
var d="";if("Browse"===app.current.app&&"Filesystem"===app.current.tab||"Search"===app.current.app||"Browse"===app.current.app&&"Database"===app.current.tab)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,h,f]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+("plist"!==b&&"smartpls"!==b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,
""]},t("Add to playlist")):"")+("song"===b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("plist"===b||"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+("plist"!==b&&"smartpls"!==b||!0!==settings.featHome?"":addMenuItem({cmd:"addPlistToHome",options:[c,f]},t("Add to homescreen"))),"Filesystem"===app.current.tab&&(d+=("dir"===b&&settings.featBookmarks?addMenuItem({cmd:"showBookmarkSave",options:[-1,f,c,b]},t("Add bookmark")):"")+("dir"===b?
addMenuItem({cmd:"updateDB",options:[dirname(c),!0]},t("Update directory")):"")+("dir"===b?addMenuItem({cmd:"rescanDB",options:[dirname(c),!0]},t("Rescan directory")):"")),"Search"===app.current.app&&(c=dirname(c),d+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-small-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+addMenuItem({cmd:"appendQueue",
options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"appendAfterQueue",options:[b,c,h,f]},t("Add after current playing song"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+"</div>");else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"All"===app.current.view)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",
...
addMenuItem({cmd:"removeFromPlaylist",options:[h.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Current"===app.current.tab)d+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},
t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-1,-1]},t("Remove all downwards"))+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+
addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Jukebox"===app.current.tab)d+=addMenuItem({cmd:"songDetails",options:[c]},t("Song details"))+addMenuItem({cmd:"delQueueJukeboxSong",options:[a.parentNode.parentNode.getAttribute("data-pos")]},t("Remove"));
else if("Home"===app.current.app){h=parseInt(a.parentNode.getAttribute("data-pos"));var g=JSON.parse(a.parentNode.getAttribute("data-href")),k="";"replaceQueue"===g.cmd&&"plist"===g.options[0]?(b="plist",c=g.options[1],k=t("Add and play playlist"),f=t("Playlist")):"appGoto"===g.cmd?(b="view",k=t("Goto view"),f=t("View")):"execScriptFromOptions"===g.cmd&&(b="script",k=t("Execute script"),f=t("Script"));d+='<h6 class="dropdown-header">'+f+"</h6>"+addMenuItem({cmd:"executeHomeIcon",options:[h]},k)+("plist"===
b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+addMenuItem({cmd:"editHomeIcon",options:[h]},t("Edit home icon"))+addMenuItem({cmd:"duplicateHomeIcon",options:[h]},t("Duplicate home icon"))+addMenuItem({cmd:"deleteHomeIcon",options:[h]},t("Delete home icon"))}new BSN.Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+d+"</div></div>",content:" "});b=a.Popover;null===a.getAttribute("data-init")&&
(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",function(l){l.target.setAttribute("data-popover","true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(n){n.preventDefault();n.stopPropagation();"A"===n.target.nodeName&&(n=n.target.getAttribute("data-href"))&&(n=JSON.parse(b64DecodeUnicode(n)),parseCmd(l,n),hideMenu())},!1);document.getElementsByClassName("popover-content")[0].addEventListener("keydown",function(n){n.preventDefault();
n.stopPropagation();if("ArrowDown"===n.key||"ArrowUp"===n.key){var p=this.getElementsByTagName("a");p=Array.prototype.slice.call(p);var q=p.indexOf(document.activeElement);do if(q="ArrowUp"===n.key?1<q?q-1:0:"ArrowDown"===n.key?q<p.length-1?q+1:q:q,0===q||q===p.length-1)break;while(!p[q].offsetHeight);p[q]&&p[q].focus()}else"Enter"===n.key?n.target.click():"Escape"===n.key&&hideMenu()},!1);var m=document.getElementById("advancedMenuLink");m&&(m.addEventListener("click",function(){var n=this.getElementsByTagName("span")[0];
n.innerText="keyboard_arrow_right"===n.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new BSN.Collapse(m));document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));b.show()}
...
a+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):""),a+="</p></div>";document.getElementById("cardPlaybackTags").innerHTML=a;(a=document.getElementById("currentLyrics"))&&lastSongObj.uri&&(b=a.getElementsByTagName("small")[0],b.classList.add("clickable"),b.addEventListener("click",function(k){k.target.parentNode.children[1].classList.toggle("expanded")},!1),getLyrics(lastSongObj.uri,a.getElementsByTagName("p")[0]))}settings.tags.includes("Title")&&(app.apps.Search.sort=
"Title");settings.tags.includes("AlbumArtist")||"AlbumArtist"!==app.apps.Browse.tabs.Database.views.List.filter||(app.apps.Browse.tabs.Database.views.List.filter="Artist",app.apps.Browse.tabs.Database.views.List.sort="Artist");!0===settings.featPlaylists?sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(k){getAllPlaylists(k,"selectJukeboxPlaylist",settings.jukeboxPlaylist)}):document.getElementById("selectJukeboxPlaylist").innerHTML='<option value="Database">'+t("Database")+"</option>";
setCols("QueueCurrent");setCols("Search");setCols("QueueLastPlayed");setCols("QueueJukebox");setCols("BrowseFilesystem");setCols("BrowsePlaylistsDetail");setCols("BrowseDatabaseDetail");setCols("Playback");addTagList("BrowseDatabaseByTagDropdown","browsetags");addTagList("BrowseNavPlaylistsDropdown","browsetags");addTagList("BrowseNavFilesystemDropdown","browsetags");addTagList("searchqueuetags","searchtags");addTagList("searchtags","searchtags");addTagList("searchDatabaseTags","browsetags");addTagList("databaseSortTagsList",
"browsetags");addTagList("dropdownSortPlaylistTags","tags");addTagList("saveSmartPlaylistSort","tags");addTagListSelect("selectSmartplsSort","tags");addTagListSelect("saveSmartPlaylistSort","tags");addTagListSelect("selectJukeboxUniqueTag","browsetags");initTagMultiSelect("inputEnabledTags","listEnabledTags",settings.allmpdtags,settings.tags);initTagMultiSelect("inputSearchTags","listSearchTags",settings.tags,settings.searchtags);initTagMultiSelect("inputBrowseTags","listBrowseTags",settings.tags,
settings.browsetags);initTagMultiSelect("inputGeneratePlsTags","listGeneratePlsTags",settings.browsetags,settings.generatePlsTags)}function resetSettings(){sendAPI("MYMPD_API_SETTINGS_RESET",{},getSettings)}
...
function filterCols(a){var b=settings.tags.slice();!1===settings.featTags&&b.push("Title");b.push("Duration");"colsQueueCurrent"===a||"colsBrowsePlaylistsDetail"===a||"colsQueueLastPlayed"===a||"colsQueueJukebox"===a?b.push("Pos"):"colsBrowseFilesystem"===a&&b.push("Type");"colsQueueLastPlayed"===a&&b.push("LastPlayed");"colsPlayback"===a&&(b.push("Filetype"),b.push("Fileformat"),b.push("LastModified"),!0===settings.featLyrics&&b.push("Lyrics"));for(var c=[],f=0;f<settings[a].length;f++)b.includes(settings[a][f])&&
c.push(settings[a][f]);settings[a]=c;logDebug("Columns for "+a+": "+c)}
...
for(var f=0;f<c.length;f++)!0===b?c[f].classList.remove("hide"):c[f].classList.add("hide");if(!0===b){b=[subdir+"/albumart/"+a.result.uri];for(c=0;c<a.result.images.length;c++)!1===isCoverfile(a.result.images[c])&&b.push(subdir+"/browse/music/"+a.result.images[c]);a=document.getElementById("tabSongPics");createImgCarousel(a,"songPicsCarousel",b)}else document.getElementById("tabSongPics").innerText=""}
...
function _setCurrentCover(a,b){if(void 0===a)clearCurrentCover();else{for(var c=b.querySelectorAll(".coverbg"),f=0;f<c.length;f++)"2"===c[f].style.zIndex?c[f].remove():c[f].style.zIndex="2";c=document.createElement("div");c.classList.add("coverbg","carousel");c.style.backgroundImage='url("'+subdir+"/albumart/"+a+'")';c.style.opacity=0;b.insertBefore(c,b.firstChild);c=new Image;c.onload=function(){b.querySelector(".coverbg").style.opacity=1};c.src=subdir+"/albumart/"+a}}
...
function songChange(a){var b=a.result.Title+":"+a.result.Artist+":"+a.result.Album+":"+a.result.uri+":"+a.result.currentSongId;if(lastSong!==b){var c="",f="",h="";mediaSessionSetMetadata(a.result.Title,a.result.Artist,a.result.Album,a.result.uri);setCurrentCover(a.result.uri);!0===settings.bgCover&&!0===settings.featCoverimage&&setBackgroundImage(a.result.uri);void 0!==a.result.Artist&&0<a.result.Artist.length&&"-"!==a.result.Artist?(c+=a.result.Artist,f+=a.result.Artist,h+=a.result.Artist+" - ",
domCache.footerArtist.innerText=a.result.Artist,domCache.footerArtist.setAttribute("data-name",encodeURI(a.result.Artist)),domCache.footerArtist.classList.add("clickable")):(domCache.footerArtist.innerText="",domCache.footerArtist.setAttribute("data-name",""),domCache.footerArtist.classList.remove("clickable"));void 0!==a.result.Album&&0<a.result.Album.length&&"-"!==a.result.Album?(c+=" - "+a.result.Album,f+="<br/>"+a.result.Album,domCache.footerAlbum.innerText=a.result.Album,domCache.footerAlbum.setAttribute("data-name",
encodeURI(a.result.Album)),domCache.footerAlbum.classList.add("clickable")):(domCache.footerAlbum.innerText="",domCache.footerAlbum.setAttribute("data-name",""),domCache.footerAlbum.classList.remove("clickable"));void 0!==a.result.Title&&0<a.result.Title.length?(h+=a.result.Title,domCache.currentTitle.innerText=a.result.Title,domCache.currentTitle.setAttribute("data-uri",encodeURI(a.result.uri)),domCache.footerTitle.innerText=a.result.Title,domCache.footerTitle.classList.add("clickable")):(domCache.currentTitle.innerText=
"",domCache.currentTitle.setAttribute("data-uri",""),domCache.footerTitle.innerText="",domCache.footerTitle.setAttribute("data-name",""),domCache.footerTitle.classList.remove("clickable"));document.title="myMPD: "+h;domCache.footerCover.title=h;void 0!==a.result.uri&&""!==a.result.uri&&-1===a.result.uri.indexOf("://")?domCache.footerTitle.classList.add("clickable"):domCache.footerTitle.classList.remove("clickable");void 0!==a.result.uri?(!0===settings.featStickers&&setVoteSongBtns(a.result.like,a.result.uri),
a.result.Filetype=filetype(a.result.uri),document.getElementById("addCurrentSongToPlaylist").removeAttribute("disabled")):(a.result.Filetype="",document.getElementById("addCurrentSongToPlaylist").setAttribute("disabled","disabled"));a.result.Fileformat=lastState?fileformat(lastState.audioFormat):"";for(h=0;h<settings.colsPlayback.length;h++){var d=document.getElementById("current"+settings.colsPlayback[h]);if(d&&"Lyrics"===settings.colsPlayback[h])getLyrics(a.result.uri,d.getElementsByTagName("p")[0]);
else if(d){var g=a.result[settings.colsPlayback[h]];void 0===g&&(g="");"Duration"===settings.colsPlayback[h]?g=beautifySongDuration(g):"LastModified"===settings.colsPlayback[h]&&(g=localeDate(g));d.getElementsByTagName("p")[0].innerText=g;d.setAttribute("data-name",encodeURI(g))}}document.getElementById("currentBooklet").innerHTML=""===a.result.bookletPath||void 0===a.result.bookletPath||!1===settings.featBrowse?"":'<span class="text-light material-icons">description</span>&nbsp;<a class="text-light" target="_blank" href="'+
...
function setCols(a,b){var c=document.getElementById(a+"ColsDropdown");c&&(c.firstChild.innerHTML=setColsChecklist(a));var f=app.current.sort;"Search"===a&&"Title"===app.apps.Search.sort&&(f=settings.tags.includes("Title")?"Title":!1===settings.featTags?"Filename":"-");if("Playback"!==a){c="";for(var h=0;h<settings["cols"+a].length;h++){var d=settings["cols"+a][h];c+='<th draggable="true" data-col="'+d+'">';if("Track"===d||"Pos"===d)d="#";c+=t(d);"Search"!==a||d!==f&&"-"+d!==f||(d=!1,0===app.current.sort.indexOf("-")&&
(d=!0),c+='<span class="sort-dir material-icons pull-right">'+(!0===d?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c=!0===settings.featTags?c+'<th data-col="Action"><a href="#" class="text-secondary align-middle material-icons material-icons-small">settings</a></th>':c+"<th></th>";if(void 0===b)document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c;else for(a=document.querySelectorAll(b),b=0;b<a.length;b++)a[b].getElementsByTagName("tr")[0].innerHTML=c}}
...
a||"BrowseNavPlaylistsDropdown"===a)c=!0===settings.featTags?'<button type="button" class="btn btn-secondary btn-sm btn-block" data-tag="Database">'+t("Database")+"</button>":"";"BrowseDatabaseByTagDropdown"===a||"BrowseNavFilesystemDropdown"===a||"BrowseNavPlaylistsDropdown"===a?("BrowseDatabaseByTagDropdown"===a&&(c+='<div class="dropdown-divider"></div>'),c+='<button type="button" class="btn btn-secondary btn-sm btn-block'+("BrowseNavPlaylistsDropdown"===a?" active":"")+'" data-tag="Playlists">'+
t("Playlists")+'</button><button type="button" class="btn btn-secondary btn-sm btn-block'+("BrowseNavFilesystemDropdown"===a?" active":"")+'" data-tag="Filesystem">'+t("Filesystem")+"</button>"):"databaseSortTagsList"===a&&(settings.tags.includes("Date")&&(c+='<button type="button" class="btn btn-secondary btn-sm btn-block" data-tag="Date">'+t("Date")+"</button>"),c+='<button type="button" class="btn btn-secondary btn-sm btn-block" data-tag="Last-Modified">'+t("Last modified")+"</button>");document.getElementById(a).innerHTML=
c}
