...
dropdownBookmarks=new Dropdown(document.getElementById("BrowseFilesystemBookmark")),collapseDBupdate=new Collapse(document.getElementById("navDBupdate"));
...
"disabled")):2==a?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):1==a&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"))});document.getElementById("selectAddToQueueMode").addEventListener("change",function(){var a=
this.options[this.selectedIndex].value;2==a?(document.getElementById("inputAddToQueueQuantity").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").value="Database"):1==a&&(document.getElementById("inputAddToQueueQuantity").removeAttribute("disabled"),document.getElementById("selectAddToQueuePlaylist").removeAttribute("disabled"))});document.getElementById("addToPlaylistPlaylist").addEventListener("change",
function(a){"New Playlist"==this.options[this.selectedIndex].text?(document.getElementById("addToPlaylistNewPlaylistDiv").classList.remove("hide"),document.getElementById("addToPlaylistNewPlaylist").focus()):document.getElementById("addToPlaylistNewPlaylistDiv").classList.add("hide")},!1);document.getElementById("selectMusicDirectory").addEventListener("change",function(a){"auto"==this.options[this.selectedIndex].value?(document.getElementById("inputMusicDirectory").value=settings.musicDirectoryValue,
document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):"none"==this.options[this.selectedIndex].value?(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").removeAttribute("readonly"))},!1);addFilterLetter("BrowseFilesystemFilterLetters");addFilterLetter("BrowseDatabaseFilterLetters");
...
function webSocketConnect(){if(null!=socket)log_info("Socket already connected");else{var a=getWsUrl();socket=new WebSocket(a);log_info("Connecting to "+a);try{socket.onopen=function(){log_info("Websocket is connected");websocketConnected=!0;null!=websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null)},socket.onmessage=function(b){try{var c=JSON.parse(b.data);log_debug("Websocket notification: "+c.type)}catch(d){log_error("Invalid JSON data received: "+b.data)}switch(c.type){case "welcome":websocketConnected=
!0;showNotification("Connected to myMPD: "+a,"","","success");appRoute();sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState,!0);break;case "update_state":parseState(c);break;case "mpd_disconnected":progressTimer&&clearTimeout(progressTimer);getSettings(!0);break;case "mpd_connected":showNotification("Connected to MPD","","","success");sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);getSettings(!0);break;case "update_queue":"Queue"===app.current.app&&getQueue();parseUpdateQueue(c);break;case "update_options":getSettings();
...
function parseSettings(){1==settings.mpdConnected&&parseMPDSettings();0!=settings.mpdHost.indexOf("/")?document.getElementById("mpdInfo_host").innerText=settings.mpdHost+":"+settings.mpdPort:document.getElementById("mpdInfo_host").innerText=settings.mpdHost;document.getElementById("inputMpdHost").value=settings.mpdHost;document.getElementById("inputMpdPort").value=settings.mpdPort;document.getElementById("inputMpdPass").value=settings.mpdPass;var a=document.getElementById("btnNotifyWeb");notificationsSupported()?
settings.notificationWeb?(toggleBtnChk("btnNotifyWeb",settings.notificationWeb),Notification.requestPermission(function(a){"permission"in Notification||(Notification.permission=a);"granted"===a?toggleBtnChk("btnNotifyWeb",1):(toggleBtnChk("btnNotifyWeb",0),settings.notificationWeb=!0)})):toggleBtnChk("btnNotifyWeb",0):(a.setAttribute("disabled","disabled"),toggleBtnChk("btnNotifyWeb",0));toggleBtnChk("btnNotifyPage",settings.notificationPage);toggleBtnChk("btnBgCover",settings.bgCover);document.getElementById("inputBgColor").value=
settings.bgColor;document.getElementById("inputBgCssFilter").value=settings.bgCssFilter;toggleBtnChk("btnFeatLocalplayer",settings.featLocalplayer);toggleBtnChk("btnLocalplayerAutoplay",settings.localplayerAutoplay);""==settings.streamUrl?(document.getElementById("selectStreamMode").value="port",document.getElementById("inputStreamUrl").value=settings.streamPort):(document.getElementById("selectStreamMode").value="url",document.getElementById("inputStreamUrl").value=settings.streamUrl);toggleBtnChk("btnCoverimage",
settings.coverimage);document.getElementById("inputCoverimageName").value=settings.coverimageName;document.getElementById("inputCoverimageSize").value=settings.coverimageSize;document.getElementById("selectLocale").value=settings.locale;document.documentElement.style.setProperty("--mympd-coverimagesize",settings.coverimageSize+"px");document.documentElement.style.setProperty("--mympd-backgroundcolor",settings.bgColor);document.documentElement.style.setProperty("--mympd-backgroundfilter",settings.bgCssFilter);
toggleBtnChk("btnLoveEnable",settings.love);document.getElementById("inputLoveChannel").value=settings.loveChannel;document.getElementById("inputLoveMessage").value=settings.loveMessage;document.getElementById("inputMaxElementsPerPage").value=settings.maxElementsPerPage;toggleBtnChk("btnStickers",settings.stickers);document.getElementById("inputLastPlayedCount").value=settings.lastPlayedCount;toggleBtnChk("btnSmartpls",settings.smartpls);for(var b=["featLocalplayer","featSyscmds"],c=0;c<b.length;c++){var d=
document.getElementsByClassName(b[c]),e=d.length,f=1==settings[b[c]]?"":"none";for(a=0;a<e;a++)d[a].style.display=f}if(settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(b='<div class="dropdown-divider"></div>',a=0;a<c;a++)b="HR"==settings.syscmdList[a]?b+'<div class="dropdown-divider"></div>':b+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+settings.syscmdList[a]+"\"]}'>"+settings.syscmdList[a]+"</a>");document.getElementById("syscmds").innerHTML=
b}else document.getElementById("syscmds").innerHTML="";dropdownMainMenu=new Dropdown(document.getElementById("mainMenu"));document.getElementById("selectJukeboxMode").value=settings.jukeboxMode;document.getElementById("inputJukeboxQueueLength").value=settings.jukeboxQueueLength;0==settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled")):2==settings.jukeboxMode?
(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):1==settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));1==settings.featLocalplayer&&(""==settings.streamUrl?(settings.mpdstream="http://",
...
"Filesystem"==app.current.tab?appRoute():"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view?appRoute():"Browse"==app.current.app&&"Database"==app.current.tab&&""!=app.current.search&&appRoute();settingsParsed="true"}
...
function getAllPlaylists(a){var b=a.data.length,c="";if(0==a.offset)if("addToPlaylistPlaylist"==playlistEl)c='<option value=""></option><option value="New Playlist">New Playlist</option>';else if("selectJukeboxPlaylist"==playlistEl||"selectAddToQueuePlaylist"==playlistEl)c='<option value="Database">Database</option>';for(var d=0;d<b;d++)c+='<option value="'+a.data[d].uri+'"',"selectJukeboxPlaylist"==playlistEl&&a.data[d].uri==settings.jukeboxPlaylist&&(c+=" selected"),c+=">"+a.data[d].uri+"</option>";
0==a.offset?document.getElementById(playlistEl).innerHTML=c:document.getElementById(playlistEl).innerHTML+=c;a.totalEntities>a.returnedEntities&&(a.offset+=settings.maxElementsPerPage,sendAPI({cmd:"MPD_API_PLAYLIST_LIST",data:{offset:a.offset,filter:"-"}},getAllPlaylists))}function updateSmartPlaylists(){sendAPI({cmd:"MPD_API_SMARTPLS_UPDATE_ALL"})}function loveSong(){sendAPI({cmd:"MPD_API_LOVE",data:{}})}
...
options:[b,c,d]},"Replace queue")+("smartpls"==b?addMenuItem({cmd:"playlistDetails",options:[c]},"View playlist"):addMenuItem({cmd:"playlistDetails",options:[c]},"Edit playlist"))+("smartpls"==b?addMenuItem({cmd:"showSmartPlaylist",options:[c]},"Edit smart playlist"):"")+(1==settings.smartpls&&"smartpls"==b?addMenuItem({cmd:"updateSmartPlaylist",options:[c]},"Update smart playlist"):"")+addMenuItem({cmd:"showRenamePlaylist",options:[c]},"Rename playlist")+addMenuItem({cmd:"showDelPlaylist",options:[c]},
"Delete playlist"):"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view?(e=document.getElementById("BrowsePlaylistsDetailList"),f+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},"Append to queue")+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},"Replace queue")+("false"==e.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[e.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},"Remove"):"")+addMenuItem({cmd:"showAddToPlaylist",
options:[c]},"Add to playlist")+(-1==c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},"Songdetails"):"")):"Queue"==app.current.app&&"Current"==app.current.tab?f+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},"Remove")+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},"Remove all upwards")+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-
...
function validatePath(a){if(""==a.value)return a.classList.add("is-invalid"),!1;if(null!=a.value.match(/^\/[\/\.\w\-]+$/))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function localeDate(a){return(new Date(1E3*a)).toLocaleString(settings.locale)}function validatePlname(a){return""==a?!1:null==a.match(/\/|\r|\n|"|'/)?!0:!1}
