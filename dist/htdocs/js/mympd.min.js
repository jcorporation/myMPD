...
a=a.substring(a.indexOf(" ")+1),a=a.substring(0,a.indexOf(" ")),""==a&&(a="contains"),document.getElementById("searchMatch").value=a)}else""==domCache.searchstr.value&&""!=app.current.search&&(domCache.searchstr.value=app.current.search);app.last.app!=app.current.app&&""!=app.current.search&&(a=settings["cols"+app.current.app].length,a--,document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+t("Searching...")+
"</td></tr>");2<=domCache.searchstr.value.length||0<domCache.searchCrumb.children.length?settings.featAdvsearch?(a=app.current.sort,b=!1,"-"==a?(a=settings.tags.includes("Title")?"Title":"-",document.getElementById("SearchList").setAttribute("data-sort",a)):0==a.indexOf("-")&&(b=!0,a=a.substring(1)),sendAPI({cmd:"MPD_API_DATABASE_SEARCH_ADV",data:{plist:"",offset:app.current.page,sort:a,sortdesc:b,expression:app.current.search,cols:settings.colsSearch}},parseSearch)):sendAPI({cmd:"MPD_API_DATABASE_SEARCH",
data:{plist:"",offset:app.current.page,filter:app.current.filter,searchstr:app.current.search,cols:settings.colsSearch}},parseSearch):(document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML="",document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"),document.getElementById("panel-heading-search").innerText="",document.getElementById("cardFooterSearch").innerText="",
document.getElementById("SearchList").classList.remove("opacity05"),setPagination(0,0));selectTag("searchtags","searchtagsdesc",app.current.filter)}else appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appGoto("Playback")}}
...
function appInitStart(){subdir=window.location.pathname.replace("/index.html","").replace(/\/$/,"");for(var a='<option value="default" data-phrase="Browser default"></option>',b=0;b<locales.length;b++)a+='<option value="'+locales[b].code+'">'+locales[b].desc+" ("+locales[b].code+")</option>";document.getElementById("selectLocale").innerHTML=a;i18nHtml(document.getElementById("modalAppInit"));"serviceWorker"in navigator&&"https"==document.URL.substring(0,5)&&"localhost"!=window.location.hostname&&
...
for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"H4"==a.target.nodeName&&gotoBrowse(a.target)},!1);document.getElementById("modalSongDetails").getElementsByTagName("tbody")[0].addEventListener("click",function(a){"A"==a.target.nodeName?void 0!=a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),
...
function(a){if("A"==a.target.nodeName){var b=a.target.parentNode.parentNode.getAttribute("data-id"),c=a.target.parentNode.parentNode.getAttribute("data-type"),d=decodeURI(a.target.parentNode.parentNode.getAttribute("data-uri")),h=a.target.parentNode.parentNode.firstChild.innerText,k=a.target.getAttribute("data-href");"delete"==k?(sendAPI({cmd:"MYMPD_API_BOOKMARK_RM",data:{id:b}},function(){sendAPI({cmd:"MYMPD_API_BOOKMARK_LIST",data:{offset:0}},parseBookmarks)}),a.preventDefault(),a.stopPropagation()):
...
!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(a){if(settings.featAdvsearch&&"TH"==a.target.nodeName){var b=a.target.getAttribute("data-col");if("Duration"!=b){var c=app.current.sort,d=!0;if(c==b||c=="-"+b)0==c.indexOf("-")?(d=!0,c.substring(1)):d=!1;0==d?(c="-"+b,d=!0):(d=!1,c=b);for(var h=document.getElementById("SearchList").getElementsByClassName("sort-dir"),k=0;k<h.length;k++)h[k].remove();app.current.sort=c;a.target.innerHTML=b+'<span class="sort-dir material-icons pull-right">'+
...
deferredPrompt.prompt();deferredPrompt.userChoice.then(function(a){"accepted"===a.outcome?logVerbose("User accepted the A2HS prompt"):logVerbose("User dismissed the A2HS prompt");deferredPrompt=null})});window.addEventListener("appinstalled",function(a){logInfo("myMPD installed as app");showNotification(t("myMPD installed as app"),"","","success")});window.addEventListener("beforeunload",function(){null!=websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null);socket.onclose=function(){};
null!=socket&&(socket.close(),socket=null);websocketConnected=!1});document.getElementById("localPlayer").addEventListener("canplay",function(){document.getElementById("alertLocalPlayback").classList.add("hide");1==settings.featLocalplayer&&1==settings.localplayerAutoplay&&localplayerPlay()})}
...
function t(a,b,c){var d=void 0;isNaN(b)&&(c=b);phrases[a]&&(d=phrases[a][locale],void 0==d&&("en-US"!=locale&&logDebug('Phrase "'+a+'" for locale '+locale+" not found"),d=phrases[a]["en-US"]));void 0==d&&(d=a);0==isNaN(b)&&(a=d.split(" |||| "),1<a.length&&(d=a[smartCount(b)]),d=d.replace("%{smart_count}",b));null!=c&&(d=d.replace(/%\{(\w+)\}/,function(a,b){return c[b]}));return d}
function i18nHtml(a){for(var b=[["data-phrase","innerText"],["data-title-phrase","title"],["data-placeholder-phrase","placeholder"]],c=0;c<b.length;c++)for(var d=a.querySelectorAll("["+b[c][0]+"]"),e=d.length,f=0;f<e;f++)d[f][b[c][1]]=t(d[f].getAttribute(b[c][0]))}
...
toggleBtnChk("btnSmartpls",settings.smartpls);for(var b=["featLocalplayer","featSyscmds"],c=0;c<b.length;c++){var d=document.getElementsByClassName(b[c]),e=d.length,f=1==settings[b[c]]?"":"none";for(a=0;a<e;a++)d[a].style.display=f}if(settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(b='<div class="dropdown-divider"></div>',a=0;a<c;a++)b="HR"==settings.syscmdList[a]?b+'<div class="dropdown-divider"></div>':b+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+
settings.syscmdList[a]+"\"]}'>"+settings.syscmdList[a]+"</a>");document.getElementById("syscmds").innerHTML=b}else document.getElementById("syscmds").innerHTML="";dropdownMainMenu=new Dropdown(document.getElementById("mainMenu"));document.getElementById("selectJukeboxMode").value=settings.jukeboxMode;document.getElementById("inputJukeboxQueueLength").value=settings.jukeboxQueueLength;0==settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled",
"disabled")):2==settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):1==settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));1==settings.featLocalplayer&&(""==settings.streamUrl?
...
"Filesystem"==app.current.tab?appRoute():"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view?appRoute():"Browse"==app.current.app&&"Database"==app.current.tab&&""!=app.current.search&&appRoute();i18nHtml(document.getElementsByTagName("body")[0]);settingsParsed="true"}
...
d[e]+'">&nbsp;&nbsp;'+t(d[e])+"</label></div>";document.getElementById(a+"ColsDropdown").firstChild.innerHTML=c;d=app.current.sort;"Search"==a&&"0/any/Title/"==app.apps.Search.state&&(d=settings.tags.includes("Title")?"Title":0==settings.featTags?"Filename":"-");if("Playback"!=a){c="";for(e=0;e<settings["cols"+a].length;e++){var f=settings["cols"+a][e];c+='<th draggable="true" data-col="'+f+'">';if("Track"==f||"Pos"==f)f="#";c+=t(f);"Search"!=a||f!=d&&"-"+f!=d||(f=!1,0==app.current.sort.indexOf("-")&&
(f=!0),c+='<span class="sort-dir material-icons pull-right">'+(1==f?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c+="<th></th>";if(void 0==b)document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c;else for(a=document.querySelectorAll(b),e=0;e<a.length;e++)a[e].getElementsByTagName("tr")[0].innerHTML=c}}function getSettings(a){0==settingsLock&&(settingsLock=!0,sendAPI({cmd:"MYMPD_API_SETTINGS_GET"},getMpdSettings,a))}
...
function saveColsPlayback(a){for(var b=document.getElementById(a+"ColsDropdown").firstChild.getElementsByTagName("input"),c=document.getElementById("cardPlaybackTags"),d=0;d<b.length;d++){var e=document.getElementById("current"+b[d].name);0==b[d].checked?e&&e.remove():e||(e=document.createElement("div"),e.innerHTML="<small>"+t(b[d].name)+"</small><h4></h4>",e.setAttribute("id","current"+b[d].name),e.setAttribute("data-tag",b[d].name),c.appendChild(e))}a={cmd:"MYMPD_API_COLS_SAVE",data:{table:"cols"+
...
function saveConnection(){var a=!0,b=document.getElementById("inputMpdHost"),c=document.getElementById("inputMpdPort"),d=document.getElementById("inputMpdPass"),e=document.getElementById("selectMusicDirectory");e=e.options[e.selectedIndex].value;"custom"==e&&(e=document.getElementById("inputMusicDirectory"),validatePath(e)||(a=!1),e=e.value);""==c.value&&(c.value="6600");0!=b.value.indexOf("/")&&(validateInt(c)||(a=!1),validateHost(b)||(a=!1));1==a&&(sendAPI({cmd:"MYMPD_API_CONNECTION_SAVE",data:{mpdHost:b.value,
mpdPort:c.value,mpdPass:d.value,musicDirectory:e}},getSettings),modalConnection.hide())}function parseOutputs(a){for(var b="",c=a.data.outputs.length,d=0;d<c;d++)b+='<button id="btnOutput'+a.data.outputs[d].id+'" data-output-id="'+a.data.outputs[d].id+'" class="btn btn-secondary btn-block',1==a.data.outputs[d].state&&(b+=" active"),b+='"><span class="material-icons float-left">volume_up</span> '+a.data.outputs[d].name+"</button>";domCache.outputs.innerHTML=b}
...
d=settings.colsQueueLastPlayed.length;d--;0==b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("Empty list")+"</td></tr>");setPagination(a.totalEntities,a.returnedEntities);document.getElementById("QueueLastPlayedList").classList.remove("opacity05")}
function parseSearch(a){-1<a.totalEntities?(document.getElementById("panel-heading-search").innerText=t("Num songs",a.totalEntities),document.getElementById("cardFooterSearch").innerText=t("Num songs",a.totalEntities)):a.returnedEntities+app.current.page<settings.maxElementsPerPage?(document.getElementById("panel-heading-search").innerText=t("Num songs",a.returnedEntities),document.getElementById("cardFooterSearch").innerText=t("Num songs",a.returnedEntities)):(document.getElementById("panel-heading-search").innerHTML=
"&ge; "+t("Num songs",settings.maxElementsPerPage),document.getElementById("cardFooterSearch").innerHTML="&ge; "+t("Num songs",settings.maxElementsPerPage));0<a.returnedEntities?(document.getElementById("searchAddAllSongs").removeAttribute("disabled"),document.getElementById("searchAddAllSongsBtn").removeAttribute("disabled")):(document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"));parseFilesystem(a)}
...
"true"),document.getElementById("btnPlaylistClear").parentNode.classList.add("hide")):(document.getElementById("BrowsePlaylistsDetailList").setAttribute("data-ro","false"),document.getElementById("btnPlaylistClear").parentNode.classList.remove("hide")),document.getElementById("BrowsePlaylistsDetailList").setAttribute("data-uri",a.uri),1==a.smartpls?document.getElementById("BrowsePlaylistsDetailList").getElementsByTagName("caption")[0].innerHTML=t("Smart playlist")+": "+a.uri+'<small class="pull-right">'+
a.totalEntities+" Songs </small>":document.getElementById("BrowsePlaylistsDetailList").getElementsByTagName("caption")[0].innerHTML=t("Playlist")+": "+a.uri+'<small class="pull-right">'+a.totalEntities+" Songs </small>",document.getElementById("BrowsePlaylistsDetailList").classList.remove("hide"),document.getElementById("BrowsePlaylistsAllList").classList.add("hide"),document.getElementById("btnBrowsePlaylistsAll").parentNode.classList.remove("hide"),settings.featTags&&document.getElementById("BrowsePlaylistsDetailColsBtn").parentNode.classList.remove("hide"));
...
"</span></td><td>"+a.data[e].name+"</td><td>"+localeDate(a.data[e].last_modified)+'</td><td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';e<d.length?d[e].replaceWith(g):c.append(g)}document.getElementById("cardFooterBrowse").innerText=t("Num playlists",a.totalEntities)}else if("Detail"==app.current.view){for(e=0;e<b;e++){f=encodeURI(a.data[e].uri);g=document.createElement("tr");0==a.smartpls&&g.setAttribute("draggable","true");g.setAttribute("id","playlistTrackId"+
...
g.innerHTML=f;e<d.length?d[e].replaceWith(g):c.append(g)}document.getElementById("cardFooterBrowse").innerText=t("Num songs",a.totalEntities)}for(e=d.length-1;e>=b;e--)d[e].remove();setPagination(a.totalEntities,a.returnedEntities);0==b&&(a=settings["cols"+list].length,a--,c.innerHTML="All"==app.current.view?'<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+a+'">'+t("No playlists found")+"</td></tr>":'<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+
a+'">'+t("Empty list")+"</td></tr>");document.getElementById(app.current.app+app.current.tab+app.current.view+"List").classList.remove("opacity05")}
...
c+('<a class="text-success" href="#">'+a.data[settings.tags[d]]+"</a>"):c+a.data[settings.tags[d]],c+="</td></tr>");c+="<tr><th>"+t("Duration")+"</th><td>"+beautifyDuration(a.data.Duration)+"</td></tr>";c=settings.featLibrary?c+("<tr><th>"+t("Filename")+'</th><td><a class="text-success" href="/library/'+encodeURI(a.data.uri)+'">'+a.data.uri+"</a></td></tr>"):c+("<tr><th>"+t("Filename")+"</th><td>"+a.data.uri+"</td></tr>");1==settings.featStickers&&(c+='<tr><th colspan="2" class="pt-3"><h5>'+t("Statistics")+
...
function getAllPlaylists(a){var b=a.data.length,c="";if(0==a.offset)if("addToPlaylistPlaylist"==playlistEl)c='<option value=""></option><option value="new">'+t("New playlist")+"</option>";else if("selectJukeboxPlaylist"==playlistEl||"selectAddToQueuePlaylist"==playlistEl)c='<option value="Database">'+t("Database")+"</option>";for(var d=0;d<b;d++)c+='<option value="'+a.data[d].uri+'"',"selectJukeboxPlaylist"==playlistEl&&a.data[d].uri==settings.jukeboxPlaylist&&(c+=" selected"),c+=">"+a.data[d].uri+
"</option>";0==a.offset?document.getElementById(playlistEl).innerHTML=c:document.getElementById(playlistEl).innerHTML+=c;a.totalEntities>a.returnedEntities&&(a.offset+=settings.maxElementsPerPage,sendAPI({cmd:"MPD_API_PLAYLIST_LIST",data:{offset:a.offset,filter:"-"}},getAllPlaylists))}function updateSmartPlaylists(){sendAPI({cmd:"MPD_API_SMARTPLS_UPDATE_ALL"})}function loveSong(){sendAPI({cmd:"MPD_API_LOVE",data:{}})}
...
"Search"==app.current.app||"Browse"==app.current.app&&"Database"==app.current.tab?(f+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+("song"==b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,e,d]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("plist"!=b&&"smartpls"!=b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c]},t("Add to playlist")):"")+("song"==b?addMenuItem({cmd:"songDetails",
options:[c]},t("Song details")):"")+("plist"==b||"smartpls"==b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+("dir"==b?addMenuItem({cmd:"showBookmarkSave",options:[-1,d,c,b]},t("Add bookmark")):""),"Search"==app.current.app&&(c=dirname(c),f+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-small-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+
...
options:[c]},t("Delete playlist")):"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view?(e=document.getElementById("BrowsePlaylistsDetailList"),f+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("false"==e.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[e.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+addMenuItem({cmd:"showAddToPlaylist",
options:[c]},t("Add to playlist"))+(-1==c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")):"Queue"==app.current.app&&"Current"==app.current.tab?f+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-
1,-1]},t("Remove all downwards"))+(-1==c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):""):"Queue"==app.current.app&&"LastPlayed"==app.current.tab&&(f+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+addMenuItem({cmd:"showAddToPlaylist",options:[c]},t("Add to playlist"))+(-1==c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):""));new Popover(a,
{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+f+"</div></div>",content:" "});b=a.Popover;a.setAttribute("data-init","true");a.addEventListener("shown.bs.popover",function(a){a.target.setAttribute("data-popover","true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(a){a.preventDefault();a.stopPropagation();if("A"==a.target.nodeName){var b=a.target.getAttribute("data-href");
...
function sendAPI(a,b,c){var d=new XMLHttpRequest;d.open("POST",subdir+"/api",!0);d.setRequestHeader("Content-type","application/json");d.onreadystatechange=function(){if(4==d.readyState)if(""!=d.responseText){var e=JSON.parse(d.responseText);"error"==e.type?(void 0!=e.number?showNotification(t(e.data,e.number,e.values),"","","danger"):showNotification(t(e.data,e.values),"","","success"),logError(e.data),1==c&&void 0!=b&&"function"==typeof b&&(logDebug('Got API response of type "'+e.type+'" calling '+
b.name),b(e))):"result"==e.type&&"ok"!=e.data?(logDebug("Got API response: "+e.data),void 0!=e.number?showNotification(t(e.data,e.number,e.values),"","","success"):showNotification(t(e.data,e.values),"","","success")):void 0!=b&&"function"==typeof b&&(logDebug('Got API response of type "'+e.type+'" calling '+b.name),b(e))}else logError("Empty response for request: "+JSON.stringify(a)),1==c&&void 0!=b&&"function"==typeof b&&(logDebug('Got API response of type "'+e.type+'" calling '+b.name),b(""))};
...
settings.mixrampdelay,notificationWeb:document.getElementById("btnNotifyWeb").classList.contains("active")?!0:!1,notificationPage:document.getElementById("btnNotifyPage").classList.contains("active")?!0:!1,jukeboxMode:e.options[e.selectedIndex].value,jukeboxPlaylist:c.options[c.selectedIndex].value,jukeboxQueueLength:document.getElementById("inputJukeboxQueueLength").value,autoPlay:document.getElementById("btnAutoPlay").classList.contains("active")?!0:!1,bgCover:document.getElementById("btnBgCover").classList.contains("active")?
...
coverimageSize:document.getElementById("inputCoverimageSize").value,locale:f.options[f.selectedIndex].value,love:document.getElementById("btnLoveEnable").classList.contains("active")?!0:!1,loveChannel:document.getElementById("inputLoveChannel").value,loveMessage:document.getElementById("inputLoveMessage").value,maxElementsPerPage:document.getElementById("inputMaxElementsPerPage").value,stickers:document.getElementById("btnStickers").classList.contains("active")?!0:!1,lastPlayedCount:document.getElementById("inputLastPlayedCount").value,
...
function validatePath(a){if(""==a.value)return a.classList.add("is-invalid"),!1;if(null!=a.value.match(/^\/[\/\.\w\-]+$/))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function localeDate(a){return(new Date(1E3*a)).toLocaleString(locale)}function validatePlname(a){return""==a?!1:null==a.match(/\/|\r|\n|"|'/)?!0:!1}function validateNotBlank(a){if(""==a.value.replace(/\s/g,""))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}
...
function validateHost(a){if(null==a.value.match("^(w-.)$"))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function logError(a){logLog(0,"ERROR: "+a)}function logWarn(a){logLog(1,"WARN: "+a)}function logInfo(a){logLog(2,"INFO: "+a)}function logVerbose(a){logLog(3,"VERBOSE: "+a)}function logDebug(a){logLog(4,"DEBUG: "+a)}function logLog(a,b){settings.loglevel>=a&&console.log(b)}appInitStart();
