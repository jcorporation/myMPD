...
function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);_addAlbum(a,c,b)}function _addAlbum(a,b,c){b="((Album == '"+c+"') AND ("+tagAlbumArtist+" == '"+b+"'))";"appendQueue"===a?addAllFromSearchPlist("queue",b,!1):"replaceQueue"===a?addAllFromSearchPlist("queue",b,!0):"addPlaylist"===a&&showAddToPlaylist("ALBUM",b)}
function searchAlbumgrid(a){for(var b="",c=document.getElementById("searchDatabaseCrumb").children,f=0;f<c.length;f++)0<f&&(b+=" AND "),b+="("+decodeURI(c[f].getAttribute("data-filter"))+")";""!==a&&(""!==b&&(b+=" AND "),c=document.getElementById("searchDatabaseMatch"),b+="("+app.current.filter+" "+c.options[c.selectedIndex].value+" '"+a+"')");2>=b.length&&(b="");appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,b)}
...
toggleBtnChk("databaseSortDesc",!0)):toggleBtnChk("databaseSortDesc",!1),selectTag("databaseSortTags",void 0,a),"Album"===app.current.tag){c=document.getElementById("searchDatabaseCrumb");f=document.getElementById("searchDatabaseStr");h="";for(var d=app.current.search.split(" AND "),g=0;g<d.length-1;g++){var k=d[g].substring(1,d[g].length-1);h+='<button data-filter="'+encodeURI(k)+'" class="btn btn-light mr-2">'+e(k)+'<span class="badge badge-secondary">&times</span></button>'}c.innerHTML=h;""===
f.value&&1<=d.length&&(h=d[d.length-1].substring(1,d[d.length-1].length-1),d=h.substring(h.indexOf("'")+1,h.length-1),f.value!==d&&(c.innerHTML+='<button data-filter="'+encodeURI(h)+'" class="btn btn-light mr-2">'+e(h)+'<span class="badge badge-secondary">&times;</span></button>'),document.getElementById("searchDatabaseMatch").value="contains");c.classList.remove("hide");document.getElementById("searchDatabaseMatch").classList.remove("hide");document.getElementById("btnDatabaseSortDropdown").removeAttribute("disabled");
document.getElementById("btnDatabaseSearchDropdown").removeAttribute("disabled");sendAPI("MPD_API_DATABASE_GET_ALBUMS",{offset:app.current.offset,limit:app.current.limit,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b},parseDatabase)}else document.getElementById("searchDatabaseCrumb").classList.add("hide"),document.getElementById("searchDatabaseMatch").classList.add("hide"),document.getElementById("btnDatabaseSortDropdown").setAttribute("disabled","disabled"),document.getElementById("btnDatabaseSearchDropdown").setAttribute("disabled",
"disabled"),document.getElementById("searchDatabaseStr").value=app.current.search,sendAPI("MPD_API_DATABASE_TAG_LIST",{offset:app.current.offset,limit:app.current.limit,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b,tag:app.current.tag},parseDatabase);else if("Browse"===app.current.app&&"Database"===app.current.tab&&"Detail"===app.current.view)document.getElementById("viewListDatabase").classList.add("hide"),document.getElementById("viewDetailDatabase").classList.remove("hide"),
"Album"===app.current.filter&&(a=settings.colsBrowseDatabaseDetail.slice(),!1===a.includes("Disc")&&a.push("Disc"),sendAPI("MPD_API_DATABASE_TAG_ALBUM_TITLE_LIST",{album:app.current.tag,searchstr:app.current.search,tag:app.current.sort,cols:a},parseAlbumDetails));else if("Search"===app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a="";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)f=b[c].substring(1,b[c].length-1),a+='<button data-filter="'+
encodeURI(f)+'" class="btn btn-light mr-2">'+e(f)+'<span class="badge badge-secondary">&times</span></button>';domCache.searchCrumb.innerHTML=a;""===domCache.searchstr.value&&1<=b.length&&(a=b[b.length-1].substring(1,b[b.length-1].length-1),b=a.substring(a.indexOf("'")+1,a.length-1),domCache.searchstr.value!==b&&(domCache.searchCrumb.innerHTML+='<button data-filter="'+encodeURI(a)+'" class="btn btn-light mr-2">'+e(a)+'<span class="ml-2 badge badge-secondary">&times;</span></button>'),a=a.substring(a.indexOf(" ")+
...
else"Album"===app.current.tag?searchAlbumgrid(this.value):appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,this.value)},!1);document.getElementById("searchDatabaseCrumb").addEventListener("click",function(d){d.preventDefault();d.stopPropagation();if("SPAN"===d.target.nodeName)d.target.parentNode.remove(),searchAlbumgrid("");else if("BUTTON"===d.target.nodeName){var g=decodeURI(d.target.getAttribute("data-filter"));document.getElementById("searchDatabaseStr").value=
...
function(d){if("SPAN"===d.target.nodeName)d.preventDefault(),d.stopPropagation(),d.target.parentNode.remove(),doSearch("");else if("BUTTON"===d.target.nodeName){d.preventDefault();d.stopPropagation();var g=decodeURI(d.target.getAttribute("data-filter"));domCache.searchstr.value=g.substring(g.indexOf("'")+1,g.length-1);var k=g.substring(0,g.indexOf(" "));selectTag("searchtags","searchtagsdesc",k);g=g.substring(g.indexOf(" ")+1);g=g.substring(0,g.indexOf(" "));document.getElementById("searchMatch").value=
g;d.target.remove();doSearch(domCache.searchstr.value)}},!1);document.getElementById("searchMatch").addEventListener("change",function(){doSearch(domCache.searchstr.value)},!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(d){if(settings.featAdvsearch&&"TH"===d.target.nodeName&&""!==d.target.innerHTML){var g=d.target.getAttribute("data-col");if("Duration"!==g){var k=app.current.sort,l=!0;if(k===g||k==="-"+g)0===k.indexOf("-")?(l=!0,g=k.substring(1)):
l=!1;!1===l?(k="-"+g,l=!0):(l=!1,k=g);for(var m=document.getElementById("SearchList").getElementsByClassName("sort-dir"),n=0;n<m.length;n++)m[n].remove();app.current.sort=k;d.target.innerHTML=t(g)+'<span class="sort-dir material-icons pull-right">'+(!0===l?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.offset,app.current.limit,app.current.filter,app.current.sort,"-",app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",
function(d){"Enter"===d.key&&(d.preventDefault(),d.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(d){"OPTION"===d.target.nodeName&&removeScriptArgument(d)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("QueueJukebox");
dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},!1);document.addEventListener("keydown",function(d){if("INPUT"!==d.target.tagName&&"SELECT"!==d.target.tagName&&"TEXTAREA"!==d.target.tagName&&!d.ctrlKey&&!d.altKey){var g=keymap[d.key];g&&"function"===typeof window[g.cmd]&&(void 0===keymap[d.key].req||
!0===settings[keymap[d.key].req])&&parseCmd(d,g)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(d){navigateTable(this,d.key)},!1);var h="";Object.keys(themes).forEach(function(d){h+='<option value="'+e(d)+'">'+t(themes[d])+"</option>"});document.getElementById("selectTheme").innerHTML=h;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",
...
function doSearch(a){if(settings.featAdvsearch){for(var b="(",c=domCache.searchCrumb.children,f=0;f<c.length;f++)b+="("+decodeURI(c[f].getAttribute("data-filter"))+")",""!==a&&(b+=" AND ");""!==a?(c=document.getElementById("searchMatch"),b+="("+app.current.filter+" "+c.options[c.selectedIndex].value+" '"+a+"'))"):b+=")";2>=b.length&&(b="");appGoto("Search",void 0,void 0,"0",app.current.limit,app.current.filter,app.current.sort,"-",b)}else appGoto("Search",void 0,void 0,"0",app.current.limit,app.current.filter,
app.current.sort,"-",a)}function parseSearch(a){0<a.result.returnedEntities?(document.getElementById("searchAddAllSongs").removeAttribute("disabled"),document.getElementById("searchAddAllSongsBtn").removeAttribute("disabled")):(document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"));parseFilesystem(a)}
...
function getLyrics(a,b){void 0===a?b.innerHTML=t("No lyrics found"):(b.classList.add("opacity05"),sendAPI("MPD_API_LYRICS_GET",{uri:a},function(c){if(c.error)b.innerText=t(c.error.message);else if(c.result.message)b.innerText=t(c.result.message);else{for(var f='<div class="lyricsTextContainer">',h=0;h<c.result.returnedEntities;h++)f+='<div class="lyricsText'+(0<h?" hide":"")+'">'+(!0===c.result.synced?parseSyncedLyrics(c.result.data[h].text):e(c.result.data[h].text).replace(/\n/g,"<br/>"))+"</div>";
f+="</div>";showSyncedLyrics=c.result.synced;1<c.result.returnedEntities?(b.innerHTML=lyrcisHeader+f,b.getElementsByClassName("lyricsHeader")[0].addEventListener("click",function(d){if("LABEL"===d.target.nodeName){d.target.parentNode.getElementsByClassName("active")[0].classList.remove("active");d.target.classList.add("active");d=parseInt(d.target.getAttribute("data-num"));for(var g=b.getElementsByClassName("lyricsText"),k=0;k<g.length;k++)k===d?g[k].classList.remove("hide"):g[k].classList.add("hide")}},
!1)):b.innerHTML=f}b.classList.remove("opacity05")},!0))}function parseSyncedLyrics(a){var b="";a=a.split("\r\n");for(var c=0;c<a.length;c++){var f=a[c].match(/^\[(\d+):(\d+)\.(\d+)\](.*)$/);if(f){var h=60*parseInt(f[1])+parseInt(f[2]);b+='<p><span data-sec="'+h+'">';b+=f[4].replace(/<(\d+):(\d+)\.(\d+)>/g,function(d,g,k,l){return'</span><span data-sec="'+(60*parseInt(g)+parseInt(k))+'">'});b+="</span></p>"}}return b+""}function loveSong(){sendAPI("MPD_API_LOVE",{})}
