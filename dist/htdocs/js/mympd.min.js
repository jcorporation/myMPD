...
else"Album"===app.current.tag?searchAlbumgrid(this.value):appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,this.value)},!1);document.getElementById("searchDatabaseMatch").addEventListener("change",function(){searchAlbumgrid(document.getElementById("searchDatabaseStr").value)});document.getElementById("searchDatabaseCrumb").addEventListener("click",function(a){"SPAN"===a.target.nodeName?(a.preventDefault(),a.stopPropagation(),
...
k.style.backgroundImage='url("'+subdir+"/browse/music/"+a.result.images[h]+'"),url("assets/coverimage-loading.svg")';g.appendChild(k)}}k=advancedSettingsDefault.clickSong.validValues[settings.advanced.clickSong];var l=advancedSettingsDefault.clickFolder.validValues[settings.advanced.clickFolder],m=advancedSettingsDefault.clickPlaylist.validValues[settings.advanced.clickPlaylist];g=a.result.returnedEntities;h=d.getElementsByTagName("tr");c=document.activeElement.parentNode.parentNode===c?!0:!1;for(var p=
0,n=0;n<g;n++){var q=document.createElement("tr"),u="";setAttEnc(q,"data-type",a.result.data[n].Type);setAttEnc(q,"data-uri",a.result.data[n].uri);q.setAttribute("tabindex",0);"Search"===app.current.app&&!0===settings.featTags&&!0===settings.featAdvsearch&&(void 0!==a.result.data[n].Album&&setAttEnc(q,"data-album",a.result.data[n].Album),void 0!==a.result.data[n][tagAlbumArtist]&&setAttEnc(q,"data-albumartist",a.result.data[n][tagAlbumArtist]));"song"===a.result.data[n].Type?setAttEnc(q,"data-name",
a.result.data[n].Title):setAttEnc(q,"data-name",a.result.data[n].name);switch(a.result.data[n].Type){case "parentDir":q.innerHTML='<td colspan="'+(f+1)+'">..</td>';q.setAttribute("title",t("Open parent folder"));break;case "dir":case "smartpls":case "plist":for(var v=0;v<settings["cols"+b].length;v++)u+='<td data-col="'+settings["cols"+b][v]+'">',"Type"===settings["cols"+b][v]?u="dir"===a.result.data[n].Type?u+'<span class="mi">folder_open</span>':u+('<span class="mi">'+("smartpls"===a.result.data[n].Type?
"queue_music":"list")+"</span>"):"Title"===settings["cols"+b][v]&&(u+=e(a.result.data[n].name)),u+="</td>";u+='<td data-col="Action"><a href="#" class="mi color-darkgrey">'+ligatureMore+"</a></td>";q.innerHTML=u;q.setAttribute("title",t("dir"===a.result.data[n].Type?l:m));break;case "song":void 0!==a.result.data[n].Duration&&(a.result.data[n].Duration=beautifySongDuration(a.result.data[n].Duration));void 0!==a.result.data[n].LastModified&&(a.result.data[n].LastModified=localeDate(a.result.data[n].LastModified));
for(v=0;v<settings["cols"+b].length;v++)u+='<td data-col="'+settings["cols"+b][v]+'">',u="Type"===settings["cols"+b][v]?u+'<span class="mi">music_note</span>':u+e(a.result.data[n][settings["cols"+b][v]]),u+="</td>";u+='<td data-col="Action"><a href="#" class="mi color-darkgrey">'+ligatureMore+"</a></td>";q.innerHTML=u;q.setAttribute("title",t(k))}n<h.length?p=!0===replaceTblRow(h[n],q)?n:p:d.append(q)}for(b=h.length-1;b>=g;b--)h[b].remove();!0===c&&focusTable(0);setPagination(a.result.totalEntities,
a.result.returnedEntities);0===g&&(d.innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="'+f+'">'+t("Empty list")+"</td></tr>")}document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05")}
...
function addPlayButton(a){var b=document.createElement("div");b.classList.add("align-self-end","album-grid-mouseover","mi","rounded-circle","clickable");b.innerText="play_arrow";b.title=t(advancedSettingsDefault.clickAlbumPlay.validValues[settings.advanced.clickAlbumPlay]);a.appendChild(b);b.addEventListener("click",function(c){c.preventDefault();c.stopPropagation();clickAlbumPlay(getAttDec(c.target.parentNode.parentNode,"data-albumartist"),getAttDec(c.target.parentNode.parentNode,"data-album"))},
!1)}
function parseAlbumDetails(a){var b=document.getElementById("viewDetailDatabaseCover");b.style.backgroundImage='url("'+subdir+"/albumart/"+a.result.data[0].uri+'"), url("'+subdir+'/assets/coverimage-loading.svg")';setAttEnc(b,"data-images",a.result.images.join(";;"));setAttEnc(b,"data-uri",a.result.data[0].uri);document.getElementById("viewDetailDatabaseInfo").innerHTML="<h1>"+e(a.result.Album)+"</h1><small> "+t("AlbumArtist")+"</small><p>"+e(a.result.AlbumArtist)+"</p>"+(""===a.result.bookletPath||!1===
settings.featBrowse?"":'<span class="text-light mi">description</span>&nbsp;<a class="text-light" target="_blank" href="'+subdir+"/browse/music/"+e(a.result.bookletPath)+'">'+t("Download booklet")+"</a>")+"</p>";b=document.getElementById("BrowseDatabaseDetailList");var c=b.getElementsByTagName("tbody")[0],d=settings.colsBrowseDatabaseDetail.length,f="";1<a.result.Discs&&(f='<tr class="not-clickable"><td><span class="mi">album</span></td><td colspan="'+d+'">'+t("Disc 1")+"</td></tr>");for(var g=a.result.returnedEntities,
h=parseInt(a.result.data[0].Disc),k=t(advancedSettingsDefault.clickSong.validValues[settings.advanced.clickSong]),l=0;l<g;l++){h<parseInt(a.result.data[l].Disc)&&(f+='<tr class="not-clickable"><td><span class="mi">album</span></td><td colspan="'+d+'">'+t("Disc")+" "+e(a.result.data[l].Disc)+"</td></tr>");a.result.data[l].Duration&&(a.result.data[l].Duration=beautifySongDuration(a.result.data[l].Duration));f+='<tr tabindex="0" title="'+t(k)+'"data-type="song" data-name="'+encodeURI(a.result.data[l].Title)+
'" data-uri="'+encodeURI(a.result.data[l].uri)+'">';for(h=0;h<settings.colsBrowseDatabaseDetail.length;h++)f+='<td data-col="'+settings.colsBrowseDatabaseDetail[h]+'">'+e(a.result.data[l][settings.colsBrowseDatabaseDetail[h]])+"</td>";f+='<td data-col="Action"><a href="#" class="mi color-darkgrey">'+ligatureMore+"</a></td></tr>";h=a.result.data[l].Disc}c.innerHTML=f;b.getElementsByTagName("tfoot")[0].innerHTML='<tr><td colspan="'+(settings.colsBrowseDatabaseDetail.length+1)+'"><small>'+t("Num songs",
a.result.totalEntities)+"&nbsp;&ndash;&nbsp;"+beautifyDuration(a.result.totalTime)+"</small></td></tr>";document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto("Browse","Database","List")}function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);_addAlbum(a,c,b)}
...
'url("'+subdir+"/browse/pics/"+c+'")';""!==c?(document.getElementById("divHomeIconLigature").classList.add("hide"),document.getElementById("homeIconPreview").innerHTML=""):(document.getElementById("divHomeIconLigature").classList.remove("hide"),document.getElementById("homeIconPreview").innerText=document.getElementById("inputHomeIconLigature").value)},!1);document.getElementById("btnHomeIconLigature").parentNode.addEventListener("show.bs.dropdown",function(){var c=document.getElementById("inputHomeIconLigature").value;
""!==c&&(document.getElementById("searchHomeIconLigature").value=c,filterHomeIconLigatures())},!1);var a="",b='<option value="all">'+t("All")+"</option>";Object.keys(materialIcons).forEach(function(c){a+='<h5 class="ml-1 mt-2">'+e(ucFirst(c))+"</h5>";b+='<option value="'+c+'">'+e(ucFirst(c))+"</option>";for(var d=0;d<materialIcons[c].length;d++)a+='<button title="'+materialIcons[c][d]+'" data-cat="'+c+'" class="btn btn-sm mi m-1">'+materialIcons[c][d]+"</button>"});document.getElementById("listHomeIconLigature").innerHTML=
a;document.getElementById("searchHomeIconCat").innerHTML=b;document.getElementById("listHomeIconLigature").addEventListener("click",function(c){"BUTTON"===c.target.nodeName&&(c.preventDefault(),selectHomeIconLigature(c.target))});document.getElementById("searchHomeIconLigature").addEventListener("click",function(c){c.stopPropagation()},!1);document.getElementById("searchHomeIconCat").addEventListener("click",function(c){c.stopPropagation()},!1);document.getElementById("searchHomeIconCat").addEventListener("change",
function(){filterHomeIconLigatures()},!1);document.getElementById("searchHomeIconLigature").addEventListener("keydown",function(c){c.stopPropagation();"Enter"===c.key&&c.preventDefault()},!1);document.getElementById("searchHomeIconLigature").addEventListener("keyup",function(c){"Enter"===c.key?(c=document.getElementById("listHomeIconLigature").getElementsByClassName("active")[0],void 0!==c&&(selectHomeIconLigature(c),dropdownHomeIconLigature.toggle())):filterHomeIconLigatures()},!1)}
function selectHomeIconLigature(a){document.getElementById("inputHomeIconLigature").value=a.getAttribute("title");document.getElementById("homeIconPreview").innerText=a.getAttribute("title");document.getElementById("homeIconPreview").style.backgroundImage="";document.getElementById("selectHomeIconImage").value=""}
...
a=document.getElementById("listHomeIconLigature").getElementsByTagName("h5");if(""===b)for(b=0;b<a.length;b++)a[b].classList.remove("hide");else for(b=0;b<a.length;b++)a[b].classList.add("hide")}
...
0,settings.maxElementsPerPage);var h="replaceQueue"===a.result.data[f].cmd?"Playlist":"appGoto"===a.result.data[f].cmd?"View":"Script",k=JSON.stringify({cmd:a.result.data[f].cmd,options:a.result.data[f].options});h='<div class="card home-icons clickable" draggable="true" tabindex="0" data-pos="'+f+"\" data-href='"+e(k)+"'  title=\""+t(h)+": "+e(a.result.data[f].name)+'"><div class="card-body mi">'+e(a.result.data[f].ligature)+'</div><div class="card-footer card-footer-grid p-2">'+e(a.result.data[f].name)+
"</div></div>";g.innerHTML=h;f<d.length?d[f].replaceWith(g):c.append(g);""!==a.result.data[f].image&&(g.getElementsByClassName("card-body")[0].style.backgroundImage='url("'+subdir+"/browse/pics/"+a.result.data[f].image+'")');""!==a.result.data[f].bgcolor&&(g.getElementsByClassName("card-body")[0].style.backgroundColor=a.result.data[f].bgcolor)}for(a=d.length-1;a>=b;a--)d[a].remove();0===b&&(c.innerHTML='<div class="ml-3"><h3>'+t("Homescreen")+"</h3><p>"+t("Homescreen welcome")+"</p><ul><li><b>"+t("View")+
"</b>: "+t("Homescreen help view")+"</li><li><b>"+t("Playlist")+"</b>: "+t("Homescreen help playlist")+"</li><li><b>"+t("Script")+"</b>: "+t("Homescreen help script")+"</li></div>")}function popoverMenuHome(a){for(var b=document.getElementById("HomeCards").getElementsByClassName("selected"),c=0;c<b.length;c++)b[c].classList.remove("selected");a.target.parentNode.classList.add("selected");showMenu(a.target,a);a.preventDefault();a.stopPropagation()}
...
function parseJukeboxList(a){var b=advancedSettingsDefault.clickSong.validValues[settings.advanced.clickSong],c=a.result.returnedEntities,d=document.getElementById("QueueJukeboxList"),f=document.activeElement.parentNode.parentNode===d?!0:!1,g=0;d=d.getElementsByTagName("tbody")[0];for(var h=d.getElementsByTagName("tr"),k=0;k<c;k++){a.result.data[k].Duration=beautifySongDuration(a.result.data[k].Duration);a.result.data[k].LastPlayed=localeDate(a.result.data[k].LastPlayed);var l=document.createElement("tr");
setAttEnc(l,"data-uri",a.result.data[k].uri);setAttEnc(l,"data-name",a.result.data[k].Title);setAttEnc(l,"data-type","song");setAttEnc(l,"data-pos",k);l.setAttribute("title",t(b));l.setAttribute("tabindex",0);for(var m="",p=0;p<settings.colsQueueJukebox.length;p++)m+='<td data-col="'+encodeURI(settings.colsQueueJukebox[p])+'">'+e(a.result.data[k][settings.colsQueueJukebox[p]])+"</td>";m+='<td data-col="Action">';""!==a.result.data[k].uri&&(m+='<a href="#" class="mi color-darkgrey">'+ligatureMore+
"</a>");m+="</td>";l.innerHTML=m;k<h.length?g=!0===replaceTblRow(h[k],l)?k:g:d.append(l)}for(b=h.length-1;b>=c;b--)h[b].remove();b=settings.colsQueueJukebox.length;0===c&&(d.innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="'+b+'">'+t("Empty list")+"</td></tr>");!0===f&&focusTable(g);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueJukeboxList").classList.remove("opacity05")}
...
toggle:"check_box check_box_outline_blank indeterminate_check_box radio_button_checked radio_button_unchecked star star_border star_border_purple500 star_half star_outline star_purple500 toggle_off toggle_on".split(" ")};
function e(a){return isNaN(a)?a.replace(/([<>"'])/g,function(b,c){if("<"===c)return"&lt;";if(">"===c)return"&gt;";if('"'===c)return"&quot;";if("'"===c)return"&apos;"}).replace(/\\u(003C|003E|0022|0027)/gi,function(b,c){if("003C"===c)return"&lt;";if("003E"===c)return"&gt;";if("0022"===c)return"&quot;";if("0027"===c)return"&apos;"}).replace(/\[\[(\w+)\]\]/g,function(b,c){return'<span class="mi">'+c+"</span>"}):a}function r(a){return a.replace(/[^\w-]/g,"_")}
...
modalEditHomeIcon=new BSN.Modal(document.getElementById("modalEditHomeIcon")),modalReally=new BSN.Modal(document.getElementById("modalReally")),dropdownMainMenu=new BSN.Dropdown(document.getElementById("mainMenu")),dropdownVolumeMenu=new BSN.Dropdown(document.getElementById("volumeMenu")),dropdownBookmarks=new BSN.Dropdown(document.getElementById("BrowseFilesystemBookmark")),dropdownLocalPlayer=new BSN.Dropdown(document.getElementById("localPlaybackMenu")),dropdownPlay=new BSN.Dropdown(document.getElementById("btnPlayDropdown")),
dropdownDatabaseSort=new BSN.Dropdown(document.getElementById("btnDatabaseSortDropdown")),dropdownNeighbors=new BSN.Dropdown(document.getElementById("btnDropdownNeighbors")),dropdownHomeIconLigature=new BSN.Dropdown(document.getElementById("btnHomeIconLigature")),collapseDBupdate=new BSN.Collapse(document.getElementById("navDBupdate")),collapseSettings=new BSN.Collapse(document.getElementById("navSettings")),collapseSyscmds=new BSN.Collapse(document.getElementById("navSyscmds")),collapseScripting=
new BSN.Collapse(document.getElementById("navScripting")),collapseJukeboxMode=new BSN.Collapse(document.getElementById("labelJukeboxMode"));
...
function appGoto(a,b,c,d,f,g,h,k,l,m){var p=document.body.scrollTop?document.body.scrollTop:document.documentElement.scrollTop;void 0!==app.apps[app.current.app].scrollPos?app.apps[app.current.app].scrollPos=p:void 0!==app.apps[app.current.app].tabs[app.current.tab].scrollPos?app.apps[app.current.app].tabs[app.current.tab].scrollPos=p:void 0!==app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos&&(app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos=
p);null===d&&(d=void 0);null===f&&(f=void 0);null===g&&(g=void 0);null===h&&(h=void 0);null===k&&(k=void 0);null===l&&(l=void 0);app.apps[a].tabs?(void 0===b&&(b=app.apps[a].active),app.apps[a].tabs[b].views?(void 0===c&&(c=app.apps[a].tabs[b].active),d="/"+encodeURIComponent(a)+"/"+encodeURIComponent(b)+"/"+encodeURIComponent(c)+"!"+encodeURIComponent(void 0===d?app.apps[a].tabs[b].views[c].offset:d)+"/"+encodeURIComponent(void 0===f?app.apps[a].tabs[b].views[c].limit:f)+"/"+encodeURIComponent(void 0===
...
document.getElementById("btnAddSmartpls").parentNode.classList.add("hide"),document.getElementById("BrowseNavPlaylists").parentNode.classList.add("hide"));var b=a.result.returnedEntities,c=document.getElementById(app.current.app+app.current.tab+app.current.view+"List"),d=c.getElementsByTagName("tbody")[0],f=d.getElementsByTagName("tr"),g=document.activeElement.parentNode.parentNode===c?!0:!1,h=0;if("All"===app.current.view){c=advancedSettingsDefault.clickPlaylist.validValues[settings.advanced.clickPlaylist];
for(var k=0;k<b;k++){var l=document.createElement("tr");setAttEnc(l,"data-uri",a.result.data[k].uri);setAttEnc(l,"data-type",a.result.data[k].Type);setAttEnc(l,"data-name",a.result.data[k].name);l.setAttribute("tabindex",0);l.setAttribute("title",t(c));l.innerHTML='<td data-col="Type"><span class="mi">'+("smartpls"===a.result.data[k].Type?"queue_music":"list")+"</span></td><td>"+e(a.result.data[k].name)+"</td><td>"+localeDate(a.result.data[k].last_modified)+'</td><td data-col="Action"><a href="#" class="mi color-darkgrey">'+
ligatureMore+"</a></td>";k<f.length?h=!0===replaceTblRow(f[k],l)?k:h:d.append(l)}}else if("Detail"===app.current.view){k=advancedSettingsDefault.clickSong.validValues[settings.advanced.clickSong];for(l=0;l<b;l++){var m=document.createElement("tr");!1===a.result.smartpls&&m.setAttribute("draggable","true");m.setAttribute("id","playlistTrackId"+a.result.data[l].Pos);setAttEnc(m,"data-type",a.result.data[l].Type);setAttEnc(m,"data-uri",a.result.data[l].uri);setAttEnc(m,"data-name",a.result.data[l].Title);
setAttEnc(m,"data-songpos",a.result.data[l].Pos);m.setAttribute("tabindex",0);m.setAttribute("title",t(k));a.result.data[l].Duration=beautifySongDuration(a.result.data[l].Duration);for(var p="",n=0;n<settings.colsBrowsePlaylistsDetail.length;n++)p+='<td data-col="'+settings.colsBrowsePlaylistsDetail[n]+'">'+e(a.result.data[l][settings.colsBrowsePlaylistsDetail[n]])+"</td>";p+='<td data-col="Action"><a href="#" class="mi color-darkgrey">'+ligatureMore+"</a></td>";m.innerHTML=p;l<f.length?h=!0===replaceTblRow(f[l],
m)?l:h:d.append(m)}h=c.getElementsByTagName("tfoot")[0];c=settings.colsBrowsePlaylistsDetail.length;c++;h.innerHTML='<tr><td colspan="'+(c+1)+'"><small>'+t("Num songs",a.result.totalEntities)+"&nbsp;&ndash;&nbsp;"+beautifyDuration(a.result.totalTime)+"</small></td></tr>"}for(h=f.length-1;h>=b;h--)f[h].remove();!0===g&&focusTable(0);setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(d.innerHTML="All"===app.current.view?'<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="3">'+
...
function playlistClear(){var a=getAttDec(document.getElementById("BrowsePlaylistsDetailList"),"data-uri");sendAPI("MPD_API_PLAYLIST_CLEAR",{uri:a});document.getElementById("BrowsePlaylistsDetailList").classList.add("opacity05")}function playlistShuffle(){var a=getAttDec(document.getElementById("BrowsePlaylistsDetailList"),"data-uri");sendAPI("MPD_API_PLAYLIST_SHUFFLE",{uri:a});document.getElementById("BrowsePlaylistsDetailList").classList.add("opacity05")}
function playlistSort(a){var b=getAttDec(document.getElementById("BrowsePlaylistsDetailList"),"data-uri");sendAPI("MPD_API_PLAYLIST_SORT",{uri:b,tag:a});document.getElementById("BrowsePlaylistsDetailList").classList.add("opacity05")}
...
(""!==settings.smartplsPrefix?"-":"")+"bestRated",b.result.type="sticker",b.result.sticker="like",b.result.maxentries=200,b.result.minvalue=2);parseSmartPlaylist(b)}function deletePlaylists(){btnWaiting(document.getElementById("btnDeletePlaylists"),!0);sendAPI("MPD_API_PLAYLIST_RM_ALL",{type:getSelectValue("selectDeletePlaylists")},function(){btnWaiting(document.getElementById("btnDeletePlaylists"),!1)})}
...
g+"</div></div>",content:" "});g=a.Popover;null===getAttDec(a,"data-init")&&(setAttEnc(a,"data-init","true"),a.addEventListener("shown.bs.popover",function(l){setAttEnc(l.target,"data-popover","true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(p){p.preventDefault();p.stopPropagation();"A"===p.target.nodeName&&(p=getAttDec(p.target,"data-href"))&&(p=JSON.parse(b64DecodeUnicode(p)),parseCmd(l,p),hideMenu())},!1);document.getElementsByClassName("popover-content")[0].addEventListener("keydown",
function(p){p.preventDefault();p.stopPropagation();if("ArrowDown"===p.key||"ArrowUp"===p.key){var n=this.getElementsByTagName("a");n=Array.prototype.slice.call(n);var q=n.indexOf(document.activeElement);do if(q="ArrowUp"===p.key?1<q?q-1:0:"ArrowDown"===p.key?q<n.length-1?q+1:q:q,0===q||q===n.length-1)break;while(!n[q].offsetHeight);n[q]&&n[q].focus()}else"Enter"===p.key?p.target.click():"Escape"===p.key&&hideMenu()},!1);var m=document.getElementById("advancedMenuLink");m&&(m.addEventListener("click",
function(){var p=this.getElementsByTagName("span")[0];p.innerText="keyboard_arrow_right"===p.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new BSN.Collapse(m));document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));g.show()}
...
(d+1)+'"><small>'+t("Num songs",a.result.totalEntities)+"</small></td></tr>":"";a.result.totalEntities>settings.maxElementsPerPage?document.getElementById("btnQueueGotoPlayingSong").parentNode.classList.remove("hide"):document.getElementById("btnQueueGotoPlayingSong").parentNode.classList.add("hide");var f=advancedSettingsDefault.clickQueueSong.validValues[settings.advanced.clickQueueSong];c=a.result.returnedEntities;var g=document.activeElement.parentNode.parentNode===b?!0:!1,h=0;setAttEnc(b,"data-version",
a.result.queueVersion);b=b.getElementsByTagName("tbody")[0];for(var k=b.getElementsByTagName("tr"),l=0;l<c;l++){a.result.data[l].Duration=beautifySongDuration(a.result.data[l].Duration);a.result.data[l].Pos++;var m=document.createElement("tr");m.setAttribute("draggable","true");m.setAttribute("id","queueTrackId"+a.result.data[l].id);m.setAttribute("tabindex",0);m.setAttribute("title",t(f));setAttEnc(m,"data-trackid",a.result.data[l].id);setAttEnc(m,"data-songpos",a.result.data[l].Pos);setAttEnc(m,
"data-duration",a.result.data[l].Duration);setAttEnc(m,"data-uri",a.result.data[l].uri);for(var p="",n=0;n<settings.colsQueueCurrent.length;n++)p+='<td data-col="'+encodeURI(settings.colsQueueCurrent[n])+'">'+e(a.result.data[l][settings.colsQueueCurrent[n]])+"</td>";p+='<td data-col="Action"><a href="#" class="mi color-darkgrey">'+ligatureMore+"</a></td>";m.innerHTML=p;l<k.length?h=!0===replaceTblRow(k[l],m)?l:h:b.append(m)}for(f=k.length-1;f>=c;f--)k[f].remove();"MPD_API_QUEUE_SEARCH"===a.result.method&&
0===c?b.innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="'+d+'">'+t("No results, please refine your search")+"</td></tr>":"MPD_API_QUEUE_LIST"===a.result.method&&0===c&&(b.innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="'+d+'">'+t("Empty queue")+"</td></tr>");!0===g&&focusTable(h);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueCurrentList").classList.remove("opacity05")}}
function parseLastPlayed(a){var b=advancedSettingsDefault.clickSong.validValues[settings.advanced.clickSong],c=a.result.returnedEntities,d=document.getElementById("QueueLastPlayedList"),f=document.activeElement.parentNode.parentNode===d?!0:!1,g=0;d=d.getElementsByTagName("tbody")[0];for(var h=d.getElementsByTagName("tr"),k=0;k<c;k++){a.result.data[k].Duration=beautifySongDuration(a.result.data[k].Duration);a.result.data[k].LastPlayed=localeDate(a.result.data[k].LastPlayed);var l=document.createElement("tr");
setAttEnc(l,"data-uri",a.result.data[k].uri);setAttEnc(l,"data-name",a.result.data[k].Title);setAttEnc(l,"data-type","song");l.setAttribute("tabindex",0);l.setAttribute("title",t(b));for(var m="",p=0;p<settings.colsQueueLastPlayed.length;p++)m+='<td data-col="'+encodeURI(settings.colsQueueLastPlayed[p])+'">'+e(a.result.data[k][settings.colsQueueLastPlayed[p]])+"</td>";m+='<td data-col="Action">';""!==a.result.data[k].uri&&(m+='<a href="#" class="mi color-darkgrey">'+ligatureMore+"</a>");m+="</td>";
l.innerHTML=m;k<h.length?g=!0===replaceTblRow(h[k],l)?k:g:d.append(l)}for(b=h.length-1;b>=c;b--)h[b].remove();b=settings.colsQueueLastPlayed.length;0===c&&(d.innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="'+b+'">'+t("Empty list")+"</td></tr>");!0===f&&focusTable(g);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueLastPlayedList").classList.remove("opacity05")}
...
function clearQueue(){showReally('{"cmd": "sendAPI", "options": [{"cmd": "MPD_API_QUEUE_CROP_OR_CLEAR"}]}',t("Do you really want to clear the queue?"))}
...
var advancedSettingsDefault={clickSong:{defaultValue:"append",validValues:{append:"Append to queue",replace:"Replace queue",view:"Song details"},inputType:"select",title:"Click song"},clickQueueSong:{defaultValue:"play",validValues:{play:"Play",view:"Song details"},inputType:"select",title:"Click song in queue"},clickPlaylist:{defaultValue:"append",validValues:{append:"Append to queue",replace:"Replace queue",view:"View playlist"},inputType:"select",title:"Click playlist"},clickFolder:{defaultValue:"view",
validValues:{append:"Append to queue",replace:"Replace queue",view:"Open folder"},inputType:"select",title:"Click folder"},clickAlbumPlay:{defaultValue:"replace",validValues:{append:"Append to queue",replace:"Replace queue"},inputType:"select",title:"Click album play button"}};
...
settings.bgColor;document.getElementsByTagName("body")[0].style.backgroundColor=settings.bgColor;toggleBtnChkCollapse("btnBgCover","collapseBackground",settings.bgCover);document.getElementById("inputBgCssFilter").value=settings.bgCssFilter;g=document.querySelectorAll(".albumartbg");for(b=0;b<g.length;b++)g[b].style.filter=settings.bgCssFilter;toggleBtnChkCollapse("btnLoveEnable","collapseLove",settings.love);document.getElementById("inputLoveChannel").value=settings.loveChannel;document.getElementById("inputLoveMessage").value=
...
g="featLocalplayer featSyscmds featMixramp featCacert featBookmarks featRegex featTimer featLyrics featScripting featScripteditor featHome".split(" ");for(b=0;b<g.length;b++){c=document.getElementsByClassName(g[b]);d=c.length;f=!0===settings[g[b]]?"":"none";for(var h=0;h<d;h++)c[h].style.display=f}g=document.getElementsByClassName("warnReadonly");for(b=0;b<g.length;b++)!1===settings.readonly?g[b].classList.add("hide"):g[b].classList.remove("hide");!0===settings.readonly?(disableEl("btnBookmarks"),
document.getElementsByClassName("groupClearCovercache")[0].classList.add("hide")):(enableEl("btnBookmarks"),document.getElementsByClassName("groupClearCovercache")[0].classList.remove("hide"));g='<optgroup data-value="player" label="'+t("Playback")+'"><option value="startplay">'+t("Start playback")+'</option><option value="stopplay">'+t("Stop playback")+"</option></optgroup>";if(!0===settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(g+='<optgroup data-value="syscmd" label="'+t("System command")+
'">',b=4<c?"":'<div class="dropdown-divider"></div>',d=0;d<c;d++)"HR"===settings.syscmdList[d]?b+='<div class="dropdown-divider"></div>':(b+='<a class="dropdown-item text-light alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+e(settings.syscmdList[d])+"\"]}'>"+e(settings.syscmdList[d])+"</a>",g+='<option value="'+e(settings.syscmdList[d])+'">'+e(settings.syscmdList[d])+"</option>");document.getElementById("syscmds").innerHTML=b;g+="</optgroup>";4<c?(document.getElementById("navSyscmds").classList.remove("hide"),
document.getElementById("syscmds").classList.add("collapse","menu-indent")):(document.getElementById("navSyscmds").classList.add("hide"),document.getElementById("syscmds").classList.remove("collapse","menu-indent"))}else document.getElementById("syscmds").innerHTML="";!0===settings.featScripting?getScriptList(!0):document.getElementById("scripts").innerHTML="";document.getElementById("selectTimerAction").innerHTML=g;toggleBtnGroupValueCollapse(document.getElementById("btnJukeboxModeGroup"),"collapseJukeboxMode",
...
function dragAndDropTable(a){var b=document.getElementById(a).getElementsByTagName("tbody")[0];b.addEventListener("dragstart",function(c){"TR"===c.target.nodeName&&(c.target.classList.add("opacity05"),c.dataTransfer.setDragImage(c.target,0,0),c.dataTransfer.effectAllowed="move",c.dataTransfer.setData("Text",c.target.getAttribute("id")),dragEl=c.target.cloneNode(!0))},!1);b.addEventListener("dragleave",function(c){c.preventDefault();if(void 0!==dragEl&&"TR"===dragEl.nodeName){var d=c.target;"TD"===
c.target.nodeName&&(d=c.target.parentNode);"TR"===d.nodeName&&d.classList.remove("dragover")}},!1);b.addEventListener("dragover",function(c){c.preventDefault();if(void 0!==dragEl&&"TR"===dragEl.nodeName){for(var d=b.getElementsByClassName("dragover"),f=d.length,g=0;g<f;g++)d[g].classList.remove("dragover");d=c.target;"TD"===c.target.nodeName&&(d=c.target.parentNode);"TR"===d.nodeName&&d.classList.add("dragover");c.dataTransfer.dropEffect="move"}},!1);b.addEventListener("dragend",function(c){c.preventDefault();
if(void 0!==dragEl&&"TR"===dragEl.nodeName){for(var d=b.getElementsByClassName("dragover"),f=d.length,g=0;g<f;g++)d[g].classList.remove("dragover");document.getElementById(c.dataTransfer.getData("Text"))&&document.getElementById(c.dataTransfer.getData("Text")).classList.remove("opacity05");dragEl=void 0}},!1);b.addEventListener("drop",function(c){c.stopPropagation();c.preventDefault();if(void 0!==dragEl&&"TR"===dragEl.nodeName){var d=c.target;"TD"===c.target.nodeName&&(d=c.target.parentNode);var f=
getAttDec(document.getElementById(c.dataTransfer.getData("Text")),"data-songpos"),g=getAttDec(d,"data-songpos");document.getElementById(c.dataTransfer.getData("Text")).remove();dragEl.classList.remove("opacity05");b.insertBefore(dragEl,d);c=b.getElementsByClassName("dragover");d=c.length;for(var h=0;h<d;h++)c[h].classList.remove("dragover");document.getElementById(a).classList.add("opacity05");"Queue"===app.current.app&&"Current"===app.current.tab?sendAPI("MPD_API_QUEUE_MOVE_TRACK",{from:f,to:g}):
"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view&&playlistMoveTrack(f,g)}},!1)}
...
function(c){c.preventDefault();void 0!==dragEl&&"TH"===dragEl.nodeName&&"TH"===c.target.nodeName&&c.target.classList.remove("dragover-th")},!1);b.addEventListener("dragover",function(c){c.preventDefault();if(void 0!==dragEl&&"TH"===dragEl.nodeName){for(var d=b.getElementsByClassName("dragover-th"),f=d.length,g=0;g<f;g++)d[g].classList.remove("dragover-th");"TH"===c.target.nodeName&&c.target.classList.add("dragover-th");c.dataTransfer.dropEffect="move"}},!1);b.addEventListener("dragend",function(c){c.preventDefault();
if(void 0!==dragEl&&"TH"===dragEl.nodeName){for(var d=b.getElementsByClassName("dragover-th"),f=d.length,g=0;g<f;g++)d[g].classList.remove("dragover-th");this.querySelector("[data-col="+c.dataTransfer.getData("Text")+"]")&&this.querySelector("[data-col="+c.dataTransfer.getData("Text")+"]").classList.remove("opacity05");dragEl=void 0}},!1);b.addEventListener("drop",function(c){c.stopPropagation();c.preventDefault();if(void 0!==dragEl&&"TH"===dragEl.nodeName){this.querySelector("[data-col="+c.dataTransfer.getData("Text")+
"]").remove();dragEl.classList.remove("opacity05");b.insertBefore(dragEl,c.target);c=b.getElementsByClassName("dragover-th");for(var d=c.length,f=0;f<d;f++)c[f].classList.remove("dragover-th");document.getElementById(a+"List")?(document.getElementById(a+"List").classList.add("opacity05"),saveCols(a)):saveCols(a,this.parentNode.parentNode)}},!1)}
...
d=getAttDec(document.getElementById("btnTimerJukeboxModeGroup").getElementsByClassName("active")[0],"data-value");g=getSelectValue("selectTimerPlaylist");-1===c.selectedIndex&&(a=!1,c.classList.add("is-invalid"));"0"===d&&"Database"===g&&"startplay"===getSelectValue(c)&&(a=!1,document.getElementById("btnTimerJukeboxModeGroup").classList.add("is-invalid"));if(!0===a){a={};h=document.getElementById("timerActionScriptArguments").getElementsByTagName("input");for(var k=0;k<h.length;k++)a[getAttDec(h[k],
"data-name")]=h[k].value;sendAPI("MYMPD_API_TIMER_SAVE",{timerid:parseInt(document.getElementById("inputTimerId").value),name:b.value,enabled:document.getElementById("btnTimerEnabled").classList.contains("active")?!0:!1,startHour:parseInt(getSelectValue("selectTimerHour")),startMinute:parseInt(getSelectValue("selectTimerMinute")),weekdays:f,action:getAttDec(c.options[c.selectedIndex].parentNode,"data-value"),subaction:getSelectValue(c),volume:parseInt(document.getElementById("inputTimerVolume").value),
playlist:g,jukeboxMode:parseInt(d),arguments:a},showListTimer)}}
...
"</td></tr>";document.getElementById("listTriggerList").innerHTML=b}else document.getElementById("listTriggerList").innerHTML='<tr class="not-clickable"><td><span class="mi">error_outline</span></td><td colspan="2">'+t("Empty list")+"</td></tr>"}function showReally(a,b){setAttEnc("modalReallyAction","data-href",a);document.getElementById("modalReallyText").innerText=b;modalReally.show()}function acknowledgeReally(a){modalReally.hide();parseCmd(a,getAttDec("modalReallyAction","data-href"))}
function clickAlbumPlay(a,b){switch(settings.advanced.clickAlbumPlay){case "append":return _addAlbum("appendQueue",a,b);case "replace":return _addAlbum("replaceQueue",a,b)}}function clickSong(a,b){switch(settings.advanced.clickSong){case "append":return appendQueue("song",a,b);case "replace":return replaceQueue("song",a,b);case "view":return songDetails(a)}}
function clickQueueSong(a,b){switch(settings.advanced.clickQueueSong){case "play":sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:a});break;case "view":return songDetails(b)}}function clickPlaylist(a,b){switch(settings.advanced.clickPlaylist){case "append":return appendQueue("plist",a,b);case "replace":return replaceQueue("plist",a,b);case "view":return playlistDetails(a)}}
function clickFolder(a,b){switch(settings.advanced.clickFolder){case "append":return appendQueue("dir",a,b);case "replace":return replaceQueue("dir",a,b);case "view":app.current.filter="-",appGoto("Browse","Filesystem",void 0,"0",app.current.limit,app.current.filter,app.current.sort,"-",a)}}function escapeMPD(a){return a.replace(/(["'])/g,function(b,c){if('"'===c)return'\\"';if("'"===c)return"\\'";if("\\"===c)return"\\\\"})}
function unescapeMPD(a){return a.replace(/(\\'|\\"|\\\\)/g,function(b,c){if('\\"'===c)return'"';if("\\'"===c)return"'";if("\\\\"===c)return"\\"})}function setAttEnc(a,b,c){"string"===typeof a&&(a=document.getElementById(a));a.setAttribute(b,encodeURI(c))}function getAttDec(a,b){"string"===typeof a&&(a=document.getElementById(a));(a=a.getAttribute(b))&&(a=decodeURI(a));return a}function disableEl(a){"string"===typeof a&&(a=document.getElementById(a));a.setAttribute("disabled","disabled")}
function enableEl(a){"string"===typeof a&&(a=document.getElementById(a));a.removeAttribute("disabled")}function getSelectValue(a){"string"===typeof a&&(a=document.getElementById(a));if(a&&0<=a.selectedIndex)return getAttDec(a.options[a.selectedIndex],"value")}function getSelectedOptionAttribute(a,b){if((a=document.getElementById(a))&&0<=a.selectedIndex)return getAttDec(a.options[a.selectedIndex],b)}
...
h+='<option value="'+l+'"'+(app.current.limit===l?" selected":"")+">"+(0<l?l:t("All"))+"</option>";h+='</select></div><div id="'+c+'PaginationBottom" class="btn-group dropup pagination">'+g+"</div></div>";k=document.getElementById(c+"ButtonsBottom");k.innerHTML=h;k.getElementsByTagName("button")[0].addEventListener("click",function(){event.preventDefault();scrollToPosY(0)},!1);k.getElementsByTagName("select")[0].addEventListener("change",function(w){w=parseInt(getSelectValue(w.target));app.current.limit!==
w&&gotoPage(app.current.offset,w)},!1);document.getElementById(c+"PaginationTop").innerHTML=g;g=app.current.offset+app.current.limit;h=[document.getElementById(c+"PaginationTop"),document.getElementById(c+"PaginationBottom")];k={};for(l=0;l<h.length;k={$jscomp$loop$prop$lastPageOffset$72:k.$jscomp$loop$prop$lastPageOffset$72},l++){var m=h[l].children[0],p=h[l].children[1],n=h[l].children[2].children[0],q=h[l].children[2].children[1],u=h[l].children[3],v=h[l].children[4];n.innerText=f+" / "+d;if(1<
d){enableEl(n);for(var y="",x=0;x<d;x++){var z=x*app.current.limit;y+='<button data-offset="'+z+'" type="button" class="btn-sm btn btn-secondary'+(z===app.current.offset?" active":"")+'">'+(x+1)+"</button>"}q.innerHTML=y;n.classList.remove("nodropdown");q.addEventListener("click",function(w){"BUTTON"===w.target.nodeName&&gotoPage(getAttDec(w.target,"data-offset"))},!1);new BSN.Dropdown(n);k.$jscomp$loop$prop$lastPageOffset$72=(d-1)*app.current.limit;k.$jscomp$loop$prop$lastPageOffset$72===app.current.offset?
disableEl(v):(enableEl(v),v.classList.remove("hide"),u.classList.remove("rounded-right"),v.addEventListener("click",function(w){return function(){event.preventDefault();gotoPage(w.$jscomp$loop$prop$lastPageOffset$72)}}(k),!1))}else-1===a?(disableEl(n),n.innerText=f,n.classList.add("nodropdown"),disableEl(v),v.classList.add("hide"),u.classList.add("rounded-right")):(disableEl(n),n.classList.add("nodropdown"),disableEl(v));0<app.current.limit&&(a>g&&0<g||-1===a&&b>=app.current.limit)?(enableEl(u),h[l].classList.remove("hide"),
u.addEventListener("click",function(){event.preventDefault();gotoPage("next")},!1)):(disableEl(u),0===l&&h[l].classList.add("hide"));0<app.current.offset?(enableEl(p),h[l].classList.remove("hide"),p.addEventListener("click",function(){event.preventDefault();gotoPage("prev")},!1),enableEl(m),m.addEventListener("click",function(){event.preventDefault();gotoPage(0)},!1)):(disableEl(p),disableEl(m))}b<app.current.limit?document.getElementById(c+"ButtonsBottom").classList.add("hide"):document.getElementById(c+
"ButtonsBottom").classList.remove("hide")}function genId(a){return"id"+a.replace(/[^\w-]/g,"")}
function parseCmd(a,b){null!==a&&void 0!==a&&a.preventDefault();var c=b;"string"===typeof b&&(c=JSON.parse(b));if("function"===typeof window[c.cmd]){for(b=0;b<c.options.length;b++)"event"===c.options[b]&&(c.options[b]=a);switch(c.cmd){case "sendAPI":sendAPI(c.options[0].cmd,{});break;case "toggleBtn":case "toggleBtnChk":case "toggleBtnGroup":case "toggleBtnGroupCollapse":case "zoomPicture":case "setPlaySettings":window[c.cmd].apply(window,[a.target].concat($jscomp.arrayFromIterable(c.options)));break;
case "toggleBtnChkCollapse":window[c.cmd].apply(window,[a.target,void 0].concat($jscomp.arrayFromIterable(c.options)));break;default:window[c.cmd].apply(window,$jscomp.arrayFromIterable(c.options))}}else logError("Can not execute cmd: "+c)}
