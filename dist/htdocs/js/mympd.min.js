...
var k='<div class="card card-grid clickable" data-uri="'+encodeURI(a.result.data[g].FirstSongUri)+'" data-album="'+encodeURI(a.result.data[g].Album)+'" data-albumartist="'+encodeURI(a.result.data[g].AlbumArtist)+'" tabindex="0"><div class="card-header covergrid-header hide"></div><div class="card-body album-cover-loading album-cover-grid bg-white" id="'+f.id+'"></div><div class="card-footer card-footer-grid p-2" title="'+a.result.data[g].AlbumArtist+": "+a.result.data[g].Album+'">'+a.result.data[g].Album+
...
var c=decodeURI(b.parentNode.getAttribute("data-uri"));showGridImage(b,c)}else"Enter"===b.key?(getCovergridTitleList(a.id),b.stopPropagation(),b.preventDefault()):" "===b.key&&(showMenu(b.target.getElementsByClassName("card-footer")[0],b),b.stopPropagation(),b.preventDefault())}}(f),!1))}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML=t("Empty list"));document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+
"List").classList.remove("opacity05");document.getElementById("cardFooterBrowse").innerText=gtPage("Num entries",a.result.returnedEntities,a.result.totalEntities)}
...
1==settings.featLocalplayer&&1==settings.localplayerAutoplay&&localplayerPlay()})}window.onerror=function(a,b,c){logError("JavaScript error: "+a+"at line "+c);!0===appInited?showNotification(t("JavaScript error"),a+"at line "+c,"","danger"):showAppInitAlert(t("JavaScript error")+": "+a+" at line "+c);return!0};appInitStart();function toggleAlert(a,b,c){a=document.getElementById(a);!1===b?(a.innerHTML="",a.classList.add("hide")):(a.innerHTML=c,a.classList.remove("hide"))}
...
document.getElementById("inputLastPlayedCount").value=settings.lastPlayedCount;toggleBtnChk("btnSmartpls",settings.smartpls);settings.featDate=settings.tags.includes("Date")?!0:!1;b="featLocalplayer featSyscmds featMixramp featCacert featBookmarks featDate featRegex".split(" ");for(c=0;c<b.length;c++)for(var d=document.getElementsByClassName(b[c]),f=d.length,g=!0===settings[b[c]]?"":"none",h=0;h<f;h++)d[h].style.display=g;b=document.getElementsByClassName("warnReadonly");for(c=0;c<b.length;c++)!1===
settings.readonly?b[c].classList.add("hide"):b[c].classList.remove("hide");!0===settings.readonly?(document.getElementById("btnBookmarks").setAttribute("disabled","disabled"),document.getElementsByClassName("groupClearCovercache")[0].classList.add("hide")):(document.getElementById("btnBookmarks").removeAttribute("disabled"),document.getElementsByClassName("groupClearCovercache")[0].classList.remove("hide"));if(settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(b=4<c?"":'<div class="dropdown-divider"></div>',
d=0;d<c;d++)b="HR"===settings.syscmdList[d]?b+'<div class="dropdown-divider"></div>':b+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+e(settings.syscmdList[d])+"\"]}'>"+e(settings.syscmdList[d])+"</a>");document.getElementById("syscmds").innerHTML=b;4<c?(document.getElementById("navSyscmds").classList.remove("hide"),document.getElementById("syscmds").classList.add("collapse","menu-indent")):(document.getElementById("navSyscmds").classList.add("hide"),
document.getElementById("syscmds").classList.remove("collapse","menu-indent"))}else document.getElementById("syscmds").innerHTML="";dropdownMainMenu=new Dropdown(document.getElementById("mainMenu"));document.getElementById("selectJukeboxMode").value=settings.jukeboxMode;document.getElementById("inputJukeboxQueueLength").value=settings.jukeboxQueueLength;0===settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled",
"disabled")):2===settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):1===settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));!0===settings.featLocalplayer&&(""===settings.streamUrl?
(settings.mpdstream="http://",null!==settings.mpdHost.match(/^127\./)||"localhost"===settings.mpdHost||null!==settings.mpdHost.match(/^\//)?settings.mpdstream+=window.location.hostname:settings.mpdstream+=settings.mpdHost,settings.mpdstream+=":"+settings.streamPort+"/"):settings.mpdstream=settings.streamUrl,b=document.getElementById("localPlayer"),b.src!==settings.mpdstream&&(b.pause(),document.getElementById("alertLocalPlayback").classList.remove("hide"),b.src=settings.mpdstream,b.load()));"auto"===
settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=settings.musicDirectory,document.getElementById("inputMusicDirectory").value=settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):"none"===settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=settings.musicDirectory,document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly",
...
settings.replaygain;for(var a="featStickers featSmartpls featPlaylists featTags featCoverimage featAdvsearch featLove".split(" "),b=0;b<a.length;b++){var c=document.getElementsByClassName(a[b]),d=c.length,f=!0===settings[a[b]]?"":"none";"featCoverimage"===a[b]&&!1===settings.coverimage&&(f="none");for(var g=0;g<d;g++)c[g].style.display=f}!1===settings.coverimage||!1===settings.featTags||0==settings.tags.includes("AlbumArtist")||0==settings.tags.includes("Album")||0==settings.tags.includes("Track")?
...
""===settings.musicDirectoryValue&&"none"!==settings.musicDirectory?document.getElementById("warnMusicDirectory").classList.remove("hide"):document.getElementById("warnMusicDirectory").classList.add("hide");!0===settings.bgCover&&!0===settings.featCoverimage&&!0===settings.coverimage?setBackgroundImage(lastSongObj.uri):clearBackgroundImage();if(!1===settings.featTags)app.apps.Browse.active="Filesystem",app.apps.Search.state="0/filename/-/",app.apps.Queue.state="0/filename/-/",settings.colsQueueCurrent=
["Pos","Title","Duration"],settings.colsQueueLastPlayed=["Pos","Title","LastPlayed"],settings.colsSearch=["Title","Duration"],settings.colsBrowseFilesystem=["Type","Title","Duration"],settings.colsBrowseDatabase=["Track","Title","Duration"],settings.colsPlayback=[];else{a="";for(b=0;b<settings.colsPlayback.length;b++)a+='<div id="current'+settings.colsPlayback[b]+'" data-tag="'+settings.colsPlayback[b]+'" data-name="'+(lastSongObj[settings.colsPlayback[b]]?encodeURI(lastSongObj[settings.colsPlayback[b]]):
"")+'"><small>'+t(settings.colsPlayback[b])+"</small><p",settings.browsetags.includes(settings.colsPlayback[b])&&(a+=' class="clickable"'),a+=">",a="Duration"===settings.colsPlayback[b]?a+(lastSongObj[settings.colsPlayback[b]]?beautifySongDuration(lastSongObj[settings.colsPlayback[b]]):""):"Fileformat"===settings.colsPlayback[b]?a+(lastState?fileformat(lastState.audioFormat):""):a+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):""),a+="</p></div>";document.getElementById("cardPlaybackTags").innerHTML=
...
function initTagMultiSelect(a,b,c,d){for(var f=[],g="",h=0;h<c.length;h++)d.includes(c[h])&&f.push(t(c[h])),g+='<div class="form-check"><button class="btn btn-secondary btn-xs clickable material-icons material-icons-small'+(d.includes(c[h])?" active":"")+'" name="'+c[h]+'">'+(d.includes(c[h])?"check":"radio_button_unchecked")+'</button><label class="form-check-label" for="'+c[h]+'">&nbsp;&nbsp;'+t(c[h])+"</label></div>";document.getElementById(a).value=f.join(", ");document.getElementById(b).innerHTML=
g;document.getElementById(b).addEventListener("click",function(a){a.stopPropagation();a.preventDefault();"BUTTON"===a.target.nodeName&&(toggleBtnChk(a.target),a.target.parentNode.parentNode.parentNode.previousElementSibling.value=getTagMultiSelectValues(a.target.parentNode.parentNode,!0))})}
...
function toggleBtnChk(a,b){var c=a;"string"===typeof a&&(c=document.getElementById(a));if(c){if(void 0===b)b=c.classList.contains("active")?!1:!0;else if(0===b||1===b)b=1===b?!0:!1;!0===b?(c.classList.add("active"),c.innerText="check"):(c.classList.remove("active"),c.innerText="radio_button_unchecked")}}
