...
!1);document.getElementById("modalSongDetails").getElementsByTagName("tbody")[0].addEventListener("click",function(a){if("A"==a.target.nodeName)if("calcFingerprint"==a.target.id){sendAPI({cmd:"MPD_API_DATABASE_FINGERPRINT",data:{uri:decodeURI(a.target.getAttribute("data-uri"))}},parseFingerprint);a.preventDefault();a=a.target.parentNode;var b=document.createElement("div");b.classList.add("spinner-border","spinner-border-sm");a.replaceChild(b,a.firstChild)}else void 0!=a.target.parentNode.getAttribute("data-tag")&&
...
!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(a){if(settings.featAdvsearch&&"TH"==a.target.nodeName){var b=a.target.getAttribute("data-col");if("Duration"!=b){var c=app.current.sort,d=!0;if(c==b||c=="-"+b)0==c.indexOf("-")?(d=!0,c.substring(1)):d=!1;0==d?(c="-"+b,d=!0):(d=!1,c=b);for(var l=document.getElementById("SearchList").getElementsByClassName("sort-dir"),k=0;k<l.length;k++)l[k].remove();app.current.sort=c;a.target.innerHTML=b+'<span class="sort-dir material-icons pull-right">'+
...
function parseFilesystem(a){var b=app.current.app+("Filesystem"==app.current.tab?app.current.tab:""),c=settings["cols"+b].length;c--;var d=a.data.length,f=document.getElementById(app.current.app+(void 0==app.current.tab?"":app.current.tab)+"List"),g=f.getElementsByTagName("tbody")[0],h=g.getElementsByTagName("tr");f=document.activeElement.parentNode.parentNode==f?!0:!1;for(var l=0,k=0;k<d;k++){var m=encodeURI(a.data[k].uri),p=document.createElement("tr");p.setAttribute("data-type",a.data[k].Type);
p.setAttribute("data-uri",m);p.setAttribute("tabindex",0);"song"==a.data[k].Type?p.setAttribute("data-name",a.data[k].Title):p.setAttribute("data-name",a.data[k].name);switch(a.data[k].Type){case "dir":case "smartpls":case "plist":m="";for(var n=0;n<settings["cols"+b].length;n++)m+='<td data-col="'+settings["cols"+b][n]+'">',"Type"==settings["cols"+b][n]?m="dir"==a.data[k].Type?m+'<span class="material-icons">folder_open</span>':m+('<span class="material-icons">'+("smartpls"==a.data[k].Type?"queue_music":
"list")+"</span>"):"Title"==settings["cols"+b][n]&&(m+=e(a.data[k].name)),m+="</td>";m+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';p.innerHTML=m;break;case "song":a.data[k].Duration=beautifySongDuration(a.data[k].Duration);m="";for(n=0;n<settings["cols"+b].length;n++)m+='<td data-col="'+settings["cols"+b][n]+'">',m="Type"==settings["cols"+b][n]?m+'<span class="material-icons">music_note</span>':m+e(a.data[k][settings["cols"+b][n]]),m+="</td>";m+=
'<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';p.innerHTML=m}k<h.length?l=1==replaceTblRow(h[k],p)?k:l:g.append(p)}for(k=h.length-1;k>=d;k--)h[k].remove();1==f&&focusTable(0);setPagination(a.totalEntities,a.returnedEntities);0==d&&(g.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+c+'">'+t("Empty list")+"</td></tr>");document.getElementById(app.current.app+(void 0==app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05");
document.getElementById("cardFooterBrowse").innerText=t("Num entries",a.totalEntities)}
...
h<f.length?g=1==replaceTblRow(f[h],k)?h:g:d.append(k)}document.getElementById("cardFooterBrowse").innerText=gtPage("Num playlists",a.returnedEntities,a.totalEntities)}else if("Detail"==app.current.view){for(h=0;h<b;h++){l=encodeURI(a.data[h].uri);k=document.createElement("tr");0==a.smartpls&&k.setAttribute("draggable","true");k.setAttribute("id","playlistTrackId"+a.data[h].Pos);k.setAttribute("data-type",a.data[h].Type);k.setAttribute("data-uri",l);k.setAttribute("data-name",a.data[h].Title);k.setAttribute("data-songpos",
a.data[h].Pos);k.setAttribute("tabindex",0);a.data[h].Duration=beautifySongDuration(a.data[h].Duration);l="";for(var m=0;m<settings.colsBrowsePlaylistsDetail.length;m++)l+='<td data-col="'+settings.colsBrowsePlaylistsDetail[m]+'">'+e(a.data[h][settings.colsBrowsePlaylistsDetail[m]])+"</td>";l+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';k.innerHTML=l;h<f.length?g=1==replaceTblRow(f[h],k)?h:g:d.append(k)}document.getElementById("cardFooterBrowse").innerText=
...
function parseFingerprint(a){var b=document.createElement("textarea");b.value=a.data.fingerprint;b.classList.add("form-control","text-monospace","small");a=document.getElementById("fingerprint");a.replaceChild(b,a.firstChild)}
