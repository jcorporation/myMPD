...
socket.close();websocketConnected=!1});document.getElementById("localPlayer").addEventListener("canplay",function(){document.getElementById("alertLocalPlayback").classList.add("hide");1==settings.featLocalplayer&&1==settings.localplayerAutoplay&&localplayerPlay()})}
...
1==settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));1==settings.featLocalplayer&&(""==settings.streamUrl?(settings.mpdstream="http://",null!=settings.mpdHost.match(/^127\./)||"localhost"==settings.mpdHost||null!=settings.mpdHost.match(/^\//)?settings.mpdstream+=window.location.hostname:settings.mpdstream+=settings.mpdHost,settings.mpdstream+=":"+settings.streamPort+"/"):
settings.mpdstream=settings.streamUrl,a=document.getElementById("localPlayer"),a.src!=settings.mpdstream&&(a.pause(),document.getElementById("alertLocalPlayback").classList.remove("hide"),a.src=settings.mpdstream,a.load()));b=document.getElementsByClassName("mixramp");for(a=0;a<b.length;a++)b[a].style.display=1==settings.featMixramp?"":"none";"Queue"==app.current.app&&"Current"==app.current.tab?getQueue():"Queue"==app.current.app&&"LastPlayed"==app.current.tab?appRoute():"Search"==app.current.app?
appRoute():"Browse"==app.current.app&&"Filesystem"==app.current.tab?appRoute():"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view?appRoute():"Browse"==app.current.app&&"Database"==app.current.tab&&""!=app.current.search&&appRoute();settingsParsed="true"}
...
1==settings.bgCover&&1==settings.featCoverimage&&1==settings.coverimage?lastSongObj.data&&-1<lastSongObj.data.cover.indexOf("coverimage-")?clearBackgroundImage():lastSongObj.data?setBackgroundImage(lastSongObj.data.cover):clearBackgroundImage():clearBackgroundImage();if(0==settings.featTags)app.apps.Browse.active="Filesystem",app.apps.Search.state="0/filename/-/",app.apps.Queue.state="0/filename/-/",settings.colsQueueCurrent=["Pos","Title","Duration"],settings.colsQueueLastPlayed=["Pos","Title","LastPlayed"],
settings.colsSearch=["Title","Duration"],settings.colsBrowseFilesystem=["Type","Title","Duration"],settings.colsBrowseDatabase=["Track","Title","Duration"],settings.colsPlayback=[];else{a="";for(f=0;f<settings.colsPlayback.length;f++)a+='<div id="current'+settings.colsPlayback[f]+'" data-tag="'+settings.colsPlayback[f]+'" data-name="'+encodeURI(lastSongObj.data?lastSongObj.data[settings.colsPlayback[f]]:"")+'"><small>'+settings.colsPlayback[f]+"</small><h4",settings.browsetags.includes(settings.colsPlayback[f])&&
...
setCols("BrowseFilesystem");setCols("BrowsePlaylistsDetail");setCols("BrowseDatabase",".tblAlbumTitles");setCols("Playback");addTagList("BrowseDatabaseByTagDropdown","browsetags");addTagList("searchqueuetags","searchtags");addTagList("searchtags","searchtags");for(f=0;f<settings.tags.length;f++)app.apps.Browse.tabs.Database.views[settings.tags[f]]={state:"0/-/-/",scrollPos:0};initTagMultiSelect("inputEnabledTags","listEnabledTags",settings.allmpdtags,settings.tags);initTagMultiSelect("inputSearchTags",
"listSearchTags",settings.tags,settings.searchtags);initTagMultiSelect("inputBrowseTags","listBrowseTags",settings.tags,settings.browsetags)}
...
function saveConnection(){var a=!0,b=document.getElementById("inputMpdHost"),c=document.getElementById("inputMpdPort"),d=document.getElementById("inputMpdPass");""==c.value&&(c.value="6600");0!=b.value.indexOf("/")&&(validateInt(c)||(a=!1),validateHost(b)||(a=!1));1==a&&(sendAPI({cmd:"MYMPD_API_CONNECTION_SAVE",data:{mpdHost:b.value,mpdPort:c.value,mpdPass:d.value}}),modalConnection.hide())}
...
lastState.data.queueVersion==a.data.queueVersion||sendAPI({cmd:"MPD_API_PLAYER_CURRENT_SONG"},songChange);if("-1"==a.data.songPos){domCache.currentTitle.innerText="Not playing";clearCurrentCover();1==settings.bgCover&&clearBackgroundImage();for(var b=document.getElementById("cardPlaybackTags").getElementsByTagName("h4"),c=0;c<b.length;c++)b[c].innerText=""}"Queue"==app.current.app&&"LastPlayed"==app.current.tab&&sendAPI({cmd:"MPD_API_QUEUE_LAST_PLAYED",data:{offset:app.current.page,cols:settings.colsQueueLastPlayed}},
...
function validateFloat(a){if(""!=a.value.replace(/[\d\-\.]/g,""))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}function validateStream(a){if(-1<a.value.indexOf("://"))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function validateHost(a){if(null==a.value.match("^(w-.)$"))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function log_error(a){log_log(0,"ERROR: "+a)}
function log_warn(a){log_log(1,"WARN: "+a)}function log_info(a){log_log(2,"INFO: "+a)}function log_verbose(a){log_log(3,"VERBOSE: "+a)}function log_debug(a){log_log(4,"DEBUG: "+a)}function log_log(a,b){settings.loglevel>=a&&console.log(b)}appInitStart();
