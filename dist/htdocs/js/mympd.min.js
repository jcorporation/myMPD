...
function parseCovergrid(a){for(var b=a.result.returnedEntities,c=document.getElementById("BrowseCovergridList"),d=c.getElementsByClassName("card"),f=0;f<b;f++){var g=document.createElement("div");g.classList.add("card","card-grid","mr-3","mb-3");g.setAttribute("data-uri",encodeURI(a.result.data[f].FirstSongUri));g.innerHTML='<div class="card-body album-cover-loading album-cover-grid"></div><div class="card-footer card-footer-grid">'+a.result.data[f].Album+"<br/><small>"+a.result.data[f].AlbumArtist+
"</small></div>";var h=!1;f<d.length?d[f].getAttribute("data-uri")!==g.getAttribute("data-uri")&&(d[f].replaceWith(g),h=!0):(c.append(g),h=!0);"IntersectionObserver"in window&&!0===h?(new IntersectionObserver(setGridImage,{root:null,rootMargin:"0px"})).observe(g):!0===h&&(g.style.backgroundImage='url("/albumart/'+a.result.data[f].uri+'")')}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML=t("Empty list"));document.getElementById(app.current.app+
(void 0===app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05");document.getElementById("cardFooterBrowse").innerText=""}function setGridImage(a,b){a.forEach(function(a){if(0<a.intersectionRatio){b.unobserve(a.target);var c=decodeURI(a.target.getAttribute("data-uri"));a.target.firstChild.style.backgroundImage='url("/albumart/'+c+'")'}})}function e(a){return isNaN(a)?a.replace(/([<>])/g,function(a,c){if("<"===c)return"&lt;";if(">"===c)return"&gt;"}):a}
...
scrollPos:0}}},Browse:{active:"Database",tabs:{Filesystem:{state:"0/-/-/",scrollPos:0},Playlists:{active:"All",views:{All:{state:"0/-/-/",scrollPos:0},Detail:{state:"0/-/-/",scrollPos:0}}},Database:{active:"AlbumArtist",views:{}},Covergrid:{state:"0/-/-/",scrollPos:0}}},Search:{state:"0/any/-/",scrollPos:0}},current:{app:"Playback",tab:void 0,view:void 0,page:0,filter:"",search:"",sort:"",scrollPos:0},last:{app:void 0,tab:void 0,view:void 0,filter:"",search:"",sort:"",scrollPos:0}},domCache={};
...
'">'+e(b[f])+"</a></li>";d+="/"}document.getElementById("BrowseBreadcrumb").innerHTML=a;doSetFilterLetter("BrowseFilesystemFilter")}else if("Browse"===app.current.app&&"Covergrid"===app.current.tab)sendAPI("MPD_API_DATABASE_GET_ALBUMS",{offset:app.current.page},parseCovergrid);else if("Search"===app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a="";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)d=b[c].substring(1,b[c].length-
...
validateFilenameList(c)||(a=!1);c=document.getElementById("inputMaxElementsPerPage");validateInt(c)||(a=!1);200<parseInt(c.value)&&(a=!1);c=document.getElementById("inputLastPlayedCount");validateInt(c)||(a=!1);document.getElementById("btnLoveEnable").classList.contains("active")&&(c=document.getElementById("inputLoveChannel"),f=document.getElementById("inputLoveMessage"),validateNotBlank(c)&&validateNotBlank(f)||(a=!1));!0===settings.featMixramp&&(c=document.getElementById("inputMixrampdb"),c.getAttribute("disabled")||
