...
function logLog(a,b){settings.loglevel>=a&&(0===a?console.error(b):1===a?console.warn(b):4===a?console.debug(b):console.log(b))}function setViewport(a){document.querySelector("meta[name=viewport]").setAttribute("content","width=device-width, initial-scale="+scale+", maximum-scale="+scale);if(!0===a)try{localStorage.setItem("scale-ratio",scale)}catch(b){logError("Can not save scale-ratio in localStorage: "+b.message)}}
...
function(){selectTimerActionChange()},!1);a="";for(b=0;24>b;b++)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerHour").innerHTML=a;a="";for(b=0;60>b;b+=5)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerMinute").innerHTML=a;document.getElementById("inputHighlightColor").addEventListener("change",function(){document.getElementById("highlightColorPreview").style.backgroundColor=this.value},!1);document.getElementById("inputBgColor").addEventListener("change",
...
if("TD"===a.target.nodeName){if(""===a.target.parentNode.getAttribute("data-point"))return!1;showEditMount(decodeURI(a.target.parentNode.getAttribute("data-url")),decodeURI(a.target.parentNode.getAttribute("data-point")))}else if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===b?unmountMount(c):"update"===b&&updateMount(a.target,c)}},!1);document.getElementById("listScriptsList").addEventListener("click",
function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){if(""===a.target.parentNode.getAttribute("data-script"))return!1;showEditScript(decodeURI(a.target.parentNode.getAttribute("data-script")))}else if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-script"));"delete"===b?deleteScript(c):"execute"===b&&execScript(a.target.getAttribute("data-href"))}},!1);document.getElementById("QueueCurrentList").addEventListener("click",
...
"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)}}},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&appGoto(app.current.app,app.current.tab,a.target.getAttribute("data-tag"),"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",
function(a){"Enter"===a.key&&(a.preventDefault(),a.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(a){"OPTION"===a.target.nodeName&&removeScriptArgument(a)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");
...
parseCmd(a,b)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);var d="";Object.keys(themes).forEach(function(a){d+='<option value="'+a+'">'+t(themes[a])+"</option>"});document.getElementById("selectTheme").innerHTML=d;window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a;domCache.btnAdd.classList.remove("hide")});domCache.btnAdd.addEventListener("click",
...
function parseScriptList(a){var b=document.createElement("optgroup");b.setAttribute("data-value","script");b.setAttribute("label",t("Script"));var c="",d="",f=a.result.data.length;if(0<f){a.result.data.sort(function(a,b){return a.metadata.order-b.metadata.order});for(var g=0,h=0;h<f;h++){var k="";0<a.result.data[h].metadata.arguments.length&&(k='"'+a.result.data[h].metadata.arguments.join('","')+'"');0<a.result.data[h].metadata.order&&(0===g&&(c=4<f?"":'<div class="dropdown-divider"></div>'),g++,
c+='<a class="dropdown-item text-light alwaysEnabled" href="#" data-href=\'{"script": "'+e(a.result.data[h].name)+'", "arguments": ['+k+"]}'>"+e(a.result.data[h].name)+"</a>");d+='<tr data-script="'+encodeURI(a.result.data[h].name)+'"><td>'+e(a.result.data[h].name)+'</td><td data-col="Action"><a href="#" title="'+t("Delete")+'" data-action="delete" class="material-icons color-darkgrey">delete</a><a href="#" title="'+t("Execute")+'" data-action="execute" class="material-icons color-darkgrey"  data-href=\'{"script": "'+
e(a.result.data[h].name)+'", "arguments": ['+k+"]}'>play_arrow</a></td></tr>";b.innerHTML+='<option data-arguments=\'{"arguments":['+k+"]}' value=\""+e(a.result.data[h].name)+'">'+e(a.result.data[h].name)+"</option>"}document.getElementById("listScriptsList").innerHTML=d}else document.getElementById("listScriptsList").innerHTML='<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="2">'+t("Empty list")+"</td></tr>";document.getElementById("scripts").innerHTML=
c;4<f?(document.getElementById("navScripting").classList.remove("hide"),document.getElementById("scripts").classList.add("collapse","menu-indent")):(document.getElementById("navScripting").classList.add("hide"),document.getElementById("scripts").classList.remove("collapse","menu-indent"));(a=document.getElementById("selectTimerAction").querySelector('optgroup[data-value="script"]'))?a.replaceWith(b):document.getElementById("selectTimerAction").appendChild(b)}
