...
a.data.dbUpdated);document.getElementById("mpdstats_dbUpdated").innerText=b.toLocaleString();document.getElementById("mympdVersion").innerText=a.data.mympdVersion;document.getElementById("mpdInfo_version").innerText=a.data.mpdVersion;document.getElementById("mpdInfo_libmpdclientVersion").innerText=a.data.libmpdclientVersion}function toggleBtn(a,b){if(a=document.getElementById(a))void 0==b&&(b=a.classList.contains("active")?0:1),1==b||1==b?a.classList.add("active"):a.classList.remove("active")}
...
lastState.data.queueVersion==a.data.queueVersion||sendAPI({cmd:"MPD_API_PLAYER_CURRENT_SONG"},songChange);if("-1"==a.data.songPos){domCache.currentTitle.innerText="Not playing";domCache.currentCover.style.backgroundImage="";"cover"==settings.background&&document.documentElement.style.setProperty("--mympd-backgroundimage",'url("")');for(var b=document.getElementById("cardPlaybackTags").getElementsByTagName("h4"),c=0;c<b.length;c++)b[c].innerText=""}"Queue"==app.current.app&&"LastPlayed"==app.current.tab&&
sendAPI({cmd:"MPD_API_QUEUE_LAST_PLAYED",data:{offset:app.current.page,cols:settings.colsQueueLastPlayed}},parseLastPlayed);lastState=a;0!=settings.mpdConnected&&0!=uiEnabled||getSettings(!0)}}
...
function beautifySongDuration(a){var b=Math.floor(a/3600),c=Math.floor(a/60)-60*b;a=a-3600*b-60*c;return(0<b?b+":"+(10>c?"0":""):"")+c+":"+(10>a?"0":"")+a}function genId(a){return"id"+a.replace(/[^\w\-]/g,"")}function validate_plname(a){return""==a?!1:null==a.match(/\/|\r|\n|"|'/)?!0:!1}function log_error(a){log_log(0,"ERROR: "+a)}function log_warn(a){log_log(1,"WARN: "+a)}function log_info(a){log_log(2,"INFO: "+a)}function log_verbose(a){log_log(3,"VERBOSE: "+a)}
