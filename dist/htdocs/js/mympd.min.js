...
function setElsState(a,b){a=document.getElementsByTagName(a);for(var c=a.length,d=0;d<c;d++)"disabled"==b?a[d].classList.contains("alwaysEnabled")||(a[d].setAttribute("disabled","disabled"),a[d].classList.add("disabled")):a[d].classList.contains("disabled")&&(a[d].removeAttribute("disabled"),a[d].classList.remove("disabled"))}function toggleUI(a){setElsState("a",a);setElsState("input",a);setElsState("button",a)}
function webSocketConnect(){var a=getWsUrl();socket=new WebSocket(a);console.log("Connecting to "+a);try{socket.onopen=function(){console.log("Websocket is connected")},socket.onmessage=function(b){try{var c=JSON.parse(b.data)}catch(d){console.log("Invalid JSON data received: "+b.data)}switch(c.type){case "welcome":websocketConnected=!0;showNotification("Connected to myMPD: "+a,"","","success");toggleAlert("alertMympdState",!1,"");appRoute();sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);toggleUI("enabled");
break;case "update_state":parseState(c);break;case "mpd_disconnected":toggleAlert("alertMpdState",!0,"Connection to MPD failed.");progressTimer&&clearTimeout(progressTimer);toggleUI("disabled");break;case "mpd_connected":showNotification("Connected to MPD","","","success");toggleAlert("alertMpdState",!1,"");sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);getSettings(!0);toggleUI("enabled");break;case "update_queue":"Queue"===app.current.app&&getQueue();sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);
break;case "update_options":getSettings();break;case "update_outputs":sendAPI({cmd:"MPD_API_PLAYER_OUTPUT_LIST"},parseOutputs);break;case "update_started":updateDBstarted(!1);break;case "update_database":case "update_finished":updateDBfinished(c.type);break;case "update_volume":parseVolume(c);break;case "update_stored_playlist":"Browse"==app.current.app&&"Playlists"==app.current.tab&&"All"==app.current.view?sendAPI({cmd:"MPD_API_PLAYLIST_LIST",data:{offset:app.current.page,filter:app.current.filter}},
parsePlaylists):"Browse"==app.current.app&&"Playlists"==app.current.tab&&"Detail"==app.current.view&&sendAPI({cmd:"MPD_API_PLAYLIST_CONTENT_LIST",data:{offset:app.current.page,filter:app.current.filter,uri:app.current.search,cols:settings.colsBrowsePlaylistsDetail}},parsePlaylists);break;case "error":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(c.data,"","","danger")}},socket.onclose=function(){console.log("Websocket is disconnected");1==appInited?(toggleAlert("alertMympdState",
!0,"Websocket connection failed."),toggleUI("disabled"),progressTimer&&clearTimeout(progressTimer)):showAppInitAlert("Websocket connection failed.");websocketConnected=!1;null!=websocketTimer&&clearTimeout(websocketTimer);websocketTimer=setTimeout(function(){console.log("Reconnecting websocket");toggleAlert("alertMympdState",!0,'Websocket connection failed. Trying to reconnect&nbsp;&nbsp;<div class="spinner-border spinner-border-sm"></div>');webSocketConnect()},3E3)}}catch(b){alert("Error: "+b)}}
...
function parseSettings(){settings.mpdConnected?parseMPDSettings():toggleAlert("alertMpdState",!0,"Connection to MPD failed.");0!=settings.mpdhost.indexOf("/")?document.getElementById("mpdHost").innerText=settings.mpdhost+":"+settings.mpdport:document.getElementById("mpdInfo_host").innerText=settings.mpdhost;var a=document.getElementById("btnnotifyWeb");notificationsSupported()?settings.notificationWeb?(toggleBtn("btnnotifyWeb",settings.notificationWeb),Notification.requestPermission(function(a){"permission"in
Notification||(Notification.permission=a);"granted"===a?toggleBtn("btnnotifyWeb",1):(toggleBtn("btnnotifyWeb",0),settings.notificationWeb=!0)})):toggleBtn("btnnotifyWeb",0):(a.setAttribute("disabled","disabled"),toggleBtn("btnnotifyWeb",0));toggleBtn("btnnotifyPage",settings.notificationPage);document.documentElement.style.setProperty("--mympd-coverimagesize",settings.coverimagesize+"px");"cover"!=settings.background?document.documentElement.style.setProperty("--mympd-backgroundcolor",settings.background):
document.documentElement.style.setProperty("--mympd-backgroundfilter",settings.backgroundFilter);for(var b=["featLocalplayer","featSyscmds"],c=0;c<b.length;c++){var d=document.getElementsByClassName(b[c]),e=d.length,f=1==settings[b[c]]?"":"none";for(a=0;a<e;a++)d[a].style.display=f}if(settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(b='<div class="dropdown-divider"></div>',a=0;a<c;a++)b="HR"==settings.syscmdList[a]?b+'<div class="dropdown-divider"></div>':b+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+
...
""}"Queue"==app.current.app&&"LastPlayed"==app.current.tab&&sendAPI({cmd:"MPD_API_QUEUE_LAST_PLAYED",data:{offset:app.current.page,cols:settings.colsQueueLastPlayed}},parseLastPlayed);lastState=a}}
...
function parseListTitles(a){var b=genId(a.Album),c=document.getElementById("card"+b),d=c.getElementsByTagName("tbody")[0],e=c.querySelector(".card-header");e.setAttribute("data-uri",encodeURI(a.data[0].uri.replace(/\/[^\/]+$/,"")));e.setAttribute("data-name",a.Album);e.setAttribute("data-type","dir");e.addEventListener("click",function(a){showMenu(this,a)},!1);e.classList.add("clickable");if(c=c.getElementsByTagName("a")[0])c.style.backgroundImage='url("'+subdir+a.cover+'")',c.setAttribute("data-uri",
encodeURI(a.data[0].uri.replace(/\/[^\/]+$/,""))),c.setAttribute("data-name",a.Album),c.setAttribute("data-type","dir"),c.addEventListener("click",function(a){showMenu(this,a)},!1);document.getElementById("albumartist"+b).innerText=a.AlbumArtist;b="";c=a.data.length;for(e=0;e<c;e++){if(a.data[e].Duration){var f=Math.floor(a.data[e].Duration/60),g=a.data[e].Duration-60*f;a.data[e].Duration=f+":"+(10>g?"0":"")+g}b+='<tr data-type="song" data-name="'+a.data[e].Title+'" data-uri="'+encodeURI(a.data[e].uri)+
'">';for(f=0;f<settings.colsBrowseDatabase.length;f++)b+='<td data-col="'+settings.colsBrowseDatabase[f]+'">'+a.data[e][settings.colsBrowseDatabase[f]]+"</td>";b+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td></tr>'}d.innerHTML=b;d.parentNode.addEventListener("click",function(a){"TD"==a.target.nodeName?appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name")):"A"==a.target.nodeName&&showMenu(a.target,
a)},!1)}
function setPagination(a,b){var c=app.current.app+(void 0==app.current.tab?"":app.current.tab),d=Math.ceil(a/settings.maxElementsPerPage);0==d&&(d=1);for(var e=["PaginationTop","PaginationBottom"],f=0;2>f;f++){document.getElementById(c+e[f]+"Page").innerText=app.current.page/settings.maxElementsPerPage+1+" / "+d;if(1<d){document.getElementById(c+e[f]+"Page").removeAttribute("disabled");for(var g="",h=0;h<d;h++)g+='<button data-page="'+h*settings.maxElementsPerPage+'" type="button" class="mr-1 mb-1 btn-sm btn btn-secondary">'+(h+
...
function songChange(a){if("song_change"==a.type){var b=a.data.Title+a.data.Artist+a.data.Album+a.data.uri+a.data.currentSongId;if(lastSong!=b){var c="",d="",e="myMPD: ";domCache.currentCover.style.backgroundImage='url("'+subdir+a.data.cover+'")';"cover"==settings.background&&(-1<a.data.cover.indexOf("coverimage-")?document.documentElement.style.setProperty("--mympd-backgroundimage",'url("")'):document.documentElement.style.setProperty("--mympd-backgroundimage",'url("'+a.data.cover+'")'));"undefined"!=
typeof a.data.Artist&&0<a.data.Artist.length&&"-"!=a.data.Artist&&(c+=a.data.Artist,d+=a.data.Artist,e+=a.data.Artist+" - ");"undefined"!=typeof a.data.Album&&0<a.data.Album.length&&"-"!=a.data.Album&&(c+=" - "+a.data.Album,d+="<br/>"+a.data.Album);"undefined"!=typeof a.data.Title&&0<a.data.Title.length?(e+=a.data.Title,domCache.currentTitle.innerText=a.data.Title,domCache.currentTitle.setAttribute("data-uri",encodeURI(a.data.uri))):(domCache.currentTitle.innerText="",domCache.currentTitle.setAttribute("data-uri",
""));document.title=e;1==settings.featStickers&&setVoteSongBtns(a.data.like,a.data.uri);for(e=0;e<settings.colsPlayback.length;e++){var f=document.getElementById("current"+settings.colsPlayback[e]);f&&(f.getElementsByTagName("h4")[0].innerText=a.data[settings.colsPlayback[e]],f.setAttribute("data-name",encodeURI(a.data[settings.colsPlayback[e]])))}if(e=document.getElementById("queueTrackId"+a.data.currentSongId))e.getElementsByTagName("td")[1].innerText=a.data.Title;"play"==playstate&&showNotification(a.data.Title,
