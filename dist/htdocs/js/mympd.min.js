...
0;a<this.onSettledCallbacks_.length;++a)g.asyncExecute(this.onSettledCallbacks_[a]);this.onSettledCallbacks_=null}};var g=new b;f.prototype.settleSameAsPromise_=function(a){var b=this.createResolveAndReject_();a.callWhenSettled_(b.resolve,b.reject)};f.prototype.settleSameAsThenable_=function(a,b){var c=this.createResolveAndReject_();try{a.call(b,c.resolve,c.reject)}catch(m){c.reject(m)}};f.prototype.then=function(a,b){function c(a,b){return"function"==typeof a?function(b){try{d(a(b))}catch(n){g(n)}}:
...
d){for(var f=$jscomp.makeIterator(a),g=f.next();!g.done;g=f.next())c(g.value).callWhenSettled_(b,d)})};f.all=function(a){var b=$jscomp.makeIterator(a),d=b.next();return d.done?c([]):new f(function(a,f){function g(b){return function(c){h[b]=c;k--;0==k&&a(h)}}var h=[],k=0;do h.push(void 0),k++,c(d.value).callWhenSettled_(g(h.length-1),f),d=b.next();while(!d.done)})};return f},"es6","es3");$jscomp.underscoreProtoCanBeSet=function(){var a={a:!0},b={};try{return b.__proto__=a,b.a}catch(c){}return!1};
...
app.current.filter)}else appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appGoto("Playback")}}
function initState(a,b){a=document.getElementById(a);a.classList.remove("text-success","text-danger","spinner-border","spinner-border-sm");a.innerText="";"blank"==b?a.innerText="":"load"==b?(a.innerText="",a.classList.add("spinner-border","spinner-border-sm")):"ok"==b?(a.innerText="check",a.classList.add("material-icons","text-success")):"error"==b&&(a.innerText="error_outline",a.classList.add("material-icons","text-danger"))}
function showAppInitAlert(a){var b=document.getElementById("modalAppInitAlert");b.innerHTML="<p>"+e(a)+'</p><a id ="appReloadBtn"class="btn btn-danger text-light" class="clickable">Reload</a>';b.classList.remove("hide");document.getElementById("appReloadBtn").addEventListener("click",function(){location.reload()},!1)}
function appInitStart(){subdir=window.location.pathname.replace("/index.html","").replace(/\/$/,"");for(var a='<option value="default" data-phrase="Browser default"></option>',b=0;b<locales.length;b++)a+='<option value="'+e(locales[b].code)+'">'+e(locales[b].desc)+" ("+e(locales[b].code)+")</option>";document.getElementById("selectLocale").innerHTML=a;i18nHtml(document.getElementById("modalAppInit"));"serviceWorker"in navigator&&"https"==document.URL.substring(0,5)&&"localhost"!=window.location.hostname&&
window.addEventListener("load",function(){navigator.serviceWorker.register("/sw.min.js",{scope:"/"}).then(function(a){logInfo("ServiceWorker registration successful.");a.update()},function(a){logError("ServiceWorker registration failed: ",a)})});appInited=!1;document.getElementsByTagName("header")[0].classList.add("hide");document.getElementsByTagName("main")[0].classList.add("hide");document.getElementsByTagName("footer")[0].classList.add("hide");initState("appInitMyMPDSettings","load");initState("appInitMPDSettings",
"blank");initState("appInitWebsocket","blank");initState("appInitApply","blank");a=document.getElementById("modalAppInitAlert");a.classList.add("hide");a.innerText="";modalAppInit.show();getSettings(!0);appInitWait()}
...
addFilterLetter("BrowsePlaylistsFilterLetters");document.getElementById("syscmds").addEventListener("click",function(a){"A"==a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);for(var a=document.querySelectorAll("[data-href]"),b=a.length,c=0;c<b;c++)a[c].classList.add("clickable"),1!=(a[c].parentNode.classList.contains("noInitChilds")?!0:!1)&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1);a=document.getElementsByClassName("pages");b=a.length;
...
!1);document.getElementById("modalSongDetails").getElementsByTagName("tbody")[0].addEventListener("click",function(a){"A"==a.target.nodeName?void 0!=a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),gotoBrowse(a.target)):"BUTTON"==a.target.nodeName&&a.target.getAttribute("data-href")&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&a.stopPropagation();sendAPI({cmd:"MPD_API_PLAYER_TOGGLE_OUTPUT",
...
a)},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(a){if("TD"==a.target.nodeName)switch(a.target.parentNode.getAttribute("data-type")){case "dir":appGoto("Browse","Filesystem",void 0,"0/"+app.current.filter+"/"+app.current.sort+"/"+decodeURI(a.target.parentNode.getAttribute("data-uri")));break;case "song":appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",
decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"))}else"A"==a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",function(a){if("A"==a.target.nodeName){var b=a.target.parentNode.parentNode.getAttribute("data-id"),c=a.target.parentNode.parentNode.getAttribute("data-type"),d=decodeURI(a.target.parentNode.parentNode.getAttribute("data-uri")),k=a.target.parentNode.parentNode.firstChild.innerText,
l=a.target.getAttribute("data-href");"delete"==l?(sendAPI({cmd:"MYMPD_API_BOOKMARK_RM",data:{id:b}},function(){sendAPI({cmd:"MYMPD_API_BOOKMARK_LIST",data:{offset:0}},parseBookmarks)}),a.preventDefault(),a.stopPropagation()):"edit"==l?showBookmarkSave(b,k,d,c):"goto"==l&&appGoto("Browse","Filesystem",null,"0/-/-/"+d)}},!1);document.getElementById("BrowsePlaylistsAllList").addEventListener("click",function(a){"TD"==a.target.nodeName?appendQueue("plist",decodeURI(a.target.parentNode.getAttribute("data-uri")),
...
a.target.getAttribute("data-phrase")?addAllFromBrowse():"Add all to playlist"==a.target.getAttribute("data-phrase")&&showAddToPlaylist(app.current.search))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&("Add all to queue"==a.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue"):"Add all to playlist"==a.target.getAttribute("data-phrase")?showAddToPlaylist("SEARCH"):"Save as smart playlist"==a.target.getAttribute("data-phrase")&&
saveSearchAsSmartPlaylist())},!1);document.getElementById("BrowseDatabaseAddAllSongsDropdown").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&("Add all to queue"==a.target.getAttribute("data-phrase")?addAllFromBrowseDatabasePlist("queue"):"Add all to playlist"==a.target.getAttribute("data-phrase")&&showAddToPlaylist("DATABASE"))},!1);document.getElementById("searchtags").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&(app.current.filter=a.target.getAttribute("data-tag"),
search(domCache.searchstr.value))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(a){"Escape"==a.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+a.target.getAttribute("data-tag")+"/"+app.current.sort+
"/"+app.current.search)},!1);a="QueueCurrentColsDropdown BrowseFilesystemColsDropdown SearchColsDropdown BrowsePlaylistsDetailColsDropdown BrowseDatabaseColsDropdown PlaybackColsDropdown QueueLastPlayedColsDropdown".split(" ");for(c=0;c<a.length;c++)document.getElementById(a[c]).addEventListener("click",function(a){"INPUT"==a.target.nodeName&&a.stopPropagation()},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",
...
a.target.getAttribute("data-col");if("Duration"!=b){var c=app.current.sort,d=!0;if(c==b||c=="-"+b)0==c.indexOf("-")?(d=!0,c.substring(1)):d=!1;0==d?(c="-"+b,d=!0):(d=!1,c=b);for(var k=document.getElementById("SearchList").getElementsByClassName("sort-dir"),l=0;l<k.length;l++)k[l].remove();app.current.sort=c;a.target.innerHTML=b+'<span class="sort-dir material-icons pull-right">'+(1==d?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+
"/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)}}},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&appGoto(app.current.app,app.current.tab,a.target.getAttribute("data-tag"),"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(a){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");
dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");window.addEventListener("hashchange",appRoute,!1);window.addEventListener("focus",function(){sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState)},!1);document.addEventListener("keydown",function(a){if("INPUT"!=a.target.tagName&&"SELECT"!=a.target.tagName&&!a.ctrlKey&&!a.altKey){var b=keymap[a.key];
b&&"function"===typeof window[b.cmd]&&(void 0==keymap[a.key].req||1==settings[keymap[a.key].req])&&parseCmd(a,b)}},!1);window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a});window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a;domCache.btnAdd.classList.remove("hide")});domCache.btnAdd.addEventListener("click",function(a){domCache.btnAdd.classList.add("hide");deferredPrompt.prompt();deferredPrompt.userChoice.then(function(a){"accepted"===
a.outcome?logVerbose("User accepted the A2HS prompt"):logVerbose("User dismissed the A2HS prompt");deferredPrompt=null})});window.addEventListener("appinstalled",function(a){logInfo("myMPD installed as app");showNotification(t("myMPD installed as app"),"","","success")});window.addEventListener("beforeunload",function(){null!=websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null);socket.onclose=function(){};null!=socket&&(socket.close(),socket=null);websocketConnected=!1});document.getElementById("localPlayer").addEventListener("canplay",
function(){document.getElementById("alertLocalPlayback").classList.add("hide");1==settings.featLocalplayer&&1==settings.localplayerAutoplay&&localplayerPlay()})}
...
"Playlists"==app.current.tab&&"Detail"==app.current.view&&sendAPI({cmd:"MPD_API_PLAYLIST_CONTENT_LIST",data:{offset:app.current.page,filter:app.current.filter,uri:app.current.search,cols:settings.colsBrowsePlaylistsDetail}},parsePlaylists);break;case "error":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.data),"","","danger")}},socket.onclose=function(){logError("Websocket is disconnected");websocketConnected=!1;1==appInited?(toggleUI(),progressTimer&&clearTimeout(progressTimer)):
(showAppInitAlert(t("Websocket connection failed")),logError("Websocket connection failed."));null!=websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null);null==websocketTimer&&(websocketTimer=setTimeout(function(){logInfo("Reconnecting websocket");toggleAlert("alertMympdState",!0,t("Websocket connection failed, trying to reconnect")+'&nbsp;&nbsp;<div class="spinner-border spinner-border-sm"></div>');webSocketConnect()},3E3));socket=null}}catch(b){logError(b)}}}
...
function toggleBtnChk(a,b){if(a=document.getElementById(a))void 0==b&&(b=a.classList.contains("active")?0:1),1==b||1==b?(a.classList.add("active"),a.innerText="check"):(a.classList.remove("active"),a.innerText="radio_button_unchecked")}
...
toggleBtnChk("btnSmartpls",settings.smartpls);for(var b=["featLocalplayer","featSyscmds"],c=0;c<b.length;c++){var d=document.getElementsByClassName(b[c]),f=d.length,g=1==settings[b[c]]?"":"none";for(a=0;a<f;a++)d[a].style.display=g}if(settings.featSyscmds){b="";c=settings.syscmdList.length;if(0<c)for(b='<div class="dropdown-divider"></div>',a=0;a<c;a++)b="HR"==settings.syscmdList[a]?b+'<div class="dropdown-divider"></div>':b+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+
e(settings.syscmdList[a])+"\"]}'>"+e(settings.syscmdList[a])+"</a>");document.getElementById("syscmds").innerHTML=b}else document.getElementById("syscmds").innerHTML="";dropdownMainMenu=new Dropdown(document.getElementById("mainMenu"));document.getElementById("selectJukeboxMode").value=settings.jukeboxMode;document.getElementById("inputJukeboxQueueLength").value=settings.jukeboxQueueLength;0==settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),
document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled")):2==settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):1==settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));
1==settings.featLocalplayer&&(""==settings.streamUrl?(settings.mpdstream="http://",null!=settings.mpdHost.match(/^127\./)||"localhost"==settings.mpdHost||null!=settings.mpdHost.match(/^\//)?settings.mpdstream+=window.location.hostname:settings.mpdstream+=settings.mpdHost,settings.mpdstream+=":"+settings.streamPort+"/"):settings.mpdstream=settings.streamUrl,a=document.getElementById("localPlayer"),a.src!=settings.mpdstream&&(a.pause(),document.getElementById("alertLocalPlayback").classList.remove("hide"),
a.src=settings.mpdstream,a.load()));b=document.getElementsByClassName("mixramp");for(a=0;a<b.length;a++)b[a].style.display=1==settings.featMixramp?"":"none";"auto"==settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=settings.musicDirectory,document.getElementById("inputMusicDirectory").value=settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):"none"==settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=
settings.musicDirectory,document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("selectMusicDirectory").value="custom",document.getElementById("inputMusicDirectory").value=settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").removeAttribute("readonly"));"Queue"==app.current.app&&"Current"==app.current.tab?getQueue():"Queue"==app.current.app&&"LastPlayed"==app.current.tab?
...
function parseState(a){if(JSON.stringify(a)===JSON.stringify(lastState))toggleUI();else{parseUpdateQueue(a);parseVolume(a);setCounter(a.data.currentSongId,a.data.totalTime,a.data.elapsedTime);lastState&&lastState.data.currentSongId==a.data.currentSongId&&lastState.data.queueVersion==a.data.queueVersion||sendAPI({cmd:"MPD_API_PLAYER_CURRENT_SONG"},songChange);if("-1"==a.data.songPos){domCache.currentTitle.innerText="Not playing";clearCurrentCover();1==settings.bgCover&&clearBackgroundImage();for(var b=
document.getElementById("cardPlaybackTags").getElementsByTagName("h4"),c=0;c<b.length;c++)b[c].innerText=""}"Queue"==app.current.app&&"LastPlayed"==app.current.tab&&sendAPI({cmd:"MPD_API_QUEUE_LAST_PLAYED",data:{offset:app.current.page,cols:settings.colsQueueLastPlayed}},parseLastPlayed);lastState=a;0!=settings.mpdConnected&&0!=uiEnabled||getSettings(!0)}}
...
function parseQueue(a){0<a.totalTime&&a.totalEntities<=settings.maxElementsPerPage?document.getElementById("cardFooterQueue").innerText=t("Num songs",a.totalEntities)+" \u2013 "+beautifyDuration(a.totalTime):0<a.totalEntities?document.getElementById("cardFooterQueue").innerText=t("Num songs",a.totalEntities):document.getElementById("cardFooterQueue").innerText="";var b=a.data.length,c=document.getElementById("QueueCurrentList");c.setAttribute("data-version",a.queueVersion);c=c.getElementsByTagName("tbody")[0];
for(var d=c.getElementsByTagName("tr"),f=0;f<b;f++){a.data[f].Duration=beautifySongDuration(a.data[f].Duration);a.data[f].Pos++;var g=document.createElement("tr");g.setAttribute("draggable","true");g.setAttribute("data-trackid",a.data[f].id);g.setAttribute("id","queueTrackId"+a.data[f].id);g.setAttribute("data-songpos",a.data[f].Pos);g.setAttribute("data-duration",a.data[f].Duration);g.setAttribute("data-uri",a.data[f].uri);for(var h="",k=0;k<settings.colsQueueCurrent.length;k++)h+='<td data-col="'+
settings.colsQueueCurrent[k]+'">'+e(a.data[f][settings.colsQueueCurrent[k]])+"</td>";h+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';g.innerHTML=h;f<d.length?d[f].replaceWith(g):c.append(g)}for(f=d.length-1;f>=b;f--)d[f].remove();d=settings.colsQueueCurrent.length;d--;"queuesearch"==a.type&&0==b?c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("No results, please refine your search")+"</td></tr>":
"queue"==a.type&&0==b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("Empty queue")+"</td></tr>");setPagination(a.totalEntities,a.returnedEntities);document.getElementById("QueueCurrentList").classList.remove("opacity05")}
function parseLastPlayed(a){document.getElementById("cardFooterQueue").innerText=t("Num songs",a.totalEntities);var b=a.data.length,c=document.getElementById("QueueLastPlayedList");c.setAttribute("data-version",a.queueVersion);c=c.getElementsByTagName("tbody")[0];for(var d=c.getElementsByTagName("tr"),f=0;f<b;f++){a.data[f].Duration=beautifySongDuration(a.data[f].Duration);a.data[f].LastPlayed=localeDate(a.data[f].LastPlayed);var g=document.createElement("tr");g.setAttribute("data-songpos",a.data[f].Pos+
1);g.setAttribute("data-uri",a.data[f].uri);g.setAttribute("data-name",a.data[f].Title);g.setAttribute("data-type","song");for(var h="",k=0;k<settings.colsQueueLastPlayed.length;k++)h+='<td data-col="'+settings.colsQueueLastPlayed[k]+'">'+e(a.data[f][settings.colsQueueLastPlayed[k]])+"</td>";h+='<td data-col="Action">';""!=a.data[f].uri&&(h+='<a href="#" class="material-icons color-darkgrey">playlist_add</a>');h+="</td>";g.innerHTML=h;f<d.length?d[f].replaceWith(g):c.append(g)}for(f=d.length-1;f>=
b;f--)d[f].remove();d=settings.colsQueueLastPlayed.length;d--;0==b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("Empty list")+"</td></tr>");setPagination(a.totalEntities,a.returnedEntities);document.getElementById("QueueLastPlayedList").classList.remove("opacity05")}
...
function parseFilesystem(a){var b=app.current.app+("Filesystem"==app.current.tab?app.current.tab:""),c=settings["cols"+b].length;c--;for(var d=a.data.length,f=document.getElementById(app.current.app+(void 0==app.current.tab?"":app.current.tab)+"List").getElementsByTagName("tbody")[0],g=f.getElementsByTagName("tr"),h=0;h<d;h++){var k=encodeURI(a.data[h].uri),l=document.createElement("tr");l.setAttribute("data-type",a.data[h].Type);l.setAttribute("data-uri",k);"song"==a.data[h].Type?l.setAttribute("data-name",
a.data[h].Title):l.setAttribute("data-name",a.data[h].name);switch(a.data[h].Type){case "dir":case "smartpls":case "plist":k="";for(var m=0;m<settings["cols"+b].length;m++)k+='<td data-col="'+settings["cols"+b][m]+'">',"Type"==settings["cols"+b][m]?k="dir"==a.data[h].Type?k+'<span class="material-icons">folder_open</span>':k+('<span class="material-icons">'+("smartpls"==a.data[h].Type?"queue_music":"list")+"</span>"):"Title"==settings["cols"+b][m]&&(k+=e(a.data[h].name)),k+="</td>";k+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';
l.innerHTML=k;break;case "song":a.data[h].Duration=beautifySongDuration(a.data[h].Duration);k="";for(m=0;m<settings["cols"+b].length;m++)k+='<td data-col="'+settings["cols"+b][m]+'">',k="Type"==settings["cols"+b][m]?k+'<span class="material-icons">music_note</span>':k+e(a.data[h][settings["cols"+b][m]]),k+="</td>";k+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';l.innerHTML=k}h<g.length?g[h].replaceWith(l):f.append(l)}for(h=g.length-1;h>=d;h--)g[h].remove();
setPagination(a.totalEntities,a.returnedEntities);0==d&&(f.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+c+'">'+t("Empty list")+"</td></tr>");document.getElementById(app.current.app+(void 0==app.current.tab?"":app.current.tab)+"List").classList.remove("opacity05");document.getElementById("cardFooterBrowse").innerText=t("Num entries",a.totalEntities)}
...
document.getElementById("BrowsePlaylistsAllList").classList.add("hide"),document.getElementById("btnBrowsePlaylistsAll").parentNode.classList.remove("hide"),settings.featTags&&document.getElementById("BrowsePlaylistsDetailColsBtn").parentNode.classList.remove("hide"));var b=a.data.length,c=document.getElementById(app.current.app+app.current.tab+app.current.view+"List").getElementsByTagName("tbody")[0],d=c.getElementsByTagName("tr");if("All"==app.current.view){for(var f=0;f<b;f++){var g=encodeURI(a.data[f].uri),
h=document.createElement("tr");h.setAttribute("data-uri",g);h.setAttribute("data-type",a.data[f].Type);h.setAttribute("data-name",a.data[f].name);h.innerHTML='<td data-col="Type"><span class="material-icons">'+("smartpls"==a.data[f].Type?"queue_music":"list")+"</span></td><td>"+e(a.data[f].name)+"</td><td>"+localeDate(a.data[f].last_modified)+'</td><td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';f<d.length?d[f].replaceWith(h):c.append(h)}document.getElementById("cardFooterBrowse").innerText=
gtPage("Num playlists",a.returnedEntities,a.totalEntities)}else if("Detail"==app.current.view){for(f=0;f<b;f++){g=encodeURI(a.data[f].uri);h=document.createElement("tr");0==a.smartpls&&h.setAttribute("draggable","true");h.setAttribute("id","playlistTrackId"+a.data[f].Pos);h.setAttribute("data-type",a.data[f].Type);h.setAttribute("data-uri",g);h.setAttribute("data-name",a.data[f].Title);h.setAttribute("data-songpos",a.data[f].Pos);a.data[f].Duration=beautifySongDuration(a.data[f].Duration);g="";for(var k=
0;k<settings.colsBrowsePlaylistsDetail.length;k++)g+='<td data-col="'+settings.colsBrowsePlaylistsDetail[k]+'">'+e(a.data[f][settings.colsBrowsePlaylistsDetail[k]])+"</td>";g+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';h.innerHTML=g;f<d.length?d[f].replaceWith(h):c.append(h)}document.getElementById("cardFooterBrowse").innerText=gtPage("Num songs",a.returnedEntities,a.totalEntities)}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.totalEntities,
a.returnedEntities);0==b&&(a=settings["cols"+list].length,a--,c.innerHTML="All"==app.current.view?'<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+a+'">'+t("No playlists found")+"</td></tr>":'<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+a+'">'+t("Empty playlist")+"</td></tr>");document.getElementById(app.current.app+app.current.tab+app.current.view+"List").classList.remove("opacity05")}
...
document.getElementById("btnBrowseDatabaseTag").parentNode.classList.add("hide");document.getElementById("BrowseDatabaseTagListCaption").innerText=app.current.view;document.getElementById("cardFooterBrowse").innerText=a.totalEntities+" Tags";b=a.data.length;c=document.getElementById(app.current.app+app.current.tab+"TagList").getElementsByTagName("tbody")[0];d=c.getElementsByTagName("tr");for(f=0;f<b;f++)g=encodeURI(a.data[f].value),h=document.createElement("tr"),h.setAttribute("data-uri",g),h.innerHTML=
'<td data-col="Type"><span class="material-icons">album</span></td><td>'+e(a.data[f].value)+"</td>",f<d.length?d[f].replaceWith(h):c.append(h);for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.totalEntities,a.returnedEntities);0==b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td>No entries found.</td></tr>');document.getElementById("BrowseDatabaseTagList").classList.remove("opacity05")}}
...
function parseListTitles(a){var b=genId(a.Album),c=document.getElementById("card"+b),d=c.getElementsByTagName("tbody")[0],f=c.querySelector(".card-footer"),g=c.querySelector(".card-header");g.setAttribute("data-uri",encodeURI(a.data[0].uri.replace(/\/[^\/]+$/,"")));g.setAttribute("data-name",a.Album);g.setAttribute("data-type","dir");g.addEventListener("click",function(a){showMenu(this,a)},!1);g.classList.add("clickable");if(c=c.getElementsByTagName("a")[0])c.style.backgroundImage='url("'+subdir+
a.cover+'"), url("'+subdir+'/assets/coverimage-loading.png")',c.setAttribute("data-uri",encodeURI(a.data[0].uri.replace(/\/[^\/]+$/,""))),c.setAttribute("data-name",a.Album),c.setAttribute("data-type","dir"),c.addEventListener("click",function(a){showMenu(this,a)},!1);document.getElementById("albumartist"+b).innerText=a.AlbumArtist;b="";c=a.data.length;for(g=0;g<c;g++){a.data[g].Duration&&(a.data[g].Duration=beautifySongDuration(a.data[g].Duration));b+='<tr data-type="song" data-name="'+a.data[g].Title+
'" data-uri="'+encodeURI(a.data[g].uri)+'">';for(var h=0;h<settings.colsBrowseDatabase.length;h++)b+='<td data-col="'+settings.colsBrowseDatabase[h]+'">'+e(a.data[g][settings.colsBrowseDatabase[h]])+"</td>";b+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td></tr>'}d.innerHTML=b;f.innerHTML=t("Num songs",a.totalEntities)+" &ndash; "+beautifyDuration(a.totalTime);d.parentNode.addEventListener("click",function(a){"TD"==a.target.nodeName?appendQueue("song",
decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name")):"A"==a.target.nodeName&&showMenu(a.target,a)},!1)}
...
function showMenu(a,b){b.preventDefault();b.stopPropagation();hideMenu();if(!a.getAttribute("data-init")){b=a.getAttribute("data-type");var c=decodeURI(a.getAttribute("data-uri")),d=a.getAttribute("data-name"),f=0;if(null==b||null==c)b=a.parentNode.parentNode.getAttribute("data-type"),c=decodeURI(a.parentNode.parentNode.getAttribute("data-uri")),d=a.parentNode.parentNode.getAttribute("data-name");lastState&&(f=lastState.data.nextSongPos);var g="";"Browse"==app.current.app&&"Filesystem"==app.current.tab||
...
b&&(b=JSON.parse(b64DecodeUnicode(b)),parseCmd(a,b),hideMenu())}},!1);if(a=document.getElementById("advancedMenuLink"))a.addEventListener("click",function(a){a=this.getElementsByTagName("span")[0];a.innerText="keyboard_arrow_right"==a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new Collapse(a)},!1);b.show()}}
...
function songChange(a){if("song_change"==a.type){var b=a.data.Title+a.data.Artist+a.data.Album+a.data.uri+a.data.currentSongId;if(lastSong!=b){var c="",d="",f="myMPD: ";setCurrentCover(a.data.cover);1==settings.bgCover&&1==settings.featCoverimage&&(-1<a.data.cover.indexOf("coverimage-")?clearBackgroundImage():setBackgroundImage(a.data.cover));"undefined"!=typeof a.data.Artist&&0<a.data.Artist.length&&"-"!=a.data.Artist&&(c+=a.data.Artist,d+=a.data.Artist,f+=a.data.Artist+" - ");"undefined"!=typeof a.data.Album&&
0<a.data.Album.length&&"-"!=a.data.Album&&(c+=" - "+a.data.Album,d+="<br/>"+a.data.Album);"undefined"!=typeof a.data.Title&&0<a.data.Title.length?(f+=a.data.Title,domCache.currentTitle.innerText=a.data.Title,domCache.currentTitle.setAttribute("data-uri",encodeURI(a.data.uri))):(domCache.currentTitle.innerText="",domCache.currentTitle.setAttribute("data-uri",""));document.title=f;1==settings.featStickers&&setVoteSongBtns(a.data.like,a.data.uri);for(f=0;f<settings.colsPlayback.length;f++){var g=document.getElementById("current"+
settings.colsPlayback[f]);g&&(g.getElementsByTagName("h4")[0].innerText=a.data[settings.colsPlayback[f]],g.setAttribute("data-name",encodeURI(a.data[settings.colsPlayback[f]])))}if(f=document.getElementById("queueTrackId"+a.data.currentSongId))f.getElementsByTagName("td")[1].innerText=a.data.Title;"play"==playstate&&showNotification(a.data.Title,c,d,"success");lastSong=b;lastSongObj=a}}}
