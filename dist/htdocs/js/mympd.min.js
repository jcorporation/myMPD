...
$jscomp.polyfillIsolated=function(a,b,c,f){var g=a.split(".");a=1===g.length;f=g[0];f=!a&&f in $jscomp.polyfills?$jscomp.polyfills:$jscomp.global;for(var d=0;d<g.length-1;d++){var h=g[d];if(!(h in f))return;f=f[h]}g=g[g.length-1];c=$jscomp.IS_SYMBOL_NATIVE&&"es6"===c?f[g]:null;b=b(c);null!=b&&(a?$jscomp.defineProperty($jscomp.polyfills,g,{configurable:!0,writable:!0,value:b}):b!==c&&($jscomp.propertyToPolyfillSymbol[g]=$jscomp.IS_SYMBOL_NATIVE?$jscomp.global.Symbol(g):$jscomp.POLYFILL_PREFIX+g,g=
$jscomp.propertyToPolyfillSymbol[g],$jscomp.defineProperty(f,g,{configurable:!0,writable:!0,value:b})))};$jscomp.polyfill("Object.is",function(a){return a?a:function(b,c){return b===c?0!==b||1/b===1/c:b!==b&&c!==c}},"es6","es3");$jscomp.polyfill("Array.prototype.includes",function(a){return a?a:function(b,c){var f=this;f instanceof String&&(f=String(f));var g=f.length;c=c||0;for(0>c&&(c=Math.max(c+g,0));c<g;c++){var d=f[c];if(d===b||Object.is(d,b))return!0}return!1}},"es7","es3");
...
$jscomp.polyfill("Symbol",function(a){if(a)return a;var b=function(g,d){this.$jscomp$symbol$id_=g;$jscomp.defineProperty(this,"description",{configurable:!0,writable:!0,value:d})};b.prototype.toString=function(){return this.$jscomp$symbol$id_};var c=0,f=function(g){if(this instanceof f)throw new TypeError("Symbol is not a constructor");return new b("jscomp_symbol_"+(g||"")+"_"+c++,g)};return f},"es6","es3");
...
"es3");$jscomp.iteratorPrototype=function(a){a={next:a};a[Symbol.iterator]=function(){return this};return a};$jscomp.iteratorFromArray=function(a,b){a instanceof String&&(a+="");var c=0,f=!1,g={next:function(){if(!f&&c<a.length){var d=c++;return{value:b(d,a[d]),done:!1}}f=!0;return{done:!0,value:void 0}}};g[Symbol.iterator]=function(){return g};return g};$jscomp.polyfill("Array.prototype.keys",function(a){return a?a:function(){return $jscomp.iteratorFromArray(this,function(b){return b})}},"es6","es3");
$jscomp.owns=function(a,b){return Object.prototype.hasOwnProperty.call(a,b)};$jscomp.assign=$jscomp.TRUST_ES6_POLYFILLS&&"function"==typeof Object.assign?Object.assign:function(a,b){for(var c=1;c<arguments.length;c++){var f=arguments[c];if(f)for(var g in f)$jscomp.owns(f,g)&&(a[g]=f[g])}return a};$jscomp.polyfill("Object.assign",function(a){return a||$jscomp.assign},"es6","es3");
...
function sendAPI(a,b,c,f){var g={jsonrpc:"2.0",id:0,method:a,params:b},d=new XMLHttpRequest;d.open("POST",subdir+"/api",!0);d.setRequestHeader("Content-type","application/json");d.onreadystatechange=function(){if(4===d.readyState)if(""!==d.responseText){var h=JSON.parse(d.responseText);if(h.error)showNotification(t(h.error.message,h.error.data),"","","danger"),logError(JSON.stringify(h.error));else if(h.result&&h.result.message&&"ok"!==h.result.message)logDebug("Got API response: "+JSON.stringify(h.result)),
showNotification(t(h.result.message,h.result.data),"","","success");else if(h.result&&h.result.message&&"ok"===h.result.message)logDebug("Got API response: "+JSON.stringify(h.result));else if(h.result&&h.result.method)logDebug("Got API response of type: "+h.result.method);else if(logError("Got invalid API response: "+JSON.stringify(h)),!0!==f)return;void 0!==c&&"function"===typeof c&&(void 0!==h.result||!0===f?(logDebug("Calling "+c.name),c(h)):logDebug("Undefined resultset, skip calling "+c.name))}else logError("Empty response for request: "+
JSON.stringify(g)),!0===f&&void 0!==c&&"function"===typeof c&&(logDebug("Got empty API response calling "+c.name),c(""))};d.send(JSON.stringify(g));logDebug("Send API request: "+a)}
...
function parseDatabase(a){var b=a.result.returnedEntities,c=document.getElementById("BrowseDatabaseCards"),f=c.getElementsByClassName("col"),g="IntersectionObserver"in window?!0:!1;0===f.length&&(c.innerHTML="");for(var d=0;d<b;d++){var h=document.createElement("div");h.classList.add("col","px-0","flex-grow-0");""===a.result.data[d].AlbumArtist&&(a.result.data[d].AlbumArtist=t("Unknown artist"));""===a.result.data[d].Album&&(a.result.data[d].Album=t("Unknown album"));if("Album"===a.result.tag){var k=
...
k+'"></div>':"")+'<div class="card-footer card-footer-grid p-2" title="'+e(a.result.data[d].value)+'">'+e(a.result.data[d].value)+"<br/></div></div>";h.innerHTML=m;m=!1;d<f.length?f[d].firstChild.getAttribute("data-picture")!==h.firstChild.getAttribute("data-picture")&&(f[d].replaceWith(h),m=!0):(c.append(h),m=!0);!0===m&&(!0===g?(new IntersectionObserver(setGridImage,{root:null,rootMargin:"0px"})).observe(h):h.firstChild.firstChild.style.backgroundImage=l,"Album"===a.result.tag&&!0===isMobile&&addPlayButton(document.getElementById(k)))}for(g=
f.length-1;g>=b;g--)f[g].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div class="ml-3 mb-3"><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}
...
!1===settings.featBrowse?"":'<span class="text-light material-icons">description</span>&nbsp;<a class="text-light" target="_blank" href="'+subdir+"/browse/music/"+e(a.result.bookletPath)+'">'+t("Download booklet")+"</a>")+"</p>";b=document.getElementById("BrowseDatabaseDetailList");var c=b.getElementsByTagName("tbody")[0],f=settings.colsBrowseDatabaseDetail.length,g="";1<a.result.Discs&&(g='<tr class="not-clickable"><td><span class="material-icons">album</span></td><td colspan="'+f+'">'+t("Disc 1")+
"</td></tr>");for(var d=a.result.returnedEntities,h=parseInt(a.result.data[0].Disc),k=0;k<d;k++){h<parseInt(a.result.data[k].Disc)&&(g+='<tr class="not-clickable"><td><span class="material-icons">album</span></td><td colspan="'+f+'">'+t("Disc")+" "+e(a.result.data[k].Disc)+"</td></tr>");a.result.data[k].Duration&&(a.result.data[k].Duration=beautifySongDuration(a.result.data[k].Duration));g+='<tr tabindex="0" data-type="song" data-name="'+a.result.data[k].Title+'" data-uri="'+encodeURI(a.result.data[k].uri)+
...
function parseHome(a){var b=a.result.returnedEntities,c=document.getElementById("HomeCards"),f=c.getElementsByClassName("col");0===f.length&&(c.innerHTML="");for(var g=0;g<b;g++){var d=document.createElement("div");d.classList.add("col","px-0","flex-grow-0");""===a.result.data[g].AlbumArtist&&(a.result.data[g].AlbumArtist=t("Unknown artist"));""===a.result.data[g].Album&&(a.result.data[g].Album=t("Unknown album"));"appGoto"===a.result.data[g].cmd&&8===a.result.data[g].options.length&&a.result.data[g].options.splice(4,
0,settings.maxElementsPerPage);var h=JSON.stringify({cmd:a.result.data[g].cmd,options:a.result.data[g].options});h='<div class="card home-icons clickable" draggable="true" tabindex="0" data-pos="'+g+"\" data-href='"+e(h)+"'  title=\""+e(a.result.data[g].name)+'"><div class="card-body material-icons">'+e(a.result.data[g].ligature)+'</div><div class="card-footer card-footer-grid p-2">'+e(a.result.data[g].name)+"</div></div>";d.innerHTML=h;g<f.length?f[g].replaceWith(d):c.append(d);""!==a.result.data[g].image&&
(d.getElementsByClassName("card-body")[0].style.backgroundImage='url("'+subdir+"/browse/pics/"+a.result.data[g].image+'")');""!==a.result.data[g].bgcolor&&(d.getElementsByClassName("card-body")[0].style.backgroundColor=a.result.data[g].bgcolor)}for(a=f.length-1;a>=b;a--)f[a].remove();0===b&&(c.innerHTML='<div class="ml-3">'+t("Homescreen welcome")+"</div>")}
...
function i18nHtml(a){for(var b=[["data-phrase","innerText"],["data-title-phrase","title"],["data-placeholder-phrase","placeholder"]],c=0;c<b.length;c++)for(var f=a.querySelectorAll("["+b[c][0]+"]"),g=f.length,d=0;d<g;d++)f[d][b[c][1]]=t(f[d].getAttribute(b[c][0]))}
...
function createImgCarousel(a,b,c){for(var f='<div id="'+b+'" class="carousel slide" data-ride="carousel"><ol class="carousel-indicators">',g=0;g<c.length;g++)f+='<li data-target="#'+b+'" data-slide-to="'+g+'"'+(0===g?' class="active"':"")+"></li>";f+='</ol><div class="carousel-inner" role="listbox">';for(g=0;g<c.length;g++)f+='<div class="carousel-item'+(0===g?" active":"")+'"><div></div></div>';a.innerHTML=f+('</div><a class="carousel-control-prev" href="#'+b+'" data-slide="prev"><span class="carousel-control-prev-icon"></span></a><a class="carousel-control-next" href="#'+
...
function parseListMounts(a){for(var b=document.getElementById("listMounts").getElementsByTagName("tbody")[0],c=b.getElementsByTagName("tr"),f=0,g=0;g<a.result.returnedEntities;g++){var d=document.createElement("tr");d.setAttribute("data-url",encodeURI(a.result.data[g].mountUrl));d.setAttribute("data-point",encodeURI(a.result.data[g].mountPoint));""===a.result.data[g].mountPoint&&d.classList.add("not-clickable");var h="<td>"+(""===a.result.data[g].mountPoint?'<span class="material-icons">home</span>':
e(a.result.data[g].mountPoint))+"</td><td>"+e(a.result.data[g].mountUrl)+"</td>";h=""!==a.result.data[g].mountPoint?h+('<td data-col="Action"><a href="#" title="'+t("Unmount")+'" data-action="unmount" class="material-icons color-darkgrey">delete</a><a href="#" title="'+t("Update")+'" data-action="update"class="material-icons color-darkgrey">refresh</a></td>'):h+"<td>&nbsp;</td>";d.innerHTML=h;g<c.length?f=!0===replaceTblRow(c[g],d)?g:f:b.append(d)}for(f=c.length-1;f>=a.result.returnedEntities;f--)c[f].remove();
...
function appGoto(a,b,c,f,g,d,h,k,l,m){var n=document.body.scrollTop?document.body.scrollTop:document.documentElement.scrollTop;void 0!==app.apps[app.current.app].scrollPos?app.apps[app.current.app].scrollPos=n:void 0!==app.apps[app.current.app].tabs[app.current.tab].scrollPos?app.apps[app.current.app].tabs[app.current.tab].scrollPos=n:void 0!==app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos&&(app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos=
n);null===f&&(f=void 0);null===g&&(g=void 0);null===d&&(d=void 0);null===h&&(h=void 0);null===k&&(k=void 0);null===l&&(l=void 0);app.apps[a].tabs?(void 0===b&&(b=app.apps[a].active),app.apps[a].tabs[b].views?(void 0===c&&(c=app.apps[a].tabs[b].active),f="/"+encodeURIComponent(a)+"/"+encodeURIComponent(b)+"/"+encodeURIComponent(c)+"!"+encodeURIComponent(void 0===f?app.apps[a].tabs[b].views[c].offset:f)+"/"+encodeURIComponent(void 0===g?app.apps[a].tabs[b].views[c].limit:g)+"/"+encodeURIComponent(void 0===
d?app.apps[a].tabs[b].views[c].filter:d)+"/"+encodeURIComponent(void 0===h?app.apps[a].tabs[b].views[c].sort:h)+"/"+encodeURIComponent(void 0===k?app.apps[a].tabs[b].views[c].tag:k)+"/"+encodeURIComponent(void 0===l?app.apps[a].tabs[b].views[c].search:l),void 0!==m&&(app.apps[a].tabs[b].views[c].scrollPos=m)):(f="/"+encodeURIComponent(a)+"/"+encodeURIComponent(b)+"!"+encodeURIComponent(void 0===f?app.apps[a].tabs[b].offset:f)+"/"+encodeURIComponent(void 0===g?app.apps[a].tabs[b].limit:g)+"/"+encodeURIComponent(void 0===
d?app.apps[a].tabs[b].filter:d)+"/"+encodeURIComponent(void 0===h?app.apps[a].tabs[b].sort:h)+"/"+encodeURIComponent(void 0===k?app.apps[a].tabs[b].tag:k)+"/"+encodeURIComponent(void 0===l?app.apps[a].tabs[b].search:l),void 0!==m&&(app.apps[a].tabs[b].scrollPos=m))):(f="/"+encodeURIComponent(a)+"!"+encodeURIComponent(void 0===f?app.apps[a].offset:f)+"/"+encodeURIComponent(void 0===g?app.apps[a].limit:g)+"/"+encodeURIComponent(void 0===d?app.apps[a].filter:d)+"/"+encodeURIComponent(void 0===h?app.apps[a].sort:
h)+"/"+encodeURIComponent(void 0===k?app.apps[a].tag:k)+"/"+encodeURIComponent(void 0===l?app.apps[a].search:l),void 0!==m&&(app.apps[a].scrollPos=m));location.hash=f}
...
"disabled"));a='<li class="breadcrumb-item"><a data-uri="" class="text-body material-icons">home</a></li>';for(var b=app.current.search.split("/"),c=b.length,f="",g=0;g<c;g++){if(c-1===g){a+='<li class="breadcrumb-item active">'+e(b[g])+"</li>";break}f+=b[g];a+='<li class="breadcrumb-item"><a class="text-body" href="#" data-uri="'+encodeURI(f)+'">'+e(b[g])+"</a></li>";f+="/"}document.getElementById("BrowseBreadcrumb").innerHTML=a;document.getElementById("searchFilesystemStr").value="-"===app.current.filter?
...
toggleBtnChk("databaseSortDesc",!0)):toggleBtnChk("databaseSortDesc",!1),selectTag("databaseSortTags",void 0,a),"Album"===app.current.tag){c=document.getElementById("searchDatabaseCrumb");f=document.getElementById("searchDatabaseStr");g="";for(var d=app.current.search.split(" AND "),h=0;h<d.length-1;h++){var k=d[h].substring(1,d[h].length-1),l=k.match(/^(\w+)\s+(\S+)\s+'(.*)'$/);g+='<button data-filter-tag="'+encodeURI(l[1])+'" data-filter-op="'+encodeURI(l[2])+'" data-filter-value="'+encodeURI(unescapeMPD(l[3]))+
'" class="btn btn-light mr-2">'+e(k)+'<span class="badge badge-secondary">&times</span></button>'}c.innerHTML=g;""===f.value&&1<=d.length&&(g=d[d.length-1].substring(1,d[d.length-1].length-1),d=g.substring(g.indexOf("'")+1,g.length-1),f.value!==d&&(f=g.match(/^(\w+)\s+(\S+)\s+'(.*)'$/),c.innerHTML+='<button data-filter-tag="'+encodeURI(f[1])+'" data-filter-op="'+encodeURI(f[2])+'" data-filter-value="'+encodeURI(unescapeMPD(f[3]))+'" class="btn btn-light mr-2">'+e(g)+'<span class="badge badge-secondary">&times</span></button>'),
...
app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a="";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)f=b[c].substring(1,b[c].length-1),g=f.match(/^(\w+)\s+(\S+)\s+'(.*)'$/),a+='<button data-filter-tag="'+encodeURI(g[1])+'" data-filter-op="'+encodeURI(g[2])+'" data-filter-value="'+encodeURI(unescapeMPD(g[3]))+'" class="btn btn-light mr-2">'+e(f)+'<span class="badge badge-secondary">&times</span></button>';domCache.searchCrumb.innerHTML=
...
playstate||"play"===playstate)&&0<currentSong.totalTime){domCache.progressPos.innerText=beautifySongDuration(Math.ceil(currentSong.totalTime/d.target.offsetWidth*d.clientX));domCache.progressPos.style.display="block";var h=domCache.progressPos.offsetWidth/2;domCache.progressPos.style.left=(d.clientX<h?d.clientX:d.clientX<window.innerWidth-h?d.clientX-h:d.clientX-2*h)+"px"}},!1);domCache.progress.addEventListener("mouseout",function(){domCache.progressPos.style.display="none"},!1);for(var a=document.querySelectorAll(".subMenu"),
b=a.length,c=0;c<b;c++)a[c].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();d=this.getElementsByTagName("span")[0];d.innerText="keyboard_arrow_right"===d.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1);document.getElementById("navbar-main").addEventListener("click",function(d){d.preventDefault();var h=d.target.getAttribute("data-href");null===h&&(h=d.target.parentNode.getAttribute("data-href"));null!==h&&parseCmd(d,h)},!1);document.getElementById("volumeMenu").parentNode.addEventListener("show.bs.dropdown",
...
document.getElementById("modalAbout").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_DATABASE_STATS",{},parseStats);getServerinfo();var d="",h;for(h in keymap)if(void 0===keymap[h].req||!0===settings[keymap[h].req])d+='<tr><td><div class="key'+(keymap[h].key&&1<keymap[h].key.length?" material-icons material-icons-small":"")+'">'+(void 0!==keymap[h].key?keymap[h].key:h)+"</div></td><td>"+t(keymap[h].desc)+"</td></tr>";document.getElementById("tbodyShortcuts").innerHTML=d});document.getElementById("modalAddToPlaylist").addEventListener("shown.bs.modal",
...
document.getElementById("selectTriggerScript").addEventListener("change",function(){selectTriggerActionChange()},!1);a="";for(b=0;24>b;b++)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerHour").innerHTML=a;a="";for(b=0;60>b;b+=5)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerMinute").innerHTML=a;document.getElementById("selectTheme").addEventListener("change",function(){var d=getSelectValue(this);"theme-default"===
d?document.getElementById("inputBgColor").value="#ccc":"theme-light"===d?document.getElementById("inputBgColor").value="#fff":"theme-dark"===d&&(document.getElementById("inputBgColor").value="#000");document.getElementById("bgColorPreview").style.backgroundColor=document.getElementById("inputBgColor").value},!1);document.getElementById("modalAddToQueue").addEventListener("shown.bs.modal",function(){document.getElementById("inputAddToQueueQuantity").classList.remove("is-invalid");document.getElementById("warnJukeboxPlaylist2").classList.add("hide");
!0===settings.featPlaylists&&sendAPI("MPD_API_PLAYLIST_LIST",{searchstr:"",offset:0,limit:0},function(d){getAllPlaylists(d,"selectAddToQueuePlaylist")})});document.getElementById("modalUpdateDB").addEventListener("hidden.bs.modal",function(){document.getElementById("updateDBprogress").classList.remove("updateDBprogressAnimate")});document.getElementById("modalSaveQueue").addEventListener("shown.bs.modal",function(){var d=document.getElementById("saveQueueName");d.focus();d.value="";d.classList.remove("is-invalid")});
...
document.getElementById("inputMpdPort").classList.remove("is-invalid");document.getElementById("inputMpdPass").classList.remove("is-invalid")});document.getElementById("btnJukeboxModeGroup").addEventListener("mouseup",function(){setTimeout(function(){var d=document.getElementById("btnJukeboxModeGroup").getElementsByClassName("active")[0].getAttribute("data-value");"0"===d?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled",
"disabled")):"2"===d?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):"1"===d&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));"0"!==d&&toggleBtnChk("btnConsume",!0);checkConsume()},100)});document.getElementById("btnConsume").addEventListener("mouseup",
...
"disabled"))},100)});document.getElementById("selectAddToQueueMode").addEventListener("change",function(){var d=this.options[this.selectedIndex].value;"2"===d?(document.getElementById("inputAddToQueueQuantity").setAttribute("disabled","disabled"),document.getElementById("inputAddToQueueQuantity").value="1",document.getElementById("selectAddToQueuePlaylist").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").value="Database"):"1"===d&&(document.getElementById("inputAddToQueueQuantity").removeAttribute("disabled"),
...
!1);document.getElementById("syscmds").addEventListener("click",function(d){"A"===d.target.nodeName&&parseCmd(d,d.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",function(d){"A"===d.target.nodeName&&execScript(d.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++){!1===a[c].classList.contains("notclickable")&&a[c].classList.add("clickable");var f=a[c].parentNode.classList.contains("noInitChilds")?
!0:!1;!1===f&&(f=a[c].parentNode.parentNode.classList.contains("noInitChilds")?!0:!1);!0!==f&&a[c].addEventListener("click",function(d){parseCmd(d,this.getAttribute("data-href"))},!1)}document.getElementById("HomeCards").addEventListener("click",function(d){if(d.target.classList.contains("card-body")){var h=d.target.parentNode.getAttribute("data-href");null!==h&&parseCmd(d,h)}else if(d.target.classList.contains("card-footer")){h=document.getElementById("HomeCards").getElementsByClassName("selected");
for(var k=0;k<h.length;k++)h[k].classList.remove("selected");d.target.parentNode.classList.add("selected");showMenu(d.target,d);d.stopPropagation()}},!1);document.getElementById("HomeCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);dragAndDropHome();document.getElementById("selectHomeIconCmd").addEventListener("change",function(){showHomeIconCmdOptions()},!1);document.getElementById("inputHomeIconLigature").addEventListener("change",function(d){document.getElementById("homeIconPreview").innerText=
d.target.value;""!==d.target.value&&(document.getElementById("selectHomeIconImage").value="",document.getElementById("homeIconPreview").style.backgroundImage="")},!1);document.getElementById("inputHomeIconBgcolor").addEventListener("change",function(d){document.getElementById("homeIconPreview").style.backgroundColor=d.target.value},!1);document.getElementById("selectHomeIconImage").addEventListener("change",function(d){d=getSelectValue(d.target);document.getElementById("homeIconPreview").style.backgroundImage=
'url("'+subdir+"/browse/pics/"+d+'")';""!==d?(document.getElementById("divHomeIconLigature").classList.add("hide"),document.getElementById("homeIconPreview").innerHTML=""):(document.getElementById("divHomeIconLigature").classList.remove("hide"),document.getElementById("homeIconPreview").innerText=document.getElementById("inputHomeIconLigature").value)},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(d){"P"===d.target.nodeName&&gotoBrowse()},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",
function(d){"A"===d.target.nodeName&&(d.preventDefault(),appGoto("Browse","Filesystem",void 0,"0",app.current.limit,app.current.filter,app.current.sort,"-",decodeURI(d.target.getAttribute("data-uri"))))},!1);document.getElementById("tbodySongDetails").addEventListener("click",function(d){if("A"===d.target.nodeName)if("calcFingerprint"===d.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(d.target.getAttribute("data-uri"))},parseFingerprint);d.preventDefault();var h=d.target.parentNode,
k=document.createElement("div");k.classList.add("spinner-border","spinner-border-sm");d.target.classList.add("hide");h.appendChild(k)}else d.target.classList.contains("external")||null===d.target.parentNode.getAttribute("data-tag")||(modalSongDetails.hide(),d.preventDefault(),gotoBrowse());else"BUTTON"===d.target.nodeName&&d.target.getAttribute("data-href")&&parseCmd(d,d.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(d){"BUTTON"===d.target.nodeName?
(d.stopPropagation(),d.preventDefault(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",{output:d.target.getAttribute("data-output-id"),state:d.target.classList.contains("active")?0:1}),toggleBtn(d.target.id)):"A"===d.target.nodeName&&(d.preventDefault(),showListOutputAttributes(decodeURI(d.target.parentNode.getAttribute("data-output-name"))))},!1);document.getElementById("listTimerList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName?d.target.parentNode.classList.contains("not-clickable")||
showEditTimer(d.target.parentNode.getAttribute("data-id")):"A"===d.target.nodeName?deleteTimer(d.target.parentNode.parentNode.getAttribute("data-id")):"BUTTON"===d.target.nodeName&&toggleTimer(d.target,d.target.parentNode.parentNode.getAttribute("data-id"))},!1);document.getElementById("listMountsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(""===d.target.parentNode.getAttribute("data-point"))return!1;showEditMount(decodeURI(d.target.parentNode.getAttribute("data-url")),
decodeURI(d.target.parentNode.getAttribute("data-point")))}else if("A"===d.target.nodeName){var h=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===h?unmountMount(k):"update"===h&&updateMount(d.target,k)}},!1);document.getElementById("listScriptsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(!1===settings.featScripteditor||""===d.target.parentNode.getAttribute("data-script"))return!1;
showEditScript(decodeURI(d.target.parentNode.getAttribute("data-script")))}else if("A"===d.target.nodeName){var h=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-script"));"delete"===h?deleteScript(k):"execute"===h?execScript(d.target.getAttribute("data-href")):"add2home"===h&&addScriptToHome(k,d.target.getAttribute("data-href"))}},!1);document.getElementById("listTriggerList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();
if("TD"===d.target.nodeName){var h=decodeURI(d.target.parentNode.getAttribute("data-trigger-id"));showEditTrigger(h)}else"A"===d.target.nodeName&&(h=d.target.getAttribute("data-action"),d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-trigger-id")),"delete"===h&&deleteTrigger(d))},!1);document.getElementById("listPartitionsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("A"===d.target.nodeName){var h=d.target.getAttribute("data-action");d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-partition"));
"delete"===h?deletePartition(d):"switch"===h&&switchPartition(d)}},!1);document.getElementById("partitionOutputsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName&&(d=decodeURI(d.target.parentNode.getAttribute("data-output")),moveOutput(d),modalPartitionOutputs.hide())},!1);document.getElementById("QueueCurrentList").addEventListener("click",function(d){"TD"===d.target.nodeName?sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:d.target.parentNode.getAttribute("data-trackid")}):
"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",function(d){"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueJukeboxList").addEventListener("click",function(d){"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(d){if("TD"===d.target.nodeName)switch(d.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":app.current.filter=
"-";appGoto("Browse","Filesystem",void 0,"0",app.current.limit,app.current.filter,app.current.sort,"-",decodeURI(d.target.parentNode.getAttribute("data-uri")));break;case "song":appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"))}else"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",
function(d){if("A"===d.target.nodeName){var h=d.target.parentNode.parentNode.getAttribute("data-id"),k=d.target.parentNode.parentNode.getAttribute("data-type"),l=decodeURI(d.target.parentNode.parentNode.getAttribute("data-uri")),m=d.target.parentNode.parentNode.firstChild.innerText,n=d.target.getAttribute("data-href");"delete"===n?(sendAPI("MYMPD_API_BOOKMARK_RM",{id:h},function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0,limit:0},parseBookmarks)}),d.preventDefault(),d.stopPropagation()):"edit"===
n?showBookmarkSave(h,m,l,k):"goto"===n&&appGoto("Browse","Filesystem",void 0,"0",void 0,"-","-","-",l)}},!1);document.getElementById("BrowsePlaylistsAllList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("plist",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowsePlaylistsDetailList").addEventListener("click",function(d){"TFOOT"!==d.target.parentNode.parentNode.nodeName&&
("TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d))},!1);document.getElementById("SearchList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseDatabaseCards").addEventListener("click",
function(d){if("Album"===app.current.tag)if(d.target.classList.contains("card-body"))appGoto("Browse","Database","Detail",0,void 0,"Album","AlbumArtist",decodeURI(d.target.parentNode.getAttribute("data-album")),decodeURI(d.target.parentNode.getAttribute("data-albumartist")));else{if(d.target.classList.contains("card-footer")){showMenu(d.target,d);for(var h=document.getElementById("BrowseDatabaseCards").getElementsByClassName("selected"),k=0;k<h.length;k++)h[k].classList.remove("selected");d.target.parentNode.classList.add("selected")}}else app.current.search=
"",document.getElementById("searchDatabaseStr").value="",appGoto(app.current.app,app.current.card,void 0,0,void 0,"Album","AlbumArtist","Album","("+app.current.tag+" == '"+decodeURI(d.target.parentNode.getAttribute("data-tag"))+"')")},!1);document.getElementById("BrowseDatabaseCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);!1===isMobile&&(document.getElementById("BrowseDatabaseCards").addEventListener("mouseover",function(d){if(d.target.classList.contains("card-body")&&
0===d.target.childNodes.length){var h=document.getElementById("BrowseDatabaseCards").getElementsByClassName("album-grid-mouseover");if(1<h.length)for(var k=0;k<h.length;k++)h[k].remove();addPlayButton(d.target)}},!1),document.getElementById("BrowseDatabaseCards").addEventListener("mouseout",function(d){!d.target.classList.contains("card-body")||null!==d.relatedTarget&&d.relatedTarget.classList.contains("album-grid-mouseover")||(d.target.innerHTML="")},!1));document.getElementById("BrowseDatabaseDetailList").addEventListener("click",
function(d){"TFOOT"!==d.target.parentNode.parentNode.nodeName&&("TD"===d.target.nodeName?appendQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d))},!1);document.getElementById("searchtags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),doSearch(domCache.searchstr.value))},!1);document.getElementById("searchDatabaseTags").addEventListener("click",
function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("databaseSortDesc").addEventListener("click",function(d){toggleBtnChk(this);d.stopPropagation();d.preventDefault();"-"===app.current.sort.charAt(0)?app.current.sort=app.current.sort.substr(1):app.current.sort="-"+app.current.sort;
appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,app.current.search)},!1);document.getElementById("databaseSortTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),d.stopPropagation(),app.current.sort=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,app.current.search))},
!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavPlaylistsDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavFilesystemDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("dropdownSortPlaylistTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),playlistSort(d.target.getAttribute("data-tag")))},
!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,"-",this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.offset,app.current.limit,d.target.getAttribute("data-tag"),app.current.sort,
"-",app.current.search)},!1);document.getElementById("searchFilesystemStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,""!==this.value?this.value:"-",app.current.sort,"-",app.current.search)},!1);document.getElementById("searchPlaylistsStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,
app.current.sort,"-",this.value)},!1);a=["PlaybackColsDropdown"];for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&d.target.classList.contains("material-icons")&&(d.stopPropagation(),d.preventDefault(),toggleBtnChk(d.target))},!1);a=["search","searchqueue","searchdatabase"];for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("submit",function(d){d.preventDefault()},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",
function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&"Album"===app.current.tag)if(""!==this.value){d=document.getElementById("searchDatabaseMatch");var h=document.createElement("button");h.classList.add("btn","btn-light","mr-2");h.setAttribute("data-filter-tag",encodeURI(app.current.filter));h.setAttribute("data-filter-op",encodeURI(d.options[d.selectedIndex].value));h.setAttribute("data-filter-value",encodeURI(this.value));h.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+
" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";document.getElementById("searchDatabaseCrumb").appendChild(h)}else searchAlbumgrid(this.value);else"Album"===app.current.tag?searchAlbumgrid(this.value):appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.limit,app.current.filter,app.current.sort,app.current.tag,this.value)},!1);document.getElementById("searchDatabaseCrumb").addEventListener("click",function(d){"SPAN"===d.target.nodeName?
(d.preventDefault(),d.stopPropagation(),d.target.parentNode.remove(),searchAlbumgrid("")):"BUTTON"===d.target.nodeName&&(d.preventDefault(),d.stopPropagation(),selectTag("searchDatabaseTags","searchDatabaseTagsDesc",decodeURI(d.target.getAttribute("data-filter-tag"))),document.getElementById("searchDatabaseStr").value=unescapeMPD(decodeURI(d.target.getAttribute("data-filter-value"))),document.getElementById("searchDatabaseMatch").value=decodeURI(d.target.getAttribute("data-filter-op")),d.target.remove(),
searchAlbumgrid(document.getElementById("searchDatabaseStr").value))},!1);domCache.searchstr.addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&settings.featAdvsearch)if(""!==this.value){d=document.getElementById("searchMatch");var h=document.createElement("button");h.classList.add("btn","btn-light","mr-2");h.setAttribute("data-filter-tag",encodeURI(app.current.filter));h.setAttribute("data-filter-op",encodeURI(d.options[d.selectedIndex].value));h.setAttribute("data-filter-value",
encodeURI(this.value));h.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";domCache.searchCrumb.appendChild(h)}else doSearch(this.value);else doSearch(this.value)},!1);domCache.searchCrumb.addEventListener("click",function(d){"SPAN"===d.target.nodeName?(d.preventDefault(),d.stopPropagation(),d.target.parentNode.remove(),doSearch("")):"BUTTON"===d.target.nodeName&&(d.preventDefault(),
d.stopPropagation(),domCache.searchstr.value=unescapeMPD(decodeURI(d.target.getAttribute("data-filter-value"))),selectTag("searchtags","searchtagsdesc",decodeURI(d.target.getAttribute("data-filter-tag"))),document.getElementById("searchMatch").value=decodeURI(d.target.getAttribute("data-filter-op")),d.target.remove(),doSearch(domCache.searchstr.value))},!1);document.getElementById("searchMatch").addEventListener("change",function(){doSearch(domCache.searchstr.value)},!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",
function(d){if(settings.featAdvsearch&&"TH"===d.target.nodeName&&""!==d.target.innerHTML){var h=d.target.getAttribute("data-col");if("Duration"!==h){var k=app.current.sort,l=!0;if(k===h||k==="-"+h)0===k.indexOf("-")?(l=!0,h=k.substring(1)):l=!1;!1===l?(k="-"+h,l=!0):(l=!1,k=h);for(var m=document.getElementById("SearchList").getElementsByClassName("sort-dir"),n=0;n<m.length;n++)m[n].remove();app.current.sort=k;d.target.innerHTML=t(h)+'<span class="sort-dir material-icons pull-right">'+(!0===l?"arrow_drop_up":
"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.offset,app.current.limit,app.current.filter,app.current.sort,"-",app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(d){"Enter"===d.key&&(d.preventDefault(),d.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(d){"OPTION"===d.target.nodeName&&removeScriptArgument(d)},!1);
document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("QueueJukebox");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",
{},parseState)},!1);document.addEventListener("keydown",function(d){if("INPUT"!==d.target.tagName&&"SELECT"!==d.target.tagName&&"TEXTAREA"!==d.target.tagName&&!d.ctrlKey&&!d.altKey){var h=keymap[d.key];h&&"function"===typeof window[h.cmd]&&(void 0===keymap[d.key].req||!0===settings[keymap[d.key].req])&&parseCmd(d,h)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(d){navigateTable(this,d.key)},!1);var g=
"";Object.keys(themes).forEach(function(d){g+='<option value="'+e(d)+'">'+t(themes[d])+"</option>"});document.getElementById("selectTheme").innerHTML=g;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",
function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("localPlayer").addEventListener("click",function(d){d.stopPropagation()});document.getElementById("localPlayer").addEventListener("canplay",function(){logDebug("localPlayer event: canplay");document.getElementById("alertLocalPlayback").classList.add("hide");document.getElementById("errorLocalPlayback").classList.add("hide")});document.getElementById("localPlayer").addEventListener("error",
...
function showNotification(a,b,c,f){if("No current song"!==a&&"No lyrics found"!==a){if(!0===settings.notificationWeb){var g=new Notification(a,{icon:"assets/favicon.ico",body:b});setTimeout(g.close.bind(g),3E3)}if(!0===settings.notificationPage||"danger"===f||"warning"===f){alertTimeout&&clearTimeout(alertTimeout);document.getElementById("alertBox")?g=document.getElementById("alertBox"):(g=document.createElement("div"),g.setAttribute("id","alertBox"),g.classList.add("toast"));var d='<div class="toast-header">';
d=("success"===f?d+'<span class="material-icons text-success mr-2">info</span>':"warning"===f?d+'<span class="material-icons text-warning mr-2">warning</span>':d+'<span class="material-icons text-danger mr-2">error</span>')+('<strong class="mr-auto">'+e(a)+'</strong><button type="button" class="ml-2 mb-1 close">&times;</button></div>');if(""!==c||""!==b)d+='<div class="toast-body">'+(""===c?e(b):c)+"</div>";g.innerHTML=d+"</div>";document.getElementById("alertBox")||(document.getElementsByTagName("main")[0].append(g),
requestAnimationFrame(function(){var h=document.getElementById("alertBox");h&&h.classList.add("alertBoxActive")}));g.getElementsByTagName("button")[0].addEventListener("click",function(){hideNotification()},!1);alertTimeout=setTimeout(function(){hideNotification()},3E3)}setStateIcon();logMessage(a,b,c,f)}}
function logMessage(a,b,c,f){"success"===f?f="Info":"warning"===f?f="Warning":"danger"===f&&(f="Error");var g=document.getElementById("logOverview"),d=!0,h=g.firstElementChild;h&&h.getAttribute("data-title")===a&&(d=!1);var k=document.createElement("div");k.classList.add("text-light");k.setAttribute("data-title",a);var l=1;!1===d&&(l+=parseInt(h.getAttribute("data-occurence")));k.setAttribute("data-occurence",l);k.innerHTML="<small>"+localeDate()+"&nbsp;&ndash;&nbsp;"+t(f)+(1<l?"&nbsp;("+l+")":"")+
"</small><p>"+e(a)+(""===c&&""===b?"":"<br/>"+(""===c?e(b):c))+"</p>";!0===d?g.insertBefore(k,g.firstElementChild):g.replaceChild(k,h);a=g.getElementsByTagName("div");10<a.length&&a[10].remove()}function clearLogOverview(){for(var a=document.getElementById("logOverview").getElementsByTagName("div"),b=a.length-1;0<=b;b--)a[b].remove();setStateIcon()}
...
function parsePartitionOutputsList(a){for(var b=document.getElementById("outputs").getElementsByTagName("button"),c=[],f=0;f<b.length;f++)c.push(parseInt(b[f].getAttribute("data-output-id")));b="";for(var g=f=0;g<a.result.data.length;g++)!1===c.includes(a.result.data[g].id)&&(b+='<tr data-output="'+encodeURI(a.result.data[g].name)+'"><td>'+e(a.result.data[g].name)+"</td></tr>",f++);0===f&&(b='<tr class="not-clickable"><td><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</td></tr>");
...
replaceTblRow(g[c],l)?c:h:f.append(l)}else if("Detail"===app.current.view){for(k=0;k<b;k++){var m=encodeURI(a.result.data[k].uri);l=document.createElement("tr");!1===a.result.smartpls&&l.setAttribute("draggable","true");l.setAttribute("id","playlistTrackId"+a.result.data[k].Pos);l.setAttribute("data-type",a.result.data[k].Type);l.setAttribute("data-uri",m);l.setAttribute("data-name",a.result.data[k].Title);l.setAttribute("data-songpos",a.result.data[k].Pos);l.setAttribute("tabindex",0);a.result.data[k].Duration=
beautifySongDuration(a.result.data[k].Duration);m="";for(var n=0;n<settings.colsBrowsePlaylistsDetail.length;n++)m+='<td data-col="'+settings.colsBrowsePlaylistsDetail[n]+'">'+e(a.result.data[k][settings.colsBrowsePlaylistsDetail[n]])+"</td>";m+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";l.innerHTML=m;k<g.length?h=!0===replaceTblRow(g[k],l)?k:h:f.append(l)}h=c.getElementsByTagName("tfoot")[0];c=settings.colsBrowsePlaylistsDetail.length;c++;
h.innerHTML='<tr><td colspan="'+(c+1)+'"><small>'+t("Num songs",a.result.totalEntities)+"&nbsp;&ndash;&nbsp;"+beautifyDuration(a.result.totalTime)+"</small></td></tr>"}for(h=g.length-1;h>=b;h--)g[h].remove();!0===d&&focusTable(0);setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(f.innerHTML="All"===app.current.view?'<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="3">'+t("No playlists found")+"</td></tr>":'<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="'+
...
function getAllPlaylists(a,b,c){var f=a.result.returnedEntities,g="";if("addToPlaylistPlaylist"===b)g='<option value=""></option><option value="new">'+t("New playlist")+"</option>";else if("selectJukeboxPlaylist"===b||"selectAddToQueuePlaylist"===b||"selectTimerPlaylist"===b)g='<option value="Database">'+t("Database")+"</option>";for(var d=0;d<f;d++)if("addToPlaylistPlaylist"!==b||"smartpls"!==a.result.data[d].Type)g+='<option value="'+e(a.result.data[d].uri)+'"',null!==c&&a.result.data[d].uri===
c&&(g+=" selected"),g+=">"+e(a.result.data[d].uri)+"</option>";document.getElementById(b).innerHTML=g}function updateSmartPlaylists(a){sendAPI("MPDWORKER_API_SMARTPLS_UPDATE_ALL",{force:a})}function removeFromPlaylist(a,b){b--;sendAPI("MPD_API_PLAYLIST_RM_TRACK",{uri:a,track:b});document.getElementById("BrowsePlaylistsDetailList").classList.add("opacity05")}
...
function saveSmartPlaylist(){var a=document.getElementById("saveSmartPlaylistName").value,b=document.getElementById("saveSmartPlaylistType").getAttribute("data-value"),c=document.getElementById("saveSmartPlaylistSort");c=c.options[c.selectedIndex].value;if(!0===validatePlname(a)){if("search"===b){var f=document.getElementById("selectSaveSmartPlaylistTag");f=f.options[f.selectedIndex].value;var g=document.getElementById("inputSaveSmartPlaylistSearchstr").value;sendAPI("MPD_API_SMARTPLS_SAVE",{type:b,
playlist:a,tag:f,searchstr:g,sort:c})}else if("sticker"===b){f=document.getElementById("selectSaveSmartPlaylistSticker");f=f.options[f.selectedIndex].value;g=document.getElementById("inputSaveSmartPlaylistStickerMaxentries");if(!validateInt(g))return;var d=document.getElementById("inputSaveSmartPlaylistStickerMinvalue");if(!validateInt(d))return;sendAPI("MPD_API_SMARTPLS_SAVE",{type:b,playlist:a,sticker:f,maxentries:parseInt(g.value),minvalue:parseInt(d.value),sort:c})}else if("newest"===b){f=document.getElementById("inputSaveSmartPlaylistNewestTimerange");
...
content:" "});c=a.Popover;null===a.getAttribute("data-init")&&(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",function(f){f.target.setAttribute("data-popover","true");document.getElementById("colChecklist"+b).addEventListener("click",function(g){"BUTTON"===g.target.nodeName&&g.target.classList.contains("material-icons")?(toggleBtnChk(g.target),g.preventDefault(),g.stopPropagation()):"BUTTON"===g.target.nodeName&&(g.preventDefault(),saveCols(b))},!1)},!1));c.show()}
function showMenuTd(a){var b=a.getAttribute("data-type"),c=decodeURI(a.getAttribute("data-uri")),f=decodeURI(a.getAttribute("data-name")),g=0;if(null===b||""===c)b=a.parentNode.getAttribute("data-type"),c=decodeURI(a.parentNode.getAttribute("data-uri")),f=a.parentNode.getAttribute("data-name");if(null===b||""===c)b=a.parentNode.parentNode.getAttribute("data-type"),c=decodeURI(a.parentNode.parentNode.getAttribute("data-uri")),f=a.parentNode.parentNode.getAttribute("data-name");lastState&&(g=lastState.nextSongPos);
var d="";if("Browse"===app.current.app&&"Filesystem"===app.current.tab||"Search"===app.current.app||"Browse"===app.current.app&&"Database"===app.current.tab&&"Detail"===app.current.view){if(d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,g,f]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+("plist"!==b&&"smartpls"!==b&&!0===settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",
...
("dir"===b?addMenuItem({cmd:"updateDB",options:[dirname(c),!0]},t("Update directory")):"")+("dir"===b?addMenuItem({cmd:"rescanDB",options:[dirname(c),!0]},t("Rescan directory")):"")),"Search"===app.current.app){var h=a.parentNode.parentNode;h.hasAttribute("data-album")&&h.hasAttribute("data-albumartist")?(b=decodeURI(h.getAttribute("data-album")),c=decodeURI(h.getAttribute("data-albumartist")),d+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+
...
addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"appendAfterQueue",options:[b,c,g,f]},t("Add after current playing song"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+"</div>")}}else if("Browse"===app.current.app&&"Database"===app.current.tab&&"List"===app.current.view)b=decodeURI(a.parentNode.getAttribute("data-albumartist")),
c=decodeURI(a.parentNode.getAttribute("data-album")),d+=addMenuItem({cmd:"appGoto",options:["Browse","Database","Detail",0,void 0,"Album",tagAlbumArtist,c,b]},t("Show album"))+addMenuItem({cmd:"_addAlbum",options:["appendQueue",b,c]},t("Append to queue"))+addMenuItem({cmd:"_addAlbum",options:["replaceQueue",b,c]},t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"_addAlbum",options:["addPlaylist",b,c]},t("Add to playlist")):"");else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&
"All"===app.current.view)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):addMenuItem({cmd:"playlistDetails",options:[c]},t("Edit playlist")))+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"showSmartPlaylist",options:[c]},t("Edit smart playlist")):"")+(!0===settings.smartpls&&"smartpls"===
b?addMenuItem({cmd:"updateSmartPlaylist",options:[c]},t("Update smart playlist")):"")+addMenuItem({cmd:"showRenamePlaylist",options:[c]},t("Rename playlist"))+addMenuItem({cmd:"showDelPlaylist",options:[c]},t("Delete playlist"))+(!0===settings.featHome?addMenuItem({cmd:"addPlistToHome",options:[c,f]},t("Add to homescreen")):"");else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view)g=document.getElementById("BrowsePlaylistsDetailList"),d+=addMenuItem({cmd:"appendQueue",
options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+("false"===g.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[g.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===
app.current.app&&"Current"===app.current.tab)b=parseInt(a.parentNode.parentNode.getAttribute("data-trackid")),d+=(b!==lastState.currentSongId?addMenuItem({cmd:"playAfterCurrent",options:[b,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Play after current playing song")):"")+addMenuItem({cmd:"delQueueSong",options:["single",b]},t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",
options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-1,-1]},t("Remove all downwards"))+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)d+=addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):
"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Jukebox"===app.current.tab)d+=addMenuItem({cmd:"songDetails",options:[c]},t("Song details"))+addMenuItem({cmd:"delQueueJukeboxSong",options:[a.parentNode.parentNode.getAttribute("data-pos")]},t("Remove"));else if("Home"===app.current.app){g=parseInt(a.parentNode.getAttribute("data-pos"));h=JSON.parse(a.parentNode.getAttribute("data-href"));var k="";"replaceQueue"===h.cmd&&
"plist"===h.options[0]?(b="plist",c=h.options[1],k=t("Add and play playlist"),f=t("Playlist")):"appGoto"===h.cmd?(b="view",k=t("Goto view"),f=t("View")):"execScriptFromOptions"===h.cmd&&(b="script",k=t("Execute script"),f=t("Script"));d+='<h6 class="dropdown-header">'+f+"</h6>"+addMenuItem({cmd:"executeHomeIcon",options:[g]},k)+("plist"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+addMenuItem({cmd:"editHomeIcon",options:[g]},t("Edit home icon"))+addMenuItem({cmd:"duplicateHomeIcon",
...
function parseQueue(a){if(a.result.offset<app.current.offset)gotoPage(a.result.offset);else{var b=document.getElementById("QueueCurrentList"),c=b.getElementsByTagName("tfoot")[0],f=settings.colsQueueCurrent.length;c.innerHTML=a.result.totalTime&&0<a.result.totalTime&&a.result.totalEntities<=app.current.limit?'<tr><td colspan="'+(f+1)+'"><small>'+t("Num songs",a.result.totalEntities)+"&nbsp;&ndash;&nbsp;"+beautifyDuration(a.result.totalTime)+"</small></td></tr>":0<a.result.totalEntities?'<tr><td colspan="'+
(f+1)+'"><small>'+t("Num songs",a.result.totalEntities)+"</small></td></tr>":"";a.result.totalEntities>settings.maxElementsPerPage?document.getElementById("btnQueueGotoPlayingSong").parentNode.classList.remove("hide"):document.getElementById("btnQueueGotoPlayingSong").parentNode.classList.add("hide");c=a.result.returnedEntities;var g=document.activeElement.parentNode.parentNode===b?!0:!1,d=0;b.setAttribute("data-version",a.result.queueVersion);b=b.getElementsByTagName("tbody")[0];for(var h=b.getElementsByTagName("tr"),
...
'<td data-col="'+settings.colsQueueCurrent[n]+'">'+e(a.result.data[k][settings.colsQueueCurrent[n]])+"</td>";m+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";l.innerHTML=m;k<h.length?d=!0===replaceTblRow(h[k],l)?k:d:b.append(l)}for(k=h.length-1;k>=c;k--)h[k].remove();"MPD_API_QUEUE_SEARCH"===a.result.method&&0===c?b.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+f+'">'+t("No results, please refine your search")+
"</td></tr>":"MPD_API_QUEUE_LIST"===a.result.method&&0===c&&(b.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+f+'">'+t("Empty queue")+"</td></tr>");!0===g&&focusTable(d);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueCurrentList").classList.remove("opacity05")}}
...
a.result.data[h].Title);k.setAttribute("data-type","song");k.setAttribute("tabindex",0);for(var l="",m=0;m<settings.colsQueueLastPlayed.length;m++)l+='<td data-col="'+settings.colsQueueLastPlayed[m]+'">'+e(a.result.data[h][settings.colsQueueLastPlayed[m]])+"</td>";l+='<td data-col="Action">';""!==a.result.data[h].uri&&(l+='<a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a>");l+="</td>";k.innerHTML=l;h<d.length?g=!0===replaceTblRow(d[h],k)?h:g:c.append(k)}for(h=d.length-1;h>=b;h--)d[h].remove();
d=settings.colsQueueLastPlayed.length;0===b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("Empty list")+"</td></tr>");!0===f&&focusTable(g);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueLastPlayedList").classList.remove("opacity05")}
...
function saveScript(){var a=!0,b=document.getElementById("inputScriptName");validatePlnameEl(b)||(a=!1);var c=document.getElementById("inputScriptOrder");validateInt(c)||(a=!1);if(!0===a){a=[];for(var f=document.getElementById("selectScriptArguments"),g=0;g<f.options.length;g++)a.push(f.options[g].text);sendAPI("MYMPD_API_SCRIPT_SAVE",{oldscript:document.getElementById("inputOldScriptName").value,script:b.value,order:parseInt(c.value),content:document.getElementById("textareaScriptContent").value,
...
function parseScriptList(a){var b=document.createElement("optgroup");b.setAttribute("data-value","script");b.setAttribute("label",t("Script"));var c="",f="",g=a.result.data.length,d=0;if(0<g){a.result.data.sort(function(l,m){return l.metadata.order-m.metadata.order});for(var h=0;h<g;h++){var k="";if(0<a.result.data[h].metadata.arguments.length){for(k=0;k<a.result.data[h].metadata.arguments.length;k++)a.result.data[h].metadata.arguments[k]=e(a.result.data[h].metadata.arguments[k]);k='"'+a.result.data[h].metadata.arguments.join('","')+
'"'}0<a.result.data[h].metadata.order&&(d++,c+='<a class="dropdown-item text-light alwaysEnabled" href="#" data-href=\'{"script": "'+e(a.result.data[h].name)+'", "arguments": ['+k+"]}'>"+e(a.result.data[h].name)+"</a>");f+='<tr data-script="'+encodeURI(a.result.data[h].name)+'"><td>'+e(a.result.data[h].name)+'</td><td data-col="Action">'+(!0===settings.featScripteditor?'<a href="#" title="'+t("Delete")+'" data-action="delete" class="material-icons color-darkgrey">delete</a>':"")+'<a href="#" title="'+
...
if(!0===settings.featSyscmds){c="";f=settings.syscmdList.length;if(0<f)for(b+='<optgroup data-value="syscmd" label="'+t("System command")+'">',c=4<f?"":'<div class="dropdown-divider"></div>',g=0;g<f;g++)"HR"===settings.syscmdList[g]?c+='<div class="dropdown-divider"></div>':(c+='<a class="dropdown-item text-light alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+e(settings.syscmdList[g])+"\"]}'>"+e(settings.syscmdList[g])+"</a>",b+='<option value="'+e(settings.syscmdList[g])+
...
d=getBtnGroupValue("btnSingleGroup");var h=getBtnGroupValue("btnJukeboxModeGroup"),k=getBtnGroupValue("btnReplaygainGroup"),l=document.getElementById("selectJukeboxUniqueTag");l=l.options[l.selectedIndex].value;var m=document.getElementById("selectJukeboxPlaylist");m=m.options[m.selectedIndex].value;"2"===h&&(l="Album");"1"===h&&!1===settings.featSearchwindow&&"Database"===m&&(b=!1,document.getElementById("warnJukeboxPlaylist").classList.remove("hide"));if(!0===b){b=document.getElementById("selectLocale");
...
function initTagMultiSelect(a,b,c,f){for(var g=[],d="",h=0;h<c.length;h++)f.includes(c[h])&&g.push(t(c[h])),d+='<div class="form-check"><button class="btn btn-secondary btn-xs clickable material-icons material-icons-small'+(f.includes(c[h])?" active":"")+'" name="'+c[h]+'">'+(f.includes(c[h])?"check":"radio_button_unchecked")+'</button><label class="form-check-label" for="'+c[h]+'">&nbsp;&nbsp;'+t(c[h])+"</label></div>";document.getElementById(b).innerHTML=d;a=document.getElementById(a);a.value=g.join(", ");
...
function isCoverfile(a){a=basename(a).toLowerCase();for(var b=a.split("."),c="png jpg jpeg svg webp tiff bmp".split(" "),f=settings.coverimageName.split(","),g=0;g<f.length;g++){var d=f[g].trim();if(a===d||b[1]&&d===b[0]&&c.includes(b[1]))return!0}return!1}
function getLyrics(a,b){void 0===a?b.innerHTML=t("No lyrics found"):(b.classList.add("opacity05"),sendAPI("MPD_API_LYRICS_GET",{uri:a},function(c){if(c.error)b.innerText=t(c.error.message);else if(c.result.message)b.innerText=t(c.result.message);else{for(var f='<span class="lyricsHeader" class="btn-group-toggle" data-toggle="buttons">',g='<div class="lyricsTextContainer">',d=0;d<c.result.returnedEntities;d++){var h=c.result.data[d].desc;h=""!==h&&""!==c.result.data[d].lang?h+(" ("+c.result.data[d].lang+
")"):""!==c.result.data[d].lang?c.result.data[d].lang:d;f+='<label data-num="'+d+'" class="btn btn-sm btn-outline-secondary mr-2'+(0===d?" active":"")+'">'+h+"</label>";g+='<div class="lyricsText'+(0<d?" hide":"")+'">'+(!0===c.result.synced?parseSyncedLyrics(c.result.data[d].text):e(c.result.data[d].text).replace(/\n/g,"<br/>"))+"</div>"}f+="</span>";g+="</div>";showSyncedLyrics=c.result.synced;1<c.result.returnedEntities?(b.innerHTML=f+g,b.getElementsByClassName("lyricsHeader")[0].addEventListener("click",
...
function parseSyncedLyrics(a){var b="";a=a.split("\r\n");for(var c=0;c<a.length;c++){var f=a[c].match(/^\[(\d+):(\d+)\.(\d+)\](.*)$/);if(f){var g=60*parseInt(f[1])+parseInt(f[2]);b+='<p><span data-sec="'+g+'">';b+=f[4].replace(/<(\d+):(\d+)\.\d+>/g,function(d,h,k){return'</span><span data-sec="'+(60*parseInt(h)+parseInt(k))+'">'});b+="</span></p>"}}return b+""}function loveSong(){sendAPI("MPD_API_LOVE",{})}
...
function showListOutputAttributes(a){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{},function(b){modalOutputAttributes.show();for(var c,f=0;f<b.result.data.length;f++)if(b.result.data[f].name===a){c=b.result.data[f];break}document.getElementById("modalOutputAttributesId").value=e(c.id);var g="<tr><td>"+t("Name")+"</td><td>"+e(c.name)+"</td></tr><tr><td>"+t("State")+"</td><td>"+(1===c.state?t("enabled"):t("disabled"))+"</td></tr><tr><td>"+t("Plugin")+"</td><td>"+e(c.plugin)+"</td></tr>",d=0;Object.keys(c.attributes).forEach(function(h){d++;
g+="<tr><td>"+e(h)+'</td><td><input name="'+e(h)+'" class="form-control border-secondary" type="text" value="'+e(c.attributes[h])+'"/></td></tr>'});0<d?document.getElementById("btnOutputAttributesSave").removeAttribute("disabled"):document.getElementById("btnOutputAttributesSave").setAttribute("disabled","disabled");document.getElementById("outputAttributesList").innerHTML=g})}
...
if(lastState&&lastState.currentSongId!==a&&(f=document.getElementById("queueTrackId"+lastState.currentSongId))){var g=f.querySelector("[data-col=Duration]");g&&(g.innerText=f.getAttribute("data-duration"));if(g=f.querySelector("[data-col=Pos]"))g.classList.remove("material-icons"),g.innerText=f.getAttribute("data-songpos");f.classList.remove("font-weight-bold")}if(a=document.getElementById("queueTrackId"+a)){if(f=a.querySelector("[data-col=Duration]"))f.innerHTML=b;(b=a.querySelector("[data-col=Pos]"))&&
...
function songChange(a){var b=a.result.Title+":"+a.result.Artist+":"+a.result.Album+":"+a.result.uri+":"+a.result.currentSongId;if(lastSong!==b){var c="",f="",g="";mediaSessionSetMetadata(a.result.Title,a.result.Artist,a.result.Album,a.result.uri);setCurrentCover(a.result.uri);!0===settings.bgCover&&!0===settings.featCoverimage&&setBackgroundImage(a.result.uri);domCache.footerArtist.classList.remove("clickable");domCache.footerAlbum.classList.remove("clickable");domCache.footerCover.classList.remove("clickable");
void 0!==a.result.Artist&&0<a.result.Artist.length&&"-"!==a.result.Artist?(c+=a.result.Artist,f+=a.result.Artist,g+=a.result.Artist+" - ",domCache.footerArtist.innerText=a.result.Artist,domCache.footerArtist.setAttribute("data-name",encodeURI(a.result.Artist)),!0===settings.featAdvsearch&&domCache.footerArtist.classList.add("clickable")):(domCache.footerArtist.innerText="",domCache.footerArtist.setAttribute("data-name",""));void 0!==a.result.Album&&0<a.result.Album.length&&"-"!==a.result.Album?(c+=
...
setVoteSongBtns(a.result.like,a.result.uri);a.result.Fileformat=lastState?fileformat(lastState.audioFormat):"";for(g=0;g<settings.colsPlayback.length;g++){var d=document.getElementById("current"+settings.colsPlayback[g]);if(d&&"Lyrics"===settings.colsPlayback[g])getLyrics(a.result.uri,d.getElementsByTagName("p")[0]);else if(d){var h=a.result[settings.colsPlayback[g]];void 0===h&&(h="");h="Duration"===settings.colsPlayback[g]?beautifySongDuration(h):"LastModified"===settings.colsPlayback[g]?localeDate(h):
0===settings.colsPlayback[g].indexOf("MUSICBRAINZ")?getMBtagLink(settings.colsPlayback[g],a.result[settings.colsPlayback[g]]):e(h);d.getElementsByTagName("p")[0].innerHTML=h;d.setAttribute("data-name",encodeURI(h));"Album"===settings.colsPlayback[g]&&null!==a.result[tagAlbumArtist]&&d.setAttribute("data-albumartist",encodeURI(a.result[tagAlbumArtist]))}}document.getElementById("currentBooklet").innerHTML=""===a.result.bookletPath||void 0===a.result.bookletPath||!1===settings.featBrowse?"":'<span class="text-light material-icons">description</span>&nbsp;<a class="text-light" target="_blank" href="'+
...
function mediaSessionSetMetadata(a,b,c,f){if(!0===settings.mediaSession&&"mediaSession"in navigator){var g=window.location.port;f=window.location.protocol+"//"+window.location.hostname+(""!==g?":"+g:"")+subdir+"/albumart/"+f;navigator.mediaSession.metadata=!0===settings.coverimage?new MediaMetadata({title:a,artist:b,album:c,artwork:[{src:f}]}):new MediaMetadata({title:a,artist:b,album:c})}}
...
d=f.getAttribute("data-songpos");document.getElementById(c.dataTransfer.getData("Text")).remove();dragEl.classList.remove("opacity05");b.insertBefore(dragEl,f);c=b.getElementsByClassName("dragover");f=c.length;for(var h=0;h<f;h++)c[h].classList.remove("dragover");document.getElementById(a).classList.add("opacity05");"Queue"===app.current.app&&"Current"===app.current.tab?sendAPI("MPD_API_QUEUE_MOVE_TRACK",{from:g,to:d}):"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view&&
playlistMoveTrack(g,d)}},!1)}
...
c.target);c=b.getElementsByClassName("dragover-th");for(var f=c.length,g=0;g<f;g++)c[g].classList.remove("dragover-th");document.getElementById(a+"List")?(document.getElementById(a+"List").classList.add("opacity05"),saveCols(a)):saveCols(a,this.parentNode.parentNode)}},!1)}
...
function setCols(a){var b=document.getElementById(a+"ColsDropdown");b&&(b.firstChild.innerHTML=setColsChecklist(a));b=app.current.sort;"Search"===a&&"Title"===app.apps.Search.sort&&(b=settings.tags.includes("Title")?"Title":!1===settings.featTags?"Filename":"-");if("Playback"!==a){for(var c="",f=0;f<settings["cols"+a].length;f++){var g=settings["cols"+a][f];c+='<th draggable="true" data-col="'+g+'">';if("Track"===g||"Pos"===g)g="#";c+=t(g);"Search"!==a||g!==b&&"-"+g!==b||(g=!1,0===app.current.sort.indexOf("-")&&
...
function saveCols(a,b){var c=document.getElementById(a+"ColsDropdown");b=void 0===b?document.getElementById(a+"List").getElementsByTagName("tr")[0]:"string"===typeof b?document.querySelector(b).getElementsByTagName("tr")[0]:b.getElementsByTagName("tr")[0];if(c){c=c.firstChild.getElementsByTagName("button");for(var f=0;f<c.length;f++)if(null!==c[f].getAttribute("name")){var g=b.querySelector("[data-col="+c[f].name+"]");!1===c[f].classList.contains("active")?g&&g.remove():g||(g=document.createElement("th"),
g.innerText=c[f].name,g.setAttribute("data-col",c[f].name),b.insertBefore(g,b.lastChild))}}a={table:"cols"+a,cols:[]};b=b.getElementsByTagName("th");for(c=0;c<b.length;c++)f=b[c].getAttribute("data-col"),"Action"!==f&&null!==f&&a.cols.push(f);sendAPI("MYMPD_API_COLS_SAVE",a,getSettings)}
function saveColsPlayback(a){for(var b=document.getElementById(a+"ColsDropdown").firstChild.getElementsByTagName("button"),c=document.getElementById("cardPlaybackTags"),f=0;f<b.length-1;f++){var g=document.getElementById("current"+b[f].name);!1===b[f].classList.contains("active")?g&&g.remove():g||(g=document.createElement("div"),g.innerHTML="<small>"+t(b[f].name)+"</small><p></p>",g.setAttribute("id","current"+b[f].name),g.setAttribute("data-tag",b[f].name),c.appendChild(g))}a={table:"cols"+a,cols:[]};
...
function saveTimer(){var a=!0,b=document.getElementById("inputTimerName");validateNotBlank(b)||(a=!1);for(var c=!1,f="btnTimerMon btnTimerTue btnTimerWed btnTimerThu btnTimerFri btnTimerSat btnTimerSun".split(" "),g=[],d=0;d<f.length;d++){var h=document.getElementById(f[d]).classList.contains("active")?!0:!1;g.push(h);!0===h&&(c=!0)}!1===c?(a=!1,document.getElementById("invalidTimerWeekdays").style.display="block"):document.getElementById("invalidTimerWeekdays").style.display="none";c=document.getElementById("selectTimerAction");
f=document.getElementById("selectTimerPlaylist");d=document.getElementById("selectTimerHour");h=document.getElementById("selectTimerMinute");var k=document.getElementById("btnTimerJukeboxModeGroup").getElementsByClassName("active")[0].getAttribute("data-value");-1===c.selectedIndex&&(a=!1,c.classList.add("is-invalid"));"0"===k&&"Database"===f.options[f.selectedIndex].value&&"startplay"===c.options[c.selectedIndex].value&&(a=!1,document.getElementById("btnTimerJukeboxModeGroup").classList.add("is-invalid"));
...
function parseListTimer(a){for(var b=document.getElementById("listTimer").getElementsByTagName("tbody")[0],c=b.getElementsByTagName("tr"),f=0,g="Mon Tue Wed Thu Fri Sat Sun".split(" "),d=0;d<a.result.returnedEntities;d++){var h=document.createElement("tr");h.setAttribute("data-id",a.result.data[d].timerid);for(var k="<td>"+e(a.result.data[d].name)+'</td><td><button name="enabled" class="btn btn-secondary btn-xs clickable material-icons material-icons-small'+(!0===a.result.data[d].enabled?" active":
"")+'">'+(!0===a.result.data[d].enabled?"check":"radio_button_unchecked")+"</button></td><td>"+zeroPad(a.result.data[d].startHour,2)+":"+zeroPad(a.result.data[d].startMinute,2)+" "+t("on")+" ",l=[],m=0;7>m;m++)!0===a.result.data[d].weekdays[m]&&l.push(t(g[m]));k+=l.join(", ")+"</td><td>"+prettyTimerAction(a.result.data[d].action,a.result.data[d].subaction)+'</td><td data-col="Action"><a href="#" class="material-icons color-darkgrey">delete</a></td>';h.innerHTML=k;d<c.length?f=!0===replaceTblRow(c[d],
h)?d:f:b.append(h)}for(f=c.length-1;f>=a.result.returnedEntities;f--)c[f].remove();0===a.result.returnedEntities&&(b.innerHTML='<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="4">'+t("Empty list")+"</td></tr>")}function prettyTimerAction(a,b){return"player"===a&&"startplay"===b?t("Start playback"):"player"===a&&"stopplay"===b?t("Stop playback"):"syscmd"===a?t("System command")+": "+e(b):"script"===a?t("Script")+": "+e(b):e(a)+": "+e(b)}
...
function setPagination(a,b){var c=app.current.app+(void 0===app.current.tab?"":app.current.tab),f=0<app.current.limit?Math.ceil(a/app.current.limit):1;0===f&&(f=1);for(var g=0<app.current.limit?app.current.offset/app.current.limit+1:1,d='<button title="'+t("First page")+'" type="button" class="btn btn-group-prepend btn-secondary material-icons">first_page</button><button title="'+t("Previous page")+'" type="button" class="btn btn-group-prepend btn-secondary material-icons">navigate_before</button><div class="btn-group"><button class="btn btn-secondary dropdown-toggle" type="button" data-toggle="dropdown"></button><div class="dropdown-menu bg-lite-dark px-2 pages dropdown-menu-right"></div></div><button title="'+
...
k.next();!l.done;l=k.next())l=l.value,h+='<option value="'+l+'"'+(app.current.limit===l?" selected":"")+">"+(0<l?l:t("All"))+"</option>";h+='</select></div><div id="'+c+'PaginationBottom" class="btn-group dropup pagination">'+d+"</div></div>";k=document.getElementById(c+"ButtonsBottom");k.innerHTML=h;k.getElementsByTagName("button")[0].addEventListener("click",function(){event.preventDefault();scrollToPosY(0)},!1);k.getElementsByTagName("select")[0].addEventListener("change",function(r){r=parseInt(getSelectValue(r.target));
app.current.limit!==r&&gotoPage(app.current.offset,r)},!1);document.getElementById(c+"PaginationTop").innerHTML=d;d=app.current.offset+app.current.limit;h=[document.getElementById(c+"PaginationTop"),document.getElementById(c+"PaginationBottom")];k={};for(l=0;l<h.length;k={$jscomp$loop$prop$lastPageOffset$73:k.$jscomp$loop$prop$lastPageOffset$73},l++){var m=h[l].children[0],n=h[l].children[1],p=h[l].children[2].children[0],q=h[l].children[2].children[1],v=h[l].children[3],u=h[l].children[4];p.innerText=
g+" / "+f;if(1<f){p.removeAttribute("disabled");for(var x="",w=0;w<f;w++){var y=w*app.current.limit;x+='<button data-offset="'+y+'" type="button" class="btn-sm btn btn-secondary'+(y===app.current.offset?" active":"")+'">'+(w+1)+"</button>"}q.innerHTML=x;p.classList.remove("nodropdown");q.addEventListener("click",function(r){"BUTTON"===r.target.nodeName&&gotoPage(r.target.getAttribute("data-offset"))},!1);new BSN.Dropdown(p);k.$jscomp$loop$prop$lastPageOffset$73=(f-1)*app.current.limit;k.$jscomp$loop$prop$lastPageOffset$73===
app.current.offset?u.setAttribute("disabled","disabled"):(u.removeAttribute("disabled"),u.classList.remove("hide"),v.classList.remove("rounded-right"),u.addEventListener("click",function(r){return function(){event.preventDefault();gotoPage(r.$jscomp$loop$prop$lastPageOffset$73)}}(k),!1))}else-1===a?(p.setAttribute("disabled","disabled"),p.innerText=g,p.classList.add("nodropdown"),u.setAttribute("disabled","disabled"),u.classList.add("hide"),v.classList.add("rounded-right")):(p.setAttribute("disabled",
"disabled"),p.classList.add("nodropdown"),u.setAttribute("disabled","disabled"));a>d&&0<d||-1===a&&b>=app.current.limit?(v.removeAttribute("disabled"),h[l].classList.remove("hide"),v.addEventListener("click",function(){event.preventDefault();gotoPage("next")},!1)):(v.setAttribute("disabled","disabled"),0===l&&h[l].classList.add("hide"));0<app.current.offset?(n.removeAttribute("disabled"),h[l].classList.remove("hide"),n.addEventListener("click",function(){event.preventDefault();gotoPage("prev")},!1),
