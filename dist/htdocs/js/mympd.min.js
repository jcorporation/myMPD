...
function(){document.getElementById("bgColorPreview").style.backgroundColor=this.value},!1);document.getElementById("modalAddToQueue").addEventListener("shown.bs.modal",function(){document.getElementById("inputAddToQueueQuantity").classList.remove("is-invalid");document.getElementById("warnJukeboxPlaylist2").classList.add("hide");!0===settings.featPlaylists&&sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(a){getAllPlaylists(a,"selectAddToQueuePlaylist")})});document.getElementById("modalUpdateDB").addEventListener("hidden.bs.modal",
...
filterCols("colsQueueLastPlayed");filterCols("colsQueueJukebox");filterCols("colsBrowsePlaylistsDetail");filterCols("colsBrowseFilesystem");filterCols("colsBrowseDatabaseDetail");filterCols("colsPlayback");if(!1===settings.featTags)app.apps.Browse.active="Filesystem",app.apps.Search.state.sort="filename",app.apps.Queue.tabs.Current.filter="filename",settings.colsQueueCurrent=["Pos","Title","Duration"],settings.colsQueueLastPlayed=["Pos","Title","LastPlayed"],settings.colsQueueJukebox=["Pos","Title"],
settings.colsSearch=["Title","Duration"],settings.colsBrowseFilesystem=["Type","Title","Duration"],settings.colsBrowseDatabase=["Track","Title","Duration"],settings.colsPlayback=[];else{a="";for(b=0;b<settings.colsPlayback.length;b++)a+='<div id="current'+settings.colsPlayback[b]+'" data-tag="'+settings.colsPlayback[b]+'" '+("Lyrics"===settings.colsPlayback[b]?"":'data-name="'+(lastSongObj[settings.colsPlayback[b]]?encodeURI(lastSongObj[settings.colsPlayback[b]]):"")+'"')+"><small>"+t(settings.colsPlayback[b])+
"</small><p",settings.browsetags.includes(settings.colsPlayback[b])&&(a+=' class="clickable"'),a+=">",a="Duration"===settings.colsPlayback[b]?a+(lastSongObj[settings.colsPlayback[b]]?beautifySongDuration(lastSongObj[settings.colsPlayback[b]]):""):"LastModified"===settings.colsPlayback[b]?a+(lastSongObj[settings.colsPlayback[b]]?localeDate(lastSongObj[settings.colsPlayback[b]]):""):"Fileformat"===settings.colsPlayback[b]?a+(lastState?fileformat(lastState.audioFormat):""):a+(lastSongObj[settings.colsPlayback[b]]?
e(lastSongObj[settings.colsPlayback[b]]):""),a+="</p></div>";document.getElementById("cardPlaybackTags").innerHTML=a;(a=document.getElementById("currentLyrics"))&&lastSongObj.uri&&(b=a.getElementsByTagName("small")[0],b.classList.add("clickable"),b.addEventListener("click",function(a){a.target.parentNode.children[1].classList.toggle("expanded")},!1),getLyrics(lastSongObj.uri,a.getElementsByTagName("p")[0]))}settings.tags.includes("Title")&&(app.apps.Search.sort="Title");settings.tags.includes("AlbumArtist")||
"AlbumArtist"!==app.apps.Browse.tabs.Database.views.List.filter||(app.apps.Browse.tabs.Database.views.List.filter="Artist",app.apps.Browse.tabs.Database.views.List.sort="Artist");!0===settings.featPlaylists?sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(a){getAllPlaylists(a,"selectJukeboxPlaylist",settings.jukeboxPlaylist)}):document.getElementById("selectJukeboxPlaylist").innerHTML='<option value="Database">'+t("Database")+"</option>";setCols("QueueCurrent");setCols("Search");setCols("QueueLastPlayed");
setCols("QueueJukebox");setCols("BrowseFilesystem");setCols("BrowsePlaylistsDetail");setCols("BrowseDatabaseDetail");setCols("Playback");addTagList("BrowseDatabaseByTagDropdown","browsetags");addTagList("BrowseNavPlaylistsDropdown","browsetags");addTagList("BrowseNavFilesystemDropdown","browsetags");addTagList("searchqueuetags","searchtags");addTagList("searchtags","searchtags");addTagList("searchDatabaseTags","browsetags");addTagList("databaseSortTagsList","browsetags");addTagList("dropdownSortPlaylistTags",
"tags");addTagList("saveSmartPlaylistSort","tags");addTagListSelect("selectSmartplsSort","tags");addTagListSelect("saveSmartPlaylistSort","tags");addTagListSelect("selectJukeboxUniqueTag","browsetags");initTagMultiSelect("inputEnabledTags","listEnabledTags",settings.allmpdtags,settings.tags);initTagMultiSelect("inputSearchTags","listSearchTags",settings.tags,settings.searchtags);initTagMultiSelect("inputBrowseTags","listBrowseTags",settings.tags,settings.browsetags);initTagMultiSelect("inputGeneratePlsTags",
"listGeneratePlsTags",settings.browsetags,settings.generatePlsTags)}function resetSettings(){sendAPI("MYMPD_API_SETTINGS_RESET",{},getSettings)}
...
app.current.app).classList.add("active");for(b=0;b<domCache.navbarBtnsLen;b++)domCache.navbarBtns[b].firstChild.setAttribute("data-href",JSON.stringify({cmd:"appGoto",options:settings.navbarIcons[b].options}))}function songDetails(a){sendAPI("MPD_API_DATABASE_SONGDETAILS",{uri:a},parseSongDetails);modalSongDetails.show()}
