...
var modalConnectionError=new Modal(document.getElementById("modalConnectionError")),modalSettings=new Modal(document.getElementById("modalSettings")),modalArchphileSettings=new Modal(document.getElementById("modalArchphileSettings")),modalNetworkSettings=new Modal(document.getElementById("modalNetworkSettings")),modalSavequeue=new Modal(document.getElementById("modalSaveQueue")),modalSongDetails=new Modal(document.getElementById("modalSongDetails")),modalAddToPlaylist=new Modal(document.getElementById("modalAddToPlaylist")),
modalRenamePlaylist=new Modal(document.getElementById("modalRenamePlaylist")),modalUpdateDB=new Modal(document.getElementById("modalUpdateDB"));
...
function appInit(){getSettings();getArchphileSettings();getNetworkSettings();sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);webSocketConnect();domCache.volumeBar.value=0;domCache.volumeBar.addEventListener("click",function(a){a.stopPropagation()},!1);domCache.volumeBar.addEventListener("change",function(a){sendAPI({cmd:"MPD_API_PLAYER_VOLUME",data:{volume:domCache.volumeBar.value}})},!1);domCache.progressBar.value=0;domCache.progressBar.addEventListener("change",function(a){currentSong&&0<=currentSong.currentSongId&&
sendAPI({cmd:"MPD_API_PLAYER_SEEK",data:{songid:currentSong.currentSongId,seek:Math.ceil(domCache.progressBar.value/100*currentSong.totalTime)}})},!1);document.getElementById("volumeIcon").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI({cmd:"MPD_API_PLAYER_OUTPUT_LIST"},parseOutputs)});document.getElementById("modalAbout").addEventListener("shown.bs.modal",function(){sendAPI({cmd:"MPD_API_DATABASE_STATS"},parseStats)});document.getElementById("modalUpdateDB").addEventListener("hidden.bs.modal",
...
document.getElementById("inputCrossfade").classList.remove("is-invalid");document.getElementById("inputMixrampdb").classList.remove("is-invalid");document.getElementById("inputMixrampdelay").classList.remove("is-invalid")});document.getElementById("modalArchphileSettings").addEventListener("shown.bs.modal",function(){getArchphileSettings();document.getElementById("settingsFrm").classList.remove("was-validated")});document.getElementById("modalNetworkSettings").addEventListener("shown.bs.modal",function(){getNetworkSettings();
document.getElementById("settingsFrm").classList.remove("was-validated")});document.getElementById("addToPlaylistPlaylist").addEventListener("change",function(a){"New Playlist"==this.options[this.selectedIndex].text?(document.getElementById("addToPlaylistNewPlaylistDiv").classList.remove("hide"),document.getElementById("addToPlaylistNewPlaylist").focus()):document.getElementById("addToPlaylistNewPlaylistDiv").classList.add("hide")},!1);addFilterLetter("BrowseFilesystemFilterLetters");addFilterLetter("BrowseDatabaseFilterLetters");
addFilterLetter("BrowsePlaylistsFilterLetters");for(var a=document.querySelectorAll("[data-href]"),b=a.length,c=0;c<b;c++)a[c].classList.add("clickable"),a[c].addEventListener("click",function(a){a.preventDefault();a=JSON.parse(this.getAttribute("data-href").replace(/'/g,'"'));if("function"===typeof window[a.cmd])switch(a.cmd){case "sendAPI":sendAPI.apply(null,$jscomp.arrayFromIterable(a.options));break;default:window[a.cmd].apply(null,$jscomp.arrayFromIterable(a.options))}},!1);a=document.getElementsByClassName("pages");
...
showAddToPlaylist(app.current.search))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&("Add all to queue"==a.target.innerText?addAllFromSearch():"Add all to playlist"==a.target.innerText&&showAddToPlaylist("SEARCH"))},!1);document.getElementById("searchtags").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,"0/"+a.target.getAttribute("data-tag")+"/"+
app.current.search)},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(a){appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+this.value)},!1);document.getElementById("searchqueuetag").addEventListener("click",function(a){"BUTTON"==a.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+a.target.getAttribute("data-tag")+"/"+app.current.search)},!1);document.getElementById("search").addEventListener("submit",
function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);document.getElementById("searchstr").addEventListener("keyup",function(a){appGoto("Search",void 0,void 0,"0/"+app.current.filter+"/"+this.value)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(a){(a=document.getElementsByClassName("popover")[0])&&a.remove()},!1);dragAndDropTable("QueueList");dragAndDropTable("BrowsePlaylistsDetailList");window.addEventListener("hashchange",
appRoute,!1);window.addEventListener("focus",function(){sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState)},!1);document.addEventListener("keydown",function(a){if("INPUT"!=a.target.tagName){switch(a.which){case 37:clickPrev();break;case 39:clickNext();break;case 32:clickPlay();break;default:return}a.preventDefault()}},!1);"serviceWorker"in navigator&&"https"==document.URL.substring(0,5)&&window.addEventListener("load",function(){navigator.serviceWorker.register("/sw.min.js",{scope:"/"}).then(function(a){console.log("ServiceWorker registration successful with scope: ",
...
"","","danger");break;case "update_queue":"Queue"===app.current.app&&getQueue();sendAPI({cmd:"MPD_API_PLAYER_STATE"},parseState);break;case "update_options":getSettings();getArchphileSettings();getNetworkSettings();break;case "update_outputs":sendAPI({cmd:"MPD_API_PLAYER_OUTPUT_LIST"},parseOutputs);break;case "update_started":updateDBstarted(!1);break;case "update_database":case "update_finished":updateDBfinished(b.type);break;case "error":showNotification(b.data,"","","danger")}}},socket.onclose=
function(){console.log("disconnected");modalConnectionError.show();setTimeout(function(){console.log("reconnect");webSocketConnect()},3E3)}}catch(a){alert("Error: "+a)}}function getWsUrl(){var a=document.URL;if("https"==a.substring(0,5)){var b="wss://";a=a.substr(8)}else b="ws://","http"==a.substring(0,4)&&(a=a.substr(7));a=a.split("#");var c=/\/$/.test(a[0])?"":"/";return b+a[0]+c+"ws"}
...
function showPassword(){var a=document.getElementById("inputNasPassword");a.type="password"===a.type?"text":"password"}function showWifiPassword(){var a=document.getElementById("inputWifiPassword");a.type="password"===a.type?"text":"password"}
function parseArchphileSettings(a){toggleBtn("btnAirplay",a.data.airplay);toggleBtn("btnUPNP",a.data.upnp);toggleBtn("btnDOP",a.data.dop);toggleBtn("btnDBupdate",a.data.dbupdate);document.getElementById("selectMixer").value=a.data.mixer;settings=a.data;settings.mpdstream="http://";settings.mpdstream="127.0.0.1"==settings.mpdhost||"localhost"==settings.mpdhost?settings.mpdstream+window.location.hostname:settings.mpdstream+settings.mpdhost;settings.mpdstream+=":"+settings.streamport+"/"}
function parseNetworkSettings(a){toggleBtn("btnEnableWifi",a.data.enablewifi);document.getElementById("inputIPaddress").value=a.data.inputipaddress;document.getElementById("inputMusicFolder").value=a.data.musicfolder;document.getElementById("selectFileSystem").value=a.data.filesystem;document.getElementById("inputWifiNetwork").value=a.data.wifinetwork;document.getElementById("inputNasUsername").value=a.data.nasusername;document.getElementById("inputNasPassword").value=a.data.naspassword;document.getElementById("inputWifiPassword").value=
a.data.wifipassword;settings=a.data;settings.mpdstream="http://";settings.mpdstream="127.0.0.1"==settings.mpdhost||"localhost"==settings.mpdhost?settings.mpdstream+window.location.hostname:settings.mpdstream+settings.mpdhost;settings.mpdstream+=":"+settings.streamport+"/"}function getSettings(){sendAPI({cmd:"MPD_API_SETTINGS_GET"},parseSettings)}function getArchphileSettings(){sendAPI({cmd:"MPD_API_GET_ARCHPHILE_SETTINGS"},parseArchphileSettings)}
function getNetworkSettings(){sendAPI({cmd:"MPD_API_GET_NETWORK_SETTINGS"},parseNetworkSettings)}function parseOutputs(a){for(var b="",c=a.data.outputs.length,e=0;e<c;e++)b+='<button id="btnOutput'+a.data.outputs[e].id+'" data-output-id="'+a.data.outputs[e].id+'" class="btn btn-secondary btn-block',1==a.data.outputs[e].state&&(b+=" active"),b+='"><span class="material-icons float-left">volume_up</span> '+a.data.outputs[e].name+"</button>";domCache.outputs.innerHTML=b}
...
function confirmArchphileSettings(){var a=document.getElementById("selectMixer");sendAPI({cmd:"MPD_API_SET_ARCHPHILE_SETTINGS",data:{mixer:a.options[a.selectedIndex].value,dop:document.getElementById("btnDOP").classList.contains("active")?1:0,airplay:document.getElementById("btnAirplay").classList.contains("active")?1:0,dbupdate:document.getElementById("btnDBupdate").classList.contains("active")?1:0,upnp:document.getElementById("btnUPNP").classList.contains("active")?1:0}},getArchphileSettings);modalArchphileSettings.hide()}
function confirmNetworkSettings(){var a=document.getElementById("selectFileSystem");sendAPI({cmd:"MPD_API_SET_NETWORK_SETTINGS",data:{inputipaddress:document.getElementById("inputIPaddress").value,filesystem:a.options[a.selectedIndex].value,musicfolder:document.getElementById("inputMusicFolder").value,nasusername:document.getElementById("inputNasUsername").value,naspassword:document.getElementById("inputNasPassword").value,wifinetwork:document.getElementById("inputWifiNetwork").value,enablewifi:document.getElementById("btnEnableWifi").classList.contains("active")?
1:0,wifipassword:document.getElementById("inputWifiPassword").value}},getNetworkSettings);modalNetworkSettings.hide()}function systemReboot(){sendAPI({cmd:"SYSTEM_REBOOT"});showNotification("System reboot","","","success")}function systemPowerOff(){sendAPI({cmd:"SYSTEM_POWEROFF"});showNotification("System shutdown. Bye!","","","success")}function addAllFromBrowse(){sendAPI({cmd:"MPD_API_QUEUE_ADD_TRACK",data:{uri:app.current.search}});showNotification("Added all songs","","","success")}
function addAllFromSearch(){2<=app.current.search.length&&(sendAPI({cmd:"MPD_API_DATABASE_SEARCH",data:{plist:"queue",filter:app.current.filter,searchstr:app.current.search,offset:0}}),showNotification("Added "+parseInt(document.getElementById("panel-heading-search").innerText)+" songs from search","","","success"))}
