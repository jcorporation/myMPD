...
function navBrowseHandler(a){"BUTTON"===a.target.nodeName&&(a=a.target.getAttribute("data-tag"),"Playlists"===a||"Filesystem"===a?appGoto("Browse",a,void 0):"Browse"===app.current.app&&"Database"!==app.current.tab?appGoto("Browse","Database",app.apps.Browse.tabs.Database.active):("Album"!==a&&(app.current.filter=a,app.current.sort=a),appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+a+"/"+app.current.search)))}
function gotoBrowse(){var a=event.target,b=a.getAttribute("data-tag"),c=decodeURI(a.getAttribute("data-name"));null===b&&(b=a.parentNode.getAttribute("data-tag"),c=decodeURI(a.parentNode.getAttribute("data-name")));""!==b&&""!==c&&"-"!==c&&settings.browsetags.includes(b)&&appGoto("Browse","Database","List","0/"+b+"/AlbumArtist/Album/"+c)}
...
function parseDatabase(a){var b=a.result.returnedEntities,c=document.getElementById("BrowseDatabaseCards"),d=c.getElementsByClassName("col");0===d.length&&(c.innerHTML="");for(var f=0;f<b;f++){var g=document.createElement("div");g.classList.add("col","px-0","flex-grow-0");""===a.result.data[f].AlbumArtist&&(a.result.data[f].AlbumArtist=t("Unknown artist"));""===a.result.data[f].Album&&(a.result.data[f].Album=t("Unknown album"));var h=genId("database"+a.result.data[f].Album+a.result.data[f].AlbumArtist);
h=void 0;var k="";"Album"===a.result.tag?(h=genId("database"+a.result.data[f].Album+a.result.data[f].AlbumArtist),k=subdir+"/albumart/"+encodeURI(a.result.data[f].FirstSongUri),h='<div class="card card-grid clickable" data-picture="'+k+'" data-uri="'+encodeURI(a.result.data[f].FirstSongUri.replace(/\/[^/]+$/,""))+'" data-album="'+encodeURI(a.result.data[f].Album)+'" data-albumartist="'+encodeURI(a.result.data[f].AlbumArtist)+'" tabindex="0"><div class="card-body album-cover-loading album-cover-grid bg-white" id="'+
h+'"></div><div class="card-footer card-footer-grid p-2" title="'+a.result.data[f].AlbumArtist+": "+a.result.data[f].Album+'">'+a.result.data[f].Album+"<br/><small>"+a.result.data[f].AlbumArtist+"</small></div></div>"):(h=genId("database"+a.result.data[f].value),k=subdir+"/tagpics/"+a.result.tag+"/"+encodeURI(a.result.data[f].value),h='<div class="card card-grid clickable" data-picture="'+k+'" data-tag="'+encodeURI(a.result.data[f].value)+'" tabindex="0">'+(!0===a.result.pics?'<div class="card-body album-cover-loading album-cover-grid bg-white" id="'+
h+'"></div>':"")+'<div class="card-footer card-footer-grid p-2" title="'+a.result.data[f].value+'">'+a.result.data[f].value+"<br/></div></div>");g.innerHTML=h;h=!1;f<d.length?d[f].firstChild.getAttribute("data-picture")!==g.firstChild.getAttribute("data-picture")&&(d[f].replaceWith(g),h=!0):(c.append(g),h=!0);"IntersectionObserver"in window&&!0===h?(new IntersectionObserver(setGridImage,{root:null,rootMargin:"0px"})).observe(g):!0===h&&(g.firstChild.firstChild.style.backgroundImage=k);!0===h&&(g.firstChild.addEventListener("click",
function(a){"Album"===app.current.tag?a.target.classList.contains("card-body")?appGoto("Browse","Database","Detail","0/Album/AlbumArtist/"+decodeURI(a.target.parentNode.getAttribute("data-album"))+"/"+decodeURI(a.target.parentNode.getAttribute("data-albumartist"))):a.target.classList.contains("card-footer")&&showMenu(a.target,a):appGoto(app.current.app,app.current.card,void 0,"0/"+app.current.tag+"/AlbumArtist/Album/"+decodeURI(a.target.parentNode.getAttribute("data-tag")))},!1),g.firstChild.addEventListener("keydown",
function(a){var b=!1;if("Enter"===a.key)"Album"===app.current.tag?appGoto("Browse","Database","Detail","0/Album/AlbumArtist/"+decodeURI(a.target.getAttribute("data-album"))+"/"+decodeURI(a.target.getAttribute("data-albumartist"))):appGoto(app.current.app,app.current.card,void 0,"0/"+app.current.tag+"/AlbumArtist/Album/"+decodeURI(a.target.getAttribute("data-tag"))),b=!0;else if(" "===a.key)"Album"===app.current.tag&&showMenu(a.target.getElementsByClassName("card-footer")[0],a),b=!0;else if("ArrowDown"===
a.key||"ArrowUp"===a.key){var c=a.target,f="ArrowDown"===a.key?null!==a.target.parentNode.nextElementSibling?a.target.parentNode.nextElementSibling.firstChild:null:null!=a.target.parentNode.previousElementSibling?a.target.parentNode.previousElementSibling.firstChild:null;null!=f&&(f.focus(),c.classList.remove("selected"),f.classList.add("selected"),b=!0,c.getBoundingClientRect().top!=f.getBoundingClientRect().top&&scrollFocusIntoView())}else"Escape"===a.key&&(b=a.target,b.blur(),b.classList.remove("selected"),
b=!0);!0===b&&(a.stopPropagation(),a.preventDefault())},!1))}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}
function setGridImage(a,b){a.forEach(function(a){if(0<a.intersectionRatio){b.unobserve(a.target);var c=decodeURI(a.target.firstChild.getAttribute("data-picture"));if(a=a.target.firstChild.getElementsByClassName("card-body")[0])a.style.backgroundImage='url("'+c+'"), url("'+subdir+'/assets/coverimage-loading.svg")'}})}
function parseAlbumDetails(a){document.getElementById("viewDetailDatabaseCover").style.backgroundImage='url("'+subdir+"/albumart/"+a.result.data[0].uri+'"), url("'+subdir+'/assets/coverimage-loading.svg")';document.getElementById("viewDetailDatabaseInfo").innerHTML="<h1>"+e(a.result.Album)+"</h1><p>"+e(a.result.AlbumArtist)+"</p>";document.getElementById("cardFooterBrowse");for(var b=document.getElementById("BrowseDatabaseDetailList").getElementsByTagName("tbody")[0],c="",d=a.result.returnedEntities,
f=0;f<d;f++){a.result.data[f].Duration&&(a.result.data[f].Duration=beautifySongDuration(a.result.data[f].Duration));c+='<tr tabindex="0" data-type="song" data-name="'+a.result.data[f].Title+'" data-uri="'+encodeURI(a.result.data[f].uri)+'">';for(var g=0;g<settings.colsBrowseDatabaseDetail.length;g++)c+='<td data-col="'+settings.colsBrowseDatabaseDetail[g]+'">'+e(a.result.data[f][settings.colsBrowseDatabaseDetail[g]])+"</td>";c+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+
ligatureMore+"</a></td></tr>"}b.innerHTML=c;document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto("Browse","Database","List")}function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);b="((Album == '"+b+"') AND (AlbumArtist == '"+c+"'))";"appendQueue"===a?addAllFromSearchPlist("queue",b,!1):"replaceQueue"===a?addAllFromSearchPlist("queue",b,!0):"addPlaylist"===a&&showAddToPlaylist("ALBUM",b)}
...
var socket=null,websocketConnected=!1,websocketTimer=null,socketRetry=0,lastSong="",lastSongObj={},lastState,currentSong={},playstate="",settingsLock=!1,settingsParsed=!1,settingsNew={},settings={loglevel:2},alertTimeout=null,progressTimer=null,deferredA2HSprompt,dragEl,appInited=!1,subdir="",uiEnabled=!0,locale=navigator.language||navigator.userLanguage,scale="1.0",isMobile=/iPhone|iPad|iPod|Android/i.test(navigator.userAgent),ligatureMore="menu",progressBarTransition="width 1s linear",app={apps:{Playback:{state:"0/-/-/-/",
scrollPos:0},Queue:{active:"Current",tabs:{Current:{state:"0/any/-/-/",scrollPos:0},LastPlayed:{state:"0/any/-/-/",scrollPos:0}}},Browse:{active:"Database",tabs:{Filesystem:{state:"0/-/-/-/",scrollPos:0},Playlists:{active:"All",views:{All:{state:"0/-/-/-/",scrollPos:0},Detail:{state:"0/-/-/-/",scrollPos:0}}},Database:{active:"List",views:{List:{state:"0/AlbumArtist/AlbumArtist/Album/",scrollPos:0},Detail:{state:"0/-/-/-/",scrollPos:0}}}}},Search:{state:"0/any/-/-/",scrollPos:0}},current:{app:"Playback",
tab:void 0,view:void 0,page:0,filter:"",search:"",sort:"",tag:"",scrollPos:0},last:{app:void 0,tab:void 0,view:void 0,filter:"",search:"",sort:"",tag:"",scrollPos:0}},domCache={};domCache.navbarBottomBtns=document.getElementById("navbar-main").getElementsByTagName("div");domCache.navbarBottomBtnsLen=domCache.navbarBottomBtns.length;domCache.counter=document.getElementById("counter");domCache.volumePrct=document.getElementById("volumePrct");domCache.volumeControl=document.getElementById("volumeControl");
...
modalExecScript=new BSN.Modal(document.getElementById("modalExecScript")),modalScripts=new BSN.Modal(document.getElementById("modalScripts")),modalPartitions=new BSN.Modal(document.getElementById("modalPartitions")),modalPartitionOutputs=new BSN.Modal(document.getElementById("modalPartitionOutputs")),modalTrigger=new BSN.Modal(document.getElementById("modalTrigger")),modalOutputAttributes=new BSN.Modal(document.getElementById("modalOutputAttributes")),dropdownMainMenu=new BSN.Dropdown(document.getElementById("mainMenu")),
dropdownVolumeMenu=new BSN.Dropdown(document.getElementById("volumeMenu")),dropdownBookmarks=new BSN.Dropdown(document.getElementById("BrowseFilesystemBookmark")),dropdownLocalPlayer=new BSN.Dropdown(document.getElementById("localPlaybackMenu")),dropdownPlay=new BSN.Dropdown(document.getElementById("btnPlayDropdown")),dropdownDatabaseSort=new BSN.Dropdown(document.getElementById("btnDatabaseSortDropdown")),dropdownNeighbors=new BSN.Dropdown(document.getElementById("btnDropdownNeighbors")),collapseDBupdate=
new BSN.Collapse(document.getElementById("navDBupdate")),collapseSettings=new BSN.Collapse(document.getElementById("navSettings")),collapseSyscmds=new BSN.Collapse(document.getElementById("navSyscmds")),collapseScripting=new BSN.Collapse(document.getElementById("navScripting")),collapseJukeboxMode=new BSN.Collapse(document.getElementById("labelJukeboxMode"));
...
app.apps[a].tabs?(void 0===b&&(b=app.apps[a].active),app.apps[a].tabs[b].views?(void 0===c&&(c=app.apps[a].tabs[b].active),a="/"+a+"/"+b+"/"+c+"!"+(void 0===d?app.apps[a].tabs[b].views[c].state:d)):a="/"+a+"/"+b+"!"+(void 0===d?app.apps[a].tabs[b].state:d)):a="/"+a+"!"+(void 0===d?app.apps[a].state:d);location.hash=encodeURI(a)}
function appRoute(){if(!1===settingsParsed)appInitStart();else{var a=decodeURI(location.hash),b=a.match(/^#\/(\w+)\/?(\w+)?\/?(\w+)?!((\d+)\/([^/]+)\/([^/]+)\/([^/]+)\/(.*))$/);if(b){app.current.app=b[1];app.current.tab=b[2];app.current.view=b[3];app.apps[app.current.app].state?(app.apps[app.current.app].state=b[4],app.current.scrollPos=app.apps[app.current.app].scrollPos):app.apps[app.current.app].tabs[app.current.tab].state?(app.apps[app.current.app].tabs[app.current.tab].state=b[4],app.apps[app.current.app].active=
app.current.tab,app.current.scrollPos=app.apps[app.current.app].tabs[app.current.tab].scrollPos):app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].state&&(app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].state=b[4],app.apps[app.current.app].active=app.current.tab,app.apps[app.current.app].tabs[app.current.tab].active=app.current.view,app.current.scrollPos=app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos);app.current.page=
parseInt(b[5]);app.current.filter=b[6];app.current.sort=b[7];app.current.tag=b[8];app.current.search=b[9];appPrepare(app.current.scrollPos);if("Playback"===app.current.app)sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange);else if("Queue"===app.current.app&&"Current"===app.current.tab)selectTag("searchqueuetags","searchqueuetagsdesc",app.current.filter),getQueue();else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)sendAPI("MPD_API_QUEUE_LAST_PLAYED",{offset:app.current.page,cols:settings.colsQueueLastPlayed},
...
parsePlaylists),doSetFilterLetter("BrowsePlaylistsFilter");else if("Browse"===app.current.app&&"Filesystem"===app.current.tab){sendAPI("MPD_API_DATABASE_FILESYSTEM_LIST",{offset:app.current.page,path:app.current.search?app.current.search:"/",filter:app.current.filter,cols:settings.colsBrowseFilesystem},parseFilesystem,!0);app.current.search?(document.getElementById("BrowseFilesystemAddAllSongs").removeAttribute("disabled"),document.getElementById("BrowseFilesystemAddAllSongsBtn").removeAttribute("disabled")):
(document.getElementById("BrowseFilesystemAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("BrowseFilesystemAddAllSongsBtn").setAttribute("disabled","disabled"));a='<li class="breadcrumb-item"><a data-uri="" class="text-body material-icons">home</a></li>';b=app.current.search.split("/");for(var c=b.length,d="",f=0;f<c;f++){if(c-1===f){a+='<li class="breadcrumb-item active">'+e(b[f])+"</li>";break}d+=b[f];a+='<li class="breadcrumb-item"><a class="text-body" href="#" data-uri="'+
encodeURI(d)+'">'+e(b[f])+"</a></li>";d+="/"}document.getElementById("BrowseBreadcrumb").innerHTML=a;doSetFilterLetter("BrowseFilesystemFilter")}else if("Browse"===app.current.app&&"Database"===app.current.tab&&"List"===app.current.view)document.getElementById("viewListDatabase").classList.remove("hide"),document.getElementById("viewDetailDatabase").classList.add("hide"),document.getElementById("searchDatabaseStr").value=app.current.search,selectTag("searchDatabaseTags","searchDatabaseTagsDesc",app.current.filter),
selectTag("BrowseDatabaseByTagDropdown","btnBrowseDatabaseByTagDesc",app.current.tag),a=app.current.sort,b=!1,"-"===app.current.sort.charAt(0)?(b=!0,a=app.current.sort.substr(1),toggleBtnChk("databaseSortDesc",!0)):toggleBtnChk("databaseSortDesc",!1),selectTag("databaseSortTags",void 0,a),"Album"===app.current.tag?sendAPI("MPD_API_DATABASE_GET_ALBUMS",{offset:app.current.page,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b},parseDatabase):sendAPI("MPD_API_DATABASE_TAG_LIST",
{offset:app.current.page,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b,tag:app.current.tag},parseDatabase),"Album"!==app.current.tag?(document.getElementById("btnDatabaseSortDropdown").setAttribute("disabled","disabled"),document.getElementById("btnDatabaseSearchDropdown").setAttribute("disabled","disabled")):(document.getElementById("btnDatabaseSortDropdown").removeAttribute("disabled"),document.getElementById("btnDatabaseSearchDropdown").removeAttribute("disabled"));else if("Browse"===
app.current.app&&"Database"===app.current.tab&&"Detail"===app.current.view)document.getElementById("viewListDatabase").classList.add("hide"),document.getElementById("viewDetailDatabase").classList.remove("hide"),"Album"===app.current.filter&&sendAPI("MPD_API_DATABASE_TAG_ALBUM_TITLE_LIST",{album:app.current.tag,search:app.current.search,tag:app.current.sort,cols:settings.colsBrowseDatabaseDetail},parseAlbumDetails);else if("Search"===app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a=
"";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)d=b[c].substring(1,b[c].length-1),a+='<button data-filter="'+encodeURI(d)+'" class="btn btn-light mr-2">'+e(d)+'<span class="ml-2 badge badge-secondary">&times</span></button>';domCache.searchCrumb.innerHTML=a;""===domCache.searchstr.value&&1<=b.length&&(a=b[b.length-1].substring(1,b[b.length-1].length-1),b=a.substring(a.indexOf("'")+1,a.length-1),domCache.searchstr.value!==b&&(domCache.searchCrumb.innerHTML+=
'<button data-filter="'+encodeURI(a)+'" class="btn btn-light mr-2">'+e(a)+'<span href="#" class="ml-2 badge badge-secondary">&times;</span></button>'),a=a.substring(a.indexOf(" ")+1),a=a.substring(0,a.indexOf(" ")),""===a&&(a="contains"),document.getElementById("searchMatch").value=a)}else""===domCache.searchstr.value&&""!==app.current.search&&(domCache.searchstr.value=app.current.search);app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app].length,a--,document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML=
'<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+t("Searching...")+"</td></tr>");2<=domCache.searchstr.value.length||0<domCache.searchCrumb.children.length?settings.featAdvsearch?(a=app.current.sort,b=!1,"-"===a?(a=settings.tags.includes("Title")?"Title":"-",document.getElementById("SearchList").setAttribute("data-sort",a)):0===a.indexOf("-")&&(b=!0,a=a.substring(1)),sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:"",offset:app.current.page,sort:a,sortdesc:b,expression:app.current.search,
...
setPagination(0,0));selectTag("searchtags","searchtagsdesc",app.current.filter)}else appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else logDebug("Invalid params: "+a),appGoto("Playback")}}
...
function appInitStart(){if(!0===isMobile)scale=localStorage.getItem("scale-ratio"),null===scale&&(scale="1.0"),setViewport(!1);else for(var a=document.getElementsByClassName("featMobile"),b=0;b<a.length;b++)a[b].classList.add("hide");subdir=window.location.pathname.replace("/index.html","").replace(/\/$/,"");a='<option value="default" data-phrase="Browser default"></option>';for(b=0;b<locales.length;b++)a+='<option value="'+e(locales[b].code)+'">'+e(locales[b].desc)+" ("+e(locales[b].code)+")</option>";
document.getElementById("selectLocale").innerHTML=a;i18nHtml(document.getElementById("splashScreenAlert"));a=document.getElementsByTagName("script")[0].src.replace(/^.*[/]/,"");"combined.js"!==a&&(settings.loglevel=4);"serviceWorker"in navigator&&"https:"===window.location.protocol&&"localhost"!==window.location.hostname&&"combined.js"===a&&window.addEventListener("load",function(){navigator.serviceWorker.register("/sw.js",{scope:"/"}).then(function(a){logInfo("ServiceWorker registration successful.");
...
0<=currentSong.currentSongId&&(domCache.progressBar.style.transition="none",domCache.progressBar.style.width=a.clientX+"px",setTimeout(function(){domCache.progressBar.style.transition=progressBarTransition},10),sendAPI("MPD_API_PLAYER_SEEK",{songid:currentSong.currentSongId,seek:Math.ceil(currentSong.totalTime*a.clientX/a.target.offsetWidth)}))},!1);domCache.progress.addEventListener("mousemove",function(a){if("pause"===playstate||"play"===playstate)domCache.progressPos.innerText=beautifySongDuration(Math.ceil(currentSong.totalTime/
a.target.offsetWidth*a.clientX)),domCache.progressPos.style.display="block",domCache.progressPos.style.left=a.clientX+"px"},!1);domCache.progress.addEventListener("mouseout",function(a){domCache.progressPos.style.display="none"},!1);for(var a=document.querySelectorAll(".subMenu"),b=a.length,c=0;c<b;c++)a[c].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();a=this.getElementsByTagName("span")[0];a.innerText="keyboard_arrow_right"===a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},
!1);document.getElementById("volumeMenu").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{},parseOutputs)});document.getElementById("btnDropdownNeighbors").parentNode.addEventListener("show.bs.dropdown",function(){!0===settings.featNeighbors?sendAPI("MPD_API_MOUNT_NEIGHBOR_LIST",{},parseNeighbors,!0):document.getElementById("dropdownNeighbors").children[0].innerHTML='<div class="list-group-item"><span class="material-icons">warning</span> '+t("Neighbors are disabled")+
"</div>"});document.getElementById("dropdownNeighbors").children[0].addEventListener("click",function(a){a.preventDefault();"A"===a.target.nodeName&&(a=a.target.getAttribute("data-value").match(/^(\w+:\/\/)(.+)$/),document.getElementById("selectMountUrlhandler").value=a[1],document.getElementById("inputMountUrl").value=a[2])});document.getElementById("BrowseFilesystemBookmark").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)});
document.getElementById("playDropdown").parentNode.addEventListener("show.bs.dropdown",function(){showPlayDropdown()});document.getElementById("playDropdown").addEventListener("click",function(a){a.preventDefault();a.stopPropagation()});a=document.querySelectorAll(".dropdown-toggle");for(b=0;b<a.length;b++)a[b].parentNode.addEventListener("show.bs.dropdown",function(){alignDropdown(this)});document.getElementById("modalTimer").addEventListener("shown.bs.modal",function(){showListTimer()});document.getElementById("modalMounts").addEventListener("shown.bs.modal",
function(){showListMounts()});document.getElementById("modalScripts").addEventListener("shown.bs.modal",function(){showListScripts()});document.getElementById("modalTrigger").addEventListener("shown.bs.modal",function(){showListTrigger()});document.getElementById("modalPartitions").addEventListener("shown.bs.modal",function(){showListPartitions()});document.getElementById("modalPartitionOutputs").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{partition:"default"},
parsePartitionOutputsList,!1)});document.getElementById("modalAbout").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_DATABASE_STATS",{},parseStats);getServerinfo();var a="",b;for(b in keymap)if(void 0===keymap[b].req||!0===settings[keymap[b].req])a+='<tr><td><div class="key'+(keymap[b].key&&1<keymap[b].key.length?" material-icons material-icons-small":"")+'">'+(void 0!==keymap[b].key?keymap[b].key:b)+"</div></td><td>"+t(keymap[b].desc)+"</td></tr>";document.getElementById("tbodyShortcuts").innerHTML=
a});document.getElementById("modalAddToPlaylist").addEventListener("shown.bs.modal",function(){document.getElementById("addStreamFrm").classList.contains("hide")?document.getElementById("addToPlaylistPlaylist").focus():(document.getElementById("streamUrl").focus(),document.getElementById("streamUrl").value="")});document.getElementById("inputTimerVolume").addEventListener("change",function(){document.getElementById("textTimerVolume").innerHTML=this.value+"&nbsp;%"},!1);document.getElementById("selectTimerAction").addEventListener("change",
function(){selectTimerActionChange()},!1);document.getElementById("selectTriggerScript").addEventListener("change",function(){selectTriggerActionChange()},!1);a="";for(b=0;24>b;b++)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerHour").innerHTML=a;a="";for(b=0;60>b;b+=5)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerMinute").innerHTML=a;document.getElementById("inputHighlightColor").addEventListener("change",function(){document.getElementById("highlightColorPreview").style.backgroundColor=
...
document.getElementById("btnJukeboxModeGroup").addEventListener("mouseup",function(){setTimeout(function(){var a=document.getElementById("btnJukeboxModeGroup").getElementsByClassName("active")[0].getAttribute("data-value");"0"===a?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled")):"2"===a?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),
document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):"1"===a&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));"0"!==a&&toggleBtnChk("btnConsume",!0);checkConsume()},100)});document.getElementById("btnConsume").addEventListener("mouseup",function(){setTimeout(function(){checkConsume()},100)});
...
function(){var a=this.options[this.selectedIndex].value;"2"===a?(document.getElementById("inputAddToQueueQuantity").setAttribute("disabled","disabled"),document.getElementById("inputAddToQueueQuantity").value="1",document.getElementById("selectAddToQueuePlaylist").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").value="Database"):"1"===a&&(document.getElementById("inputAddToQueueQuantity").removeAttribute("disabled"),document.getElementById("selectAddToQueuePlaylist").removeAttribute("disabled"))});
...
settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):"none"===this.options[this.selectedIndex].value?(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").removeAttribute("readonly"))},!1);addFilterLetter("BrowseFilesystemFilterLetters");addFilterLetter("BrowsePlaylistsFilterLetters");
document.getElementById("syscmds").addEventListener("click",function(a){"A"===a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",function(a){"A"===a.target.nodeName&&execScript(a.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++)!1===a[c].classList.contains("notclickable")&&a[c].classList.add("clickable"),!0!==(a[c].parentNode.classList.contains("noInitChilds")?
!0:!1)&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"P"===a.target.nodeName&&gotoBrowse(a.target)},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",function(a){"A"===
a.target.nodeName&&(a.preventDefault(),appGoto("Browse","Filesystem",void 0,"0/"+app.current.filter+"/"+app.current.sort+"/-/"+decodeURI(a.target.getAttribute("data-uri"))))},!1);document.getElementById("tbodySongDetails").addEventListener("click",function(a){if("A"===a.target.nodeName)if("calcFingerprint"===a.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(a.target.getAttribute("data-uri"))},parseFingerprint);a.preventDefault();var b=a.target.parentNode,c=document.createElement("div");
c.classList.add("spinner-border","spinner-border-sm");a.target.classList.add("hide");b.appendChild(c)}else null!==a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),gotoBrowse(a.target));else"BUTTON"===a.target.nodeName&&a.target.getAttribute("data-href")&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(a){"BUTTON"===a.target.nodeName?(a.stopPropagation(),a.preventDefault(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",
{output:a.target.getAttribute("data-output-id"),state:a.target.classList.contains("active")?0:1}),toggleBtn(a.target.id)):"A"===a.target.nodeName&&(a.preventDefault(),showListOutputAttributes(decodeURI(a.target.parentNode.getAttribute("data-output-name"))))},!1);document.getElementById("listTimerList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();"TD"===a.target.nodeName?a.target.parentNode.classList.contains("not-clickable")||showEditTimer(a.target.parentNode.getAttribute("data-id")):
"A"===a.target.nodeName?deleteTimer(a.target.parentNode.parentNode.getAttribute("data-id")):"BUTTON"===a.target.nodeName&&toggleTimer(a.target,a.target.parentNode.parentNode.getAttribute("data-id"))},!1);document.getElementById("listMountsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){if(""===a.target.parentNode.getAttribute("data-point"))return!1;showEditMount(decodeURI(a.target.parentNode.getAttribute("data-url")),decodeURI(a.target.parentNode.getAttribute("data-point")))}else if("A"===
a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===b?unmountMount(c):"update"===b&&updateMount(a.target,c)}},!1);document.getElementById("listScriptsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){if(!1===settings.featScripteditor||""===a.target.parentNode.getAttribute("data-script"))return!1;showEditScript(decodeURI(a.target.parentNode.getAttribute("data-script")))}else if("A"===
a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-script"));"delete"===b?deleteScript(c):"execute"===b&&execScript(a.target.getAttribute("data-href"))}},!1);document.getElementById("listTriggerList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){var b=decodeURI(a.target.parentNode.getAttribute("data-trigger-id"));showEditTrigger(b)}else"A"===a.target.nodeName&&(b=a.target.getAttribute("data-action"),
a=decodeURI(a.target.parentNode.parentNode.getAttribute("data-trigger-id")),"delete"===b&&deleteTrigger(a))},!1);document.getElementById("listPartitionsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action");a=decodeURI(a.target.parentNode.parentNode.getAttribute("data-partition"));"delete"===b?deletePartition(a):"switch"===b&&switchPartition(a)}},!1);document.getElementById("partitionOutputsList").addEventListener("click",
function(a){a.stopPropagation();a.preventDefault();"TD"===a.target.nodeName&&(a=decodeURI(a.target.parentNode.getAttribute("data-output")),moveOutput(a),modalPartitionOutputs.hide())},!1);document.getElementById("QueueCurrentList").addEventListener("click",function(a){"TD"===a.target.nodeName?sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:a.target.parentNode.getAttribute("data-trackid")}):"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",
...
app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.tag+"/"+app.current.search))},!1);document.getElementById("databaseSortDesc").addEventListener("click",function(a){toggleBtnChk(this);a.stopPropagation();a.preventDefault();"-"===app.current.sort.charAt(0)?app.current.sort=app.current.sort.substr(1):app.current.sort="-"+app.current.sort;appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.tag+
"/"+app.current.search)},!1);document.getElementById("databaseSortTags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(a.preventDefault(),a.stopPropagation(),app.current.sort=a.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.tag+"/"+app.current.search))},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",function(a){"Escape"===a.key?this.blur():appGoto(app.current.app,
app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.tag+"/"+this.value)},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(a){navBrowseHandler(a)},!1);document.getElementById("BrowseNavPlaylistsDropdown").addEventListener("click",function(a){navBrowseHandler(a)},!1);document.getElementById("BrowseNavFilesystemDropdown").addEventListener("click",function(a){navBrowseHandler(a)},!1);document.getElementById("dropdownSortPlaylistTags").addEventListener("click",
...
"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+app.current.filter+"/"+app.current.sort+"/-/"+app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(a){"Enter"===a.key&&(a.preventDefault(),a.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(a){"OPTION"===a.target.nodeName&&removeScriptArgument(a)},!1);document.getElementsByTagName("body")[0].addEventListener("click",
function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("hashchange",appRoute,!1);window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},
!1);document.addEventListener("keydown",function(a){if("INPUT"!==a.target.tagName&&"SELECT"!==a.target.tagName&&"TEXTAREA"!==a.target.tagName&&!a.ctrlKey&&!a.altKey){var b=keymap[a.key];b&&"function"===typeof window[b.cmd]&&(void 0===keymap[a.key].req||!0===settings[keymap[a.key].req])&&parseCmd(a,b)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);var d="";Object.keys(themes).forEach(function(a){d+=
'<option value="'+e(a)+'">'+t(themes[a])+"</option>"});document.getElementById("selectTheme").innerHTML=d;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();clickCheckLocalPlayerState(a)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();
clickCheckLocalPlayerState(a)},!1);document.getElementById("localPlayer").addEventListener("click",function(a){a.stopPropagation()});document.getElementById("localPlayer").addEventListener("canplay",function(){logDebug("localPlayer event: canplay");document.getElementById("alertLocalPlayback").classList.add("hide");document.getElementById("errorLocalPlayback").classList.add("hide")});document.getElementById("localPlayer").addEventListener("error",function(){logError("localPlayer event: error");document.getElementById("errorLocalPlayback").classList.remove("hide")})}
...
a&&a.classList.add("alertBoxActive")}));f.getElementsByTagName("button")[0].addEventListener("click",function(){hideNotification()},!1);alertTimeout=setTimeout(function(){hideNotification()},3E3)}setStateIcon("newMessage");logMessage(a,b,c,d)}
...
"</small><p>"+e(a)+(""===c&&""===b?"":"<br/>"+(""===c?e(b):c))+"</p>";!0===g?f.insertBefore(k,f.firstElementChild):f.replaceChild(k,h);a=f.getElementsByTagName("div");10<a.length&&a[10].remove();document.getElementById("navState").children[0].classList.add("text-success");setTimeout(function(){document.getElementById("navState").children[0].classList.remove("text-success")},250)}
...
function toggleUI(){var a="disabled";!0===websocketConnected&&!0===settings.mpdConnected&&(a="enabled");var b="disabled"===a?!1:!0;b!==uiEnabled&&(logDebug("Setting ui state to "+a),setElsState("a",a,"tag"),setElsState("input",a,"tag"),setElsState("button",a,"tag"),setElsState("clickable",a,"class"),uiEnabled=b);!0===settings.mpdConnected?toggleAlert("alertMpdState",!1,""):(toggleAlert("alertMpdState",!0,t("MPD disconnected")),logMessage(t("MPD disconnected"),"","","danger"));!0===websocketConnected?
toggleAlert("alertMympdState",!1,""):(toggleAlert("alertMympdState",!0,t("Websocket is disconnected")),logMessage(t("Websocket is disconnected"),"","","danger"));setStateIcon()}function moveOutput(a){sendAPI("MPD_API_PARTITION_OUTPUT_MOVE",{name:a})}
...
l.setAttribute("data-name",a.result.data[h].name);l.setAttribute("tabindex",0);l.innerHTML='<td data-col="Type"><span class="material-icons">'+("smartpls"===a.result.data[h].Type?"queue_music":"list")+"</span></td><td>"+e(a.result.data[h].name)+"</td><td>"+localeDate(a.result.data[h].last_modified)+'</td><td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";h<f.length?g=!0===replaceTblRow(f[h],l)?h:g:d.append(l)}else if("Detail"===app.current.view)for(h=
0;h<b;h++){l=encodeURI(a.result.data[h].uri);k=document.createElement("tr");!1===a.result.smartpls&&k.setAttribute("draggable","true");k.setAttribute("id","playlistTrackId"+a.result.data[h].Pos);k.setAttribute("data-type",a.result.data[h].Type);k.setAttribute("data-uri",l);k.setAttribute("data-name",a.result.data[h].Title);k.setAttribute("data-songpos",a.result.data[h].Pos);k.setAttribute("tabindex",0);a.result.data[h].Duration=beautifySongDuration(a.result.data[h].Duration);l="";for(var m=0;m<settings.colsBrowsePlaylistsDetail.length;m++)l+=
'<td data-col="'+settings.colsBrowsePlaylistsDetail[m]+'">'+e(a.result.data[h][settings.colsBrowsePlaylistsDetail[m]])+"</td>";l+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";k.innerHTML=l;h<f.length?g=!0===replaceTblRow(f[h],k)?h:g:d.append(k)}for(g=f.length-1;g>=b;g--)f[g].remove();!0===c&&focusTable(0);setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(d.innerHTML="All"===app.current.view?'<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="3">'+
...
function hideMenu(){var a=document.querySelector("[data-popover]");a&&((new BSN.Popover(a,{})).hide(),a.removeAttribute("data-popover"),a.parentNode.parentNode.classList.contains("selected")?focusTable(void 0,a.parentNode.parentNode.parentNode.parentNode):"Browse"===app.current.app&&"Database"===app.current.tab&&focusTable(void 0,a.parentNode.parentNode.parentNode.parentNode))}
...
"Detail"===app.current.view?(f=document.getElementById("BrowsePlaylistsDetailList"),g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("false"===f.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[f.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):
"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")):"Queue"===app.current.app&&"Current"===app.current.tab?g+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-
1,-1]},t("Remove all downwards"))+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):""):"Queue"===app.current.app&&"LastPlayed"===app.current.tab&&(g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):
""));new BSN.Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+g+"</div></div>",content:" "});b=a.Popover;null===a.getAttribute("data-init")&&(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",function(a){a.target.setAttribute("data-popover","true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(b){b.preventDefault();b.stopPropagation();
"A"===b.target.nodeName&&(b=b.target.getAttribute("data-href"))&&(b=JSON.parse(b64DecodeUnicode(b)),parseCmd(a,b),hideMenu())},!1);document.getElementsByClassName("popover-content")[0].addEventListener("keydown",function(a){a.preventDefault();a.stopPropagation();if("ArrowDown"===a.key||"ArrowUp"===a.key){var b=this.getElementsByTagName("a");b=Array.prototype.slice.call(b);var c=b.indexOf(document.activeElement);do if(c="ArrowUp"===a.key?1<c?c-1:0:"ArrowDown"===a.key?c<b.length-1?c+1:c:c,0===c||c===
b.length-1)break;while(!b[c].offsetHeight);b[c]&&b[c].focus()}else"Enter"===a.key?a.target.click():"Escape"===a.key&&hideMenu()},!1);var b=document.getElementById("advancedMenuLink");b&&(b.addEventListener("click",function(){var a=this.getElementsByTagName("span")[0];a.innerText="keyboard_arrow_right"===a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new BSN.Collapse(b));document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));b.show()}
...
"")+'<a href="#" title="'+t("Execute")+'" data-action="execute" class="material-icons color-darkgrey"  data-href=\'{"script": "'+e(a.result.data[h].name)+'", "arguments": ['+k+"]}'>play_arrow</a></td></tr>";b.innerHTML+='<option data-arguments=\'{"arguments":['+k+"]}' value=\""+e(a.result.data[h].name)+'">'+e(a.result.data[h].name)+"</option>"}document.getElementById("listScriptsList").innerHTML=d}else document.getElementById("listScriptsList").innerHTML='<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="2">'+
t("Empty list")+"</td></tr>";document.getElementById("scripts").innerHTML=c;4<f?(document.getElementById("navScripting").classList.remove("hide"),document.getElementById("scripts").classList.add("collapse","menu-indent")):(document.getElementById("navScripting").classList.add("hide"),document.getElementById("scripts").classList.remove("collapse","menu-indent"));document.getElementById("selectTriggerScript").innerHTML=b.innerHTML;(a=document.getElementById("selectTimerAction").querySelector('optgroup[data-value="script"]'))?
a.replaceWith(b):document.getElementById("selectTimerAction").appendChild(b)}
...
function search(a){if(settings.featAdvsearch){for(var b="(",c=domCache.searchCrumb.children,d=0;d<c.length;d++)b+="("+decodeURI(c[d].getAttribute("data-filter"))+")",""!==a&&(b+=" AND ");""!==a?(c=document.getElementById("searchMatch"),b+="("+app.current.filter+" "+c.options[c.selectedIndex].value+" '"+a+"'))"):b+=")";2>=b.length&&(b="");appGoto("Search",void 0,void 0,"0/"+app.current.filter+"/"+app.current.sort+"/-/"+b)}else appGoto("Search",void 0,void 0,"0/"+app.current.filter+"/"+app.current.sort+
"/-/"+a)}function parseSearch(a){0<a.result.returnedEntities?(document.getElementById("searchAddAllSongs").removeAttribute("disabled"),document.getElementById("searchAddAllSongsBtn").removeAttribute("disabled")):(document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"));parseFilesystem(a)}
...
for(c=0;c<a.length;c++)a[c].children[0].style.backgroundImage='url("'+encodeURI(b[c])+'")';a=document.getElementById("songPicsCarousel");new BSN.Carousel(a,{interval:!1,pause:!1})}else document.getElementById("tabSongPics").innerText=""}function isCoverfile(a){a=basename(a).toLowerCase();for(var b=a.split("."),c="png jpg jpeg svg webp tiff bmp".split(" "),d=settings.coverimageName.split(","),f=0;f<d.length;f++){var g=d[f].trim();if(a===g||b[1]&&g===b[0]&&c.includes(b[1]))return!0}return!1}
...
function setCounter(a,b,c){currentSong.totalTime=b;currentSong.elapsedTime=c;currentSong.currentSongId=a;var d=Math.ceil(domCache.progress.offsetWidth*c/b);0===d&&(domCache.progressBar.style.transition="none");domCache.progressBar.style.width=d+"px";0===d&&setTimeout(function(){domCache.progressBar.style.transition=progressBarTransition},10);b=beautifySongDuration(c)+"&nbsp;/&nbsp;"+beautifySongDuration(b);domCache.counter.innerHTML=b;if(lastState&&lastState.currentSongId!==a&&(c=document.getElementById("queueTrackId"+
lastState.currentSongId))){if(d=c.querySelector("[data-col=Duration]"))d.innerText=c.getAttribute("data-duration");if(d=c.querySelector("[data-col=Pos]"))d.classList.remove("material-icons"),d.innerText=c.getAttribute("data-songpos");c.classList.remove("font-weight-bold")}if(a=document.getElementById("queueTrackId"+a)){if(c=a.querySelector("[data-col=Duration]"))c.innerHTML=b;(b=a.querySelector("[data-col=Pos]"))&&!b.classList.contains("material-icons")&&(b.classList.add("material-icons"),b.innerText=
"play_arrow");a.classList.add("font-weight-bold")}progressTimer&&clearTimeout(progressTimer);"play"===playstate&&(progressTimer=setTimeout(function(){currentSong.elapsedTime++;requestAnimationFrame(function(){setCounter(currentSong.currentSongId,currentSong.totalTime,currentSong.elapsedTime)})},1E3))}
function parseState(a){if(JSON.stringify(a.result)===JSON.stringify(lastState))toggleUI();else{parseUpdateQueue(a);parseVolume(a);setCounter(a.result.currentSongId,a.result.totalTime,a.result.elapsedTime);lastState&&lastState.currentSongId===a.result.currentSongId&&lastState.queueVersion===a.result.queueVersion||sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange);if("-1"===a.result.songPos){domCache.currentTitle.innerText="Not playing";document.title="myMPD";var b=document.getElementById("footerTitle");
...
function _setCurrentCover(a,b){if(void 0===a)clearCurrentCover();else{for(var c=b.querySelectorAll(".coverbg"),d=0;d<c.length;d++)"2"===c[d].style.zIndex?c[d].remove():c[d].style.zIndex="2";c=document.createElement("div");c.classList.add("coverbg");c.style.backgroundImage='url("'+subdir+"/albumart/"+a+'")';c.style.opacity=0;b.insertBefore(c,b.firstChild);c=new Image;c.onload=function(){b.querySelector(".coverbg").style.opacity=1};c.src=subdir+"/albumart/"+a}}
function clearCurrentCover(a){_clearCurrentCover(domCache.currentCover);_clearCurrentCover(domCache.footerCover)}function _clearCurrentCover(a){a=a.querySelectorAll(".coverbg");for(var b=0;b<a.length;b++)"2"===a[b].style.zIndex?a[b].remove():(a[b].style.zIndex="2",a[b].style.opacity="0")}
...
"",domCache.currentTitle.setAttribute("data-uri",""),domCache.footerTitle.innerText="",domCache.footerTitle.setAttribute("data-name",""),domCache.footerTitle.classList.remove("clickable"));document.title="myMPD: "+f;domCache.footerCover.title=f;void 0!==a.result.uri&&""!==a.result.uri&&-1===a.result.uri.indexOf("://")?footerTitle.classList.add("clickable"):footerTitle.classList.remove("clickable");void 0!==a.result.uri?(!0===settings.featStickers&&setVoteSongBtns(a.result.like,a.result.uri),a.result.Filetype=
filetype(a.result.uri)):a.result.Filetype="";a.result.Fileformat=lastState?fileformat(lastState.audioFormat):"";for(f=0;f<settings.colsPlayback.length;f++){var g=document.getElementById("current"+settings.colsPlayback[f]);if(g&&"Lyrics"===settings.colsPlayback[f])getLyrics(a.result.uri,g.getElementsByTagName("p")[0]);else if(g){var h=a.result[settings.colsPlayback[f]];void 0===h&&(h="");"Duration"===settings.colsPlayback[f]?h=beautifySongDuration(h):"LastModified"===settings.colsPlayback[f]&&(h=localeDate(h));
g.getElementsByTagName("p")[0].innerText=h;g.setAttribute("data-name",encodeURI(h))}}if(f=document.getElementById("queueTrackId"+a.result.currentSongId))f.getElementsByTagName("td")[1].innerText=a.result.Title;"play"===playstate&&showNotification(a.result.Title,c,d,"success");lastSong=b;lastSongObj=a.result}}function gotoTagList(){appGoto(app.current.app,app.current.tab,app.current.view,"0/-/-/-/")}function volumeStep(a){chVolume("up"===a?settings.volumeStep:0-settings.volumeStep)}
...
{},function(a){getAllPlaylists(a,"selectTimerPlaylist","Database")});document.getElementById("inputTimerId").value="0";document.getElementById("inputTimerName").value="";toggleBtnChk("btnTimerEnabled",!0);document.getElementById("selectTimerHour").value="12";document.getElementById("selectTimerMinute").value="0";document.getElementById("selectTimerAction").value="startplay";document.getElementById("inputTimerVolume").value="50";document.getElementById("selectTimerPlaylist").value="Database";toggleBtnGroupValue(document.getElementById("btnTimerJukeboxModeGroup"),
...
function parseEditTimer(a){var b=a.result.playlist;sendAPI("MPD_API_PLAYLIST_LIST_ALL",{},function(a){getAllPlaylists(a,"selectTimerPlaylist",b)});document.getElementById("inputTimerId").value=a.result.timerid;document.getElementById("inputTimerName").value=a.result.name;toggleBtnChk("btnTimerEnabled",a.result.enabled);document.getElementById("selectTimerHour").value=a.result.startHour;document.getElementById("selectTimerMinute").value=a.result.startMinute;document.getElementById("selectTimerAction").value=
...
app.current.tab,app.current.view,"0/"+b+"/"+app.current.sort+"/"+app.current.tag+"/"+app.current.search)},!1)}function selectTag(a,b,c){a=document.getElementById(a);var d=a.querySelector(".active");d&&d.classList.remove("active");if(d=a.querySelector("[data-tag="+c+"]"))d.classList.add("active"),void 0!==b&&(document.getElementById(b).innerText=d.innerText,document.getElementById(b).setAttribute("data-phrase",d.innerText))}
