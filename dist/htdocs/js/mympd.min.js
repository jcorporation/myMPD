...
function sendAPI(a,b,c,d){var f={jsonrpc:"2.0",id:0,method:a,params:b},g=new XMLHttpRequest;g.open("POST",subdir+"/api",!0);g.setRequestHeader("Content-type","application/json");g.onreadystatechange=function(){if(4===g.readyState)if(""!==g.responseText){var a=JSON.parse(g.responseText);a.error?(showNotification(t(a.error.message,a.error.data),"","","danger"),logError(JSON.stringify(a.error)),!0===d&&void 0!==c&&"function"===typeof c&&(logDebug("Got API response of type error calling "+c.name),c(a))):
a.result&&a.result.message&&"ok"!==a.result.message?(logDebug("Got API response: "+JSON.stringify(a.result)),showNotification(t(a.result.message,a.result.data),"","","success")):a.result&&a.result.message&&"ok"===a.result.message?logDebug("Got API response: "+JSON.stringify(a.result)):a.result&&void 0!==c&&"function"===typeof c?(logDebug('Got API response of type "'+a.result.method+'" calling '+c.name),c(a)):logError("Got invalid API response: "+JSON.stringify(a))}else logError("Empty response for request: "+
...
function logLog(a,b){settings.loglevel>=a&&(0===a?console.error(b):1===a?console.warn(b):console.log(b))}
...
app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appGoto("Playback")}}function showAppInitAlert(a){document.getElementById("splashScreenAlert").innerHTML='<a id="appReloadBtn" class="btn btn-danger text-light clickable">Reload</a><p class="text-danger">'+t(a)+"</p>";document.getElementById("appReloadBtn").addEventListener("click",function(){location.reload()},!1)}
...
function(a){"A"===a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++)a[c].classList.add("clickable"),!0!==(a[c].parentNode.classList.contains("noInitChilds")?!0:!1)&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},
!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"H4"===a.target.nodeName&&gotoBrowse(a.target)},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",function(a){"A"===a.target.nodeName&&(a.preventDefault(),appGoto("Browse","Filesystem",void 0,"0/"+app.current.filter+"/"+app.current.sort+"/"+decodeURI(a.target.getAttribute("data-uri"))))},!1);document.getElementById("modalSongDetails").getElementsByTagName("tbody")[0].addEventListener("click",
function(a){if("A"===a.target.nodeName)if("calcFingerprint"===a.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(a.target.getAttribute("data-uri"))},parseFingerprint);a.preventDefault();var b=a.target.parentNode,c=document.createElement("div");c.classList.add("spinner-border","spinner-border-sm");a.target.classList.add("hide");b.appendChild(c)}else void 0!==a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),gotoBrowse(a.target));else"BUTTON"===
a.target.nodeName&&a.target.getAttribute("data-href")&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(a.stopPropagation(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",{output:a.target.getAttribute("data-output-id"),state:a.target.classList.contains("active")?0:1}),toggleBtn(a.target.id))},!1);document.getElementById("QueueCurrentList").addEventListener("click",function(a){"TD"===a.target.nodeName?
sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:a.target.parentNode.getAttribute("data-trackid")}):"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",function(a){"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(a){if("TD"===a.target.nodeName)switch(a.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":appGoto("Browse","Filesystem",
...
a.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue"):"Add all to playlist"===a.target.getAttribute("data-phrase")?showAddToPlaylist("SEARCH"):"Save as smart playlist"===a.target.getAttribute("data-phrase")&&saveSearchAsSmartPlaylist())},!1);document.getElementById("BrowseDatabaseAddAllSongsDropdown").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&("Add all to queue"===a.target.getAttribute("data-phrase")?addAllFromBrowseDatabasePlist("queue"):"Add all to playlist"===
a.target.getAttribute("data-phrase")&&showAddToPlaylist("DATABASE"))},!1);document.getElementById("searchtags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(app.current.filter=a.target.getAttribute("data-tag"),search(domCache.searchstr.value))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(a){"Escape"===a.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+this.value)},!1);
document.getElementById("searchqueuetags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+a.target.getAttribute("data-tag")+"/"+app.current.sort+"/"+app.current.search)},!1);a="QueueCurrentColsDropdown BrowseFilesystemColsDropdown SearchColsDropdown BrowsePlaylistsDetailColsDropdown BrowseDatabaseColsDropdown PlaybackColsDropdown QueueLastPlayedColsDropdown".split(" ");for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",
...
!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(a){if(settings.featAdvsearch&&"TH"===a.target.nodeName){var b=a.target.getAttribute("data-col");if("Duration"!==b){var c=app.current.sort,d=!0;if(c===b||c==="-"+b)0===c.indexOf("-")?(d=!0,b=c.substring(1)):d=!1;!1===d?(c="-"+b,d=!0):(d=!1,c=b);for(var l=document.getElementById("SearchList").getElementsByClassName("sort-dir"),k=0;k<l.length;k++)l[k].remove();app.current.sort=c;a.target.innerHTML=
b+'<span class="sort-dir material-icons pull-right">'+(!0===d?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)}}},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&appGoto(app.current.app,app.current.tab,a.target.getAttribute("data-tag"),"0/"+app.current.filter+"/"+app.current.sort+"/"+
app.current.search)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");window.addEventListener("hashchange",appRoute,!1);window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",
{},parseState)},!1);document.addEventListener("keydown",function(a){if("INPUT"!==a.target.tagName&&"SELECT"!==a.target.tagName&&!a.ctrlKey&&!a.altKey){var b=keymap[a.key];b&&"function"===typeof window[b.cmd]&&(void 0===keymap[a.key].req||!0===settings[keymap[a.key].req])&&parseCmd(a,b)}},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);window.addEventListener("beforeinstallprompt",
...
1==settings.localplayerAutoplay&&localplayerPlay()})}appInitStart();function toggleAlert(a,b,c){a=document.getElementById(a);!1===b?(a.innerHTML="",a.classList.add("hide")):(a.innerHTML=c,a.classList.remove("hide"))}
...
playlistEl&&a.result.data[d].uri===settings.jukeboxPlaylist&&(c+=" selected"),c+=">"+e(a.result.data[d].uri)+"</option>";0===a.result.offset?document.getElementById(playlistEl).innerHTML=c:document.getElementById(playlistEl).innerHTML+=c;a.result.totalEntities>a.returnedEntities&&(a.result.offset+=settings.maxElementsPerPage,sendAPI("MPD_API_PLAYLIST_LIST",{offset:a.result.offset,filter:"-"},getAllPlaylists))}function updateSmartPlaylists(){sendAPI("MPD_API_SMARTPLS_UPDATE_ALL",{})}
...
function parseSmartPlaylist(a){var b=document.getElementById("saveSmartPlaylistName");b.value=a.result.data.playlist;b.classList.remove("is-invalid");document.getElementById("saveSmartPlaylistType").value=a.result.data.type;document.getElementById("saveSmartPlaylistSearch").classList.add("hide");document.getElementById("saveSmartPlaylistSticker").classList.add("hide");document.getElementById("saveSmartPlaylistNewest").classList.add("hide");var c;settings.featTags&&(c='<option value="any">'+t("Any Tag")+
"</option>");c+='<option value="filename">'+t("Filename")+"</option>";for(var d=0;d<settings.searchtags.length;d++)c+='<option value="'+settings.searchtags[d]+'">'+t(settings.searchtags[d])+"</option>";document.getElementById("selectSaveSmartPlaylistTag").innerHTML=c;"search"===a.result.type?(document.getElementById("saveSmartPlaylistSearch").classList.remove("hide"),document.getElementById("selectSaveSmartPlaylistTag").value=a.result.tag,document.getElementById("inputSaveSmartPlaylistSearchstr").value=
a.result.searchstr,settings.featAdvsearch?(document.getElementById("selectSaveSmartPlaylistTag").parentNode.classList.add("hide"),document.getElementById("inputSaveSmartPlaylistSearchstr").parentNode.classList.replace("col-md-6","col-md-12")):(document.getElementById("selectSaveSmartPlaylistTag").parentNode.classList.remove("hide"),document.getElementById("inputSaveSmartPlaylistSearchstr").parentNode.classList.replace("col-md-12","col-md-6"))):"sticker"===a.result.type?(document.getElementById("saveSmartPlaylistSticker").classList.remove("hide"),
...
d=document.activeElement.parentNode.parentNode===c?!0:!1,f=0;c.setAttribute("data-version",a.queueVersion);c=c.getElementsByTagName("tbody")[0];for(var g=c.getElementsByTagName("tr"),h=0;h<b;h++){a.result.data[h].Duration=beautifySongDuration(a.result.data[h].Duration);a.result.data[h].Pos++;var l=document.createElement("tr");l.setAttribute("draggable","true");l.setAttribute("data-trackid",a.result.data[h].id);l.setAttribute("id","queueTrackId"+a.result.data[h].id);l.setAttribute("data-songpos",a.result.data[h].Pos);
l.setAttribute("data-duration",a.result.data[h].Duration);l.setAttribute("data-uri",a.result.data[h].uri);l.setAttribute("tabindex",0);for(var k="",m=0;m<settings.colsQueueCurrent.length;m++)k+='<td data-col="'+settings.colsQueueCurrent[m]+'">'+e(a.result.data[h][settings.colsQueueCurrent[m]])+"</td>";k+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';l.innerHTML=k;h<g.length?f=!0===replaceTblRow(g[h],l)?h:f:c.append(l)}for(h=g.length-1;h>=b;h--)g[h].remove();
g=settings.colsQueueCurrent.length;g--;"queuesearch"===a.type&&0===b?c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+g+'">'+t("No results, please refine your search")+"</td></tr>":"queue"===a.type&&0===b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+g+'">'+t("Empty queue")+"</td></tr>");!0===d&&focusTable(f);setPagination(a.result.totalEntities,a.returnedEntities);document.getElementById("QueueCurrentList").classList.remove("opacity05")}
...
"/"+a)}
function parseSearch(a){document.getElementById("panel-heading-search").innerText=gtPage("Num songs",a.returnedEntities,a.result.totalEntities);document.getElementById("cardFooterSearch").innerText=gtPage("Num songs",a.returnedEntities,a.result.totalEntities);0<a.returnedEntities?(document.getElementById("searchAddAllSongs").removeAttribute("disabled"),document.getElementById("searchAddAllSongsBtn").removeAttribute("disabled")):(document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),
...
app.current.app&&"Database"===app.current.tab&&""!==app.current.search&&appRoute();i18nHtml(document.getElementsByTagName("body")[0]);settingsParsed="true"}
...
""===settings.musicDirectoryValue&&"none"!==settings.musicDirectory?document.getElementById("warnMusicDirectory").classList.remove("hide"):document.getElementById("warnMusicDirectory").classList.add("hide");!0===settings.bgCover&&!0===settings.featCoverimage&&!0===settings.coverimage?lastSongObj.cover&&-1<lastSongObj.cover.indexOf("coverimage-")?clearBackgroundImage():lastSongObj.cover?setBackgroundImage(lastSongObj.cover):clearBackgroundImage():clearBackgroundImage();if(!1===settings.featTags)app.apps.Browse.active=
"Filesystem",app.apps.Search.state="0/filename/-/",app.apps.Queue.state="0/filename/-/",settings.colsQueueCurrent=["Pos","Title","Duration"],settings.colsQueueLastPlayed=["Pos","Title","LastPlayed"],settings.colsSearch=["Title","Duration"],settings.colsBrowseFilesystem=["Type","Title","Duration"],settings.colsBrowseDatabase=["Track","Title","Duration"],settings.colsPlayback=[];else{a="";for(b=0;b<settings.colsPlayback.length;b++)a+='<div id="current'+settings.colsPlayback[b]+'" data-tag="'+settings.colsPlayback[b]+
'" data-name="'+(lastSongObj[settings.colsPlayback[b]]?encodeURI(lastSongObj[settings.colsPlayback[b]]):"")+'"><small>'+t(settings.colsPlayback[b])+"</small><h4",settings.browsetags.includes(settings.colsPlayback[b])&&(a+=' class="clickable"'),a+=">"+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):"")+"</h4></div>";document.getElementById("cardPlaybackTags").innerHTML=a}!settings.tags.includes("AlbumArtist")&&settings.featTags&&(settings.tags.includes("Artist")?app.apps.Browse.tabs.Database.active=
...
locale:g.options[g.selectedIndex].value,love:document.getElementById("btnLoveEnable").classList.contains("active")?!0:!1,loveChannel:document.getElementById("inputLoveChannel").value,loveMessage:document.getElementById("inputLoveMessage").value,maxElementsPerPage:document.getElementById("inputMaxElementsPerPage").value,stickers:document.getElementById("btnStickers").classList.contains("active")?!0:!1,lastPlayedCount:document.getElementById("inputLastPlayedCount").value,smartpls:document.getElementById("btnSmartpls").classList.contains("active")?
!0:!1,taglist:document.getElementById("inputEnabledTags").value.replace(/\s/g,""),searchtaglist:document.getElementById("inputSearchTags").value.replace(/\s/g,""),browsetaglist:document.getElementById("inputBrowseTags").value.replace(/\s/g,"")},getSettings);modalSettings.hide()}}
...
c="",d=0;d<b.length;d++)!0===b[d].checked&&(c+=b[d].name+", ");a.target.parentNode.parentNode.parentNode.previousElementSibling.value=c.replace(/(,\s)$/,"")}});document.getElementById(a).value=f.replace(/(,\s)$/,"");document.getElementById(b).innerHTML=g}
...
document.getElementById("mpdstats_dbUpdated").innerText=localeDate(a.result.dbUpdated);document.getElementById("mympdVersion").innerText=a.result.mympdVersion;document.getElementById("mpdInfo_version").innerText=a.result.mpdVersion;document.getElementById("mpdInfo_libmpdclientVersion").innerText=a.result.libmpdclientVersion}
...
"";document.getElementById("headerTitle").removeAttribute("title");clearCurrentCover();!0===settings.bgCover&&clearBackgroundImage();for(var b=document.getElementById("cardPlaybackTags").getElementsByTagName("h4"),c=0;c<b.length;c++)b[c].innerText=""}lastState=a.result;!1!==settings.mpdConnected&&!1!==uiEnabled||getSettings(!0)}}
...
function songChange(a){var b=a.result.uri+":"+a.result.currentSongId;if(lastSong!==b){var c="",d="",f="";setCurrentCover(a.result.cover);!0===settings.bgCover&&!0===settings.featCoverimage&&(-1<a.result.cover.indexOf("coverimage-")?clearBackgroundImage():setBackgroundImage(a.result.cover));void 0!==a.result.Artist&&0<a.result.Artist.length&&"-"!==a.result.Artist&&(c+=a.result.Artist,d+=a.result.Artist,f+=a.result.Artist+" - ");void 0!==a.result.Album&&0<a.result.Album.length&&"-"!==a.result.Album&&
(c+=" - "+a.result.Album,d+="<br/>"+a.result.Album);void 0!==a.result.Title&&0<a.result.Title.length?(f+=a.result.Title,domCache.currentTitle.innerText=a.result.Title,domCache.currentTitle.setAttribute("data-uri",encodeURI(a.result.uri))):(domCache.currentTitle.innerText="",domCache.currentTitle.setAttribute("data-uri",""));document.title="myMPD: "+f;document.getElementById("headerTitle").innerText=f;document.getElementById("headerTitle").title=f;!0===settings.featStickers&&setVoteSongBtns(a.result.like,
a.result.uri);logDebug("colsPlayback: "+JSON.stringify(settings.colsPlayback));for(f=0;f<settings.colsPlayback.length;f++){var g=document.getElementById("current"+settings.colsPlayback[f]);g&&(g.getElementsByTagName("h4")[0].innerText=a.result[settings.colsPlayback[f]],g.setAttribute("data-name",encodeURI(a.result[settings.colsPlayback[f]])))}if(f=document.getElementById("queueTrackId"+a.result.currentSongId))f.getElementsByTagName("td")[1].innerText=a.result.Title;"play"===playstate&&showNotification(a.result.Title,
c,d,"success");lastSong=b;lastSongObj=a.result}}function gotoTagList(){appGoto(app.current.app,app.current.tab,app.current.view,"0/-/-/")}function chVolume(a){a=parseInt(domCache.volumeBar.value)+a;0>a?a=0:100<a&&(a=100);domCache.volumeBar.value=a;sendAPI("MPD_API_PLAYER_VOLUME_SET",{volume:a})}function clickTitle(){var a=decodeURI(domCache.currentTitle.getAttribute("data-uri"));""!==a&&songDetails(a)}
function focusTable(a,b){null===b&&(b=document.getElementById(app.current.app+(void 0!==app.current.tab?app.current.tab:"")+(void 0!==app.current.view?app.current.view:"")+"List"),null===b&&(b=document.getElementById(app.current.app+app.current.tab+"TagList")),"Browse"!==app.current.app||"Database"!==app.current.tab||document.getElementById("BrowseDatabaseAlbumList").classList.contains("hide")||(b=document.getElementById("BrowseDatabaseAlbumList").getElementsByTagName("table")[0]));if(null!==b){var c=
...
a||g!==d&&"-"+g!==d||(g=!1,0===app.current.sort.indexOf("-")&&(g=!0),c+='<span class="sort-dir material-icons pull-right">'+(!0===g?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c+="<th></th>";if(void 0===b)document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c;else for(a=document.querySelectorAll(b),b=0;b<a.length;b++)a[b].getElementsByTagName("tr")[0].innerHTML=c}}
...
for(b=0;b<c.length;b++)(d=c[b].getAttribute("data-tag"))&&a.cols.push(d);sendAPI("MYMPD_API_COLS_SAVE",a,getSettings)}function replaceTblRow(a,b){var c=!1;a.querySelector("[data-popover]")&&hideMenu();a.classList.contains("selected")&&(b.classList.add("selected"),b.focus(),c=!0);a.replaceWith(b);return c}function dirname(a){return a.replace(/\/[^/]*$/,"")}function basename(a){return a.split("/").reverse()[0]}function scrollToPosY(a){document.body.scrollTop=a;document.documentElement.scrollTop=a}
