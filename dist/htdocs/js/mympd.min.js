...
function parseDatabase(a){var b=a.result.returnedEntities,c=document.getElementById("BrowseDatabaseCards"),d=c.getElementsByClassName("col");0===d.length&&(c.innerHTML="");for(var f=0;f<b;f++){var g=document.createElement("div");g.classList.add("col","px-0","flex-grow-0");""===a.result.data[f].AlbumArtist&&(a.result.data[f].AlbumArtist=t("Unknown artist"));""===a.result.data[f].Album&&(a.result.data[f].Album=t("Unknown album"));var h=void 0;h=void 0;var k="";"Album"===a.result.tag?(h=genId("database"+
a.result.data[f].Album+a.result.data[f].AlbumArtist),k=subdir+"/albumart/"+encodeURI(a.result.data[f].FirstSongUri),h='<div class="card card-grid clickable" data-picture="'+k+'" data-uri="'+encodeURI(a.result.data[f].FirstSongUri.replace(/\/[^/]+$/,""))+'" data-type="dir" data-name="'+encodeURI(a.result.data[f].Album)+'" data-album="'+encodeURI(a.result.data[f].Album)+'" data-albumartist="'+encodeURI(a.result.data[f].AlbumArtist)+'" tabindex="0"><div class="card-body album-cover-loading album-cover-grid bg-white" id="'+
...
function(a){"Album"===app.current.tag?a.target.classList.contains("card-body")?appGoto("Browse","Database","Detail","0/Album/AlbumArtist/"+decodeURI(a.target.parentNode.getAttribute("data-album"))+"/"+decodeURI(a.target.parentNode.getAttribute("data-albumartist"))):a.target.classList.contains("card-footer")&&showMenu(a.target,a):appGoto(app.current.app,app.current.card,void 0,"0/Album/AlbumArtist/Album/("+app.current.tag+" == '"+decodeURI(a.target.parentNode.getAttribute("data-tag"))+"')")},!1),g.firstChild.addEventListener("keydown",
function(a){var b=!1;if("Enter"===a.key)"Album"===app.current.tag?appGoto("Browse","Database","Detail","0/Album/AlbumArtist/"+decodeURI(a.target.getAttribute("data-album"))+"/"+decodeURI(a.target.getAttribute("data-albumartist"))):appGoto(app.current.app,app.current.card,void 0,"0/"+app.current.tag+"/AlbumArtist/Album/("+app.current.tag+" == '"+decodeURI(a.target.getAttribute("data-tag"))+"')"),b=!0;else if(" "===a.key)"Album"===app.current.tag&&showMenu(a.target.getElementsByClassName("card-footer")[0],
a),b=!0;else if("ArrowDown"===a.key||"ArrowUp"===a.key){var c=a.target,f="ArrowDown"===a.key?null!==a.target.parentNode.nextElementSibling?a.target.parentNode.nextElementSibling.firstChild:null:null!==a.target.parentNode.previousElementSibling?a.target.parentNode.previousElementSibling.firstChild:null;null!==f&&(f.focus(),c.classList.remove("selected"),f.classList.add("selected"),b=!0,scrollFocusIntoView())}else"Escape"===a.key&&(b=a.target,b.blur(),b.classList.remove("selected"),b=!0);!0===b&&(a.stopPropagation(),
a.preventDefault())},!1))}for(f=d.length-1;f>=b;f--)d[f].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}
...
function parseAlbumDetails(a){document.getElementById("viewDetailDatabaseCover").style.backgroundImage='url("'+subdir+"/albumart/"+a.result.data[0].uri+'"), url("'+subdir+'/assets/coverimage-loading.svg")';document.getElementById("viewDetailDatabaseInfo").innerHTML="<h1>"+e(a.result.Album)+"</h1><p>"+e(a.result.AlbumArtist)+"</p>";for(var b=document.getElementById("BrowseDatabaseDetailList").getElementsByTagName("tbody")[0],c="",d=a.result.returnedEntities,f=0;f<d;f++){a.result.data[f].Duration&&
(a.result.data[f].Duration=beautifySongDuration(a.result.data[f].Duration));c+='<tr tabindex="0" data-type="song" data-name="'+a.result.data[f].Title+'" data-uri="'+encodeURI(a.result.data[f].uri)+'">';for(var g=0;g<settings.colsBrowseDatabaseDetail.length;g++)c+='<td data-col="'+settings.colsBrowseDatabaseDetail[g]+'">'+e(a.result.data[f][settings.colsBrowseDatabaseDetail[g]])+"</td>";c+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td></tr>"}b.innerHTML=
c;document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto("Browse","Database","List")}function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);b="((Album == '"+b+"') AND (AlbumArtist == '"+c+"'))";"appendQueue"===a?addAllFromSearchPlist("queue",b,!1):"replaceQueue"===a?addAllFromSearchPlist("queue",b,!0):"addPlaylist"===a&&showAddToPlaylist("ALBUM",b)}
...
tab:void 0,view:void 0,page:0,filter:"",search:"",sort:"",tag:"",scrollPos:0},last:{app:void 0,tab:void 0,view:void 0,filter:"",search:"",sort:"",tag:"",scrollPos:0}},domCache={};domCache.navbarBtns=document.getElementById("navbar-main").getElementsByTagName("div");domCache.navbarBtnsLen=domCache.navbarBtns.length;domCache.counter=document.getElementById("counter");domCache.volumePrct=document.getElementById("volumePrct");domCache.volumeControl=document.getElementById("volumeControl");
...
parseInt(a[5]);app.current.filter=a[6];app.current.sort=a[7];app.current.tag=a[8];app.current.search=a[9];appPrepare(app.current.scrollPos);if("Playback"===app.current.app)sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange);else if("Queue"===app.current.app&&"Current"===app.current.tab)selectTag("searchqueuetags","searchqueuetagsdesc",app.current.filter),getQueue();else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)sendAPI("MPD_API_QUEUE_LAST_PLAYED",{offset:app.current.page,cols:settings.colsQueueLastPlayed},
...
encodeURI(d)+'">'+e(b[f])+"</a></li>";d+="/"}document.getElementById("BrowseBreadcrumb").innerHTML=a;doSetFilterLetter("BrowseFilesystemFilter")}else if("Browse"===app.current.app&&"Database"===app.current.tab&&"List"===app.current.view)if(document.getElementById("viewListDatabase").classList.remove("hide"),document.getElementById("viewDetailDatabase").classList.add("hide"),selectTag("searchDatabaseTags","searchDatabaseTagsDesc",app.current.filter),selectTag("BrowseDatabaseByTagDropdown","btnBrowseDatabaseByTagDesc",
app.current.tag),a=app.current.sort,b=!1,"-"===app.current.sort.charAt(0)?(b=!0,a=app.current.sort.substr(1),toggleBtnChk("databaseSortDesc",!0)):toggleBtnChk("databaseSortDesc",!1),selectTag("databaseSortTags",void 0,a),"Album"===app.current.tag){c=document.getElementById("searchDatabaseCrumb");d=document.getElementById("searchDatabaseStr");f="";for(var g=app.current.search.split(" AND "),h=0;h<g.length-1;h++){var k=g[h].substring(1,g[h].length-1);f+='<button data-filter="'+encodeURI(k)+'" class="btn btn-light mr-2">'+
e(k)+'<span class="badge badge-secondary">&times</span></button>'}c.innerHTML=f;""===d.value&&1<=g.length&&(f=g[g.length-1].substring(1,g[g.length-1].length-1),g=f.substring(f.indexOf("'")+1,f.length-1),d.value!==g&&(c.innerHTML+='<button data-filter="'+encodeURI(f)+'" class="btn btn-light mr-2">'+e(f)+'<span class="badge badge-secondary">&times;</span></button>'),document.getElementById("searchDatabaseMatch").value="contains");c.classList.remove("hide");document.getElementById("searchDatabaseMatch").classList.remove("hide");
document.getElementById("btnDatabaseSortDropdown").removeAttribute("disabled");document.getElementById("btnDatabaseSearchDropdown").removeAttribute("disabled");sendAPI("MPD_API_DATABASE_GET_ALBUMS",{offset:app.current.page,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b},parseDatabase)}else document.getElementById("searchDatabaseCrumb").classList.add("hide"),document.getElementById("searchDatabaseMatch").classList.add("hide"),document.getElementById("btnDatabaseSortDropdown").setAttribute("disabled",
"disabled"),document.getElementById("btnDatabaseSearchDropdown").setAttribute("disabled","disabled"),document.getElementById("searchDatabaseStr").value=app.current.search,sendAPI("MPD_API_DATABASE_TAG_LIST",{offset:app.current.page,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b,tag:app.current.tag},parseDatabase);else if("Browse"===app.current.app&&"Database"===app.current.tab&&"Detail"===app.current.view)document.getElementById("viewListDatabase").classList.add("hide"),
document.getElementById("viewDetailDatabase").classList.remove("hide"),"Album"===app.current.filter&&sendAPI("MPD_API_DATABASE_TAG_ALBUM_TITLE_LIST",{album:app.current.tag,search:app.current.search,tag:app.current.sort,cols:settings.colsBrowseDatabaseDetail},parseAlbumDetails);else if("Search"===app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a="";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)d=b[c].substring(1,b[c].length-
1),a+='<button data-filter="'+encodeURI(d)+'" class="btn btn-light mr-2">'+e(d)+'<span class="badge badge-secondary">&times</span></button>';domCache.searchCrumb.innerHTML=a;""===domCache.searchstr.value&&1<=b.length&&(a=b[b.length-1].substring(1,b[b.length-1].length-1),b=a.substring(a.indexOf("'")+1,a.length-1),domCache.searchstr.value!==b&&(domCache.searchCrumb.innerHTML+='<button data-filter="'+encodeURI(a)+'" class="btn btn-light mr-2">'+e(a)+'<span class="ml-2 badge badge-secondary">&times;</span></button>'),
a=a.substring(a.indexOf(" ")+1),a=a.substring(0,a.indexOf(" ")),""===a&&(a="contains"),document.getElementById("searchMatch").value=a)}else""===domCache.searchstr.value&&""!==app.current.search&&(domCache.searchstr.value=app.current.search);app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app].length,a--,document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+
t("Searching...")+"</td></tr>");2<=domCache.searchstr.value.length||0<domCache.searchCrumb.children.length?settings.featAdvsearch?(a=app.current.sort,b=!1,"-"===a?(a=settings.tags.includes("Title")?"Title":"-",document.getElementById("SearchList").setAttribute("data-sort",a)):0===a.indexOf("-")&&(b=!0,a=a.substring(1)),sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:"",offset:app.current.page,sort:a,sortdesc:b,expression:app.current.search,cols:settings.colsSearch,replace:!1},parseSearch)):sendAPI("MPD_API_DATABASE_SEARCH",
...
app.current.filter)}else appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appPrepare(0),appGoto("Playback")}}function showAppInitAlert(a){document.getElementById("splashScreenAlert").innerHTML='<p class="text-danger">'+t(a)+'</p><p><a id="appReloadBtn" class="btn btn-danger text-light clickable">'+t("Reload")+"</a></p>";document.getElementById("appReloadBtn").addEventListener("click",function(){clearAndReload()},!1)}
function clearAndReload(){"serviceWorker"in navigator&&caches.keys().then(function(a){a.forEach(function(a){caches.delete(a)})});location.reload()}
...
a.target.offsetWidth*a.clientX));domCache.progressPos.style.display="block";var b=domCache.progressPos.offsetWidth/2;domCache.progressPos.style.left=(a.clientX<b?a.clientX:a.clientX<window.innerWidth-b?a.clientX-b:a.clientX-2*b)+"px"}},!1);domCache.progress.addEventListener("mouseout",function(){domCache.progressPos.style.display="none"},!1);for(var a=document.querySelectorAll(".subMenu"),b=a.length,c=0;c<b;c++)a[c].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();a=this.getElementsByTagName("span")[0];
a.innerText="keyboard_arrow_right"===a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1);document.getElementById("volumeMenu").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{},parseOutputs)});document.getElementById("btnDropdownNeighbors").parentNode.addEventListener("show.bs.dropdown",function(){!0===settings.featNeighbors?sendAPI("MPD_API_MOUNT_NEIGHBOR_LIST",{},parseNeighbors,!0):document.getElementById("dropdownNeighbors").children[0].innerHTML=
'<div class="list-group-item"><span class="material-icons">warning</span> '+t("Neighbors are disabled")+"</div>"});document.getElementById("dropdownNeighbors").children[0].addEventListener("click",function(a){a.preventDefault();"A"===a.target.nodeName&&(a=a.target.getAttribute("data-value").match(/^(\w+:\/\/)(.+)$/),document.getElementById("selectMountUrlhandler").value=a[1],document.getElementById("inputMountUrl").value=a[2])});document.getElementById("BrowseFilesystemBookmark").parentNode.addEventListener("show.bs.dropdown",
...
!1);addFilterLetter("BrowseFilesystemFilterLetters");addFilterLetter("BrowsePlaylistsFilterLetters");document.getElementById("syscmds").addEventListener("click",function(a){"A"===a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",function(a){"A"===a.target.nodeName&&execScript(a.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++)!1===a[c].classList.contains("notclickable")&&
a[c].classList.add("clickable"),!0!==(a[c].parentNode.classList.contains("noInitChilds")?!0:!1)&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"P"===a.target.nodeName&&gotoBrowse()},!1);
document.getElementById("BrowseBreadcrumb").addEventListener("click",function(a){"A"===a.target.nodeName&&(a.preventDefault(),appGoto("Browse","Filesystem",void 0,"0/"+app.current.filter+"/"+app.current.sort+"/-/"+decodeURI(a.target.getAttribute("data-uri"))))},!1);document.getElementById("tbodySongDetails").addEventListener("click",function(a){if("A"===a.target.nodeName)if("calcFingerprint"===a.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(a.target.getAttribute("data-uri"))},parseFingerprint);
a.preventDefault();var b=a.target.parentNode,c=document.createElement("div");c.classList.add("spinner-border","spinner-border-sm");a.target.classList.add("hide");b.appendChild(c)}else null!==a.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),a.preventDefault(),gotoBrowse());else"BUTTON"===a.target.nodeName&&a.target.getAttribute("data-href")&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(a){"BUTTON"===a.target.nodeName?
...
showEditTimer(a.target.parentNode.getAttribute("data-id")):"A"===a.target.nodeName?deleteTimer(a.target.parentNode.parentNode.getAttribute("data-id")):"BUTTON"===a.target.nodeName&&toggleTimer(a.target,a.target.parentNode.parentNode.getAttribute("data-id"))},!1);document.getElementById("listMountsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){if(""===a.target.parentNode.getAttribute("data-point"))return!1;showEditMount(decodeURI(a.target.parentNode.getAttribute("data-url")),
decodeURI(a.target.parentNode.getAttribute("data-point")))}else if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===b?unmountMount(c):"update"===b&&updateMount(a.target,c)}},!1);document.getElementById("listScriptsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){if(!1===settings.featScripteditor||""===a.target.parentNode.getAttribute("data-script"))return!1;
showEditScript(decodeURI(a.target.parentNode.getAttribute("data-script")))}else if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action"),c=decodeURI(a.target.parentNode.parentNode.getAttribute("data-script"));"delete"===b?deleteScript(c):"execute"===b&&execScript(a.target.getAttribute("data-href"))}},!1);document.getElementById("listTriggerList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("TD"===a.target.nodeName){var b=decodeURI(a.target.parentNode.getAttribute("data-trigger-id"));
showEditTrigger(b)}else"A"===a.target.nodeName&&(b=a.target.getAttribute("data-action"),a=decodeURI(a.target.parentNode.parentNode.getAttribute("data-trigger-id")),"delete"===b&&deleteTrigger(a))},!1);document.getElementById("listPartitionsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();if("A"===a.target.nodeName){var b=a.target.getAttribute("data-action");a=decodeURI(a.target.parentNode.parentNode.getAttribute("data-partition"));"delete"===b?deletePartition(a):
"switch"===b&&switchPartition(a)}},!1);document.getElementById("partitionOutputsList").addEventListener("click",function(a){a.stopPropagation();a.preventDefault();"TD"===a.target.nodeName&&(a=decodeURI(a.target.parentNode.getAttribute("data-output")),moveOutput(a),modalPartitionOutputs.hide())},!1);document.getElementById("QueueCurrentList").addEventListener("click",function(a){"TD"===a.target.nodeName?sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:a.target.parentNode.getAttribute("data-trackid")}):"A"===
a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",function(a){"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(a){if("TD"===a.target.nodeName)switch(a.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":appGoto("Browse","Filesystem",void 0,"0/"+app.current.filter+"/"+app.current.sort+"/-/"+decodeURI(a.target.parentNode.getAttribute("data-uri")));
break;case "song":appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"))}else"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",function(a){if("A"===a.target.nodeName){var b=a.target.parentNode.parentNode.getAttribute("data-id"),
...
(a.stopPropagation(),a.preventDefault(),toggleBtnChk(a.target))},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);document.getElementById("searchdatabase").addEventListener("submit",function(){return!1},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",function(a){if("Escape"===a.key)this.blur();else if("Enter"===a.key&&"Album"===app.current.tag)if(""!==
this.value){a=document.getElementById("searchDatabaseMatch");var b=document.createElement("button");b.classList.add("btn","btn-light","mr-2");b.setAttribute("data-filter",encodeURI(app.current.filter+" "+a.options[a.selectedIndex].value+" '"+this.value+"'"));b.innerHTML=e(app.current.filter)+" "+a.options[a.selectedIndex].value+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";document.getElementById("searchDatabaseCrumb").appendChild(b)}else searchAlbumgrid(this.value);
...
function(a){"OPTION"===a.target.nodeName&&removeScriptArgument(a)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("Search");dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("hashchange",
...
0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);var d="";Object.keys(themes).forEach(function(a){d+='<option value="'+e(a)+'">'+t(themes[a])+"</option>"});document.getElementById("selectTheme").innerHTML=d;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(a){a.stopPropagation();a.preventDefault();clickCheckLocalPlayerState(a)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",
...
function setCounter(a,b,c){currentSong.totalTime=b;currentSong.elapsedTime=c;currentSong.currentSongId=a;var d=0<b?Math.ceil(domCache.progress.offsetWidth*c/b):0;0===d&&(domCache.progressBar.style.transition="none");domCache.progressBar.style.width=d+"px";0===d&&setTimeout(function(){domCache.progressBar.style.transition=progressBarTransition},10);b=beautifySongDuration(c)+"&nbsp;/&nbsp;"+beautifySongDuration(b);domCache.counter.innerHTML=b;if(lastState&&lastState.currentSongId!==a&&(c=document.getElementById("queueTrackId"+
...
(h=localeDate(h));g.getElementsByTagName("p")[0].innerText=h;g.setAttribute("data-name",encodeURI(h))}}if(f=document.getElementById("queueTrackId"+a.result.currentSongId))f.getElementsByTagName("td")[1].innerText=a.result.Title;"play"===playstate&&showNotification(a.result.Title,c,d,"success");lastSong=b;lastSongObj=a.result}}function gotoTagList(){appGoto(app.current.app,app.current.tab,app.current.view,"0/-/-/-/")}
...
c=app.current.search.replace(/\/?([^/]+)$/,""),a=a.insertRow(0),a.setAttribute("data-type","parentDir"),a.setAttribute("tabindex",0),a.setAttribute("data-uri",encodeURI(c)),a.innerHTML='<td colspan="'+b+'">..</td>'));scrollFocusIntoView()}}
