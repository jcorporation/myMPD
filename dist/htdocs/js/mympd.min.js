...
g=document.createElement("tr");g.setAttribute("data-uri",f);g.setAttribute("data-type",a.data[e].Type);g.setAttribute("data-name",a.data[e].name);g.innerHTML='<td data-col="Type"><span class="material-icons">'+("smartpls"==a.data[e].Type?"queue_music":"list")+"</span></td><td>"+a.data[e].name+"</td><td>"+localeDate(a.data[e].last_modified)+'</td><td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';e<d.length?d[e].replaceWith(g):c.append(g)}document.getElementById("cardFooterBrowse").innerText=
gtPage("Num playlists",a.returnedEntities,a.totalEntities)}else if("Detail"==app.current.view){for(e=0;e<b;e++){f=encodeURI(a.data[e].uri);g=document.createElement("tr");0==a.smartpls&&g.setAttribute("draggable","true");g.setAttribute("id","playlistTrackId"+a.data[e].Pos);g.setAttribute("data-type",a.data[e].Type);g.setAttribute("data-uri",f);g.setAttribute("data-name",a.data[e].Title);g.setAttribute("data-songpos",a.data[e].Pos);a.data[e].Duration=beautifySongDuration(a.data[e].Duration);f="";for(var h=
0;h<settings.colsBrowsePlaylistsDetail.length;h++)f+='<td data-col="'+settings.colsBrowsePlaylistsDetail[h]+'">'+a.data[e][settings.colsBrowsePlaylistsDetail[h]]+"</td>";f+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">playlist_add</a></td>';g.innerHTML=f;e<d.length?d[e].replaceWith(g):c.append(g)}document.getElementById("cardFooterBrowse").innerText=gtPage("Num songs",a.returnedEntities,a.totalEntities)}for(e=d.length-1;e>=b;e--)d[e].remove();setPagination(a.totalEntities,
...
document.getElementById(c+e[f]).classList.remove("hide"),document.getElementById(c+"ButtonsBottom").classList.remove("hide")):document.getElementById(c+e[f]+"Prev").setAttribute("disabled","disabled")}}
...
function getAllPlaylists(a){var b=a.data.length,c="";if(0==a.offset)if("addToPlaylistPlaylist"==playlistEl)c='<option value=""></option><option value="new">'+t("New playlist")+"</option>";else if("selectJukeboxPlaylist"==playlistEl||"selectAddToQueuePlaylist"==playlistEl)c='<option value="Database">'+t("Database")+"</option>";for(var d=0;d<b;d++)if("addToPlaylistPlaylist"!=playlistEl||"smartpls"!=a.data[d].Type)c+='<option value="'+a.data[d].uri+'"',"selectJukeboxPlaylist"==playlistEl&&a.data[d].uri==
settings.jukeboxPlaylist&&(c+=" selected"),c+=">"+a.data[d].uri+"</option>";0==a.offset?document.getElementById(playlistEl).innerHTML=c:document.getElementById(playlistEl).innerHTML+=c;a.totalEntities>a.returnedEntities&&(a.offset+=settings.maxElementsPerPage,sendAPI({cmd:"MPD_API_PLAYLIST_LIST",data:{offset:a.offset,filter:"-"}},getAllPlaylists))}function updateSmartPlaylists(){sendAPI({cmd:"MPD_API_SMARTPLS_UPDATE_ALL"})}function loveSong(){sendAPI({cmd:"MPD_API_LOVE",data:{}})}
...
function logWarn(a){logLog(1,"WARN: "+a)}function logInfo(a){logLog(2,"INFO: "+a)}function logVerbose(a){logLog(3,"VERBOSE: "+a)}function logDebug(a){logLog(4,"DEBUG: "+a)}function logLog(a,b){settings.loglevel>=a&&console.log(b)}appInitStart();
