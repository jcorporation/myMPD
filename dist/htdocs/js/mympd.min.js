...
function parseBookmarks(a){for(var b='<table class="table table-sm table-dark table-borderless mb-0">',c=0;c<a.data.length;c++)b+='<tr data-id="'+a.data[c].id+'" data-type="'+a.data[c].type+'" data-uri="'+encodeURI(a.data[c].uri)+'"><td class="nowrap"><a class="text-light" href="#" data-href="goto">'+a.data[c].name+'</a></td><td><a class="text-light material-icons material-icons-small" href="#" data-href="edit">edit</a></td><td><a class="text-light material-icons material-icons-small" href="#" data-href="delete">delete</a></td></tr>';
0==a.data.length&&(b+='<tr><td class="text-light nowrap">No bookmarks found.</td></tr>');b+="</table>";document.getElementById("BrowseFilesystemBookmarks").innerHTML=b}
...
function saveSettings(){var a=!0,b=document.getElementById("inputCrossfade");b.getAttribute("disabled")||validateInt(b)||(a=!1);b=document.getElementById("inputJukeboxQueueLength");validateInt(b)||(a=!1);1==settings.mixramp&&(b=document.getElementById("inputMixrampdb"),b.getAttribute("disabled")||validateFloat(b)||(a=!1),b=document.getElementById("inputMixrampdelay"),b.getAttribute("disabled")||("nan"==b.value&&(b.value="-1"),validateFloat(b)||(a=!1)));if(1==a){a=document.getElementById("selectReplaygain");
b=document.getElementById("selectJukeboxPlaylist");var c=document.getElementById("selectJukeboxMode");sendAPI({cmd:"MYMPD_API_SETTINGS_SET",data:{consume:document.getElementById("btnConsume").classList.contains("active")?1:0,random:document.getElementById("btnRandom").classList.contains("active")?1:0,single:document.getElementById("btnSingle").classList.contains("active")?1:0,repeat:document.getElementById("btnRepeat").classList.contains("active")?1:0,replaygain:a.options[a.selectedIndex].value,crossfade:document.getElementById("inputCrossfade").value,
mixrampdb:1==settings.mixramp?document.getElementById("inputMixrampdb").value:settings.mixrampdb,mixrampdelay:1==settings.mixramp?document.getElementById("inputMixrampdelay").value:settings.mixrampdelay,notificationWeb:document.getElementById("btnnotifyWeb").classList.contains("active")?!0:!1,notificationPage:document.getElementById("btnnotifyPage").classList.contains("active")?!0:!1,jukeboxMode:c.options[c.selectedIndex].value,jukeboxPlaylist:b.options[b.selectedIndex].value,jukeboxQueueLength:document.getElementById("inputJukeboxQueueLength").value,
autoPlay:document.getElementById("btnAutoPlay").classList.contains("active")?!0:!1}},getSettings);modalSettings.hide()}}function addAllFromBrowseFilesystem(){sendAPI({cmd:"MPD_API_QUEUE_ADD_TRACK",data:{uri:app.current.search}});showNotification("Added all songs","","","success")}
...
function beautifySongDuration(a){var b=Math.floor(a/3600),c=Math.floor(a/60)-60*b;a=a-3600*b-60*c;return(0<b?b+":"+(10>c?"0":""):"")+c+":"+(10>a?"0":"")+a}function genId(a){return"id"+a.replace(/[^\w\-]/g,"")}function validatePlname(a){return""==a?!1:null==a.match(/\/|\r|\n|"|'/)?!0:!1}function validateInt(a){if(""!=a.value.replace(/\d/g,""))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}
function validateFloat(a){if(""!=a.value.replace(/\d\-\./g,""))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}function log_error(a){log_log(0,"ERROR: "+a)}function log_warn(a){log_log(1,"WARN: "+a)}function log_info(a){log_log(2,"INFO: "+a)}function log_verbose(a){log_log(3,"VERBOSE: "+a)}function log_debug(a){log_log(4,"DEBUG: "+a)}function log_log(a,b){settings.loglevel>=a&&console.log(b)}appInitStart();
