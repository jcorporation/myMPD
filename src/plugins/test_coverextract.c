...
int main(int argc, char **argv) {
    ...
    int (*plugin_coverextract)(const char *, char *, int, char *, int, const bool);
    
    handle_plugins_coverextract = dlopen("/usr/share/mympd/lib/mympd_coverextract.so", RTLD_LAZY);
    if (!handle_plugins_coverextract) {
        printf("ERROR loading plugin %s: %s\n", "/usr/share/mympd/lib/mympd_coverextract.so", dlerror());
        return 1;
    }
    *(void **) (&plugin_coverextract) = dlsym(handle_plugins_coverextract, "coverextract");
    if ((error = dlerror()) != NULL)  {
        printf("ERROR loading plugin %s: %s\n", "/usr/share/mympd/lib/mympd_coverextract.so", error);
        return 1;
    }
    if (argc == 2) {
