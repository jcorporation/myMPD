...
/**
 * Returns the path / basename for an uri to save it in the cover cache
...
/**
 * Writes the coverimage (as binary buffer) to the cover cache,
...
bool cache_disk_cover_write_file(sds cachedir, const char *uri, const char *mime_type, sds binary, int offset) {
    ...
    sds filepath = cache_disk_cover_get_basename(cachedir, uri, offset);
