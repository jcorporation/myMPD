...
void mympd_api_handler(struct t_mympd_state *mympd_state, struct t_work_request *request) {
    ...
    switch(request->cmd_id) {
        ...
        case INTERNAL_API_CACHES_CREATE:
            if (worker_threads > 5) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true,
        ...
        case MYMPD_API_PICTURE_LIST:
            if (json_get_string(request->data, "$.params.type", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                response->data = mympd_api_settings_picture_list(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_HOME_ICON_SAVE: {
            if (mympd_state->home_list.length > LIST_HOME_ICONS_MAX) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_HOME, JSONRPC_SEVERITY_ERROR, "Too many home icons");
                break;
            }
                ...
                json_get_array_string(request->data, "$.params.options", &options, vcb_isname, 10, &error) == true)
            {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_HOME, JSONRPC_SEVERITY_ERROR, "Can not save home icon");
                }
            }
            list_clear(&options);
            break;
        ...
        case MYMPD_API_HOME_ICON_MOVE:
                ...
                json_get_long(request->data, "$.params.to", 0, LIST_HOME_ICONS_MAX, &long_buf2, &error) == true)
            {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_HOME, JSONRPC_SEVERITY_ERROR, "Can not move home icon");
                }
            }
            break;
        case MYMPD_API_HOME_ICON_RM:
            if (json_get_long(request->data, "$.params.pos", 0, LIST_HOME_ICONS_MAX, &long_buf1, &error) == true) {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_HOME, JSONRPC_SEVERITY_ERROR, "Can not delete home icon");
                }
            }
            break;
        case MYMPD_API_HOME_ICON_GET:
            if (json_get_long(request->data, "$.params.pos", 0, LIST_HOME_ICONS_MAX, &long_buf1, &error) == true) {
                response->data = mympd_api_home_icon_get(&mympd_state->home_list, response->data, request->cmd_id, request->id, long_buf1);
            }
            break;
        ...
        case MYMPD_API_SCRIPT_GET:
            if (json_get_string(request->data, "$.params.script", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                response->data = mympd_api_script_get(config->workdir, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_SCRIPT_LIST: {
            if (json_get_bool(request->data, "$.params.all", &bool_buf1, &error) == true) {
                response->data = mympd_api_script_list(config->workdir, response->data, request->cmd_id, request->id, bool_buf1);
            }
            break;
        }
            ...
            if (json_get_string(request->data, "$.params.table", 1, NAME_LEN_MAX, &sds_buf1, vcb_isalnum, &error) == true) {
                ...
                if (rc == true) {
                    ...
                    if (mympd_api_settings_cols_save(mympd_state, sds_buf1, cols) == true) {
                        ...
                        response->data = jsonrpc_respond_message_phrase(response->data, request->cmd_id, request->id, true,
        ...
        case MYMPD_API_PLAYER_OPTIONS_SET: {
            if (mympd_state->mpd_state->conn_state != MPD_CONNECTED) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true,
                    JSONRPC_FACILTY_MPD, JSONRPC_SEVERITY_ERROR, "Can't set playback options: MPD not connected");
                break;
            }
            if (json_iterate_object(request->data, "$.params", mympd_api_settings_mpd_options_set, mympd_state, NULL, 100, &error) == true) {
                ...
                //respond with ok
                response->data = jsonrpc_respond_ok(response->data, request->cmd_id, request->id, JSONRPC_FACILTY_MPD);
            }
            else {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true,
                    JSONRPC_FACILTY_MPD, JSONRPC_SEVERITY_ERROR, "Can't set playback options");
            }
            break;
        }
        case MYMPD_API_SETTINGS_GET:
            response->data = mympd_api_settings_get(mympd_state, response->data, request->cmd_id, request->id);
            break;
        ...
        case MYMPD_API_COVERCACHE_CROP:
            ...
            if (int_buf1 >= 0) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_TIMER_LIST:
            response->data = mympd_api_timer_list(&mympd_state->timer_list, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_TIMER_GET:
            if (json_get_int(request->data, "$.params.timerid", USER_TIMER_ID_MIN, USER_TIMER_ID_MAX, &int_buf1, &error) == true) {
                response->data = mympd_api_timer_get(&mympd_state->timer_list, response->data, request->cmd_id, request->id, int_buf1);
            }
            break;
        ...
        case MYMPD_API_LYRICS_GET:
            if (json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                response->data = mympd_api_lyrics_get(&mympd_state->lyrics, mympd_state->music_directory_value, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        ...
        case MYMPD_API_JUKEBOX_RM:
            if (json_get_long(request->data, "$.params.pos", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true) {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_JUKEBOX, JSONRPC_SEVERITY_ERROR, "Could not remove song from jukebox queue");
                }
            }
            break;
        ...
        case MYMPD_API_TRIGGER_LIST:
            response->data = mympd_api_trigger_list(&mympd_state->trigger_list, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_TRIGGER_GET:
            if (json_get_long(request->data, "$.params.id", 0, LIST_TRIGGER_MAX, &long_buf1, &error) == true) {
                response->data = mympd_api_trigger_get(&mympd_state->trigger_list, response->data, request->cmd_id, request->id, long_buf1);
            }
            break;
        case MYMPD_API_TRIGGER_SAVE: {
            if (mympd_state->trigger_list.length > LIST_TRIGGER_MAX) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_TRIGGER, JSONRPC_SEVERITY_ERROR, "Too many triggers defined");
                break;
            }
                ...
                json_get_object_string(request->data, "$.params.arguments", arguments, vcb_isname, 10, &error) == true)
            {
                ...
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_TRIGGER, JSONRPC_SEVERITY_ERROR, "Could not save trigger");
            }
            list_clear(arguments);
            FREE_PTR(arguments);
            break;
        ...
        case MYMPD_API_TRIGGER_RM:
            if (json_get_long(request->data, "$.params.id", 0, LIST_TRIGGER_MAX, &long_buf1, &error) == true) {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_TRIGGER, JSONRPC_SEVERITY_ERROR, "Could not delete trigger");
                }
            }
            break;
        ...
        case MYMPD_API_PLAYER_OUTPUT_ATTRIBUTS_SET: {
                ...
                json_get_object_string(request->data, "$.params.attributes", &attributes, vcb_isalnum, 10, &error) == true)
            {
                ...
                while (current != NULL) {
                    ...
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_output_set", &result);
                    if (result == false) {
                        break;
                    }
        ...
        case INTERNAL_API_STICKERCACHE_CREATED:
            if (request->extra != NULL) {
                ...
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_STICKER, JSONRPC_SEVERITY_ERROR, "Sticker cache is NULL");
            }
        ...
        case INTERNAL_API_ALBUMCACHE_CREATED:
            if (request->extra != NULL) {
                ...
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_DATABASE, JSONRPC_SEVERITY_ERROR, "Album cache is NULL");
            }
        ...
        case MYMPD_API_MESSAGE_SEND:
                ...
                json_get_string(request->data, "$.params.message", 1, CONTENT_LEN_MAX, &sds_buf2, vcb_isname, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, bool_buf1, "mpd_run_send_message", &result);
            }
            break;
        case MYMPD_API_LIKE:
            if (mympd_state->mpd_state->feat_mpd_stickers == false) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_STICKER, JSONRPC_SEVERITY_ERROR, "MPD stickers are disabled");
                ...
                json_get_int(request->data, "$.params.like", 0, 2, &int_buf1, &error) == true)
            {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_STICKER, JSONRPC_SEVERITY_ERROR, "Failed to set like, unknown error");
                }
        ...
        case MYMPD_API_PLAYER_STATE:
            response->data = mympd_api_status_get(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_PLAYER_CLEARERROR:
            rc = mpd_run_clearerror(mympd_state->mpd_state->conn);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_clearerror", &result);
            break;
        case MYMPD_API_DATABASE_UPDATE:
        case MYMPD_API_DATABASE_RESCAN: {
            long update_id = mympd_api_status_updatedb_id(mympd_state);
            if (update_id == -1) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_DATABASE, JSONRPC_SEVERITY_ERROR, "Error getting MPD status");
                break;
            }
            if (update_id > 0) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false, JSONRPC_FACILITY_DATABASE, JSONRPC_SEVERITY_INFO, "Database update already started");
                break;
            }
            if (json_get_string(request->data, "$.params.uri", 0, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                ...
                if (request->cmd_id == MYMPD_API_DATABASE_UPDATE) {
                    ...
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, (uint_buf1 == 0 ? false : true), "mpd_run_update", &result);
                }
                else {
                    uint_buf1 = mpd_run_rescan(mympd_state->mpd_state->conn, sds_buf1);
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, (uint_buf1 == 0 ? false : true), "mpd_run_rescan", &result);
                }
            }
            break;
        }
        ...
        case MYMPD_API_SMARTPLS_SEARCH_SAVE:
            if (mympd_state->mpd_state->feat_mpd_smartpls == false) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_PLAYLIST, JSONRPC_SEVERITY_ERROR, "Smart playlists are not supported");
                break;
            }
            rc = false;
            if (request->cmd_id == MYMPD_API_SMARTPLS_STICKER_SAVE) {
                ...
                {
                    rc = mpd_client_smartpls_save(config->workdir, JSONRPC_FACILITY_STICKER, sds_buf1, sds_buf2, int_buf1, int_buf2, sds_buf3);
                }
            }
        ...
        case MYMPD_API_SMARTPLS_GET:
            if (json_get_string(request->data, "$.params.plist", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                response->data = mympd_api_smartpls_get(config, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_PLAYER_PAUSE:
            rc = mpd_run_pause(mympd_state->mpd_state->conn, true);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_pause", &result);
            break;
        ...
        case MYMPD_API_PLAYER_PREV:
            ...
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_previous", &result);
            break;
        case MYMPD_API_PLAYER_NEXT:
            rc = mpd_run_next(mympd_state->mpd_state->conn);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_next", &result);
            break;
        case MYMPD_API_PLAYER_PLAY:
            rc = mpd_run_play(mympd_state->mpd_state->conn);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_play", &result);
            break;
        case MYMPD_API_PLAYER_STOP:
            rc = mpd_run_stop(mympd_state->mpd_state->conn);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_stop", &result);
            break;
        case MYMPD_API_QUEUE_CLEAR:
            rc = mpd_run_clear(mympd_state->mpd_state->conn);
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_clear", &result);
            break;
        ...
        case MYMPD_API_QUEUE_RM_SONG:
            if (json_get_uint_max(request->data, "$.params.songId", &uint_buf1, &error) == true) {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_delete_id", &result);
            }
            break;
        case MYMPD_API_QUEUE_RM_RANGE:
                ...
                json_get_int(request->data, "$.params.end", -1, MPD_PLAYLIST_LENGTH_MAX, &int_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_delete_range", &result);
            }
            break;
        case MYMPD_API_QUEUE_MOVE_SONG:
                ...
                json_get_uint(request->data, "$.params.to", 0, MPD_PLAYLIST_LENGTH_MAX, &uint_buf2, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_move", &result);
            }
            break;
        case MYMPD_API_QUEUE_PRIO_SET:
                ...
                json_get_uint(request->data, "$.params.priority", 0, MPD_QUEUE_PRIO_MAX, &uint_buf2, &error) == true)
            {
                ...
                if (rc == true) {
                    ...
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_QUEUE, JSONRPC_SEVERITY_ERROR, "Failed to set song priority");
                }
            }
            break;
        ...
        case MYMPD_API_PLAYER_PLAY_SONG:
            if (json_get_uint_max(request->data, "$.params.songId", &uint_buf1, &error) == true) {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_play_id", &result);
            }
            break;
        case MYMPD_API_PLAYER_OUTPUT_LIST:
            if (json_get_string(request->data, "$.params.partition", 0, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true) {
                if (sdslen(sds_buf1) == 0) {
                    response->data = mympd_api_status_output_list(mympd_state, response->data, request->cmd_id, request->id);
                }
                else {
                    response->data = mympd_api_status_partition_output_list(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
                }
            }
            break;
        case MYMPD_API_PLAYER_OUTPUT_TOGGLE:
                ...
                json_get_uint(request->data, "$.params.state", 0, 1, &uint_buf2, &error) == true)
            {
                if (uint_buf2 == 1) {
                    ...
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_enable_output", &result);
                }
                else {
                    rc = mpd_run_disable_output(mympd_state->mpd_state->conn, uint_buf1);
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_disable_output", &result);
                }
            }
            break;
        case MYMPD_API_PLAYER_VOLUME_SET:
            if (json_get_uint(request->data, "$.params.volume", 0, 100, &uint_buf1, &error) == true) {
                if (uint_buf1 > mympd_state->volume_max || uint_buf1 < mympd_state->volume_min) {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, "player", JSONRPC_SEVERITY_ERROR, "Invalid volume level");
                }
                else {
                    rc = mpd_run_set_volume(mympd_state->mpd_state->conn, uint_buf1);
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_set_volume", &result);
                }
            }
            break;
        case MYMPD_API_PLAYER_VOLUME_GET:
            response->data = mympd_api_status_volume_get(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_PLAYER_SEEK_CURRENT:
                ...
                json_get_bool(request->data, "$.params.relative", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_seek_current", &result);
            }
            break;
        case MYMPD_API_QUEUE_LIST: {
            ...
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_queue_list(mympd_state, response->data, request->cmd_id, request->id, long_buf1, long_buf2, &tagcols);
            }
            break;
        }
        case MYMPD_API_QUEUE_LAST_PLAYED: {
            ...
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_stats_last_played_list(mympd_state, response->data, request->cmd_id, request->id, long_buf1, long_buf2, sds_buf1, &tagcols);
            }
            break;
        }
        case MYMPD_API_PLAYER_CURRENT_SONG: {
            response->data = mympd_api_status_current_song(mympd_state, response->data, request->cmd_id, request->id);
            break;
        }
        case MYMPD_API_DATABASE_SONGDETAILS:
            if (json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                response->data = mympd_api_songdetails(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_DATABASE_COMMENTS:
            if (json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                response->data = mympd_api_read_comments(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_DATABASE_FINGERPRINT:
            if (mympd_state->mpd_state->feat_mpd_fingerprint == false) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_DATABASE, JSONRPC_SEVERITY_ERROR, "Fingerprint command not supported");
                break;
            }
            if (json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                response->data = mympd_api_fingerprint(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_PLAYLIST_RENAME:
            ...
            {
                response->data = mympd_api_playlist_rename(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, sds_buf2);
            }
            break;
        case MYMPD_API_PLAYLIST_RM:
            ...
            {
                response->data = mympd_api_playlist_delete(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, bool_buf1);
            }
            break;
        case MYMPD_API_PLAYLIST_RM_ALL:
            if (json_get_string(request->data, "$.params.type", 1, NAME_LEN_MAX, &sds_buf1, vcb_isalnum, &error) == true) {
                response->data = mympd_api_playlist_delete_all(mympd_state, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        case MYMPD_API_PLAYLIST_LIST:
            ...
            {
                response->data = mympd_api_playlist_list(mympd_state, response->data, request->cmd_id, request->id, long_buf1, long_buf2, sds_buf1, uint_buf1);
            }
            break;
        case MYMPD_API_PLAYLIST_CONTENT_LIST: {
            ...
            if (json_get_string(request->data, "$.params.plist", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_playlist_content_list(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, long_buf1, long_buf2, sds_buf2, &tagcols);
            }
            break;
        }
        case MYMPD_API_PLAYLIST_CONTENT_APPEND_URI:
                ...
                json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf2, vcb_isuri, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_add", &result);
                if (result == true) {
                    response->data = jsonrpc_respond_message_phrase(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_PLAYLIST_CONTENT_INSERT_URI:
            if (mympd_state->mpd_state->feat_mpd_whence == false) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_PLAYLIST, JSONRPC_SEVERITY_ERROR, "Method not supported");
                break;
            }
            if (json_get_string(request->data, "$.params.plist", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true &&
                json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf2, vcb_isuri, &error) == true &&
                json_get_uint(request->data, "$.params.to", 0, MPD_PLAYLIST_LENGTH_MAX, &uint_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_add_to", &result);
                if (result == true) {
                    response->data = jsonrpc_respond_message_phrase(response->data, request->cmd_id, request->id, false,
                        JSONRPC_FACILITY_PLAYLIST, JSONRPC_SEVERITY_INFO, "Updated the playlist %{playlist}", 2, "playlist", sds_buf1);
                }
        ...
        case MYMPD_API_PLAYLIST_CONTENT_REPLACE_URI:
                ...
                json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf2, vcb_isuri, &error) == true)
            {
                ...
                if (rc == false) {
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_clear", &result);
                    break;
                }
                rc = mpd_run_playlist_add(mympd_state->mpd_state->conn, sds_buf1, sds_buf2);
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_add", &result);
        ...
        case MYMPD_API_PLAYLIST_CONTENT_APPEND_SEARCH:
            ...
            {
                response->data = mpd_client_search_adv(mympd_state->mpd_state, response->data, request->cmd_id, request->id,
        ...
        case MYMPD_API_PLAYLIST_CONTENT_MOVE_SONG:
                ...
                json_get_uint(request->data, "$.params.to", 0, MPD_PLAYLIST_LENGTH_MAX, &uint_buf2, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_move", &result);
            }
            break;
        case MYMPD_API_PLAYLIST_CONTENT_RM_SONG:
                ...
                json_get_uint(request->data, "$.params.pos", 0, MPD_PLAYLIST_LENGTH_MAX, &uint_buf1, &error) == true)
            {
                ...
                response->data = mympd_respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_delete", &result);
            }
            break;
        case MYMPD_API_PLAYLIST_CONTENT_RM_RANGE:
                ...
                json_get_int(request->data, "$.params.end", -1, MPD_PLAYLIST_LENGTH_MAX, &int_buf1, &error) == true)
            {
                ...
                response->data = mympd_respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_playlist_delete_range", &result);
            }
            break;
        case MYMPD_API_PLAYLIST_CONTENT_SHUFFLE:
            if (json_get_string(request->data, "$.params.plist", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                rc = mpd_client_playlist_shuffle(mympd_state->mpd_state);
                if (rc == true) {
        ...
        case MYMPD_API_DATABASE_FILESYSTEM_LIST: {
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                if (strcmp(sds_buf3, "plist") == 0) {
                    response->data = mympd_api_playlist_content_list(mympd_state, response->data, request->cmd_id, request->id, sds_buf2, long_buf1, long_buf2, sds_buf1, &tagcols);
                }
                else {
                    response->data = mympd_api_browse_filesystem(mympd_state, response->data, request->cmd_id, request->id, sds_buf2, long_buf1, long_buf2, sds_buf1, &tagcols);
                }
            }
            break;
        }
        case MYMPD_API_QUEUE_INSERT_URI:
            if (mympd_state->mpd_state->feat_mpd_whence == false) {
                response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_QUEUE, JSONRPC_SEVERITY_ERROR, "Method not supported");
                break;
            }
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = mympd_respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_add_whence", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false, JSONRPC_FACILITY_QUEUE, JSONRPC_SEVERITY_INFO, "Updated the queue");
                }
            }
            break;
        case MYMPD_API_QUEUE_REPLACE_URI:
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mympd_api_queue_replace_with_song", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false, JSONRPC_FACILITY_QUEUE, JSONRPC_SEVERITY_INFO, "Replaced the queue");
                }
            }
            break;
        case MYMPD_API_QUEUE_APPEND_URI:
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_add", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false, JSONRPC_FACILITY_QUEUE, JSONRPC_SEVERITY_INFO, "Updated the queue");
        ...
        case MYMPD_API_QUEUE_APPEND_PLAYLIST:
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_load", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_QUEUE_INSERT_PLAYLIST:
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_load_range_to", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_QUEUE_REPLACE_PLAYLIST:
                ...
                json_get_bool(request->data, "$.params.play", &bool_buf1, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mympd_api_queue_replace_with_playlist", &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_QUEUE_APPEND_SEARCH:
            ...
            {
                response->data = mpd_client_search_adv(mympd_state->mpd_state, response->data, request->cmd_id, request->id,
                    sds_buf1, NULL, false, JSONRPC_FACILITY_QUEUE, UINT_MAX, 0, 0, 0, NULL, &mympd_state->sticker_cache, &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_QUEUE_INSERT_SEARCH:
            ...
            {
                response->data = mpd_client_search_adv(mympd_state->mpd_state, response->data, request->cmd_id, request->id,
                    sds_buf1, NULL, false, JSONRPC_FACILITY_QUEUE, uint_buf1, uint_buf2, 0, 0, NULL, &mympd_state->sticker_cache, &result);
                if (result == true &&
                    check_start_play(mympd_state, bool_buf1, &response->data, request->cmd_id, request->id) == true)
                {
                    response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, false,
        ...
        case MYMPD_API_QUEUE_SAVE:
            if (json_get_string(request->data, "$.params.plist", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_save", &result);
            }
            break;
        case MYMPD_API_QUEUE_SEARCH: {
            ...
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_queue_search(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, long_buf1, long_buf2, sds_buf2, &tagcols);
            }
            break;
        }
        case MYMPD_API_QUEUE_SEARCH_ADV: {
            ...
            if (json_get_string(request->data, "$.params.expression", 0, EXPRESSION_LEN_MAX, &sds_buf1, vcb_isname, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_queue_search_adv(mympd_state, response->data, request->cmd_id, request->id,
        ...
        case MYMPD_API_DATABASE_SEARCH: {
            ...
            if (json_get_string(request->data, "$.params.searchstr", 1, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mpd_client_search(mympd_state->mpd_state, response->data, request->cmd_id, request->id,
        ...
        case MYMPD_API_QUEUE_SHUFFLE:
            ...
            response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_shuffle", &result);
            break;
        case MYMPD_API_DATABASE_STATS:
            response->data = mympd_api_stats_get(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case INTERNAL_API_ALBUMART:
            if (json_get_string(request->data, "$.params.uri", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                response->data = mympd_api_albumart_getcover(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, &response->binary);
            }
            break;
        case MYMPD_API_DATABASE_ALBUMS_GET:
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_bool(request->data, "$.params.sortdesc", &bool_buf1, &error) == true)
            {
                response->data = mympd_api_browse_album_list(mympd_state, response->data, request->cmd_id, request->id,
        ...
        case MYMPD_API_DATABASE_TAG_LIST:
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_bool(request->data, "$.params.sortdesc", &bool_buf1, &error) == true)
            {
                response->data = mympd_api_browse_tag_list(mympd_state, response->data, request->cmd_id, request->id,
        ...
        case MYMPD_API_DATABASE_TAG_ALBUM_TITLE_LIST: {
            ...
            if (json_get_string(request->data, "$.params.album", 1, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true &&
                ...
                json_get_tags(request->data, "$.params.cols", &tagcols, COLS_MAX, &error) == true)
            {
                response->data = mympd_api_browse_album_songs(mympd_state, response->data, request->cmd_id, request->id, sds_buf1, &albumartists, &tagcols);
            }
        ...
        case INTERNAL_API_TIMER_STARTPLAY:
            ...
            {
                response->data = mympd_api_timer_startplay(mympd_state, response->data, request->cmd_id, request->id, uint_buf1, sds_buf1, uint_buf2);
            }
            break;
        case MYMPD_API_URLHANDLERS:
            response->data = mympd_api_mounts_urlhandler_list(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_PARTITION_LIST:
            response->data = mympd_api_partition_list(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_PARTITION_NEW:
            if (json_get_string(request->data, "$.params.name", 1, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true) {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_newpartition", &result);
            }
            break;
        case MYMPD_API_PARTITION_SWITCH:
            if (json_get_string(request->data, "$.params.name", 1, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true) {
                rc = mpd_run_switch_partition(mympd_state->mpd_state->conn, sds_buf1);
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_switch_partition", &result);
            }
            break;
        case MYMPD_API_PARTITION_RM:
            if (json_get_string(request->data, "$.params.name", 1, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true) {
                rc = mpd_run_delete_partition(mympd_state->mpd_state->conn, sds_buf1);
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_delete_partition", &result);
            }
            break;
        case MYMPD_API_PARTITION_OUTPUT_MOVE: {
            ...
            if (json_get_array_string(request->data, "$.params.outputs", &outputs, vcb_isname, 10, &error) == true) {
                ...
                while ((current = list_shift_first(&outputs)) != NULL) {
                    ...
                    response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_move_output", &result);
                    if (result == false) {
                        break;
                    }
                }
        ...
        case MYMPD_API_MOUNT_LIST:
            response->data = mympd_api_mounts_list(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_MOUNT_NEIGHBOR_LIST:
            response->data = mympd_api_mounts_neighbor_list(mympd_state, response->data, request->cmd_id, request->id);
            break;
        case MYMPD_API_MOUNT_MOUNT:
                ...
                json_get_string(request->data, "$.params.mountPoint", 1, FILEPATH_LEN_MAX, &sds_buf2, vcb_isfilepath, &error) == true)
            {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_mount", &result);
            }
            break;
        case MYMPD_API_MOUNT_UNMOUNT:
            if (json_get_string(request->data, "$.params.mountPoint", 1, FILEPATH_LEN_MAX, &sds_buf1, vcb_isfilepath, &error) == true) {
                ...
                response->data = respond_with_mpd_error_or_ok(mympd_state->mpd_state, response->data, request->cmd_id, request->id, rc, "mpd_run_unmount", &result);
            }
            break;
        case MYMPD_API_WEBRADIO_FAVORITE_LIST:
            if (json_get_long(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &long_buf1, &error) == true &&
                ...
                json_get_string(request->data, "$.params.searchstr", 0, NAME_LEN_MAX, &sds_buf1, vcb_isname, &error) == true)
            {
                response->data = mympd_api_webradio_list(config->workdir, response->data, request->cmd_id, request->id, sds_buf1, long_buf1, long_buf2);
            }
            break;
        case MYMPD_API_WEBRADIO_FAVORITE_GET:
            if (json_get_string(request->data, "$.params.filename", 1, FILENAME_LEN_MAX, &sds_buf1, vcb_isfilename, &error) == true) {
                response->data = mympd_api_webradio_get(config->workdir, response->data, request->cmd_id, request->id, sds_buf1);
            }
            break;
        ...
        case MYMPD_API_WEBRADIO_FAVORITE_RM:
            ...
            break;
        default:
            response->data = jsonrpc_respond_message(response->data, request->cmd_id, request->id, true, JSONRPC_FACILITY_GENERAL, JSONRPC_SEVERITY_ERROR, "Unknown request");
    ...
    if (sdslen(error) > 0) {
        ...
        MYMPD_LOG_ERROR("Error processing method \"%s\"", request->cmd_id);
    }
    FREE_SDS(error);
...
static bool check_start_play(struct t_mympd_state *mympd_state, bool play, sds *buffer, sds method, long id) {
    if (play == true) {
        ...
        if (rc == false) {
            *buffer = jsonrpc_respond_message(*buffer, method, id,
