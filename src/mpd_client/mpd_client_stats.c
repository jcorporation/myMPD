...
sds mpd_client_put_last_played_songs(t_config *config, t_mpd_client_state *mpd_client_state, sds buffer, sds method, int request_id, 
...
sds mpd_client_put_stats(t_config *config, t_mpd_client_state *mpd_client_state, sds buffer, sds method, int request_id) {
                                          ...
                                          unsigned entity_count, long last_played, const char *uri, const t_tags *tagcols)
{
    ...
    if (check_rc_error_and_recover(mpd_client_state->mpd_state, NULL, NULL, 0, false, rc, "mpd_send_list_meta") == false) {
        buffer = put_empty_song_tags(buffer, mpd_client_state, tagcols, uri);
    }
    else {
