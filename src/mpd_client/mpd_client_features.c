...
void mpd_client_mpd_features(struct t_mympd_state *mympd_state) {
    ...
    MYMPD_LOG_NOTICE("MPD protocol version: %u.%u.%u",
        mympd_state->mpd_shared_state->protocol[0],
        mympd_state->mpd_shared_state->protocol[1],
        mympd_state->mpd_shared_state->protocol[2]
    );
    ...
    if (mpd_connection_cmp_server_version(mympd_state->partition_state->conn, 0, 22, 0) >= 0) {
        mympd_state->mpd_shared_state->feat_mpd_partitions = true;
    ...
    if (mpd_connection_cmp_server_version(mympd_state->partition_state->conn, 0, 22, 4) >= 0 ) {
        mympd_state->mpd_shared_state->feat_mpd_binarylimit = true;
    ...
    if (mpd_connection_cmp_server_version(mympd_state->partition_state->conn, 0, 23, 3) >= 0 ) {
        mympd_state->mpd_shared_state->feat_mpd_playlist_rm_range = true;
    ...
    if (mpd_connection_cmp_server_version(mympd_state->partition_state->conn, 0, 23, 5) >= 0 ) {
        mympd_state->mpd_shared_state->feat_mpd_whence = true;
    ...
    if (mpd_connection_cmp_server_version(mympd_state->partition_state->conn, 0, 24, 0) >= 0 ) {
        mympd_state->mpd_shared_state->feat_mpd_advqueue = true;
    ...
    extra->feat_mpd_albumart = mympd_state->mpd_shared_state->feat_mpd_albumart;
    extra->mpd_stream_port = mympd_state->mpd_stream_port;
    extra->mpd_host = sdsdup(mympd_state->mpd_shared_state->mpd_host);
...
static void mpd_client_feature_tags(struct t_mympd_state *mympd_state) {
    ...
    if (mympd_state->mpd_shared_state->feat_mpd_tags == true) {
        check_tags(mympd_state->tag_list_search, "tag_list_search",
            &mympd_state->mpd_shared_state->tag_types_search, &mympd_state->mpd_shared_state->tag_types_mympd);
        check_tags(mympd_state->tag_list_browse, "tag_list_browse",
            &mympd_state->mpd_shared_state->tag_types_browse, &mympd_state->mpd_shared_state->tag_types_mympd);
        check_tags(mympd_state->smartpls_generate_tag_list, "smartpls_generate_tag_list",
            &mympd_state->smartpls_generate_tag_types, &mympd_state->mpd_shared_state->tag_types_mympd);
    }
}
...
static void mpd_client_feature_mpd_tags(struct t_mympd_state *mympd_state) {
    ...
    if (mpd_send_list_tag_types(mympd_state->partition_state->conn) == true) {
        ...
        while ((pair = mpd_recv_tag_type_pair(mympd_state->partition_state->conn)) != NULL) {
            ...
            if (tag != MPD_TAG_UNKNOWN) {
                ...
                mympd_state->mpd_shared_state->tag_types_mpd.tags[mympd_state->mpd_shared_state->tag_types_mpd.len++] = tag;
            }
            else {
    ...
    if (mympd_state->mpd_shared_state->tag_types_mpd.len == 0) {
        ...
        mympd_state->mpd_shared_state->feat_mpd_tags = true;
        MYMPD_LOG_NOTICE("%s", logline);
        check_tags(mympd_state->mpd_shared_state->tag_list, "tag_list", &mympd_state->mpd_shared_state->tag_types_mympd, &mympd_state->mpd_shared_state->tag_types_mpd);
        enable_mpd_tags(mympd_state->partition_state, &mympd_state->mpd_shared_state->tag_types_mympd);
    }

    bool has_albumartist = mpd_client_tag_exists(&mympd_state->mpd_shared_state->tag_types_mympd, MPD_TAG_ALBUM_ARTIST);
    if (has_albumartist == true) {
        mympd_state->mpd_shared_state->tag_albumartist = MPD_TAG_ALBUM_ARTIST;
    }
    else {
        MYMPD_LOG_WARN("AlbumArtist tag not enabled");
        mympd_state->mpd_shared_state->tag_albumartist = MPD_TAG_ARTIST;
    }
    FREE_SDS(logline);
}
...
static void mpd_client_feature_music_directory(struct t_mympd_state *mympd_state) {
    ...
    if (strncmp(mympd_state->mpd_shared_state->mpd_host, "/", 1) == 0 &&
        strncmp(mympd_state->music_directory, "auto", 4) == 0)
    {
        ...
        if (mpd_send_command(mympd_state->partition_state->conn, "config", NULL) == true) {
            ...
            while ((pair = mpd_recv_pair(mympd_state->partition_state->conn)) != NULL) {
                ...
                {
                    mympd_state->mpd_shared_state->music_directory_value = sds_replace(mympd_state->mpd_shared_state->music_directory_value, pair->value);
                }
                mpd_return_pair(mympd_state->partition_state->conn, pair);
            }
        }
        ...
        mympd_state->mpd_shared_state->music_directory_value = sds_replace(mympd_state->mpd_shared_state->music_directory_value, mympd_state->music_directory);
    }
    ...
    strip_slash(mympd_state->mpd_shared_state->music_directory_value);
    MYMPD_LOG_INFO("Music directory is \"%s\"", mympd_state->mpd_shared_state->music_directory_value);

    //set feat_library
    if (sdslen(mympd_state->mpd_shared_state->music_directory_value) == 0) {
        MYMPD_LOG_WARN("Disabling library feature, music directory not defined");
        mympd_state->mpd_shared_state->feat_mpd_library = false;
    }
    else if (testdir("MPD music_directory", mympd_state->mpd_shared_state->music_directory_value, false) == DIR_EXISTS) {
        MYMPD_LOG_NOTICE("Enabling library feature");
        mympd_state->mpd_shared_state->feat_mpd_library = true;
    }
    else {
