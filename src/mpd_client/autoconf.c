...
void mpd_client_autoconf(struct t_mympd_state *mympd_state) {
    ...
    if (mpd_host_env != NULL) {
        ...
        if (vcb_isname(mpd_host) == true) {
            if (mpd_host[0] != '@' && strstr(mpd_host, "@") != NULL) {
                ...
                mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, tokens[1]);
                mympd_state->mpd_shared_state->mpd_pass = sds_replace(mympd_state->mpd_shared_state->mpd_pass, tokens[0]);
                sdsfreesplitres(tokens,count);
            }
            else {
                //no password
                mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, mpd_host);
            }
            MYMPD_LOG_NOTICE("Setting mpd host to \"%s\"", mympd_state->mpd_shared_state->mpd_host);
            mpd_configured = true;
        }
        FREE_SDS(mpd_host);
    }
    ...
    if (mpd_port_env != NULL) {
        ...
        if (vcb_isdigit(mpd_port) == true) {
            ...
            if (port == 0) {
                mympd_state->mpd_shared_state->mpd_port = 6600;
                MYMPD_LOG_NOTICE("Setting mpd port to \"%d\"", mympd_state->mpd_shared_state->mpd_port);
            }
            else if (port > MPD_PORT_MIN && port <= MPD_PORT_MAX) {
                mympd_state->mpd_shared_state->mpd_port = port;
                MYMPD_LOG_NOTICE("Setting mpd port to \"%d\"", mympd_state->mpd_shared_state->mpd_port);
            }
            else {
    ...
    if (mpd_timeout_env != NULL) {
        ...
        if (vcb_isdigit(mpd_timeout) == true) {
            ...
            if (timeout >= MPD_TIMEOUT_MIN && timeout < MPD_TIMEOUT_MAX) {
                mympd_state->mpd_shared_state->mpd_timeout = timeout;
                MYMPD_LOG_NOTICE("Setting mpd timeout to \"%d\"", mympd_state->mpd_shared_state->mpd_timeout);
            }
            else {
    ...
    if (xdg_runtime_dir != NULL) {
        ...
        if (access(socket, F_OK ) == 0) { /* Flawfinder: ignore */
            mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, socket);
            FREE_SDS(socket);
            return;
        }
        FREE_SDS(socket);
    ...
    if (access("/run/mpd/socket", F_OK ) == 0) { /* Flawfinder: ignore */
        mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, "/run/mpd/socket");
        return;
    }
    if (access("/var/run/mpd/socket", F_OK ) == 0) { /* Flawfinder: ignore */
        mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, "/var/run/mpd/socket");
        return;
    }
    if (access("/var/lib/mpd/socket", F_OK ) == 0) { /* Flawfinder: ignore */
        //gentoo default 
        mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, "/var/lib/mpd/socket");
        return;
    }
    //fallback to localhost:6600
    mympd_state->mpd_shared_state->mpd_host = sds_replace(mympd_state->mpd_shared_state->mpd_host, MYMPD_MPD_HOST);
    mympd_state->mpd_shared_state->mpd_port = MYMPD_MPD_PORT;
}
