...
bool mpd_worker_cache_init(struct t_mpd_worker_state *mpd_worker_state) {
    ...
    if (mpd_worker_state->mpd_state->feat_tags == true) {
        if (rc == true) {
            t_work_request *request = create_request(-1, 0, INTERNAL_API_ALBUMCACHE_CREATED, NULL);
    ...
    if (mpd_worker_state->mpd_state->feat_stickers == true) {
        if (rc == true) {
            t_work_request *request2 = create_request(-1, 0, INTERNAL_API_STICKERCACHE_CREATED, NULL);
