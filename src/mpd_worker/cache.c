...
bool mpd_worker_cache_init(struct t_mpd_worker_state *mpd_worker_state) {
    ...
    if (mpd_worker_state->partition_state->mpd_shared_state->feat_mpd_tags == true) {
        album_cache.cache = raxNew();
    }
    struct t_cache sticker_cache;
    sticker_cache.cache = NULL;
    if (mpd_worker_state->partition_state->mpd_shared_state->feat_mpd_stickers == true) {
        sticker_cache.cache = raxNew();
    }

    bool rc = true;
    if (mpd_worker_state->partition_state->mpd_shared_state->feat_mpd_tags == true ||
        mpd_worker_state->partition_state->mpd_shared_state->feat_mpd_stickers == true)
    {
...
static bool _cache_init(struct t_mpd_worker_state *mpd_worker_state, rax *album_cache, rax *sticker_cache) {
    ...
    do {
        ...
        sds key = sdsempty();
        const bool create_album_cache = mpd_worker_state->partition_state->mpd_shared_state->feat_mpd_tags &&
            mpd_client_tag_exists(&mpd_worker_state->partition_state->mpd_shared_state->tag_types_mympd, MPD_TAG_ALBUM) &&
            mpd_client_tag_exists(&mpd_worker_state->partition_state->mpd_shared_state->tag_types_mympd, mpd_worker_state->partition_state->mpd_shared_state->tag_albumartist);
        ...
        while ((song = mpd_recv_song(mpd_worker_state->partition_state->conn)) != NULL) {
            ...
            if (create_album_cache == true) {
                ...
                //construct the key
                key = album_cache_get_key(song, key, mpd_worker_state->partition_state->mpd_shared_state->tag_albumartist);
                if (sdslen(key) > 0) {
                    void *old_data;
                    if (raxTryInsert(album_cache, (unsigned char *)key, sdslen(key), (void *)song, &old_data) == 0) {
                        ...
                        album_cache_append_tags(album, song, &mpd_worker_state->partition_state->mpd_shared_state->tag_types_mympd);
                        //set album data
